...
label: step7-fa-c-round-1

# Final Adjudicator queue — phase-2-next-20, group c, round 1

This is the exact queue frozen in `research/phase-2-next-20-step7-fa-c-round-1.json`. It contains 2 item(s).
Work in the numbered order below. Do not substantively review the next item until the recorder accepts the current one.

## 1. `ex-independent-sums-via-characteristic-functions` (run)

1. Read `items/ex-independent-sums-via-characteristic-functions.md`, its cited dependencies, pair/page context, proof contract, judge and Alpha evidence, and this group's conventions.
...
3. Write concrete evidence to `research/phase-2-next-20-step7-fa-c-1-ex-independent-sums-via-characteristic-functions.md`, including exact source URLs and what they support, or explain why the mathematics was familiar.
...
```bash
node tools/step7-terminal-resolution.mjs record --run phase-2-next-20 --id ex-independent-sums-via-characteristic-functions --resolved-by final-adjudicator --group c --queue research/phase-2-next-20-step7-fa-c-round-1.json --state-dir /home/lazyinspirit/Projects/prestige-math-library/.autopilot/phase-2-next-20 --disposition accepted-after-review --source-status verified --basis-file research/phase-2-next-20-step7-fa-c-1-ex-independent-sums-via-characteristic-functions.md
node tools/step7-terminal-resolution.mjs record --run phase-2-next-20 --id ex-independent-sums-via-characteristic-functions --resolved-by final-adjudicator --group c --queue research/phase-2-next-20-step7-fa-c-round-1.json --state-dir /home/lazyinspirit/Projects/prestige-math-library/.autopilot/phase-2-next-20 --disposition repaired --source-status verified --basis-file research/phase-2-next-20-step7-fa-c-1-ex-independent-sums-via-characteristic-functions.md
```
...
## 2. `lem-martingale-differences-are-orthogonal-in-l2` (run)

1. Read `items/lem-martingale-differences-are-orthogonal-in-l2.md`, its cited dependencies, pair/page context, proof contract, judge and Alpha evidence, and this group's conventions.
2. Independently review the current repair, the Terra verdict and any later licensed correction. If unfamiliar or uncertain, use web search and verify against authoritative sources.
3. Write concrete evidence to `research/phase-2-next-20-step7-fa-c-2-lem-martingale-differences-are-orthogonal-in-l2.md`, including exact source URLs and what they support, or explain why the mathematics was familiar.
4. Accept or repair the queued item and its own contracts/metadata using existing suppliers. Run focused checks and record the final decision. If new items or supplier edits are necessary, escalate instead. Do not launch another judge, reopen settled items, or expand scope.
5. Record the exact final bytes with exactly one of these commands:

```bash
node tools/step7-terminal-resolution.mjs record --run phase-2-next-20 --id lem-martingale-differences-are-orthogonal-in-l2 --resolved-by final-adjudicator --group c --queue research/phase-2-next-20-step7-fa-c-round-1.json --state-dir /home/lazyinspirit/Projects/prestige-math-library/.autopilot/phase-2-next-20 --disposition accepted-after-review --source-status verified --basis-file research/phase-2-next-20-step7-fa-c-2-lem-martingale-differences-are-orthogonal-in-l2.md
node tools/step7-terminal-resolution.mjs record --run phase-2-next-20 --id lem-martingale-differences-are-orthogonal-in-l2 --resolved-by final-adjudicator --group c --queue research/phase-2-next-20-step7-fa-c-round-1.json --state-dir /home/lazyinspirit/Projects/prestige-math-library/.autopilot/phase-2-next-20 --disposition repaired --source-status verified --basis-file research/phase-2-next-20-step7-fa-c-2-lem-martingale-differences-are-orthogonal-in-l2.md
```
