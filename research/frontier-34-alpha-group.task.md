...
Run `node tools/scope-decisions.mjs refresh --run frontier-34 --group <your-group>`.
Resolve every pending row in
`research/frontier-34-alpha-<your-group>-scope-decisions.json` as `stands` or
`owner-decision` with current evidence, then run its `check` command.
Run whole-run `manifest-deps`, `validate-plan`, and `extcheck`.

Write `research/frontier-34-alpha-<your-group>-step3-scaffold-review.md` and
`research/frontier-34-alpha-<your-group>-step3-verdicts.json`. The verdict file has
