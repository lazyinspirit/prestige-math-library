# Contract-rework certification, run `frontier-18`
...
newest `beta-contract-rework-*.result.json` in `research/frontier-18-dispatch/` —
...
4. **On a pass: close the ledger row IN PLACE** — one defect, one row: set its
   ...
   obligation row with `node tools/obligations.mjs close --run frontier-18 --id
