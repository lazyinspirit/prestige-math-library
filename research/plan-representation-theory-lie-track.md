...
After the ten revocations and the six RL-12 proof additions, every one of the 225 ids individually
...
RL-12 31; RL-13 21 after two; RL-14 19 after one revocation and two definition
moves to RL-12; and RL-15 19 after two.
The totals are 225 and zero published impact. The three new Phase-2-pair B items
...
Serre presentation for symmetrizable matrices, and develop real/imaginary roots
and the Weyl action. The presentation proof is Kleshchev Theorem 9.3.5,
pp.125--126, not the introductory presentation discussion. Its local Casimir,
primitive-vector and relation-module prerequisites are supplied below using
§§2.3, 9.1 and 9.3; no RL-14 result or formal-character machinery is assumed.
The invariant form is constructed from the maximal-ideal quotient before
Serre generation. Maximality also proves that no nonzero element of either
nilpotent half commutes with every opposite simple generator. The Serre
quotient's Weyl symmetry must be proved directly from its relations.
...
| `def-kac-moody-category-o` | Definition: weight modules with finite-dimensional weight spaces and support in finitely many downward cones form the Kac--Moody version of $\mathcal O$. | Show that finite downward-cone support makes all positive-root sums pointwise finite; no finite-length assertion. | literature-derived | not-applicable | Kleshchev §9.1, pp.116--118; Perrin §10.1, pp.85--87 |
| `def-kac-moody-verma-module` | Definition: $M_A(\lambda)=U\mathfrak g(A)\otimes_{U\mathfrak b}\mathbb C_\lambda$. | Infinite-dimensional standard module. | literature-derived | not-applicable | Kleshchev §9.1, pp.116--117; Perrin §10.1, pp.85--88 |
| `def-generalized-casimir-on-restricted-kac-moody-modules` | Define restricted modules, choose $\rho(h_i)=1$, and define the Cartan term plus the root-degree dual-basis sum $\Omega$. | Finite root spaces and restrictedness make the operator pointwise finite; prove independence of the dual bases. | literature-derived | not-applicable | Kleshchev §2.3, pp.32--34 |
| `thm-generalized-kac-moody-casimir-is-central-and-scalar-on-highest-weight-modules` | Prove the dual-basis commutator cancellation, centrality of $\Omega$, and its scalar $(\lambda+2\rho\mid\lambda)$ on a module generated by a highest-weight vector. | Depends on the invariant form and the preceding definition, never on the Serre-presentation theorem. | literature-derived | ai-altered | Kleshchev Lemma 2.3.1, Theorem 2.3.5 and Corollary 2.3.6, pp.32--36 |
| `lem-bounded-above-kac-moody-weight-modules-are-generated-by-primitive-vectors` | Define a primitive vector as a highest-weight vector modulo a submodule. Prove that an object of $\mathcal O$ is generated by primitive vectors under $U(\mathfrak n^-)$. | Downward-cone induction; prove the needed Verma PBW freeness and one-dimensional highest-weight space inline. No formal characters or finite composition series. | literature-derived | ai-altered | Kleshchev §9.1, especially Lemma 9.1.3, pp.116--118 |
| `lem-enveloping-quotient-kernels-and-augmentation-intersections` | For a Lie quotient prove $\ker U(\theta)=\mathfrak r U(\mathfrak g)$; for a subalgebra prove $\mathfrak r\cap\mathfrak r U_0(\mathfrak g)=[\mathfrak r,\mathfrak r]$. | Universal-property inverse maps and an ordered PBW basis; supplies the relation-module injection. | literature-derived | ai-altered | Kleshchev Lemmas 9.3.1--9.3.3, pp.122--124 |
| `lem-kac-moody-relation-module-embeds-in-verma-modules-and-obeys-the-casimir-constraint` | Construct $\mathfrak r^-/[\mathfrak r^-,\mathfrak r^-]\hookrightarrow\bigoplus_i M(-\alpha_i)$ using the free contragredient negative half. Prove injectivity with the augmentation lemma; primitive generation and Casimir imply that ideal generators have $(\alpha\mid\alpha)=2(\rho\mid\alpha)$. | Include the final minimum-height lift from the abelianized relation module to the ideal. | literature-derived | ai-altered | Kleshchev Proposition 9.3.4, pp.124--125 |
| `lem-the-serre-quotient-has-weyl-symmetry-and-no-residual-kac-moody-kernel` | First prove that Serre elements vanish in the maximal-ideal quotient, using adjoint rank-one calculations and the no-commuting-vector consequence of maximality. On the quotient by Serre elements, prove local nilpotence and use finite exponential operators for Weyl symmetry. A minimum-height residual-kernel weight contradicts the preceding Casimir constraint. | Never assume that vanishing implies generation. The Weyl action on the Serre quotient is proved from its relations, not from its still-unproved identification with $\mathfrak g(A)$. | literature-derived | ai-altered | Kleshchev Lemma 3.1.1, §3.2 and Theorem 9.3.5 proof, pp.37--42, 125--126 |
| `thm-serre-presentation-of-a-kac-moody-algebra` | Theorem: for symmetrizable $A$, the ideals $\mathfrak r\cap\widetilde{\mathfrak n}^{\pm}$ are generated by the corresponding Serre elements, so $\mathfrak g(A)$ has the Chevalley--Serre presentation. | Uses the preceding relation-module and residual-kernel lemmas, not later integrable representation theory. | literature-derived | ai-altered | Kleshchev §9.3, Theorem 9.3.5, pp.125--126 |
...
Perrin §§10--11, pp.85--100. The category-O and Verma-module definitions now
...
**Local chamber interface (2026-09-11).** The three local coroot/sign/stabilizer
items below supply the complete conditional chamber algorithm and zero-wall
generated stabilizer formula from published RL12 GCM results. Apply those
results also to the transpose realization. The unused finite DG root-system
page is removed from RL14's canonical prerequisites; no finite-type assumption
or new pair is introduced. Full argument: `phase-2-next-20-rl14-chamber-local-resolution.md`.
...
| `def-kac-moody-real-coroot-inversions` | Definition: transpose-realization coroot signs, length and inversion sets. | Fixes local chamber conventions. | literature-derived | not-applicable | Kleshchev §§3.3–3.4, pp42–47 |
| `lem-kac-moody-reduced-words-and-coroot-signs` | Lemma: root/coroot sign exchange, deletion and finite inversion sets from the published GCM automorphisms. | Supplies chamber descent without a new Coxeter pair. | literature-derived | ai-altered | Kleshchev Lemmas3.3.1–3.3.3, pp42–44 |
| `lem-kac-moody-chamber-boundary-stabilizers` | Lemma: unique dominant orbit representative, full zero-wall generated stabilizer, and terminating negative-label descent when the orbit meets the chamber. | Supplies the complete boundary clause locally. | literature-derived | ai-altered | Kleshchev Proposition3.4.1(i)–(iii), p47, dualized |
...
| `lem-an-integrable-o-module-is-generated-by-dominant-maximal-weights` | Lemma: highest vectors have dominant weights and generate integrable highest-weight quotients; primitive highest vectors in successive subquotients give the generation route. This does not assert generation by globally maximal support weights alone. | Starts complete reducibility. | literature-derived | ai-altered | Kleshchev §10.4, pp.139--142; Perrin §10.2, pp.91--92 |
...
| `ex-simple-root-string-in-an-integrable-kac-moody-module` | Example: display the finite cyclic $i$-string generated by an $i$-highest vector and its reflection symmetry; the entire support on an arbitrary root line need not be finite. | Makes local nilpotence/Weyl invariance concrete. | literature-derived | ai-altered | Kleshchev §3.1--2, pp.37--49 |
...
| I58 | Klesh §§1.4--1.5 quotient; §§9.1,9.3 symmetrizable Serre proof | included | `lem-the-sum-of-triangularly-disjoint-graded-ideals-is-disjoint-from-h`, `def-kac-moody-algebra-associated-to-a-gcm`, `thm-serre-presentation-of-a-kac-moody-algebra`, `lem-bounded-above-kac-moody-weight-modules-are-generated-by-primitive-vectors`, `lem-enveloping-quotient-kernels-and-augmentation-intersections`, `lem-kac-moody-relation-module-embeds-in-verma-modules-and-obeys-the-casimir-constraint`, `lem-the-serre-quotient-has-weyl-symmetry-and-no-residual-kac-moody-kernel`, `ex-rank-one-gcm-recovers-sl2`, `ex-the-a2-serre-relations` |
