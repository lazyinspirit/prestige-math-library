<!-- Dispatched as-is. `frontier-15` and `<i>` are substituted by tools/dispatch.mjs
     from the engine own --vars; nothing is copied by hand. -->

# Authoring brief — run `frontier-15`, batch `<i>` (step 5)
...
`research/frontier-15-batch-<i>.notes.md`: current substage, owned artifact paths,
...
- update only your own `research/frontier-15-batch-<i>.proof-contracts.json` so it
...
- keep your own `research/frontier-15-batch-<i>.coverage.json` **true of what you
...
research/frontier-15-batch-<i>.proof-contracts.json --strict` before reporting.
...
run: frontier-15
role: alpha
label: contract-rework-1-b2

# Contract-worksheet rework, batch 2, run `frontier-15`

You are the contract-rework lane for batch 2, standing in for the owning
Beta under an owner substitution (2026-08-17: the Codex weekly limit is hit;
the owner instructed Opus 5 so the run closes today — recorded on obligation
row `contract-rework-b2`). The batch-2 proof-contract boundary worksheets
were GENERATED rather than written: defect-ledger row `f15-a-rr-005` records that
518 boundary rows across 88 batch-2 items in
`research/frontier-15-batch-2.proof-contracts.json` follow the shape
`"<title>: <stock clause for this axis> for <statement>"`, with
`not_applicable` reasons drawn from a fixed set of seven phrasings — 26 items
generated end to end, 62 partly. Two of those rows were later found false
(`f15-S9-006`); neither hid a mathematical defect, but a worksheet that
restates the title is not a disposition of anything.
...
2. **Touch ONLY `research/frontier-15-batch-2.proof-contracts.json`** — never
   item text, page files, or any other batch's contracts. The items are
   frozen and judged; their contracts are not.
3. Where writing an honest row reveals the PROOF actually fails a case, do
   not paper over it: leave the row stating the gap and record the finding at
   the end of your report — that is an escalation, not yours to repair here.
