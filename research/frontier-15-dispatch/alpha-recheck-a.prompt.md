...
An `applied` entry that changed nothing is the failure this catches, and it
is the only reason the fix pass can be trusted without a person reading it.
A Beta that reports a fix it did not make produces exactly the same result
file as one that did.

## What to do

1. Read your own report:
   `research/frontier-15-alpha-<your-group>-step3-scaffold-review.md`
2. Read each Beta's `## Step-3 fix pass` section in
   `research/frontier-15-batch-<i>.notes.md`
3. For every finding id, open the scaffold and confirm the disposition is
   true of the file as it now stands.
4. Where a Beta pushed back, adjudicate: is the pushback right? A
