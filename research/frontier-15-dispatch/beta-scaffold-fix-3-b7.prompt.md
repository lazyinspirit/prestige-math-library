<!-- Dispatched as-is. `frontier-15` and `7` are substituted by tools/dispatch.mjs
...
# Scaffolding brief — run `frontier-15`, batch `7` (steps 1 and 2)
...
You are **Beta-7**, running **GPT 5.6 Sol via the Codex subscription plan
...
`research/frontier-15-batch-7.notes.md`: current substage, owned artifact paths,
...
`research/frontier-15-batch-7.coverage.json`:
...
```
node tools/source-fetch-check.mjs --coverage research/frontier-15-batch-7.coverage.json --stamp
```
...
```
node tools/coverage-checklist.mjs research/frontier-15-batch-7.coverage.json
```
...
- `research/frontier-15-batch-7.pages.json` — your batch's page objects in final
...
- `research/frontier-15-batch-7.notes.md` — prose-scaffold amendments as precise
  ...
  route, and necessity.
- `research/frontier-15-batch-7.proof-contracts.json` — the version-1
...
run: frontier-15
role: beta
label: scaffold-fix-3-b7
covers: 7

> **Scaffold-fix round.** Your batch number is in the "This dispatch" section
> appended below; substitute it wherever `<i>` appears.

# Batch `<i>` of run `frontier-15` — post-recheck scaffold repair

The step-3 recheck closed with your batch still carrying `insufficient`
pairs. The findings live in the CLOSURE RECEIPT, not in prose:
...
1. For each of your findings: apply the remedy to your batch's scaffold
   files (`research/frontier-15-batch-<i>.pages.json`, `.coverage.json`,
   `.notes.md`), or push back with the reason if the finding is wrong —
...
4. Append `## Scaffold-fix round` to
   `research/frontier-15-batch-<i>.notes.md`, one entry per finding id:
   `applied` / `pushed back` (+ reason), with what changed.
