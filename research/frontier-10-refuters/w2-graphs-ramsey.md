...
## Your cluster — `w2-graphs-ramsey`

These are the graph-theory and Ramsey items **not** already assigned to the Kuratowski refuter.

Plane graphs: everything here rests on a **polygonal** Jordan curve theorem, not the general one, so every topological step must stay inside the polygonal category. Check `thm-polygonal-jordan-curve`, `lem-polygonal-arc-does-not-separate-the-plane` and the parity argument in `lem-polygonal-crossing-parity-is-locally-constant` — the parity argument must handle a ray that passes through a **vertex** or runs **along** an edge, and the general-position lemma is what rules those out. Then check Euler's formula's induction and the edge-bound corollaries: \(m \le 3n-6\) needs \(n \ge 3\), and \(m \le 2n-4\) needs triangle-freeness and \(n \ge 3\).

Ramsey: check `thm-finite-ramsey-for-uniform-subsets` proves the \(k\)-uniform case by the right induction (on both the uniformity and the target sizes), and that `thm-infinite-ramsey-on-the-naturals` builds its nested infinite sets with a licensed recursion. `thm-van-der-waerden-with-monochromatic-difference` has a double induction — verify both parameters actually decrease. Check `thm-canonical-ramsey-theorem-for-pairs` genuinely produces all four canonical types and that they are exhaustive.

Attack the three `ai-generated` Statements on these pages by trying to build the object each says cannot exist.
...
- `cor-ramsey-recursion-and-diagonal-upper-bound`
- `thm-diagonal-ramsey-counting-lower-bound`
- `thm-finite-ramsey-for-uniform-subsets`
- `thm-konig-infinity-lemma-for-ordered-trees`
- `lem-infinite-pigeonhole-on-the-naturals`
- `thm-infinite-ramsey-on-the-naturals`
- `cor-infinite-ramsey-for-dedekind-infinite-sets`
- `cor-finite-ramsey-by-konig-compactness`
- `thm-canonical-ramsey-theorem-for-pairs`
- `lem-colour-focussing-for-arithmetic-progressions`

**ramsey-theory-examples** (1)

- `cex-infinite-branching-is-not-enough-for-konig`

**plane-graphs-euler-and-the-five-colour-theorem** (12)
...
**plane-graphs-euler-and-the-five-colour-theorem-examples** (2)
