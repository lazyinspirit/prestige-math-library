...
## Your cluster — `w2-fubini-rest`

These are the Fubini/change-of-variables items **not** already assigned to the fatal-repair refuter.

The Fubini half: the library's theorem is for the **Riemann** integral, where the section function need not be integrable for every section. Check that every statement carries the exceptional-section machinery and that the exceptional set is shown to have content zero, not merely measure zero. `thm-riemann-fubini-on-product-rectangles` and `thm-jordan-fubini-by-sections` are the two to hold to that standard, and `cor-cavalieri-principle-for-jordan-content` is derived from the second.

The examples half: the coordinate Jacobians (polar, cylindrical, spherical, hyperspherical) are computations, so check them **numerically**. Recompute each determinant. Verify the stated parameter box is genuinely seam-free and injective on it, and that the recovery of the parameters from the point is correct including the poles and the origin, where these coordinate systems degenerate. `ex-hyperspherical-coordinate-jacobian` carries an induction — check the base case and that the sine powers come out as claimed.

Also verify the three counterexamples really refute what they claim: non-global-injectivity of polar coordinates, sign reversal without the absolute Jacobian, and double counting under a non-injective substitution.

## Your assigned items

Every one of these is `high` or `critical` on `risk-report.mjs`. Read each in full, from disk, with every dependency it cites.

**fubini-and-change-of-variables** (11)
...
**fubini-and-change-of-variables-examples** (10)
...
- `ex-fubini-computes-the-integral-of-x-exp-xy`
- `ex-polar-change-of-variables-on-an-annular-sector`
- `ex-cylindrical-coordinate-jacobian`
- `ex-spherical-coordinate-jacobian`
- `ex-hyperspherical-coordinate-jacobian`
- `cex-omitting-the-absolute-jacobian-reverses-sign`
- `cex-noninjective-change-of-variables-double-counts`
