# Local closure of the loxodromic pole obligation

Root read the full prior draft and all direct suppliers. The following full orbit-chain proof has been installed in the draft item; explicit precheck/render, strict single-item contract, whole batch citation fidelity and36-item content policy pass. Root recorded the current owner repaired decision; independent review remains required. It uses neither Morse nor properness. Primary-source search found the standard classification, but the Urbański PDF follow-up read timed out; no complete external-source reading is claimed.
...
For later quantitative use, there are $\tau>0$, an integer $s\ge1$ and $C\ge0$ such that, with $a_n=d(o,g^no)=d(o,g^{-n}o)$, one has $a_n\ge n\tau$ and $(g^{\pm n}o|g^{\pm m}o)_o\ge a_n-C$ for every $m\ge n\ge s$, with matching signs. These constants depend on the given orbit and product constant, not on any additional boundary point.
