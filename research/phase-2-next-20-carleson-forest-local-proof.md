...
This proof uses the exact statements of `lem-carleson-size-selection` and `lem-carleson-single-tree-estimate`. The size-selection proof is now supplied locally (see `phase-2-next-20-carleson-size-local-proof.md`), awaiting ordinary review. The single-tree proof remains incomplete, so the forest must remain escalated. No maximal inequality or Fremlin backing is thereby accepted.
