...
label: step7-fa-h-round-1

# Final Adjudicator queue — phase-2-nine-step-25, group h, round 1

This is the exact queue frozen in `research/phase-2-nine-step-25-step7-fa-h-round-1.json`. It contains 2 item(s).
Work in the numbered order below. Do not substantively review the next item until the recorder accepts the current one.

## 1. `cex-separately-continuous-not-jointly-continuous` (published)

1. Read `items/cex-separately-continuous-not-jointly-continuous.md`, its cited dependencies, pair/page context, proof contract, judge and Alpha evidence, and this group's conventions.
...
3. Write concrete evidence to `research/phase-2-nine-step-25-step7-fa-h-1-cex-separately-continuous-not-jointly-continuous.md`, including exact source URLs and what they support, or explain why the mathematics was familiar.
...
```bash
node tools/step7-terminal-resolution.mjs record --run phase-2-nine-step-25 --id cex-separately-continuous-not-jointly-continuous --resolved-by final-adjudicator --group h --queue research/phase-2-nine-step-25-step7-fa-h-round-1.json --state-dir /home/lazyinspirit/Projects/prestige-math-library/.autopilot/phase-2-nine-step-25 --disposition accepted-after-review --source-status verified --basis-file research/phase-2-nine-step-25-step7-fa-h-1-cex-separately-continuous-not-jointly-continuous.md
node tools/step7-terminal-resolution.mjs record --run phase-2-nine-step-25 --id cex-separately-continuous-not-jointly-continuous --resolved-by final-adjudicator --group h --queue research/phase-2-nine-step-25-step7-fa-h-round-1.json --state-dir /home/lazyinspirit/Projects/prestige-math-library/.autopilot/phase-2-nine-step-25 --disposition repaired --source-status verified --basis-file research/phase-2-nine-step-25-step7-fa-h-1-cex-separately-continuous-not-jointly-continuous.md
```
...
## 2. `def-metric-continuity` (published)

1. Read `items/def-metric-continuity.md`, its cited dependencies, pair/page context, proof contract, judge and Alpha evidence, and this group's conventions.
2. Independently review the current repair, the Terra verdict and any later licensed correction. If unfamiliar or uncertain, use web search and verify against authoritative sources.
3. Write concrete evidence to `research/phase-2-nine-step-25-step7-fa-h-2-def-metric-continuity.md`, including exact source URLs and what they support, or explain why the mathematics was familiar.
4. Accept or repair the queued item and its own contracts/metadata using existing suppliers. Run focused checks and record the final decision. If new items or supplier edits are necessary, escalate instead. Do not launch another judge, reopen settled items, or expand scope.
5. Record the exact final bytes with exactly one of these commands:

```bash
node tools/step7-terminal-resolution.mjs record --run phase-2-nine-step-25 --id def-metric-continuity --resolved-by final-adjudicator --group h --queue research/phase-2-nine-step-25-step7-fa-h-round-1.json --state-dir /home/lazyinspirit/Projects/prestige-math-library/.autopilot/phase-2-nine-step-25 --disposition accepted-after-review --source-status verified --basis-file research/phase-2-nine-step-25-step7-fa-h-2-def-metric-continuity.md
node tools/step7-terminal-resolution.mjs record --run phase-2-nine-step-25 --id def-metric-continuity --resolved-by final-adjudicator --group h --queue research/phase-2-nine-step-25-step7-fa-h-round-1.json --state-dir /home/lazyinspirit/Projects/prestige-math-library/.autopilot/phase-2-nine-step-25 --disposition repaired --source-status verified --basis-file research/phase-2-nine-step-25-step7-fa-h-2-def-metric-continuity.md
```
