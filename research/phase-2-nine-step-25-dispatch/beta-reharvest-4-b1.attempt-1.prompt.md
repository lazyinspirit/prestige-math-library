...
role: beta
label: reharvest-4-b1
covers: 1
output: research/phase-2-nine-step-25-alpha-5b.md

> The dispatch block supplies batch `1`.

# Reharvest — batch `1`, `phase-2-nine-step-25`

Read `research/phase-2-nine-step-25-reharvest-plan.json` and use its `work[]` rows belonging
...
Follow `briefs/beta-scaffold.md` for five recovery retries, documented source
drops and owner escalation on mathematical doubt. For each orphaned result,
update only this batch's manifest, coverage and notes with a
...
found, supply a complete local alternative and its dependencies. If not fully
confident, escalate to the owner and keep the result and batch blocked.
