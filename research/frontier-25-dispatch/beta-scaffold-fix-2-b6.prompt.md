...
run: frontier-25
role: beta
label: scaffold-fix-2-b6
covers: 6

> The dispatch block supplies batch `6`.

# Post-recheck scaffold repair — batch `6`, `frontier-25`

Read `research/frontier-25-scaffold-closure.json` and take only `work[]` entries
whose pages belong to batch `6` in the scope ledger. Apply or push back on
...
For batch 6's Mac Lane finding, use the reader-openable full paper at
...
The required locator is §5, “Higher Identity Laws,” Theorem 5.2 (paper pp.
42–43; PDF pages 15–16), including condition (5.6), the pentagon (3.5), the
first condition of (5.3), and the two conditions (5.7). A local full-body fetch
on 2026-08-30 returned HTTP 200, `application/pdf`, 7,206,775 bytes.
