...
label: assign
covers: all
output: research/phase-2-next-17-alpha-groups.json

# Assign batches to group Alphas

This dispatch produces only `research/phase-2-next-17-alpha-groups.json`. It assigns the
...
```sh
node tools/alpha-groups.mjs --run phase-2-next-17 --facts
```
...
```sh
node tools/alpha-groups.mjs --run phase-2-next-17
```
