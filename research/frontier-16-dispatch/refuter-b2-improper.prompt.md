...
# Read-only proof refutation task — frontier-16 batch 2, improper-integral pair

Audit the current post-reader text for the A/B pair
`improper-and-parameter-dependent-multiple-integrals` and
`improper-and-parameter-dependent-multiple-integrals-examples`, using
...
in the sequence `F2I-1`, `F2I-2`, and so on, the item id, exact
