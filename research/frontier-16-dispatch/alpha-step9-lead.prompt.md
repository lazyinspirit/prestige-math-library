...
label: step9-lead
covers: all
output: research/frontier-16-alpha-step9.md

# Step 9 — the scope-denial sweep, run `frontier-16`
...
1. Collect every `deferred` and `out-of-scope` disposition from all
   7 `research/frontier-16-batch-<i>.coverage.json` files.
...
```
grep '"disposition":"open"' research/defect-ledger.jsonl | grep '"run":"frontier-16"'
```
...
`research/frontier-16-alpha-step9.md`: every decline, its verdict (`stands` /
