...
# Batch `<i>` of run `frontier-16` — step-3 fix pass

The group Alpha that reviewed your batch returned numbered findings. **Find
which Alpha reviewed you**: check each
`research/frontier-16-alpha-<group>-step3-scaffold-review.md` for your batch
...
```
node tools/coverage-checklist.mjs research/frontier-16-batch-<i>.coverage.json
node tools/content-policy.mjs --manifest-only research/frontier-16-batch-<i>.pages.json
node tools/validate-plan.mjs research/plan-spec.json
```

5. Append `## Step-3 fix pass` to `research/frontier-16-batch-<i>.notes.md`, one
