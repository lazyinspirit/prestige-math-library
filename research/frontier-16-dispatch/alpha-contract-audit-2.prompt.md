# Alpha brief — run `frontier-16`

You are Alpha on run `frontier-16`. The engine dispatches you at several stages; your
...
`research/frontier-16-alpha-groups.json`, with a rationale per group: batches are
...
`research/frontier-16-published-dependency-repairs.md`; take a dedicated touch
...
block, and for each one read `research/frontier-16-batch-i.pages.json`, `.notes.md`
...
1. `research/frontier-16-alpha-<g>-step3-scaffold-review.md` — your reasoning per pair.
2. `research/frontier-16-alpha-<g>-step3-verdicts.json` — the same verdicts, machine
...
update yourself, naming the batch: `node tools/splice-plan.mjs --run frontier-16
...
`research/frontier-16-impact.json` against exactly that computed scope. Generate the
template first —

```
node tools/impact-audit.mjs --touches research/frontier-16-touches.json \
  --from pre-author --to post-6b --template research/frontier-16-impact.json
```
...
```
node tools/step8-guard.mjs --touches research/frontier-16-touches.json \
  --baseline pre-step8 --adjudications research/frontier-16-judge-adjudications.jsonl
```
...
`research/frontier-16-judge-closure.json` and sweeps exactly the ids that need it.

**A repair updates the owning BATCH contract**
(`research/frontier-16-batch-N.proof-contracts.json` for that batch), never only the
merged file:
...
`frontier-16-audit-coverage.json` with a real reviewer, a concrete attestation, and a
...
defer if you could. Run `node tools/defect-ledger.mjs stats --run frontier-16
...
9. The consolidated **fatal-error ledger**: per publish-blocking error, the
   ...
   30-second edits.
10. The **course-pathway receipt**, `research/frontier-16-pathway.json`, written by
...
half, `research/frontier-16-alpha-<g>-6b-findings.json` — a JSON array with one row
...
`research/frontier-16-dispatch/` and read the LATEST prior result and log for your
...
run: frontier-16
role: alpha
label: contract-audit-2

# Contract-detector adjudication, run `frontier-16`
...
```
node tools/boundary-audit.mjs research/frontier-16-batch-*.proof-contracts.json --fail-on-contradicted --fail-on-template
node tools/citation-fidelity.mjs research/frontier-16-proof-contracts.json --fail-on-missing-quote
node tools/gate-liveness.mjs --run frontier-16 --contracts research/frontier-16-proof-contracts.json --checklists research/frontier-16-batch-1.coverage.json,research/frontier-16-batch-2.coverage.json,research/frontier-16-batch-3.coverage.json,research/frontier-16-batch-4.coverage.json,research/frontier-16-batch-5.coverage.json,research/frontier-16-batch-6.coverage.json,research/frontier-16-batch-7.coverage.json --min-checks 1
```
...
`research/frontier-16-batch-<i>.proof-contracts.json` entry. Then re-merge and
...
why. Write `research/frontier-16-alpha-contract-audit.md`: per candidate, the
