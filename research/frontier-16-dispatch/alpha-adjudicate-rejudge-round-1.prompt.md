...
run: frontier-16
role: alpha
label: adjudicate-rejudge-round-1
covers: all

# Step 8 — fatal-only adjudication of the paired-judge verdicts, run `frontier-16`
...
```
node tools/touchlog.mjs snap research/frontier-16-touches.json pre-step8
```
...
For every rejection in `research/frontier-16-judge.jsonl`:

1. Read the frozen verdict and the current text on disk.
2. Record in `research/frontier-16-judge-adjudications.jsonl`:
   `{id, model, context_sha256, item_sha256, outcome, defect_type?}` —
...
`research/frontier-16-alpha-step8.md`: every rejection, its adjudication, the
