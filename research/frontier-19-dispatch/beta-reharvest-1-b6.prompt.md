...
run: frontier-19
role: beta
label: reharvest-1-b6
covers: 6

> **Re-harvest round.** Your batch number is in the "This dispatch" section
> appended below; substitute it wherever `6` appears.

# Batch `6` of run `frontier-19` — a source is gone; its results are not
...
```
node tools/source-backing.mjs --coverage research/frontier-19-batch-6.coverage.json \
...
node tools/url-sweep.mjs --coverage research/frontier-19-batch-6.coverage.json --recover --fail-on-dead
node tools/source-fetch-check.mjs --coverage research/frontier-19-batch-6.coverage.json --stamp
```

## Report

Append `## Re-harvest round` to `research/frontier-19-batch-6.notes.md`: the dead
...
`research/frontier-19-batch-6.coverage.json` and `.notes.md`. Keep `original_url`
