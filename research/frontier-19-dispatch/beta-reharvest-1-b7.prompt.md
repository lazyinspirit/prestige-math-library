<!-- Dispatched as-is. `frontier-19` and `7` are substituted by tools/dispatch.mjs
...
# Scaffolding brief — run `frontier-19`, batch `7` (steps 1 and 2)
...
You are **Beta-7**, running **Claude Opus 5 via the claude CLI
...
`research/frontier-19-batch-7.notes.md`: current substage, owned artifact paths,
...
- `research/frontier-19-batch-7.pages.json` — your batch's page objects in final
...
- `research/frontier-19-batch-7.notes.md` — prose-scaffold amendments as precise
  ...
  route, and necessity.
- `research/frontier-19-batch-7.proof-contracts.json` — the version-1
...
run: frontier-19
role: beta
label: reharvest-1-b7
covers: 7

> **Re-harvest round.** Your batch number is in the "This dispatch" section
> appended below; substitute it wherever `7` appears.

# Batch `7` of run `frontier-19` — a source is gone; its results are not
...
```
node tools/source-backing.mjs --coverage research/frontier-19-batch-7.coverage.json \
...
node tools/url-sweep.mjs --coverage research/frontier-19-batch-7.coverage.json --recover --fail-on-dead
node tools/source-fetch-check.mjs --coverage research/frontier-19-batch-7.coverage.json --stamp
```

## Report

Append `## Re-harvest round` to `research/frontier-19-batch-7.notes.md`: the dead
...
`research/frontier-19-batch-7.coverage.json` and `.notes.md`. Keep `original_url`
