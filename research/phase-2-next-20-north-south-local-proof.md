# Direct uniform north–south product argument

Root completed and installed the argument, with exact contracts and current owner repaired decision. Explicit precheck/render, strict three-item contracts,36-item content policy and batch citation fidelity pass. This is local operator work, not independent review. The quantitative bound from the pole proof is exposed explicitly in its Definition so this consumer does not hide a proof-only dependency.
