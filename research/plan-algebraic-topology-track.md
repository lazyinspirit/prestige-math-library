...
- `def-fiber-transport-and-monodromy-action` — **definition** — for a Hurewicz fibration, path transport gives unbased fiber equivalences and the homology local system. Homotopy groups have moving basepoints; a fixed-group action is asserted only with the explicit path-connected/simple-action hypothesis and endpoint-correction proof. For: precise local coefficients, without an unqualified fixed-based action. **Provenance:** statement `literature-derived`; proof `not-applicable`. **Source locator:** May, *A Concise Course in Algebraic Topology*, Ch. 7 and Ch. 9 §§3--5, pp. 49--56, 68--72.
- `prop-fibers-over-one-path-component-are-fiber-homotopy-equivalent` — **proposition** — for Hurewicz fibrations, transport along a path supplies an equivalence independent up to homotopy of endpoint-fixed path homotopy. For Serre fibrations retain the sharp weak-equivalence zigzag through the interval pullback; the refined Cantor-cylinder counterexample refutes general fiber homotopy equivalence. For: exact hypotheses in reducing computations to one fiber. **Provenance:** statement `literature-derived`; proof `ai-altered`. **Source locator:** May, *A Concise Course in Algebraic Topology*, Ch. 7 and Ch. 9 §§3--5, pp. 49--56, 68--72.
