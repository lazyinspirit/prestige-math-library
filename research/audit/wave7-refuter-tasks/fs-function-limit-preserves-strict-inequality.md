...
## Target item — `fs-function-limit-preserves-strict-inequality`

Normalized current SHA-256: `85e2150c541b0c58ac474f26d86e1d3117b7fafefc6f0faf95ed5f87ebbc779d`
...
```json
{
  ...
  "rationale": "Lebl’s order theorem preserves only non-strict inequalities, and standard positive functions may tend to zero. The local false statement is the corresponding strict strengthening with its elementary witness.",
...
```json
{
  ...
  "derivations": [
    {
      "id": "step-1-1",
      "claim": "The point $0$ is a limit point of $\\mathbb{R}$.",
      "step": "1.1",
      "inputs": [
        "L2"
      ]
    ...
    {
      "id": "step-1-2",
      "claim": "The strict hypothesis holds with $\\eta := 1$: every $x \\in \\mathbb{R}$ with $0 < |x - 0| < 1$ has $|x| \\ne 0$, hence $|x| > 0 = f(x)$, that is $f(x) < g(x)$.",
      "step": "1.2",
      "inputs": [
        "L3",
        "L4"
    ...
    {
      "id": "step-2-1",
      "claim": "Both limits exist and are equal to $0$. For $f$: $|f(x) - 0| = 0 < \\varepsilon$ for every $x$ and every real $\\varepsilon > 0$, any $\\delta$ serving. For $g$: given a real $\\varepsilon > 0$ take $\\delta := \\varepsilon$; every $x$ with $0 < |x - 0| < \\delta$ satisfies $\\bigl| |x| - 0 \\bigr| = |x| < \\varepsilon$.",
      "step": "2.1",
      "inputs": [
        "1.1",
        "L1",
    ...
    {
      "id": "step-3-1",
      "claim": "So $f(x) < g(x)$ throughout a punctured neighbourhood of $0$ while $\\lim_{x \\to 0} f(x) = 0 = \\lim_{x \\to 0} g(x)$; the asserted strict inequality $0 < 0$ is impossible by trichotomy, so the claim is false.",
      "step": "3.1",
      "inputs": [
        "1.2",
        "2.1",
  ...
  "boundaries": [
    ...
    },
    {
      "case": "zero",
      "status": "checked",
      "evidence": "step 1.1: The point $0$ is a limit point of $\\mathbb{R}$."
    },
    {
      "case": "one",
      "status": "checked",
      "evidence": "step 1.2: The strict hypothesis holds with $\\eta := 1$: every $x \\in \\mathbb{R}$ with $0 < |x - 0| < 1$ has $|x| \\ne 0$, hence $|x| > 0 = f(x)$, that is $f(x) < g(x)$."
    },
    {
      "case": "degenerate",
      "status": "checked",
      "evidence": "step 2.1: Both limits exist and are equal to $0$. For $f$: $|f(x) - 0| = 0 < \\varepsilon$ for every $x$ and every real $\\varepsilon > 0$, any $\\delta$ serving. For $g$: given a real $\\varepsilon > 0$ take $\\delta := \\varepsilon$; every $x$ with $0 < |x - 0| < \\delta$ sa"
    },
    {
      "case": "endpoints",
      "status": "not_applicable",
  ...
  "risk_review": {
    ...
    "notes": "critical risk (8): 6 declared dependencies; 5 cited facts; boundary-sensitive language; analytic limiting/completeness language, routed by risk-report.mjs. The read-only DeepSeek V4 Pro audit-refuter received the complete target and every cited dependency and returned CLEAN. Boundary evidence: The counterexample (A=ℝ, c=0, f(x)=0, g(x)=|x|) satisfies all hypotheses: c is a limit point of A, the punctured neighbourhood condition holds with η=1, limits exist and equal 0, so the strict conclusion would force 0<0. No empty, zero, degenerate, or endpoint boundary issues affect the refutation. Checked surface: The title, the public claim, all numbered steps (1.1, 1.2, 2.1, 3.1), and the Remarks were read and verified; no mathematical falsehood, invalid inference, or missing hypothesis was detected. ``` Alpha adjudicated the return against the current item and cited dependencies on disk and found no fatal mathematical or dependency-citation defect. Evidence: research/audit/wave7-preserved-refuters/fs-function-limit-preserves-strict-inequality--efb6da4072d77c32.result.json."
  }
}
```
...
[
  {
    "source": "fs-function-limit-preserves-strict-inequality",
