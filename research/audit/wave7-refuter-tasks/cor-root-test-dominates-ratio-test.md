...
- critical risk (13): 13 declared dependencies; 13 cited facts; 7 numbered proof steps; biconditional / both-direction claim; boundary-sensitive language; analytic limiting/completeness language

## Target item — `cor-root-test-dominates-ratio-test`

Normalized current SHA-256: `a368416aad2cb954177ccc0af47fd66639687a3d5d4eec916ae9f4a1ecbe2cf4`
...
```json
{
  ...
  "rationale": "The CSUDH notes give the full ratio-root liminf/limsup chain, state that the root test is strictly stronger, and use the same sequence 2^{(-1)^n-n} as witness. The item shifts indices and expands both convergence and divergence translations through local test statements. Its proof follows the sourced inequality-and-witness argument.",
...
```json
{
  ...
  "derivations": [
    {
      "id": "step-1-1",
      "claim": "Put $b_k := |a_k|$. Since $a_k \\ne 0$ we have $b_k > 0$ for every $k$, so [L1] applies to $(b_k)$.",
      "step": "1.1",
      "inputs": [
        "L1",
        "L3",
    ...
    {
      "id": "step-1-2",
      "claim": "For the sequence $a_k = 2^{-k} t_k$ of the Statement every term is nonzero, $\\limsup_k \\rho_k = 1/2 < 1$, and neither $\\limsup_k q_k = 2 < 1$ nor $\\liminf_k q_k = 1/8 > 1$ holds.",
      "step": "1.2",
      "inputs": [
        "L7"
      ]
    ...
    {
      "id": "step-2-1",
      "claim": "For this $(b_k)$ the ratio family is $b_{k+1}/b_k = |a_{k+1}|/|a_k| = q_k$ and the root family is $b_{k+1}^{1/(k+1)} = |a_{k+1}|^{1/(k+1)} = \\rho_k$.",
      "step": "2.1",
      "inputs": [
        "1.1"
      ]
    ...
    {
      "id": "step-3-1",
      "claim": "Therefore $\\liminf_k q_k \\le \\liminf_k \\rho_k \\le \\limsup_k \\rho_k \\le \\limsup_k q_k$, which is the displayed chain.",
      "step": "3.1",
      "inputs": [
        "1.1",
        "2.1",
    ...
    {
      "id": "step-4-1",
      "claim": "Suppose $\\limsup_k q_k < 1$. By the chain, $\\limsup_k \\rho_k \\le \\limsup_k q_k < 1$, so the root test applies to the family $(a_k)_{k \\ge 1}$ and gives convergence of $\\sum_{k \\ge 1}|a_k|$ and of $\\sum_{k \\ge 1} a_k$, hence of $\\sum |a_k|$ and of $\\sum a_k$; the ratio test gives the same conclusions. That is claim 1.",
      "step": "4.1",
      "inputs": [
        "3.1",
        "L4",
    ...
    {
      "id": "step-4-2",
      "claim": "Suppose $\\liminf_k q_k > 1$. By the chain and [L2], $\\limsup_k \\rho_k \\ge \\liminf_k \\rho_k \\ge \\liminf_k q_k > 1$, so the root test gives divergence of $\\sum_{k \\ge 1} a_k$, hence of $\\sum a_k$; the ratio test gives the same conclusion. That is claim 2.",
      "step": "4.2",
      "inputs": [
        "L2",
        "3.1",
    ...
    {
      "id": "step-5-1",
      "claim": "So for that sequence the root test gives convergence of $\\sum_{k \\ge 1} |a_k|$ while neither half of the ratio test applies, and the converse of claims 1 and 2 fails.",
      "step": "5.1",
      "inputs": [
        "1.2",
        "L4",
  ...
  "boundaries": [
    ...
    },
    {
      "case": "zero",
      "status": "checked",
      "evidence": "Statement excludes zero a_k; step 1.1 then makes b_k = |a_k| strictly positive before applying the ratio-root inequality."
    },
    {
      "case": "one",
      "status": "checked",
      "evidence": "steps 4.1-4.2 use strict comparison with 1, while step 1.2 checks the witness whose ratio bounds straddle 1."
    },
    {
      "case": "degenerate",
      "status": "checked",
      "evidence": "step 1.2 checks the oscillating witness directly; no convergence of the ratio family is assumed."
    },
    {
      "case": "endpoints",
      "status": "checked",
      "evidence": "steps 3.1-4.2 preserve all four liminf/limsup directions and both strict test thresholds."
    },
    {
  ...
  "risk_review": {
    ...
    "notes": "critical risk (13): 13 declared dependencies; 13 cited facts; 7 numbered proof steps; biconditional / both-direction claim; boundary-sensitive language; analytic limiting/completeness language, routed by risk-report.mjs. The read-only DeepSeek V4 Pro audit-refuter received the complete target and every cited dependency and returned CLEAN. Boundary evidence: The target explicitly excludes zero terms (a_k ≠ 0), so b_k = |a_k| > 0 and the ratio‑root inequality applies. Strict thresholds (< 1, > 1) are used everywhere and are preserved by the chain and the tests. The witness a_k = 2^{−k}(−1)^k is correctly computed, its root limit = 1/2 < 1 while its ratios oscillate between 1/8 and 2, so the root test decides convergence and the ratio test is silent. The index shift in the root family (ρ_k = |a_{k+1}|^{1/(k+1)}) is exactly the convention of the cited root test and inequality; no off‑by‑one or missing term issue exists. Divergence via root test uses limsup > 1, and the chain plus liminf ≤ limsup guarantees limsup ρ_k > 1 when liminf q_k > 1. Tail invariance (lem‑series‑tail‑invariance) correctly lifts conclusions from the 1‑tail to the full series. Checked surface: Read the title, the full public statement, every numbered proof step (1.1–5.1), and all Remarks. ``` Alpha adjudicated the return against the current item and cited dependencies on disk and found no fatal mathematical or dependency-citation defect. Evidence: research/audit/wave7-preserved-refuters/cor-root-test-dominates-ratio-test--b4e8767682c7a671.result.json."
  }
}
```
...
[
  {
    "source": "cor-root-test-dominates-ratio-test",
