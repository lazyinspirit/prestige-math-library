...
- critical risk (12): 17 declared dependencies; 19 cited facts; biconditional / both-direction claim; boundary-sensitive language; analytic limiting/completeness language
- top-20 dependency-cone consumer (17 generated manifest edges)

## Target item — `cex-dirichlet-has-no-limit-anywhere`

Normalized current SHA-256: `9be60fb38d0cf896ea07901443f39fa051c719c0497ab24d4c60d7db32620c14`
...
```json
{
  ...
  "rationale": "The source defines the rational indicator and states that it is discontinuous everywhere because rationals and irrationals are dense. The local item restates this as nonexistence of every punctured limit and proves that semantically equivalent form with rational and irrational sequences.",
...
```json
{
  ...
  "derivations": [
    {
      "id": "step-1-1",
      "claim": "Let $c \\in \\mathbb{R}$ be arbitrary. Then $c$ is a limit point of $\\mathbb{R}$, the domain of $\\mathbf{1}_{\\mathbb{Q}}$, so the question of a limit at $c$ is well posed.",
      "step": "1.1",
      "inputs": [
        "L5"
      ]
    ...
    {
      "id": "step-1-2",
      "claim": "Let $S$ be either $\\mathbb{Q}$ or $X$, and let $\\varepsilon > 0$ be an arbitrary real. Applying [L1] at the real $c + \\varepsilon/2$ with the radius $\\varepsilon/2 > 0$, the neighbourhood $N_{\\varepsilon/2}(c + \\varepsilon/2)$ meets $S$; and by [L3] every $y$ in that neighbourhood satisfies $c < y < c + \\varepsilon$, hence $y \\ne c$ and $0 < |y - c| < \\varepsilon$. So every neighbourhood of $c$ meets $S \\setminus \\{c\\}$.",
      "step": "1.2",
      "inputs": [
        "L1",
        "L3",
    ...
    {
      "id": "step-2-1",
      "claim": "By [L1] again, step 1.2 says exactly that $c$ lies in the closure of $\\mathbb{Q} \\setminus \\{c\\}$ and in the closure of $X \\setminus \\{c\\}$. Hence [L2] supplies a sequence $(q_k)$ with all terms in $\\mathbb{Q} \\setminus \\{c\\}$ converging to $c$, and a sequence $(u_k)$ with all terms in $X \\setminus \\{c\\}$ converging to $c$.",
      "step": "2.1",
      "inputs": [
        "L1",
        "1.2",
    ...
    {
      "id": "step-3-1",
      "claim": "Every term of $(q_k)$ lies in $\\mathbb{Q}$, so $\\mathbf{1}_{\\mathbb{Q}}(q_k) = 1$ for every $k$ and the image sequence is the constant sequence $1$, converging to $1$; every term of $(u_k)$ lies in $X$, so $\\mathbf{1}_{\\mathbb{Q}}(u_k) = 0$ for every $k$ and that image sequence converges to $0$. The reals $1$ and $0$ are distinct.",
      "step": "3.1",
      "inputs": [
        "2.1",
        "L6",
    ...
    {
      "id": "step-4-1",
      "claim": "Both sequences have all their terms in $\\mathbb{R} \\setminus \\{c\\}$ and converge to $c$, while their image sequences converge to distinct reals; by [L4] the function $\\mathbf{1}_{\\mathbb{Q}}$ has no limit at $c$. Since $c \\in \\mathbb{R}$ was arbitrary, it has a limit at no point of $\\mathbb{R}$.",
      "step": "4.1",
      "inputs": [
        "L4",
        "1.1",
  ...
  "boundaries": [
    ...
    },
    {
      "case": "zero",
      "status": "checked",
      "evidence": "step 1.2: Let $S$ be either $\\mathbb{Q}$ or $X$, and let $\\varepsilon > 0$ be an arbitrary real. Applying [L1] at the real $c + \\varepsilon/2$ with the radius $\\varepsilon/2 > 0$, the neighbourhood $N_{\\varepsilon/2}(c + \\varepsilon/2)$ meets $S$; and by [L3] every $y$ "
    },
    {
      "case": "one",
      "status": "checked",
      "evidence": "step 1.1: Let $c \\in \\mathbb{R}$ be arbitrary. Then $c$ is a limit point of $\\mathbb{R}$, the domain of $\\mathbf{1}_{\\mathbb{Q}}$, so the question of a limit at $c$ is well posed."
    },
    {
      "case": "degenerate",
      "status": "checked",
      "evidence": "step 3.1: Every term of $(q_k)$ lies in $\\mathbb{Q}$, so $\\mathbf{1}_{\\mathbb{Q}}(q_k) = 1$ for every $k$ and the image sequence is the constant sequence $1$, converging to $1$; every term of $(u_k)$ lies in $X$, so $\\mathbf{1}_{\\mathbb{Q}}(u_k) = 0$ for every $k$ and t"
    },
    {
      "case": "endpoints",
      "status": "not_applicable",
  ...
  "risk_review": {
    ...
    "notes": "critical risk (12): 17 declared dependencies; 19 cited facts; biconditional / both-direction claim; boundary-sensitive language; analytic limiting/completeness language, routed by risk-report.mjs. The read-only DeepSeek V4 Pro audit-refuter received the complete target and every cited dependency and returned CLEAN. Boundary evidence: The proof works for every real c. Step 1.1 verifies c is a limit point of ℝ, so the limit question is well‑posed. Step 1.2 constructs, for each ε>0, a point of S\\{c} (S = ℚ or X) lying in (c, c+ε), which shows every neighbourhood of c meets S\\{c}. This gives c ∈ closure(S\\{c}) without needing left‑side symmetry. The sequential criterion (L4) applies because ℝ is the domain and the supplied sequences avoid c. No special cases (c=0, c rational/irrational, etc.) break the reasoning, and no counterexample to the refuted claim exists. Checked surface: The title, the public Statement refuted, every numbered proof step (1.1 through 4.1), and the Remarks were read and verified. ``` Alpha adjudicated the return against the current item and cited dependencies on disk and found no fatal mathematical or dependency-citation defect. Evidence: research/audit/wave7-preserved-refuters/cex-dirichlet-has-no-limit-anywhere--24d97efb59e14ba9.result.json."
  }
}
```
...
[
  {
    "source": "cex-dirichlet-has-no-limit-anywhere",
