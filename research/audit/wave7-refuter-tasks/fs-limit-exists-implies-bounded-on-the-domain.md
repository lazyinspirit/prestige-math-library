...
- critical risk (9): 16 declared dependencies; 15 cited facts; boundary-sensitive language; analytic limiting/completeness language

## Target item — `fs-limit-exists-implies-bounded-on-the-domain`

Normalized current SHA-256: `0edbab7827b34a545547d879fecfac8f1572256b192cd5c5a9256cb6158aa270`
...
```json
{
  ...
  "rationale": "Lebl supplies only local boundedness near the limit point. The local false statement strengthens that to global boundedness and uses the standard 1/x-on-(0,infinity) witness at c=1.",
...
```json
{
  ...
  "derivations": [
    {
      "id": "step-1-1",
      "claim": "The point $1$ lies in $A = (0,\\infty)$ and is a limit point of $A$: given a real $\\varepsilon > 0$, let $\\rho$ be the smaller of $\\varepsilon$ and $1$, so $\\rho > 0$; then $1 + \\rho/2 > 1 > 0$ lies in $A$ and satisfies $0 < |(1 + \\rho/2) - 1| = \\rho/2 < \\varepsilon$.",
      "step": "1.1",
      "inputs": [
        "L2",
        "L3",
    ...
    {
      "id": "step-1-2",
      "claim": "$f$ is well defined on $A$: every $x \\in A$ has $x > 0$, hence $x \\ne 0$ and $x^{-1}$ exists, with $x^{-1} > 0$.",
      "step": "1.2",
      "inputs": [
        "L4",
        "L7"
    ...
    {
      "id": "step-2-1",
      "claim": "The limit of $f$ at $1$ exists and equals $1$. Let $\\varepsilon > 0$ be an arbitrary real and let $\\delta$ be the smaller of $1/2$ and $\\varepsilon/2$, so $\\delta > 0$. For $x \\in A$ with $0 < |x - 1| < \\delta$ we get $x > 1 - 1/2 = 1/2 > 0$, hence $0 < 1/x < 2$ by [L4]; and $|1/x - 1| = |(1 - x)/x| = |x - 1| \\cdot (1/x) < \\delta \\cdot 2 \\le \\varepsilon$.",
      "step": "2.1",
      "inputs": [
        "L4",
        "1.1",
    ...
    {
      "id": "step-2-2",
      "claim": "The image $f(A)$ has no upper bound. Let $M$ be an arbitrary real; by [L6] fix a natural $n \\ge 1$ with $M < n \\cdot 1_{\\mathbb{R}}$, and note $n \\cdot 1_{\\mathbb{R}} > 0$. Then $x := (n \\cdot 1_{\\mathbb{R}})^{-1}$ satisfies $x > 0$, so $x \\in A$, and $f(x) = x^{-1} = n \\cdot 1_{\\mathbb{R}} > M$. So no real bounds $f(A)$ above, and $f(A)$ is not bounded.",
      "step": "2.2",
      "inputs": [
        "L6",
        "1.2",
    ...
    {
      "id": "step-3-1",
      "claim": "So $f$ has a limit at the limit point $c = 1$ of its domain and is unbounded on that domain: the claim is false, while [[lem-limit-implies-local-boundedness]] remains true and gives boundedness on $A \\cap N^{*}_{1/2}(1)$, where indeed $0 < f(x) < 2$ by step 2.1.",
      "step": "3.1",
      "inputs": [
        "2.1",
        "2.2",
  ...
  "boundaries": [
    ...
    },
    {
      "case": "zero",
      "status": "checked",
      "evidence": "step 1.1: The point $1$ lies in $A = (0,\\infty)$ and is a limit point of $A$: given a real $\\varepsilon > 0$, let $\\rho$ be the smaller of $\\varepsilon$ and $1$, so $\\rho > 0$; then $1 + \\rho/2 > 1 > 0$ lies in $A$ and satisfies $0 < |(1 + \\rho/2) - 1| = \\rho/2 < \\varep"
    },
    {
      "case": "one",
      "status": "checked",
    ...
    },
    {
      "case": "degenerate",
      "status": "checked",
      "evidence": "step 2.1: The limit of $f$ at $1$ exists and equals $1$. Let $\\varepsilon > 0$ be an arbitrary real and let $\\delta$ be the smaller of $1/2$ and $\\varepsilon/2$, so $\\delta > 0$. For $x \\in A$ with $0 < |x - 1| < \\delta$ we get $x > 1 - 1/2 = 1/2 > 0$, hence $0 < 1/x < "
    },
    {
      "case": "endpoints",
      "status": "not_applicable",
  ...
  "risk_review": {
    ...
    "notes": "critical risk (9): 16 declared dependencies; 15 cited facts; boundary-sensitive language; analytic limiting/completeness language, routed by risk-report.mjs. The read-only DeepSeek V4 Pro audit-refuter received the complete target and every cited dependency and returned CLEAN. Boundary evidence: Zero‑case (c=1, δ>0, ε‑δ argument for arbitrarily small ε) verified. One‑case (c=1 is a limit point of A and belongs to A) explicitly checked; the construction in step 1.1 works for every ε>0. Degenerate‑domain (A is non‑degenerate; the limit point is not isolated) not an issue. Endpoints (domain (0,∞) is unbounded; the unboundedness argument uses no special endpoint logic). Nonempty‑choice: all existence claims are constructive (Archimedean property gives n; field axioms give the inverse). Iff‑reverse/forward not applicable (no biconditional asserted). All boundary checks pass. Checked surface: The title, the public false claim, the full refutation (steps 1.1, 1.2, 2.1, 2.2, 3.1), and the three Remarks were read and found mathematically correct and logically valid. The counterexample f(x)=1/x on (0,∞) at c=1 satisfies all hypotheses of the false claim and violates its conclusion, so the refutation is sound. Alpha adjudicated the return against the current item and cited dependencies on disk and found no fatal mathematical or dependency-citation defect. Evidence: research/audit/wave7-preserved-refuters/fs-limit-exists-implies-bounded-on-the-domain--b39e576ba31e68d6.result.json."
  }
}
```
...
[
  {
    "source": "fs-limit-exists-implies-bounded-on-the-domain",
