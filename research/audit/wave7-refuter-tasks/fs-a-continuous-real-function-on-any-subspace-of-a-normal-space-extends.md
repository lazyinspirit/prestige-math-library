...
- critical risk (12): 12 declared dependencies; 8 cited facts; 7 numbered proof steps; boundary-sensitive language; quotient or equivalence-class construction; analytic limiting/completeness language

## Target item — `fs-a-continuous-real-function-on-any-subspace-of-a-normal-space-extends`

Normalized current SHA-256: `68db09d7debb80d30825c0b828b6ba07699956589484bbc3059abc9838506dd5`
...
```json
{
  ...
  "rationale": "Tietze requires a closed subspace; this item turns the standard missing hypothesis into a false universal and uses the classical 1/x obstruction.",
...
```json
{
  "citations": [
    {
      "fact": "L1",
      "source": "thm-metric-spaces-are-completely-normal",
  ...
  "derivations": [
    {
      "id": "step-1.1",
      "claim": "$f$ is continuous on $A$, by [L2] with $0 \\notin A$; and $\\mathbb{R}$ is normal, by [L1]. [given, L1, L2]",
      "step": "1.1",
      "inputs": [
        "given",
        "L1",
    ...
    {
      "id": "step-1.2",
      "claim": "For every real $M$, there is $x \\in A$ with $f(x) > M$: if $M \\le 0$, take $x:=1$, so $f(1)=1>0\\ge M$; if $M>0$, [L6] applied to $\\varepsilon := 1/(M+1) > 0$ gives a natural $n \\ge 1$ with $1/n < 1/(M+1)$, hence $n > M+1 > M$; taking $x := 1/n \\in (0,1] = A$ gives $f(x) = 1/x = n > M$. [given, L6, algebra, choose]",
      "step": "1.2",
      "inputs": [
        "given",
        "L6",
    ...
    {
      "id": "step-1.3",
      "claim": "Suppose, toward a contradiction, that a continuous $F : \\mathbb{R} \\to \\mathbb{R}$ exists with $F|_A = f$. [assume-contra]",
      "step": "1.3",
      "inputs": [
        "assume-contra"
      ]
    ...
    {
      "id": "step-2.1",
      "claim": "Under step 1.3: $F|_{[0,1]}$ is continuous, by [L3] applied to $F$ on $\\mathbb{R} \\supseteq [0,1]$. [step 1.3, L3]",
      "step": "2.1",
      "inputs": [
        "step 1.3",
        "L3",
    ...
    {
      "id": "step-3.1",
      "claim": "Under step 1.3: by [L5] applied to $F|_{[0,1]}$ (step 2.1) and $K := [0,1]$ (step 1.4), fix a real $M_0 \\ge 0$ with $|F(x)| \\le M_0$ for every $x \\in [0,1]$. [step 2.1, step 1.4, L5, choose]",
      "step": "3.1",
      "inputs": [
        "step 2.1",
        "step 1.4",
    ...
    {
      "id": "step-4.1",
      "claim": "Under step 1.3: for $x \\in A$, $F(x) = f(x)$ (step 1.3) and $x \\in [0,1]$, so $f(x) \\le |F(x)| \\le M_0$ by step 3.1; but step 1.2 applied with $M := M_0$ gives $x_0 \\in A$ with $f(x_0) > M_0$, contradicting $f(x_0) \\le M_0$. [step 1.3, step 3.1, step 1.2, discharge-contradiction] ∎",
      "step": "4.1",
      "inputs": [
        "step 1.3",
        "step 3.1",
  ...
  "risk_review": {
    ...
    "notes": "critical risk (12): 12 declared dependencies; 8 cited facts; 7 numbered proof steps; boundary-sensitive language; quotient or equivalence-class construction; analytic limiting/completeness language, routed by risk-report.mjs. The read-only DeepSeek V4 Pro audit-refuter received the complete target and every cited dependency and returned CLEAN. Boundary evidence: The refutation uses a nonempty subspace \\(A=(0,1]\\); the empty subspace trivially satisfies the universal statement but does not rescue it, as a single counterexample suffices. The unboundedness argument treats the case \\(M\\le 0\\) (taking \\(x=1\\)) separately from \\(M>0\\), covering all reals. The endpoint \\(0\\) is not in \\(A\\) and the extension is required to be continuous on all of \\(\\mathbb{R}\\), so no continuity condition on \\(f\\) at \\(0\\) is needed. Compactness of \\([0,1]\\) is correctly applied, and the boundedness theorem is invoked on the precise domain \\([0,1]\\). No non‑empty choice is used; the construction of \\(x_0\\) in step 1.2 is explicit from the Archimedean property. Checked surface: The title, the public claim (the universal statement is false), every numbered proof step (1.1 – 4.1), and the two Remarks were read in full and found to be mathematically correct. No false assertion, invalid inference, or missing necessary hypothesis was detected. Alpha adjudicated the return against the current item and cited dependencies on disk and found no fatal mathematical or dependency-citation defect. Evidence: research/audit/wave7-preserved-refuters/fs-a-continuous-real-function-on-any-subspace-of-a-normal-space-extends--3c04e811d0814257.result.json."
  }
}
```
...
[
  {
    "source": "fs-a-continuous-real-function-on-any-subspace-of-a-normal-space-extends",
