...
- critical risk (17): 10 declared dependencies; 12 cited facts; 7 numbered proof steps; biconditional / both-direction claim; existence, choice, uniqueness, or well-definedness; boundary-sensitive language; quotient or equivalence-class construction; analytic limiting/completeness language

## Target item — `lem-sign-preservation-near-a-limit`

Normalized current SHA-256: `877be8e4cd95dcca6d6e365222e375e136a75d91ff3d1316958ed727fe58eee2`
...
```json
{
  ...
  "rationale": "The standard limit inequalities in Lebl give eventual sign preservation and a positive lower bound away from zero for a nonzero limit. The local statement packages both signs and the limit-point consequence for the nonzero domain.",
...
```json
{
  ...
  "derivations": [
    {
      "id": "step-1-1",
      "claim": "Since $L \\ne 0$ we have $|L| \\ne 0$ while $|L| \\ge 0$, so trichotomy gives $|L| > 0$, and $\\varepsilon := |L|/2 > 0$ with $|L| - |L|/2 = |L|/2$.",
      "step": "1.1",
      "inputs": [
        "L2",
        "L5",
    ...
    {
      "id": "step-2-1",
      "claim": "Apply [L1] with this $\\varepsilon$: fix a real $\\delta > 0$ such that every $x \\in A$ with $0 < |x - c| < \\delta$ satisfies $|f(x) - L| < |L|/2$.",
      "step": "2.1",
      "inputs": [
        "L1",
        "1.1",
    ...
    {
      "id": "step-3-1",
      "claim": "For every such $x$ the reverse triangle inequality gives $\\bigl| |f(x)| - |L| \\bigr| \\le |f(x) - L| < |L|/2$, hence $|f(x)| - |L| > -|L|/2$ and so $|f(x)| > |L| - |L|/2 = |L|/2 > 0$; in particular $|f(x)| \\ne 0$ and therefore $f(x) \\ne 0$.",
      "step": "3.1",
      "inputs": [
        "2.1",
        "L2",
    ...
    {
      "id": "step-3-2",
      "claim": "If $L > 0$ then $|L| = L$, and for every such $x$ the estimate $|f(x) - L| < L/2$ gives $-L/2 < f(x) - L$, that is $f(x) > L - L/2 = L/2 > 0$.",
      "step": "3.2",
      "inputs": [
        "2.1",
        "L2",
    ...
    {
      "id": "step-3-3",
      "claim": "If $L < 0$ then $|L| = -L$, and for every such $x$ the estimate $|f(x) - L| < -L/2$ gives $f(x) - L < -L/2$, that is $f(x) < L - L/2 = L/2 < 0$.",
      "step": "3.3",
      "inputs": [
        "2.1",
        "L2",
    ...
    {
      "id": "step-4-1",
      "claim": "Let $\\eta > 0$ be an arbitrary real and let $\\rho$ be the smaller of $\\delta$ and $\\eta$, so $\\rho > 0$. Since $c$ is a limit point of $A$ there is $x \\in A$ with $0 < |x - c| < \\rho$; that $x$ satisfies $0 < |x - c| < \\delta$, hence $f(x) \\ne 0$ by step 3.1, so $x \\in A_0$ and $0 < |x - c| < \\eta$. As $\\eta$ was arbitrary, $c$ is a limit point of $A_0$.",
      "step": "4.1",
      "inputs": [
        "3.1",
        "L4",
    ...
    {
      "id": "step-5-1",
      "claim": "So on $A \\cap N^{*}_{\\delta}(c)$ the function is bounded away from $0$ by $|L|/2$ and carries the sign of $L$, and $c$ remains a limit point of the set $A_0$ where $f$ does not vanish.",
      "step": "5.1",
      "inputs": [
        "3.1",
        "3.2",
  ...
  "boundaries": [
    ...
    },
    {
      "case": "zero",
      "status": "checked",
      "evidence": "step 1.1: Since $L \\ne 0$ we have $|L| \\ne 0$ while $|L| \\ge 0$, so trichotomy gives $|L| > 0$, and $\\varepsilon := |L|/2 > 0$ with $|L| - |L|/2 = |L|/2$."
    },
    {
      "case": "one",
      "status": "checked",
      "evidence": "step 2.1: Apply [L1] with this $\\varepsilon$: fix a real $\\delta > 0$ such that every $x \\in A$ with $0 < |x - c| < \\delta$ satisfies $|f(x) - L| < |L|/2$."
    },
    {
      "case": "degenerate",
      "status": "checked",
      "evidence": "step 3.1: For every such $x$ the reverse triangle inequality gives $\\bigl| |f(x)| - |L| \\bigr| \\le |f(x) - L| < |L|/2$, hence $|f(x)| - |L| > -|L|/2$ and so $|f(x)| > |L| - |L|/2 = |L|/2 > 0$; in particular $|f(x)| \\ne 0$ and therefore $f(x) \\ne 0$."
    },
    {
      "case": "endpoints",
      "status": "not_applicable",
    ...
    },
    {
      ...
      "status": "checked",
      "evidence": "step 2.1, step 4.1: all existence or selection moves were checked; any countable-choice use is explicitly tagged in the cited step."
    },
    {
      "case": "iff-forward",
      "status": "not_applicable",
  ...
  "risk_review": {
    ...
    "notes": "critical risk (17): 10 declared dependencies; 12 cited facts; 7 numbered proof steps; biconditional / both-direction claim; existence, choice, uniqueness, or well-definedness; boundary-sensitive language; quotient or equivalence-class construction; analytic limiting/completeness language, routed by risk-report.mjs. The read-only DeepSeek V4 Pro audit-refuter received the complete target and every cited dependency and returned CLEAN. Boundary evidence: Checked L≠0⇒|L|>0; positivity of |L|/2; application of the limit definition yields δ; reverse triangle inequality gives |f| > |L|/2; sign cases L>0 and L<0 handled correctly; limit-point property of A used to pick x with 0<|x−c|<min(δ,η); that x satisfies f(x)≠0, hence lies in A₀, proving c is a limit point of A₀. No counterexample or boundary failure found. Checked surface: The title, the full public statement, every numbered proof step (1.1, 2.1, 3.1, 3.2, 3.3, 4.1, 5.1), and the Remarks were read and found correct. ``` Alpha adjudicated the return against the current item and cited dependencies on disk and found no fatal mathematical or dependency-citation defect. Evidence: research/audit/wave7-preserved-refuters/lem-sign-preservation-near-a-limit--3b46349164025056.result.json."
  }
}
```
