...
- critical risk (12): 17 declared dependencies; 20 cited facts; 6 numbered proof steps; existence, choice, uniqueness, or well-definedness; boundary-sensitive language; analytic limiting/completeness language
- top-20 dependency-cone consumer (17 generated manifest edges)

## Target item — `thm-cesaro-mean-theorem`

Normalized current SHA-256: `a4b12ed7b8eee01c7f8966bc5a4d5ae31a12a5013744b2290888ede22ff7384f`
...
```json
{
  ...
  "rationale": "The source states Cauchy’s theorem that s_n tending to s implies its arithmetic averages tend to s, with the same quantifiers and conclusion. The numbered head-tail proof is local.",
...
```json
{
  ...
  "derivations": [
    {
      "id": "step-1-1",
      "claim": "Let $\\varepsilon > 0$ be an arbitrary real; choose $K \\in \\mathbb{N}$ with $|x_k - L| < \\varepsilon/2$ for every $k \\ge K$.",
      "step": "1.1",
      "inputs": [
        "L4",
        "L6",
    ...
    {
      "id": "step-1-2",
      "claim": "Put $C := \\sum_{k<K} |x_k - L|$, a real with $C \\ge 0$ because every summand is $\\ge 0$.",
      "step": "1.2",
      "inputs": [
        "L2",
        "L6"
    ...
    {
      "id": "step-2-1",
      "claim": "For every $n \\ge K$: since $\\sum_{k=0}^{n} L = (n+1)L$, one has $\\sigma_n - L = (n+1)^{-1}\\sum_{k=0}^{n}(x_k - L)$, hence $|\\sigma_n - L| \\le (n+1)^{-1}\\sum_{k=0}^{n}|x_k - L| = (n+1)^{-1}\\big(C + \\sum_{k=K}^{n}|x_k - L|\\big) \\le (n+1)^{-1}\\big(C + (n+1-K)(\\varepsilon/2)\\big) \\le C(n+1)^{-1} + \\varepsilon/2$, the last two steps using $|x_k - L| < \\varepsilon/2$ for $K \\le k \\le n$ and $n+1-K \\le n+1$.",
      "step": "2.1",
      "inputs": [
        "1.1",
        "1.2",
    ...
    {
      "id": "step-2-2",
      "claim": "Since $\\varepsilon\\,(2(C+1))^{-1} > 0$, choose a natural $m \\ge 1$ with $1/m < \\varepsilon\\,(2(C+1))^{-1}$; then for every $n \\ge m$ one has $n + 1 > m$, so $C(n+1)^{-1} \\le C/m \\le (C+1)/m < \\varepsilon/2$.",
      "step": "2.2",
      "inputs": [
        "1.2",
        "L5",
    ...
    {
      "id": "step-3-1",
      "claim": "Let $N$ be the larger of $K$ and $m$; for every $n \\ge N$ both estimates apply and $|\\sigma_n - L| < \\varepsilon/2 + \\varepsilon/2 = \\varepsilon$.",
      "step": "3.1",
      "inputs": [
        "2.1",
        "2.2",
    ...
    {
      "id": "step-4-1",
      "claim": "As $\\varepsilon > 0$ was arbitrary, $(\\sigma_n)$ converges to $L$, so $\\lim_n \\sigma_n$ exists and equals $L = \\lim_k x_k$.",
      "step": "4.1",
      "inputs": [
        "3.1",
        "L1",
  ...
  "boundaries": [
    ...
    },
    {
      "case": "zero",
      "status": "checked",
      "evidence": "step 1.1: Let $\\varepsilon > 0$ be an arbitrary real; choose $K \\in \\mathbb{N}$ with $|x_k - L| < \\varepsilon/2$ for every $k \\ge K$."
    },
    {
      "case": "one",
      "status": "checked",
      "evidence": "step 2.1: For every $n \\ge K$: since $\\sum_{k=0}^{n} L = (n+1)L$, one has $\\sigma_n - L = (n+1)^{-1}\\sum_{k=0}^{n}(x_k - L)$, hence $|\\sigma_n - L| \\le (n+1)^{-1}\\sum_{k=0}^{n}|x_k - L| = (n+1)^{-1}\\big(C + \\sum_{k=K}^{n}|x_k - L|\\big) \\le (n+1)^{-1}\\big(C + (n+1-K)(\\va"
    },
    {
      "case": "degenerate",
      "status": "checked",
      "evidence": "step 4.1: As $\\varepsilon > 0$ was arbitrary, $(\\sigma_n)$ converges to $L$, so $\\lim_n \\sigma_n$ exists and equals $L = \\lim_k x_k$."
    },
    {
      "case": "endpoints",
      "status": "not_applicable",
    ...
    },
    {
      ...
      "status": "checked",
      "evidence": "step 1.1, step 2.2: all existence or selection moves were checked; any countable-choice use is explicitly tagged in the cited step."
    },
    {
      "case": "iff-forward",
      "status": "checked",
  ...
  "risk_review": {
    ...
    "notes": "critical risk (12): 17 declared dependencies; 20 cited facts; 6 numbered proof steps; existence, choice, uniqueness, or well-definedness; boundary-sensitive language; analytic limiting/completeness language, routed by risk-report.mjs. The read-only DeepSeek V4 Pro audit-refuter received the complete target and every cited dependency and returned CLEAN. Boundary evidence: Checked empty head sum (K=0, C=0) where the C+1 trick avoids division by zero. Checked n=K (tail non‑empty). Constant sequence (K=0, C=0) works. The proof does not require the whole sequence to be bounded; the head sum is a finite constant. The denominator n+1 is always positive. All index shifts and finite‑sum splits are valid for ℕ including 0. Checked surface: The title, public statement, Facts & Assumptions, all six numbered proof steps (1.1–4.1), and the three Remarks were read and verified. ``` Alpha adjudicated the return against the current item and cited dependencies on disk and found no fatal mathematical or dependency-citation defect. Evidence: research/audit/wave7-preserved-refuters/thm-cesaro-mean-theorem--8ab0d3b33592cc62.result.json."
  }
}
```
...
[
  {
    "source": "thm-cesaro-mean-theorem",
