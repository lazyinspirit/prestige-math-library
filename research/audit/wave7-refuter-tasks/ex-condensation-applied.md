...
- critical risk (13): 12 declared dependencies; 13 cited facts; 7 numbered proof steps; biconditional / both-direction claim; boundary-sensitive language; analytic limiting/completeness language

## Target item — `ex-condensation-applied`

Normalized current SHA-256: `1af48cc8ac6e19fc11f0b1dce2a31474776302609c49368f2f3950ff10d586e5`
...
```json
{
  ...
  "rationale": "The sources apply condensation to the p-series and obtain the geometric ratio 2^{1-p}, with the threshold p>1. The item restricts to rational exponents, displays the algebra separately, and tabulates p=1/2,1,2. Its verification follows the sourced calculation under local rational-power notation.",
...
```json
{
  ...
  "derivations": [
    {
      "id": "step-1-1",
      "claim": "Each $a_k = \\iota(k)^{-p}$ is positive, and $a_j \\ge a_k$ whenever $1 \\le j \\le k$, since $\\iota(j)^{p} \\le \\iota(k)^{p}$ for $p > 0$ and reciprocation reverses the order; so condensation applies.",
      "step": "1.1",
      "inputs": [
        "L3",
        "L5",
    ...
    {
      "id": "step-1-2",
      "claim": "For every $j \\in \\mathbb{N}$: $2^{j} a_{2^{j}} = 2^{j}\\big(2^{j}\\big)^{-p} = 2^{j} \\cdot 2^{-jp} = 2^{\\,j - jp} = 2^{\\,(1-p)j} = \\big(2^{\\,1-p}\\big)^{j}$, reading each integer exponent as a rational one.",
      "step": "1.2",
      "inputs": [
        "L2",
        "algebra"
    ...
    {
      "id": "step-2-1",
      "claim": "So the condensed series is the geometric series of ratio $r := 2^{\\,1-p}$, which is positive; and $r < 1$ exactly when $1 - p < 0$, since $2 > 1$ makes $t \\mapsto 2^{t}$ strictly increasing and $2^{0} = 1$.",
      "step": "2.1",
      "inputs": [
        "1.2",
        "L2",
    ...
    {
      "id": "step-3-1",
      "claim": "At $p = 2$: the ratio is $2^{-1} = 1/2$, so the condensed series converges with sum $1/(1 - 1/2) = 2$, and $\\sum_{k \\ge 1} 1/k^{2}$ converges.",
      "step": "3.1",
      "inputs": [
        "1.2",
        "2.1",
    ...
    {
      "id": "step-3-2",
      "claim": "At $p = 1$: the ratio is $2^{0} = 1$, the condensed terms are constantly $1$, so the condensed series diverges and $\\sum_{k \\ge 1} 1/k$ diverges.",
      "step": "3.2",
      "inputs": [
        "1.2",
        "2.1",
    ...
    {
      "id": "step-3-3",
      "claim": "At $p = 1/2$: the ratio is $2^{1/2}$, which exceeds $1$ because $2 > 1$ and $1/2 > 0$; so the condensed series diverges and $\\sum_{k \\ge 1} k^{-1/2}$ diverges.",
      "step": "3.3",
      "inputs": [
        "1.2",
        "2.1",
    ...
    {
      "id": "step-4-1",
      "claim": "The three verdicts agree with the $p$-series theorem, whose content is exactly step 2.1 together with the geometric threshold.",
      "step": "4.1",
      "inputs": [
        "2.1",
        "3.1",
  ...
  "boundaries": [
    {
      ...
      "reason": "The original family begins at k = 1 and the condensed family at j = 0; both first terms are explicit."
    },
    {
      "case": "zero",
      "status": "checked",
      "evidence": "Statement excludes p = 0 and k = 0; step 1.1 checks positivity for every admitted term."
    },
    {
      "case": "one",
      "status": "checked",
      "evidence": "steps 2.1 and 3.2 compute the exact p = 1 boundary, where the condensed ratio is 1 and the series diverges."
    },
    {
      "case": "degenerate",
      "status": "not_applicable",
      "reason": "The hypothesis p > 0 excludes the only collapsed exponent relevant to the reciprocal definition."
    },
    {
      "case": "endpoints",
      "status": "checked",
      "evidence": "steps 3.1-3.3 check p = 1/2, 1, and 2 on both sides of the strict p > 1 threshold."
    },
    {
  ...
  "risk_review": {
    ...
    "notes": "critical risk (13): 12 declared dependencies; 13 cited facts; 7 numbered proof steps; biconditional / both-direction claim; boundary-sensitive language; analytic limiting/completeness language, routed by risk-report.mjs. The read-only DeepSeek V4 Pro audit-refuter received the complete target and every cited dependency and returned CLEAN. Boundary evidence: The domain is $p \\in \\mathbb{Q}$, $p>0$; the boundary $p=1$ (ratio $1$, divergence) and the endpoints $p=1/2$ and $p=2$ are checked directly and agree with the claimed threshold. The zero case $p=0$ is excluded by hypothesis, and $k=0$ is excluded because $1/0^p$ is undefined; no other degenerate or empty cases arise. The equivalence chain $\\sum 1/k^p$ converges $\\iff$ condensed series converges $\\iff$ $2^{1-p}<1$ $\\iff$ $p>1$ is valid for all admitted $p$, and no counterexample was found. Checked surface: The title, the public claim in the Example section, every numbered verification step (1.1–4.1), and the Remarks were read. No mathematical defect, missing hypothesis, or invalid inference was found. Alpha adjudicated the return against the current item and cited dependencies on disk and found no fatal mathematical or dependency-citation defect. Evidence: research/audit/wave7-preserved-refuters/ex-condensation-applied--80b568a248a6d796.result.json."
  }
}
```
...
[
  {
    "source": "ex-condensation-applied",
