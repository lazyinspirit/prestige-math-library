...
- critical risk (12): 11 declared dependencies; 11 cited facts; 7 numbered proof steps; existence, choice, uniqueness, or well-definedness; boundary-sensitive language; analytic limiting/completeness language
- Wave 7 ai-generated statement seed

## Target item — `cex-limit-not-unique-at-an-isolated-point`

Normalized current SHA-256: `902e4980112625db6c42ef7f85adb625f18fb4a9084073536ad043eaa34dca67`
...
```json
{
  ...
  "evidence": "trivial",
  "urls": [],
  "rationale": "Positive novelty determination: the exact domain {0} union [1,2] is a bespoke witness chosen to show vacuity at 0 and ordinary uniqueness at 1 in one example. The delta=1 check directly shows every real satisfies the raw formula at the isolated point; the separate false Remark is reported.",
...
```json
{
  ...
  "derivations": [
    {
      "id": "step-1-1",
      "claim": "$A = \\{0\\} \\cup [1,2]$ is a subset of $\\mathbb{R}$, and $f$ is the constant $0$ on $A$; both $0$ and $1$ belong to $A$.",
      "step": "1.1",
      "inputs": [
        "L4"
      ]
    ...
    {
      "id": "step-1-2",
      "claim": "$0$ is an isolated point of $A$ and not a limit point of $A$: $N_{1}(0) \\cap A = \\{0\\}$, since an element of $A$ is either $0$, with $|0 - 0| = 0 < 1$, or an element of $[1,2]$, with $|y - 0| = y \\ge 1$ and hence outside $N_1(0)$.",
      "step": "1.2",
      "inputs": [
        "L2",
        "L3",
    ...
    {
      "id": "step-1-3",
      "claim": "The reals $0$ and $1$ are distinct.",
      "step": "1.3",
      "inputs": [
        "L6"
      ]
    ...
    {
      "id": "step-2-1",
      "claim": "Take $\\delta := 1$. No $x \\in A$ satisfies $0 < |x - 0| < 1$: such an $x$ would lie in $N^{*}_{1}(0) \\cap A$, which is contained in $N_1(0) \\cap A = \\{0\\}$ and excludes $0$, hence is empty. So for every real $L$ and every real $\\varepsilon > 0$ the choice $\\delta = 1$ makes the implication vacuously true, and **every** real $L$ satisfies the displayed formula at $c = 0$.",
      "step": "2.1",
      "inputs": [
        "1.2",
        "L1",
    ...
    {
      "id": "step-2-2",
      "claim": "By contrast $1 \\in A$ *is* a limit point of $A$: given a real $\\rho > 0$, let $\\sigma$ be the smaller of $\\rho$ and $1$, so $\\sigma > 0$; then $1 + \\sigma/2$ satisfies $1 \\le 1 + \\sigma/2 \\le 1 + 1/2 \\le 2$, so it lies in $[1,2] \\subseteq A$, and $0 < |(1 + \\sigma/2) - 1| = \\sigma/2 < \\rho$. There [[def-function-limit]] applies, [[lem-function-limit-unique]] gives at most one $L$, and in fact $\\lim_{x \\to 1} f(x) = 0$, since $|f(x) - 0| = |0| = 0 < \\varepsilon$ for every $x \\in A$ and every real $\\varepsilon > 0$.",
      "step": "2.2",
      "inputs": [
        "1.1",
        "L1",
    ...
    {
      "id": "step-3-1",
      "claim": "In particular $L = 0$ and $L = 1$ both satisfy the formula at $c = 0$, and they are distinct: more than one real satisfies it, so the claim is refuted. This is why [[def-function-limit]] is stated only at a limit point, and why $\\lim_{x \\to 0} f(x)$ is left undefined on this domain.",
      "step": "3.1",
      "inputs": [
        "1.3",
        "2.1",
    ...
    {
      "id": "step-4-1",
      "claim": "So on one and the same domain the formula pins down a unique value at the limit point $1$ and no value at all at the isolated point $0$: uniqueness of the limit is a property of limit points, not of arbitrary points of the domain.",
      "step": "4.1",
      "inputs": [
        "2.2",
        "3.1"
  ...
  "boundaries": [
    {
      "case": "empty",
      "status": "checked",
      "evidence": "step 2.1: Take $\\delta := 1$. No $x \\in A$ satisfies $0 < |x - 0| < 1$: such an $x$ would lie in $N^{*}_{1}(0) \\cap A$, which is contained in $N_1(0) \\cap A = \\{0\\}$ and excludes $0$, hence is empty. So for every real $L$ and every real $\\varepsilon > 0$ the choice $\\de"
    },
    {
      "case": "zero",
      "status": "checked",
      "evidence": "step 1.1: $A = \\{0\\} \\cup [1,2]$ is a subset of $\\mathbb{R}$, and $f$ is the constant $0$ on $A$; both $0$ and $1$ belong to $A$."
    },
    {
      "case": "one",
      "status": "checked",
  ...
  "risk_review": {
    ...
    "notes": "critical risk (12): 11 declared dependencies; 11 cited facts; 7 numbered proof steps; existence, choice, uniqueness, or well-definedness; boundary-sensitive language; analytic limiting/completeness language, routed by risk-report.mjs. The read-only DeepSeek V4 Pro audit-refuter received the complete target and every cited dependency and returned CLEAN. Boundary evidence: The counterexample depends on the vacuous satisfaction of the ε-δ formula at the isolated point 0. The target correctly verifies that N*_1(0) ∩ A is empty (step 2.1), making the antecedent false for every x ∈ A; therefore the implication holds for every L, refuting the claim of at most one L. The boundary case of an empty punctured neighbourhood is handled without error, and the construction for the limit point at 1 (step 2.2) works for all positive radii, including arbitrarily small ones. No degenerate or endpoint edge cases introduce a flaw. Checked surface: The title, the public claim (Statement refuted), all numbered proof steps (1.1–4.1), and the Remarks were read in full. ``` Alpha adjudicated the return against the current item and cited dependencies on disk and found no fatal mathematical or dependency-citation defect. Evidence: research/audit/wave7-preserved-refuters/cex-limit-not-unique-at-an-isolated-point--14c9af6b71c6e559.result.json."
  }
}
```
...
[
  {
    "source": "cex-limit-not-unique-at-an-isolated-point",
    "sourcePage": "limits-of-real-functions-examples",
    "batch": "wave7-real-analysis-completeness-limits",
    "target": "fs-limit-unique-at-every-point-of-the-domain",
    "declared_target": "fs-limit-unique-at-every-point-of-the-domain",
