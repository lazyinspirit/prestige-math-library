...
- critical risk (10): 15 declared dependencies; 15 cited facts; 9 numbered proof steps; boundary-sensitive language; analytic limiting/completeness language

## Target item — `fs-ratio-limsup-at-least-one-diverges`

Normalized current SHA-256: `7952092a00c39f8884b6ff7fa20c07028f55400956e2d658cd6fd40b9041049f`
...
```json
{
  ...
  "rationale": "The sourced ratio test requires liminf>1 for divergence, while the CSUDH notes give oscillating-ratio geometric witnesses showing why limsup cannot replace it. The item formulates the pedagogical false alteration and uses a closely related bounded oscillating multiplier. The detailed computation is local.",
...
```json
{
  ...
  "derivations": [
    {
      "id": "step-1-1",
      "claim": "Each $s_k$ is $1$ or $-1$, since $|s_k| = 1$; so $c_k = 2 + s_k$ is $3$ or $1$, in either case $1 \\le c_k \\le 3$ and $c_k > 0$.",
      "step": "1.1",
      "inputs": [
        "L1",
        "given"
    ...
    {
      "id": "step-2-1",
      "claim": "Hence $a_k = c_k 2^{-k} > 0$ for every $k$, so in particular $a_k \\ne 0$ and the claim applies to $(a_k)$.",
      "step": "2.1",
      "inputs": [
        "1.1",
        "L2",
    ...
    {
      "id": "step-2-2",
      "claim": "For every $k$, $0 < a_k = c_k 2^{-k} \\le 3 \\cdot 2^{-k}$.",
      "step": "2.2",
      "inputs": [
        "1.1",
        "L2"
    ...
    {
      "id": "step-3-1",
      "claim": "The ratios are $q_k := |a_{k+1}/a_k| = a_{k+1}/a_k = \\dfrac{c_{k+1}}{c_k}\\cdot\\dfrac{1}{2}$, so $q_k = (1/3)(1/2) = 1/6$ when $s_k = 1$ and $q_k = 3 \\cdot (1/2) = 3/2$ when $s_k = -1$, using $s_{k+1} = -s_k$.",
      "step": "3.1",
      "inputs": [
        "1.1",
        "2.1",
    ...
    {
      "id": "step-3-2",
      "claim": "The geometric series $\\sum_k (1/2)^{k}$ converges, since $|1/2| < 1$; hence so does $\\sum_k 3 \\cdot 2^{-k}$, and by comparison so does $\\sum_k a_k$.",
      "step": "3.2",
      "inputs": [
        "2.2",
        "L2",
    ...
    {
      "id": "step-4-1",
      "claim": "For every $n \\in \\mathbb{N}$ there is an index $k \\ge n$ with $s_k = -1$, namely $k = o_n$, since $o_n \\ge n$; so $q_k = 3/2$ at some index $k \\ge n$, for every $n$.",
      "step": "4.1",
      "inputs": [
        "3.1",
        "L1"
    ...
    {
      "id": "step-5-1",
      "claim": "Therefore every tail supremum satisfies $\\sup\\{q_k : k \\ge n\\} \\ge 3/2$, so $3/2$ is a lower bound of the set of tail suprema and $\\limsup_k q_k \\ge 3/2 \\ge 1$.",
      "step": "5.1",
      "inputs": [
        "4.1",
        "L3"
    ...
    {
      "id": "step-6-1",
      "claim": "So $(a_k)$ has nonzero terms and $\\limsup_k |a_{k+1}/a_k| \\ge 1$, yet $\\sum a_k$ converges; the claim fails for it and is therefore false.",
      "step": "6.1",
      "inputs": [
        "2.1",
        "5.1",
    ...
    {
      "id": "step-7-1",
      "claim": "Nothing in the ratio test is contradicted: its divergence half requires $\\liminf_k q_k > 1$, and here $\\liminf_k q_k \\le 1/6 < 1$, since $q_k = 1/6$ at indices $e_n \\ge n$ for every $n$.",
      "step": "7.1",
      "inputs": [
        "6.1",
        "3.1",
  ...
  "boundaries": [
    ...
    },
    {
      "case": "zero",
      "status": "checked",
      "evidence": "steps 1.1-2.1 prove every witness term is positive and hence nonzero before ratios are formed."
    },
    {
      "case": "one",
      "status": "checked",
      "evidence": "steps 4.1-5.1 place ratios 3/2 arbitrarily late and verify the non-strict boundary limsup >= 1."
    },
    {
      "case": "degenerate",
      "status": "checked",
      "evidence": "steps 3.1 and 7.1 check both alternating ratio values, so the oscillatory sequence is not mistaken for a convergent-ratio case."
    },
    {
      "case": "endpoints",
      "status": "checked",
      "evidence": "steps 5.1-7.1 distinguish the false limsup >= 1 condition from the true strict liminf > 1 condition."
    },
    {
      "case": "nonempty-choice",
      "status": "checked",
      "evidence": "steps 4.1: every selected index or tolerance is supplied by the cited limit, order-bound, Archimedean, or divergence clause; no selection from an unproved nonempty family is hidden."
    },
    {
      "case": "iff-forward",
      "status": "not_applicable",
  ...
  "risk_review": {
    ...
    "notes": "critical risk (10): 15 declared dependencies; 15 cited facts; 9 numbered proof steps; boundary-sensitive language; analytic limiting/completeness language, routed by risk-report.mjs. The read-only DeepSeek V4 Pro audit-refuter received the complete target and every cited dependency and returned CLEAN. Boundary evidence: The false claim asserts divergence whenever limsup ≥ 1; the witness has limsup ≥ 3/2 > 1, so the boundary condition (≥) is met. All terms are positive, hence nonzero, so ratios are well‑defined. The witness oscillates, taking the two ratio values 1/6 and 3/2 arbitrarily late, and the limsup argument correctly uses the fact that 3/2 occurs in every tail. The series converges by direct comparison with a convergent geometric series, so the counterexample is valid. No degenerate or endpoint case invalidates the refutation. Checked surface: The title, the public false claim, every numbered refutation step (1.1–7.1), and the Remarks were read. No mathematical error, missing hypothesis, or incorrect citation was found. The refutation is logically sound and the counterexample correctly disproves the claimed implication. ``` Alpha adjudicated the return against the current item and cited dependencies on disk and found no fatal mathematical or dependency-citation defect. Evidence: research/audit/wave7-preserved-refuters/fs-ratio-limsup-at-least-one-diverges--621f88dda8a0b96e.result.json."
  }
}
```
...
[
  {
    "source": "fs-ratio-limsup-at-least-one-diverges",
