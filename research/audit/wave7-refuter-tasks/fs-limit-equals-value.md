...
- critical risk (11): 7 declared dependencies; 6 cited facts; existence, choice, uniqueness, or well-definedness; boundary-sensitive language; analytic limiting/completeness language

## Target item — `fs-limit-equals-value`

Normalized current SHA-256: `75065f18095c5312153f44a3e77ff6111ecc7bba8cd6f7e2a14ff92cda634927`
...
```json
{
  ...
  "rationale": "The sources distinguish a function limit from the value at the point and standard removable discontinuities witness their possible disagreement. The local false universal isolates that missing continuity hypothesis.",
...
```json
{
  ...
  "derivations": [
    {
      "id": "step-1-1",
      "claim": "The point $c = 0$ lies in $A = \\mathbb{R}$ and is a limit point of $\\mathbb{R}$: for every real $\\varepsilon > 0$ the punctured neighbourhood $N^{*}_{\\varepsilon}(0)$ is nonempty and is contained in $\\mathbb{R}$, so it meets $\\mathbb{R}$.",
      "step": "1.1",
      "inputs": [
        "L2"
      ]
    ...
    {
      "id": "step-1-2",
      "claim": "$f$ is a well-defined function on $\\mathbb{R}$, since by trichotomy every real either equals $0$ or does not, exclusively; and the reals $0$ and $1$ are distinct.",
      "step": "1.2",
      "inputs": [
        "L4"
      ]
    ...
    {
      "id": "step-2-1",
      "claim": "The limit of $f$ at $0$ exists and equals $0$: given an arbitrary real $\\varepsilon > 0$, take $\\delta := 1 > 0$; every $x \\in \\mathbb{R}$ with $0 < |x - 0| < 1$ has $|x| \\ne 0$, hence $x \\ne 0$, hence $f(x) = 0$ and $|f(x) - 0| = |0| = 0 < \\varepsilon$.",
      "step": "2.1",
      "inputs": [
        "1.1",
        "1.2",
    ...
    {
      "id": "step-3-1",
      "claim": "Yet $f(0) = 1$, and $1 \\ne 0 = \\lim_{x \\to 0} f(x)$. So at the point $c = 0$ of the domain, which is a limit point of the domain, the limit exists and differs from the value: the claim is false.",
      "step": "3.1",
      "inputs": [
        "1.2",
        "2.1",
  ...
  "boundaries": [
    {
      "case": "empty",
      "status": "checked",
      "evidence": "step 1.1: The point $c = 0$ lies in $A = \\mathbb{R}$ and is a limit point of $\\mathbb{R}$: for every real $\\varepsilon > 0$ the punctured neighbourhood $N^{*}_{\\varepsilon}(0)$ is nonempty and is contained in $\\mathbb{R}$, so it meets $\\mathbb{R}$."
    },
    {
      "case": "zero",
      "status": "checked",
    ...
    },
    {
      "case": "one",
      "status": "checked",
      "evidence": "step 1.2: $f$ is a well-defined function on $\\mathbb{R}$, since by trichotomy every real either equals $0$ or does not, exclusively; and the reals $0$ and $1$ are distinct."
    },
    {
      "case": "degenerate",
      "status": "checked",
  ...
  "risk_review": {
    ...
    "notes": "critical risk (11): 7 declared dependencies; 6 cited facts; existence, choice, uniqueness, or well-definedness; boundary-sensitive language; analytic limiting/completeness language, routed by risk-report.mjs. The read-only DeepSeek V4 Pro audit-refuter received the complete target and every cited dependency and returned CLEAN. Boundary evidence: The counterexample uses \\(A=\\mathbb{R}\\), \\(c=0\\), and \\(f\\) defined by \\(f(x)=0\\) for \\(x\\neq 0\\) and \\(f(0)=1\\). Checked: \\(c=0\\) is a limit point of \\(\\mathbb{R}\\) (step 1.1, every punctured neighbourhood meets \\(\\mathbb{R}\\)); \\(f\\) is well‑defined on \\(\\mathbb{R}\\) (step 1.2, partition into \\(x=0\\) and \\(x\\neq 0\\) is exclusive and exhaustive, \\(0\\neq 1\\)); under the ε‑δ definition of limit (def‑function‑limit) the limit of \\(f\\) at \\(0\\) exists and equals \\(0\\) (step 2.1, \\(\\delta=1\\) works for every \\(\\varepsilon>0\\) because \\(0<|x|<1\\Rightarrow x\\neq 0\\Rightarrow f(x)=0\\) and \\(|0|=0<\\varepsilon\\)); \\(f(0)=1\\) and \\(1\\neq 0\\) (step 3.1). No empty, zero, endpoint, or degenerate cases affect the refutation; no non‑empty choice, hidden division, or extended‑real pitfalls are present. The witness directly contradicts the universal claim, so the refutation is sound. Checked surface: The title (“FALSE: \\(\\lim_{x\\to c}f(x)=f(c)\\) whenever both sides exist”), the public Statement, every numbered step (1.1, 1.2, 2.1, 3.1), and the Remarks were read and verified for mathematical accuracy and logical validity. All are consistent and free of defect. ``` Alpha adjudicated the return against the current item and cited dependencies on disk and found no fatal mathematical or dependency-citation defect. Evidence: research/audit/wave7-preserved-refuters/fs-limit-equals-value--b4890994479bf7fe.result.json."
  }
}
```
