...
- critical risk (11): 8 declared dependencies; 9 cited facts; existence, choice, uniqueness, or well-definedness; boundary-sensitive language; analytic limiting/completeness language

## Target item — `fs-limit-unique-at-every-point-of-the-domain`

Normalized current SHA-256: `32bf79f6288efb5bd1aae35c53eb256e996c7130f8739dbe9eede56c4c27c634`
...
```json
{
  ...
  "rationale": "The sources identify isolated points and the punctured epsilon-delta formula’s vacuity there. The local false universal deliberately removes the cluster-point hypothesis from limit uniqueness.",
...
```json
{
  ...
  "derivations": [
    {
      "id": "step-1-1",
      "claim": "The point $0$ lies in $A$, and $N_{1}(0) \\cap A = \\{0\\}$: an element of $A$ is either $0$, which satisfies $|0| = 0 < 1$, or an element of $[1,2]$, which satisfies $|y| = y \\ge 1$ and so is not in $N_1(0)$. Hence $0$ is an isolated point of $A$ and not a limit point of $A$.",
      "step": "1.1",
      "inputs": [
        "L2",
        "L3",
    ...
    {
      "id": "step-2-1",
      "claim": "Take $\\delta := 1$. No $x \\in A$ satisfies $0 < |x - 0| < 1$: such an $x$ would lie in $N^{*}_{1}(0) \\cap A$, which is contained in $N_1(0) \\cap A = \\{0\\}$ and excludes $0$, hence is empty. So for every real $L$ and every real $\\varepsilon > 0$ the choice $\\delta = 1$ makes the implication in $(\\ast)$ vacuously true, and every real $L$ satisfies $(\\ast)$ at $c = 0$.",
      "step": "2.1",
      "inputs": [
        "1.1",
        "L1",
    ...
    {
      "id": "step-3-1",
      "claim": "In particular $L = 0$ and $L = 1$ both satisfy $(\\ast)$ at $c = 0$, and they are distinct: more than one real satisfies the formula, so the claim is false.",
      "step": "3.1",
      "inputs": [
        "1.2",
        "2.1",
  ...
  "boundaries": [
    ...
    },
    {
      "case": "zero",
      "status": "checked",
      "evidence": "step 1.1: The point $0$ lies in $A$, and $N_{1}(0) \\cap A = \\{0\\}$: an element of $A$ is either $0$, which satisfies $|0| = 0 < 1$, or an element of $[1,2]$, which satisfies $|y| = y \\ge 1$ and so is not in $N_1(0)$. Hence $0$ is an isolated point of $A$ and not a limit"
    },
    {
      "case": "one",
      "status": "checked",
...
[
  {
    "source": "fs-limit-unique-at-every-point-of-the-domain",
