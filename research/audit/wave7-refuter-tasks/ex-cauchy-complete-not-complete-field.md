...
- critical risk (10): 14 declared dependencies; 13 cited facts; 9 numbered proof steps; boundary-sensitive language; analytic limiting/completeness language
- Wave 7 ai-generated statement seed

## Target item — `ex-cauchy-complete-not-complete-field`

Normalized current SHA-256: `d45bf2e41a696506d6e30e9b2a4ee701adc944f24abdcc90e7c96383b76d3294`
...
```json
{
  ...
  "evidence": "none",
  "urls": [],
  "rationale": "Positive novelty determination: the exact property inventory for this library’s K=R((t^{-1})) is a bespoke composition rather than an established named result. Its ordered-field, Cauchy-complete, non-Archimedean, non-LUB, shrinking-NIP, non-BW, and non-MCT assertions follow from the source-backed local Laurent and completeness results cited in the item.",
...
```json
{
  ...
  "derivations": [
    ...
    {
      "id": "step-1-2",
      "claim": "$K$ is not Archimedean: $t$ exceeds every canonical natural.",
      "step": "1.2",
      "inputs": [
        "L2"
      ]
    ...
    {
      "id": "step-1-4",
      "claim": "$K$ does not have (LUB): the canonical naturals are nonempty and bounded above and have no supremum in $K$.",
      "step": "1.4",
      "inputs": [
        "L4"
      ]
    ...
    {
      "id": "step-1-5",
      "claim": "$K$ has (NIP) in the shrinking form of [[def-completeness-properties]].",
      "step": "1.5",
      "inputs": [
        "L5"
      ]
    ...
    {
      "id": "step-2-1",
      "claim": "So $K$ is a Cauchy complete, non-Archimedean ordered field without the least-upper-bound property, which is what this example asserts, and it is the witness used in [[fs-cauchy-complete-implies-lub]] and in [[fs-nested-intervals-implies-lub]].",
      "step": "2.1",
      "inputs": [
        "1.1",
        "1.2",
    ...
    {
      "id": "step-2-2",
      "claim": "$K$ has neither (BW) nor (MCT), since either would force $K$ to be Archimedean, which step 1.2 denies.",
      "step": "2.2",
      "inputs": [
        "1.2",
        "L6"
    ...
    {
      "id": "step-2-3",
      "claim": "A concrete convergent Cauchy sequence: let $f^{(n)} := \\sum_{k=0}^{n} t^{-k}$, the function taking the value $1$ at each index $0 \\le j \\le n$ and $0$ elsewhere. For $n > m$ the difference $f^{(n)} - f^{(m)}$ vanishes at every index $j \\le m$, so $|f^{(n)} - f^{(m)}| < t^{-m}$; since the monomials $t^{-m}$ get below every positive element of $K$, the sequence is Cauchy in $K$. Its limit is the element $L$ with $L(j) = 1$ for $j \\ge 0$ and $L(j) = 0$ for $j < 0$, which lies in $K$ because its support is bounded below, and $f^{(n)} - L$ vanishes at every index $j \\le n$, so $|f^{(n)} - L| < t^{-n}$ and $f^{(n)} \\to L$ in $K$.",
      "step": "2.3",
      "inputs": [
        "1.1",
        "1.3",
    ...
    {
      "id": "step-3-1",
      "claim": "The table of the Example is therefore established in every row, and $K$ separates Cauchy completeness from the least-upper-bound property.",
      "step": "3.1",
      "inputs": [
        "2.1",
        "2.2",
  ...
  "boundaries": [
    {
      "case": "empty",
      "status": "checked",
      "evidence": "step 1.4: $K$ does not have (LUB): the canonical naturals are nonempty and bounded above and have no supremum in $K$."
    },
    {
      "case": "zero",
      "status": "checked",
      "evidence": "step 2.3: A concrete convergent Cauchy sequence: let $f^{(n)} := \\sum_{k=0}^{n} t^{-k}$, the function taking the value $1$ at each index $0 \\le j \\le n$ and $0$ elsewhere. For $n > m$ the difference $f^{(n)} - f^{(m)}$ vanishes at every index $j \\le m$, so $|f^{(n)} - f"
    },
    {
      "case": "one",
      "status": "checked",
      "evidence": "step 2.2: $K$ has neither (BW) nor (MCT), since either would force $K$ to be Archimedean, which step 1.2 denies."
    },
    {
      "case": "degenerate",
    ...
    },
    {
      "case": "endpoints",
      "status": "checked",
      "evidence": "step 1.5: $K$ has (NIP) in the shrinking form of [[def-completeness-properties]]."
    },
    {
  ...
  "risk_review": {
    ...
    "notes": "critical risk (10): 14 declared dependencies; 13 cited facts; 9 numbered proof steps; boundary-sensitive language; analytic limiting/completeness language, routed by risk-report.mjs. The read-only DeepSeek V4 Pro audit-refuter received the complete target and every cited dependency and returned CLEAN. Boundary evidence: Checked empty/nonempty sets (the set of canonical naturals is nonempty; nested intervals are assumed nondegenerate or degenerate only in the limit, covered by the corollary), zero element (the absolute value and field operations pose no division-by-zero issues), and endpoint/index cases (the Cauchy sequence starts at \\(n=0\\) and uses natural indices throughout; the shrinking condition on intervals uses the field order consistently). No boundary counterexample invalidates any claim. Checked surface: The title, the public claim (the property table and the statements that \\(K\\) is Cauchy complete, non‑Archimedean, and lacks LUB), all numbered verification steps (1.1–3.1), and the four Remarks were read and verified against the supplied sources. The logical structure is sound and no false claim or invalid inference is present. ``` Alpha adjudicated the return against the current item and cited dependencies on disk and found no fatal mathematical or dependency-citation defect. Evidence: research/audit/wave7-preserved-refuters/ex-cauchy-complete-not-complete-field--39e1760d7655f943.result.json."
  }
}
```
...
[
  {
    "source": "ex-cauchy-complete-not-complete-field",
