...
- critical risk (10): 4 declared dependencies; 4 cited facts; 7 numbered proof steps; biconditional / both-direction claim; analytic limiting/completeness language

## Target item — `lem-series-linearity`

Normalized current SHA-256: `e978bd1f7ffdba98e1c8ec34ff7a1b7d097721dd08b4ee1fb57c21df31092eeb`
...
```json
{
  ...
  "rationale": "Linearity follows from the sourced definition of a series as a limit of partial sums and the standard algebra of limits. The item combines addition, scalar multiplication, and the nonzero-scalar iff/divergence form into one library-facing lemma. The proof is locally composed from finite-sum and limit laws.",
...
```json
{
  ...
  "derivations": [
    {
      "id": "step-1-1",
      "claim": "The partial sums of $\\sum (a_k + b_k)$ are $\\sum_{k<n}(a_k + b_k) = s_n + t_n$, and those of $\\sum c\\,a_k$ are $\\sum_{k<n} c\\,a_k = c\\,s_n$.",
      "step": "1.1",
      "inputs": [
        "L1",
        "given"
    ...
    {
      "id": "step-1-2",
      "claim": "Assume $\\sum a_k$ and $\\sum b_k$ converge, say $s_n \\to S$ and $t_n \\to T$.",
      "step": "1.2",
      "inputs": [
        "given"
      ]
    ...
    {
      "id": "step-2-1",
      "claim": "Then $s_n + t_n \\to S + T$, so $\\sum(a_k + b_k)$ converges with sum $S + T$, which is claim 1.",
      "step": "2.1",
      "inputs": [
        "1.2",
        "1.1",
    ...
    {
      "id": "step-2-2",
      "claim": "Likewise $c\\,s_n \\to c\\,S$, so $\\sum c\\,a_k$ converges with sum $c\\,S$, which is claim 2.",
      "step": "2.2",
      "inputs": [
        "1.2",
        "1.1",
    ...
    {
      "id": "step-3-1",
      "claim": "For claim 3, let $c \\ne 0$ and let $(a_k)$ be arbitrary. If $\\sum a_k$ converges then $\\sum c\\,a_k$ converges by claim 2.",
      "step": "3.1",
      "inputs": [
        "2.2"
      ]
    ...
    {
      "id": "step-3-2",
      "claim": "Conversely, if $\\sum c\\,a_k$ converges then applying claim 2 to the sequence $(c\\,a_k)$ and the scalar $1/c$, which exists since $c \\ne 0$, shows that $\\sum (1/c)(c\\,a_k) = \\sum a_k$ converges.",
      "step": "3.2",
      "inputs": [
        "2.2",
        "algebra"
    ...
    {
      "id": "step-4-1",
      "claim": "The two implications are claim 3, and its contrapositive form is the statement about divergence.",
      "step": "4.1",
      "inputs": [
        "3.1",
        "3.2"
  ...
  "boundaries": [
    {
      "case": "empty",
      "status": "checked",
      "evidence": "step 1.1 applies the finite-sum identities also at n = 0, where all three partial sums are empty and equal to 0."
    },
    {
      "case": "zero",
      "status": "checked",
      "evidence": "step 2.2 includes c = 0 in claim 2; steps 3.1-3.2 explicitly require c != 0 only for the converse in claim 3."
    },
    {
      "case": "one",
      "status": "checked",
      "evidence": "steps 2.2 and 3.1 include c = 1, where the scaled series is unchanged."
    },
    {
      "case": "degenerate",
      "status": "checked",
      "evidence": "steps 1.1-2.2 include zero sequences and equal input sequences without a distinct branch."
    },
    {
      "case": "endpoints",
      "status": "not_applicable",
      "reason": "The Statement has no interval endpoint or variable finite-index endpoint beyond the index conventions already fixed in its hypotheses."
    },
    {
    ...
    },
    {
      "case": "iff-forward",
      "status": "checked",
      "evidence": "step 3.1 proves convergence of sum a_k implies convergence of sum c a_k for c != 0."
    },
    {
      "case": "iff-reverse",
      "status": "checked",
      "evidence": "step 3.2 applies the scalar 1/c and proves the reverse implication."
    }
  ],
  "finite_smoke": [],
  "risk_review": {
    ...
    "notes": "critical risk (10): 4 declared dependencies; 4 cited facts; 7 numbered proof steps; biconditional / both-direction claim; analytic limiting/completeness language, routed by risk-report.mjs. The read-only DeepSeek V4 Pro audit-refuter received the complete target and every cited dependency and returned CLEAN. Boundary evidence: The proof contract already examined empty partial sums, zero scalar, one scalar, degenerate (zero) sequences, and both directions of the biconditional. All checks pass: the finite-sum identities hold at n=0, claim 2 works for c=0, claim 3 correctly requires c≠0, c=1 is trivial, and the forward/reverse implications are explicitly proved. No counterexample was found; the proof’s use of limits and finite sums is standard and applies without hidden assumptions. Checked surface: The title, the public statement (claims 1‑3), all seven numbered proof steps, and the Remarks were read and assessed. The proof is logically valid, the biconditional is correctly established, and the remarks contain no false mathematical claims. Alpha adjudicated the return against the current item and cited dependencies on disk and found no fatal mathematical or dependency-citation defect. Evidence: research/audit/wave7-preserved-refuters/lem-series-linearity--ffe73bdd0518b14b.result.json."
  }
}
```
