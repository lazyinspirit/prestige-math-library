...
- critical risk (16): 6 declared dependencies; 8 cited facts; 8 numbered proof steps; biconditional / both-direction claim; existence, choice, uniqueness, or well-definedness; boundary-sensitive language; quotient or equivalence-class construction; analytic limiting/completeness language

## Target item — `lem-limit-is-local`

Normalized current SHA-256: `4a096e2634a5a9a285fcbf8f8923c78cab524fe74639589f0f6d5dd4fb8b7edc`
...
```json
{
  ...
  "rationale": "Lebl develops limits on restricted domains and the fact that only punctured-neighborhood values matter. The local item packages equality-near-c and restriction as two clauses.",
...
```json
{
  ...
  "derivations": [
    {
      "id": "step-1-1",
      "claim": "For claim 1, assume $\\lim_{x \\to c} f(x) = L$ and let $\\varepsilon > 0$ be an arbitrary real.",
      "step": "1.1",
      "inputs": [
        "L1",
        "assume-hyp"
    ...
    {
      "id": "step-1-2",
      "claim": "For claim 2, $B \\subseteq A$ and $c$ is a limit point of $B$; hence $c$ is a limit point of $A$, since for every real $\\delta > 0$ a point $x \\in B$ with $0 < |x - c| < \\delta$ is also a point of $A$ with $0 < |x - c| < \\delta$.",
      "step": "1.2",
      "inputs": [
        "L2"
      ]
    ...
    {
      "id": "step-1-3",
      "claim": "For claim 2, assume $\\lim_{x \\to c} f(x) = L$ and let $\\varepsilon > 0$ be an arbitrary real.",
      "step": "1.3",
      "inputs": [
        "L1",
        "assume-hyp"
    ...
    {
      "id": "step-2-1",
      "claim": "By [L1] fix a real $\\delta_0 > 0$ such that every $x \\in A$ with $0 < |x - c| < \\delta_0$ satisfies $|f(x) - L| < \\varepsilon$, and put $\\delta$ to be the smaller of $\\delta_0$ and $\\eta$, so $\\delta > 0$.",
      "step": "2.1",
      "inputs": [
        "L1",
        "1.1",
    ...
    {
      "id": "step-2-2",
      "claim": "By [L1] fix a real $\\delta > 0$ such that every $x \\in A$ with $0 < |x - c| < \\delta$ satisfies $|f(x) - L| < \\varepsilon$.",
      "step": "2.2",
      "inputs": [
        "L1",
        "1.3",
    ...
    {
      "id": "step-3-1",
      "claim": "Every $x \\in A$ with $0 < |x - c| < \\delta$ satisfies both $0 < |x - c| < \\delta_0$ and $0 < |x - c| < \\eta$, so $g(x) = f(x)$ and $|g(x) - L| = |f(x) - L| < \\varepsilon$; as $\\varepsilon > 0$ was arbitrary, $\\lim_{x \\to c} g(x) = L$.",
      "step": "3.1",
      "inputs": [
        "2.1",
        "L1",
    ...
    {
      "id": "step-3-2",
      "claim": "Every $x \\in B$ with $0 < |x - c| < \\delta$ lies in $A$ and satisfies $0 < |x - c| < \\delta$, so $f|_B(x) = f(x)$ and therefore $|f(x) - L| < \\varepsilon$; as $\\varepsilon > 0$ was arbitrary, and $c$ is a limit point of $B$, $\\lim_{x \\to c} f|_B(x) = L$.",
      "step": "3.2",
      "inputs": [
        "2.2",
        "L1",
    ...
    {
      "id": "step-4-1",
      "claim": "The hypothesis of claim 1 is symmetric in $f$ and $g$, so interchanging their roles in steps 1.1, 2.1 and 3.1 gives the implication in the other direction, and claim 1 is proved; claim 2 is steps 1.2 and 3.2.",
      "step": "4.1",
      "inputs": [
        "1.1",
        "2.1",
  ...
  "boundaries": [
    ...
    },
    {
      "case": "zero",
      "status": "checked",
      "evidence": "step 1.1: For claim 1, assume $\\lim_{x \\to c} f(x) = L$ and let $\\varepsilon > 0$ be an arbitrary real."
    },
    {
      "case": "one",
      "status": "checked",
  ...
  "risk_review": {
    ...
    "notes": "critical risk (16): 6 declared dependencies; 8 cited facts; 8 numbered proof steps; biconditional / both-direction claim; existence, choice, uniqueness, or well-definedness; boundary-sensitive language; quotient or equivalence-class construction; analytic limiting/completeness language, routed by risk-report.mjs. The read-only DeepSeek V4 Pro audit-refuter received the complete target and every cited dependency and returned CLEAN. Boundary evidence: No counterexample found for either claim. The biconditional in claim 1 is proved in both directions (forward by ε‑δ with δ = min(δ₀, η); reverse by symmetry). Claim 2’s deduction that c is a limit point of A is either given by the preamble or validly inferred from B ⊆ A and c limit point of B; the restriction limit follows directly because the δ witnessing the limit on A serves for B. The proof uses no choice, handles the punctured neighbourhood correctly, and does not rely on vacuous cases because c is a limit point of the relevant sets. The converse of claim 2 is correctly noted as false and is not part of the proof. Checked surface: The title, the full public Statement (including both claims and the surrounding text), every numbered proof step (1.1 through 4.1), the Facts & Assumptions, and the Remarks were read and verified. Alpha adjudicated the return against the current item and cited dependencies on disk and found no fatal mathematical or dependency-citation defect. Evidence: research/audit/wave7-preserved-refuters/lem-limit-is-local--99d63131b2158d04.result.json."
  }
}
```
