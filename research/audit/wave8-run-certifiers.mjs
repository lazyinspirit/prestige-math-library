...
const taskDir = join(root, "research/audit/wave8-certifier-tasks");
const dispatchDir = join(root, "research/audit/wave8-dispatch");
const brief = "research/audit/wave8-certifier-brief.md";
const concurrency = 6;
...
const passed = ({ label }) => {
  ...
  try {
    ...
    return row.ok === true && row.model === "gpt-5.6-terra" &&
      /VERDICT:\s*CERTIFIED/.test(row.tail ?? "");
  } catch { return false; }
};

const pending = tasks.filter((task) => !passed(task));
console.log(`wave8 certifiers: ${tasks.length} prepared; ${tasks.length - pending.length} certified; ${pending.length} pending; concurrency ${concurrency}`);
let cursor = 0;
const failures = [];
const runOne = (task) => new Promise((resolve) => {
  ...
  const child = spawn(process.execPath, [
    ...
    "--label", task.label, "--run", "wave8", "--task", task.path,
    "--timeout", "7200", "--json",
...
console.log(`wave8 certifiers: ${tasks.length - failures.length}/${tasks.length} dispatches passed; ${failures.length} failed`);
