...
## Wave 10 target — `cex-dirichlet-is-not-riemann-integrable`

Selection: mandatory Wave 10 high/critical proof-bearing route (risk report classifies all 49 contracts critical).

Normalized current SHA-256: `21e1b58b310ddd95860ab7300849188db7072bb395b95d219efbdf59bded1a48`
...
```json
{
  ...
  "rationale": "The sources show that every interval contains rationals and irrationals, forcing lower sums zero and upper sums one for the Dirichlet indicator. The item reproduces that standard proof.",
...
```json
{
  ...
  "derivations": [
    {
      "id": "cex-dirichlet-is-not-riemann-integrable-step-1-1",
      "claim": "$g$ is bounded, with $0 \\le g(x) \\le 1$ for every $x \\in [0,1]$, so its Darboux sums and integrals are defined by [L3]. [given, L3]",
      "step": "1.1",
      "inputs": [
        "L3"
      ]
    ...
    {
      "id": "cex-dirichlet-is-not-riemann-integrable-step-1-2",
      "claim": "Let $P = (n,t)$ be any partition of $[0,1]$ and $i < n$. By [L2] the interval $(t_i,t_{i+1})$ is nonempty and open, so by [L1] it contains a rational and an irrational, both lying in $I_i$. Hence $g[I_i] = \\{0,1\\}$ and, by [L4], $m_i = 0$ and $M_i = 1$. [given, L1, L2, L4]",
      "step": "1.2",
      "inputs": [
        "L2",
        "L1",
    ...
    {
      "id": "cex-dirichlet-is-not-riemann-integrable-step-2-1",
      "claim": "Therefore $L(g,P) = \\sum_{i<n}0\\cdot\\Delta_i = 0$ and $U(g,P) = \\sum_{i<n}1\\cdot\\Delta_i = \\sum_{i<n}\\Delta_i = 1$, for every partition $P$ of $[0,1]$, by [L3], [L5] and [L2]. [step 1.2, L2, L3, L5]",
      "step": "2.1",
      "inputs": [
        "L3",
        "L5",
    ...
    {
      "id": "cex-dirichlet-is-not-riemann-integrable-step-3-1",
    ...
    {
      "id": "cex-dirichlet-is-not-riemann-integrable-step-4-1",
      "claim": "So $g$ is bounded on $[0,1]$, an interval with $0 < 1$, and is not Riemann integrable: [A1] is refuted. [step 1.1, step 3.1, A1] ∎",
      "step": "4.1",
      "inputs": [
        "A1",
        "1.1",
