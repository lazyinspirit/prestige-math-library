...
## Wave 10 target — `ex-refinement-improves-the-darboux-sums`

Selection: mandatory Wave 10 high/critical proof-bearing route (risk report classifies all 49 contracts critical).

Normalized current SHA-256: `0e89c0c6d213cc159ef097195b66c9d685721c2b8fe1abc6384458a373eacaa2`
...
```json
{
  ...
  "rationale": "The sources give refinement monotonicity and standard Darboux-sum computations. The exact x-squared partitions and numerical comparison are a local worked example.",
...
```json
{
  ...
  "derivations": [
    {
      "id": "ex-refinement-improves-the-darboux-sums-step-1-1",
      "claim": "For $P_0$ the single subinterval is $[0,1]$, so by [L3] $m_0 = 0^{2} = 0$ and $M_0 = 1^{2} = 1$, and by [L2] and [L5], $L(f,P_0) = 0 \\cdot 1 = 0$ and $U(f,P_0) = 1 \\cdot 1 = 1$. [given, L1, L2, L3, L5]",
      "step": "1.1",
      "inputs": [
        "L3",
        "L2",
    ...
    {
      "id": "ex-refinement-improves-the-darboux-sums-step-1-2",
      "claim": "For $P_1$ the two subintervals are $[0,\\tfrac12]$ and $[\\tfrac12,1]$, each of length $\\tfrac12$, so by [L3] the extreme values are $m_0 = 0$, $M_0 = (\\tfrac12)^{2} = \\tfrac14$ on the first and $m_1 = \\tfrac14$, $M_1 = 1$ on the second. [given, L1, L3, L6]",
      "step": "1.2",
      "inputs": [
        "L3",
        "L1",
    ...
    {
      "id": "ex-refinement-improves-the-darboux-sums-step-2-1",
      "claim": "Hence by [L2], [L5] and [L6], $L(f,P_1) = 0\\cdot\\tfrac12 + \\tfrac14\\cdot\\tfrac12 = \\tfrac18$ and $U(f,P_1) = \\tfrac14\\cdot\\tfrac12 + 1\\cdot\\tfrac12 = \\tfrac18 + \\tfrac12 = \\tfrac58$. [step 1.2, L2, L5, L6]",
      "step": "2.1",
      "inputs": [
        "L2",
        "L5",
    ...
    {
      "id": "ex-refinement-improves-the-darboux-sums-step-3-1",
      "claim": "Comparing with step 1.1 and using [L6]: $L(f,P_0) = 0 < \\tfrac18 = L(f,P_1)$ and $U(f,P_1) = \\tfrac58 < 1 = U(f,P_0)$, while $L(f,P_1) = \\tfrac18 < \\tfrac58 = U(f,P_1)$. This is the chain of [L4] for the refinement $P_1$ of $P_0$, here with every inequality strict. [step 1.1, step 2.1, L1, L4, L6] ∎",
      "step": "3.1",
      "inputs": [
        "1.1",
        "L6",
...
[
  {
    "source": "ex-refinement-improves-the-darboux-sums",
