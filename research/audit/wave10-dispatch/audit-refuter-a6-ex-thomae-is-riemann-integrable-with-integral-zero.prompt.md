...
## Wave 10 target — `ex-thomae-is-riemann-integrable-with-integral-zero`

Selection: mandatory Wave 10 high/critical proof-bearing route (risk report classifies all 49 contracts critical).

Normalized current SHA-256: `5290a20ee6aeddadacd32e34aa09f1f6dd2cdecb8c874d6689d22c6055f10b73`
...
```json
{
  ...
  "rationale": "The sources establish Thomae’s integrability and zero integral. The item gives both a discontinuity-set proof and direct Darboux control.",
...
```json
{
  ...
  "derivations": [
    {
      "id": "ex-thomae-is-riemann-integrable-with-integral-zero-step-1-1",
      "claim": "$t$ is bounded on $[0,1]$, with $0 \\le t(x) \\le 1$ for every $x$, by [L1]. [given, L1, L10]",
      "step": "1.1",
      "inputs": [
        "L1",
        "L10"
    ...
    {
      "id": "ex-thomae-is-riemann-integrable-with-integral-zero-step-1-2",
      "claim": "By [L2], $t$ is continuous at every irrational point of $[0,1]$, so its set of discontinuities in $[0,1]$ is contained in $\\mathbb{Q}\\cap[0,1]$, which is at most countable by [L3]; a subset of it is then at most countable as well. [given, L2, L3]",
      "step": "1.2",
      "inputs": [
        "L2",
        "L3"
    ...
    {
      "id": "ex-thomae-is-riemann-integrable-with-integral-zero-step-2-1",
      "claim": "By [L4] applied on $[0,1]$, with $0 < 1$, $t$ is Riemann integrable on $[0,1]$. [step 1.1, step 1.2, L4]",
      "step": "2.1",
      "inputs": [
        "L4",
        "1.1",
    ...
    {
      "id": "ex-thomae-is-riemann-integrable-with-integral-zero-step-2-2",
      "claim": "**Every lower sum is $0$.** Let $P = (n,s)$ be a partition of $[0,1]$ and $i < n$. By [L6] the interval $(s_i,s_{i+1})$ is nonempty and open, so by [L5] it contains an irrational $y$, and $y \\in I_i \\subseteq [0,1]$, so $t(y) = 0$ by [L1]. Since $t \\ge 0$ by [L1], the value $0$ is the least element of $t[I_i]$ and $m_i = 0$ by [L8]. Hence $L(t,P) = 0$ by [L7] and [L9]. [step 1.1, L1, L5, L6, L7, L8, L9]",
      "step": "2.2",
      "inputs": [
        "L6",
        "L5",
    ...
    {
      "id": "ex-thomae-is-riemann-integrable-with-integral-zero-step-3-1",
      "claim": "The set of lower sums is therefore $\\{0\\}$ and $\\underline{\\int_0^1}t = 0$ by [L8]; since $t$ is integrable by step 2.1, $\\int_0^1 t = 0$ by [L7]. [step 2.1, step 2.2, L7, L8] ∎",
      "step": "3.1",
      "inputs": [
        "L8",
        "2.1",
...
## Complete current text of every declared or cited item (30)
