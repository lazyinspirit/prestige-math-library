...
## Wave 10 target — `cor-zero-derivative-implies-constant`

Selection: mandatory Wave 10 high/critical proof-bearing route (risk report classifies all 49 contracts critical).

Normalized current SHA-256: `b0e4e7698b7004e27c467ae9f84fc8b05b2542544a9c7ecbf4fb5442e99b9aac`
...
```json
{
  ...
  "rationale": "The source gives the standard zero-derivative-implies-constant corollary of the mean value theorem. The item supplies the interval endpoint bookkeeping.",
...
```json
{
  ...
  "derivations": [
    {
      "id": "cor-zero-derivative-implies-constant-step-1-1",
      "claim": "If $I$ has at most one element then $f$ is constant on $I$ and there is nothing to prove, the second claim following likewise. So assume $I$ has at least two elements and let $u, v \\in I$ with $u < v$ be arbitrary. [given, L2]",
      "step": "1.1",
      "inputs": [
        "L2"
      ]
    ...
    {
      "id": "cor-zero-derivative-implies-constant-step-2-1",
      "claim": "By [L2] the segment $[u,v]$ is contained in $I$, and $u < v$, so $[u,v]$ is a nondegenerate interval. The restriction $f|_{[u,v]}$ is continuous on $[u,v]$ by [L5]. [step 1.1, L2, L5]",
      "step": "2.1",
      "inputs": [
        "L2",
        "L5",
    ...
    {
      "id": "cor-zero-derivative-implies-constant-step-2-2",
      "claim": "Let $x \\in (u,v)$. By [L3] the point $x$ is interior to $I$, so $f$ is differentiable at $x$ with $f'(x) = 0$ by hypothesis. By [L4] the point $x$ is a limit point of $[u,v]$, so $f|_{[u,v]}$ is differentiable at $x$ with $(f|_{[u,v]})'(x) = f'(x) = 0$. [step 1.1, L3, L4]",
      "step": "2.2",
      "inputs": [
        "L3",
        "L4",
    ...
    {
      "id": "cor-zero-derivative-implies-constant-step-3-1",
      "claim": "By steps 2.1 and 2.2 the function $f|_{[u,v]}$ satisfies the hypotheses of [L1] on $[u,v]$, so there is $c \\in (u,v)$ with $f(v) - f(u) = (f|_{[u,v]})'(c)\\,(v-u) = 0 \\cdot (v-u) = 0$. Hence $f(u) = f(v)$. [step 2.1, step 2.2, L1]",
      "step": "3.1",
      "inputs": [
        "2.1",
        "2.2",
    ...
    {
      "id": "cor-zero-derivative-implies-constant-step-4-1",
      "claim": "Any two distinct points of $I$ can be named $u$ and $v$ with $u < v$, and step 3.1 then gives $f(u) = f(v)$; at a single point the equality is trivial. So $f$ takes one and the same value at every point of $I$, and $f$ is constant on $I$. [step 1.1, step 3.1]",
      "step": "4.1",
      "inputs": [
        "3.1",
        "1.1"
    ...
    {
      "id": "cor-zero-derivative-implies-constant-step-5-1",
      "claim": "**Second claim.** Put $h := f + (-1)g$, so $h(x) = f(x) - g(x)$ on $I$. By [L6] the function $h$ is continuous on $I$. If $I$ has at most one element the claim is trivial; otherwise every point of $I$ is a limit point of $I$ by [L4], so at every interior point $x$ of $I$ the sum rule of [L6] applies and gives that $h$ is differentiable at $x$ with $h'(x) = f'(x) - g'(x) = 0$. By step 4.1, applied to $h$ in place of $f$, the function $h$ is constant on $I$; writing $k$ for its value, $f(x) = g(x) + k$ for every $x \\in I$. [step 4.1, L4, L6] ∎",
      "step": "5.1",
      "inputs": [
        "L6",
        "L4",
  ...
  "boundaries": [
    ...
    },
    {
      "case": "zero",
      "status": "checked",
      "evidence": "Step 2.2 explicitly includes the zero-valued, vanishing, or zero-index case."
    },
    {
      "case": "one",
      "status": "checked",
