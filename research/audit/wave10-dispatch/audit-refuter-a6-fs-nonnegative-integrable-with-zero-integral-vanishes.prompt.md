...
## Wave 10 target — `fs-nonnegative-integrable-with-zero-integral-vanishes`

Selection: mandatory Wave 10 high/critical proof-bearing route (risk report classifies all 49 contracts critical).

Normalized current SHA-256: `a41f24ceb01521f40048a36d3b15e1eaf3ae2003d7b44148f66913ea597b45b4`
...
```json
{
  ...
  "rationale": "The source establishes that Thomae’s nonnegative function is Riemann integrable with integral zero although it is not identically zero. The item packages this as a counterexample to the false pointwise conclusion.",
...
```json
{
  ...
  "derivations": [
    {
      "id": "fs-nonnegative-integrable-with-zero-integral-vanishes-step-1-1",
      "claim": "$t$ is nonnegative and bounded on $[0,1]$, with $0 \\le t(x) \\le 1$ for every $x$, by [L1]. [given, L1, L10]",
      "step": "1.1",
      "inputs": [
        "L1",
        "L10"
    ...
    {
      "id": "fs-nonnegative-integrable-with-zero-integral-vanishes-step-1-2",
      "claim": "$t$ is continuous at every irrational point of $[0,1]$ by [L2], so its set of discontinuities in $[0,1]$ is contained in $\\mathbb{Q} \\cap [0,1]$, which is at most countable by [L3]; every subset of it is then at most countable by [L3] as well. [given, L2, L3]",
      "step": "1.2",
      "inputs": [
        "L2",
        "L3"
    ...
    {
      "id": "fs-nonnegative-integrable-with-zero-integral-vanishes-step-1-3",
      "claim": "Separately, and independently of everything below, $t$ does not vanish identically: $2^{-1}$ is a rational point of $[0,1]$, so $t(2^{-1}) > 0$ by [L1] and [L10]. [L1, L10]",
      "step": "1.3",
      "inputs": [
        "L1",
        "L10"
    ...
    {
      "id": "fs-nonnegative-integrable-with-zero-integral-vanishes-step-2-1",
      "claim": "By [L4] applied to $[0,1]$, with $0 < 1$, the function $t$ is Riemann integrable on $[0,1]$. [step 1.1, step 1.2, L4]",
      "step": "2.1",
      "inputs": [
        "L4",
        "1.1",
    ...
    {
      "id": "fs-nonnegative-integrable-with-zero-integral-vanishes-step-2-2",
      "claim": "Every lower Darboux sum of $t$ is $0$: let $P$ be a partition of $[0,1]$ and $i < n$; the open interval $(t^{P}_i, t^{P}_{i+1})$ is nonempty by [L6] and contains an irrational $y$ by [L5], and $y \\in I_i \\subseteq [0,1]$, so $t(y) = 0$ by [L1]. Since $t \\ge 0$ by [L1], the value $0$ is the least element of $t[I_i]$ and $m_i = 0$ by [L8]. Hence $L(t,P) = \\sum_{i<n}0\\cdot\\Delta_i = 0$ by [L7] and [L9]. [step 1.1, L1, L5, L6, L7, L8, L9]",
      "step": "2.2",
      "inputs": [
        "L6",
        "L5",
    ...
    {
      "id": "fs-nonnegative-integrable-with-zero-integral-vanishes-step-3-1",
      "claim": "The set of lower sums is therefore $\\{0\\}$, so $\\underline{\\int_0^1} t = 0$ by [L8], and since $t$ is integrable by step 2.1 its integral is $\\int_0^1 t = 0$ by [L7]. [step 2.1, step 2.2, L7, L8]",
      "step": "3.1",
      "inputs": [
        "L8",
        "2.1",
    ...
    {
      "id": "fs-nonnegative-integrable-with-zero-integral-vanishes-step-4-1",
      "claim": "So $t$ is a nonnegative Riemann integrable function on $[0,1]$ with $\\int_0^1 t = 0$ that is not identically zero; [A1] fails at $t$ and the claim is false. [step 1.1, step 3.1, step 1.3, A1] ∎",
      "step": "4.1",
      "inputs": [
        "A1",
        "1.1",
...
[
  {
    "source": "fs-nonnegative-integrable-with-zero-integral-vanishes",
...
## Complete current text of every declared or cited item (29)
