...
## Wave 10 target — `thm-monotone-implies-integrable`

Selection: mandatory Wave 10 high/critical proof-bearing route (risk report classifies all 49 contracts critical).

Normalized current SHA-256: `db784f42a761d2d86dabb1af6f7ab1e8ee398a0c7441051d1f2c22e8d193f14f`
...
```json
{
  ...
  "urls": [
    ...
    "https://www.math.ucdavis.edu/~hunter/m125b/125bLectureNotes_5-26-11.pdf"
  ],
  "rationale": "The sources prove that bounded monotone functions on closed intervals are Riemann integrable. The item writes the telescoping uniform-partition estimate for both monotonicity directions.",
...
```json
{
  ...
  "derivations": [
    {
      "id": "thm-monotone-implies-integrable-step-1-1",
      "claim": "By [L1] there are two cases, $f$ nondecreasing and $f$ nonincreasing, and they exhaust the hypothesis. Every $x \\in [a,b]$ satisfies $a \\le x \\le b$ ([[def-interval]]). [given, L1, cases]",
      "step": "1.1",
      "inputs": [
        "L1"
      ]
    ...
    {
      "id": "thm-monotone-implies-integrable-step-1-2",
      "claim": "**Case: $f$ is nondecreasing.** Then $f(a) \\le f(x) \\le f(b)$ for every $x \\in [a,b]$, so $f$ is bounded by [L8]; and for $i < N$ and $x \\in I_i$ one has $t_i \\le x \\le t_{i+1}$, hence $f(t_i) \\le f(x) \\le f(t_{i+1})$. Since $f(t_i)$ and $f(t_{i+1})$ themselves lie in $f[I_i]$, they are its least and greatest elements, so $m_i = f(t_i)$ and $M_i = f(t_{i+1})$ by [L3]. [assume-case up, given, L2, L3, L8]",
      "step": "1.2",
      "inputs": [
        "L8",
        "L3",
    ...
    {
      "id": "thm-monotone-implies-integrable-step-1-3",
      "claim": "**Case: $f$ is nonincreasing.** Then $f(b) \\le f(x) \\le f(a)$ for every $x \\in [a,b]$, so $f$ is bounded; and for $i < N$ and $x \\in I_i$ one has $f(t_{i+1}) \\le f(x) \\le f(t_i)$, so $m_i = f(t_{i+1})$ and $M_i = f(t_i)$ by [L3]. [assume-case down, given, L2, L3, L8]",
      "step": "1.3",
      "inputs": [
        "L3",
        "L2",
    ...
    {
      "id": "thm-monotone-implies-integrable-step-2-1",
      "claim": "In the nondecreasing case, $M_i - m_i = f(t_{i+1}) - f(t_i)$, so by [L4], [L2] and [L5], $U(f,U_N) - L(f,U_N) = \\sum_{i<N}\\bigl(f(t_{i+1}) - f(t_i)\\bigr)\\frac{b-a}{\\iota(N)} = \\frac{b-a}{\\iota(N)}\\bigl(f(t_N) - f(t_0)\\bigr) = \\frac{b-a}{\\iota(N)}\\bigl(f(b) - f(a)\\bigr)$, and $f(b) - f(a) \\ge 0$, so this equals $|f(b)-f(a)|(b-a)/\\iota(N)$ by [L8]. [step 1.2, L2, L4, L5, L8]",
      "step": "2.1",
      "inputs": [
        "L4",
        "L2",
    ...
    {
      "id": "thm-monotone-implies-integrable-step-2-2",
      "claim": "In the nonincreasing case the same computation gives $U(f,U_N) - L(f,U_N) = \\frac{b-a}{\\iota(N)}\\bigl(f(a) - f(b)\\bigr)$ with $f(a) - f(b) \\ge 0$, which is again $|f(b)-f(a)|(b-a)/\\iota(N)$ by [L8]. The two cases of step 1.1 exhaust the hypothesis, so the displayed identity holds for every monotone $f$, which is also bounded. [step 1.3, L2, L4, L5, L8, cases-exhaustive]",
      "step": "2.2",
      "inputs": [
        "L8",
        "1.1",
    ...
    {
      "id": "thm-monotone-implies-integrable-step-3-1",
      "claim": "Let a real $\\varepsilon > 0$ be given and put $\\eta := \\varepsilon \\cdot \\bigl((|f(b)-f(a)| + 1)(b-a)\\bigr)^{-1}$, a positive real by [L8]. By [L7] fix a natural $N \\ge 1$ with $1/\\iota(N) < \\eta$. [step 2.2, L7, L8, choose]",
      "step": "3.1",
      "inputs": [
        "L8",
        "L7",
    ...
    {
      "id": "thm-monotone-implies-integrable-step-4-1",
      "claim": "Then $U(f,U_N) - L(f,U_N) = |f(b)-f(a)|\\,(b-a)/\\iota(N) \\le (|f(b)-f(a)|+1)(b-a)/\\iota(N) < (|f(b)-f(a)|+1)(b-a)\\,\\eta = \\varepsilon$: the first inequality because $(b-a)/\\iota(N) > 0$ and $|f(b)-f(a)| \\le |f(b)-f(a)|+1$, and the second because $(|f(b)-f(a)|+1)(b-a) > 0$ and $1/\\iota(N) < \\eta$. So $U(f,U_N) - L(f,U_N) < \\varepsilon$. [step 2.1, step 2.2, step 3.1, L8]",
      "step": "4.1",
      "inputs": [
        "2.1",
        "2.2",
    ...
    {
      "id": "thm-monotone-implies-integrable-step-5-1",
      "claim": "Since the real $\\varepsilon > 0$ was arbitrary and step 4.1 produced a partition with $U - L < \\varepsilon$, criterion [L6] applies: $f$ is bounded by steps 1.2 and 1.3 and Riemann integrable on $[a,b]$. [step 1.2, step 1.3, step 4.1, L6] ∎",
      "step": "5.1",
      "inputs": [
        "4.1",
        "L6",
...
## Complete current text of every declared or cited item (24)
