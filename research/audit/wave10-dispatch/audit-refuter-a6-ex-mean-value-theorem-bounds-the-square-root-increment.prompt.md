...
## Wave 10 target — `ex-mean-value-theorem-bounds-the-square-root-increment`

Selection: mandatory Wave 10 high/critical proof-bearing route (risk report classifies all 49 contracts critical).

Normalized current SHA-256: `baeeefa9cb6b8d505969fdc5c80e39eaae829371717e637854d88210d830ee47`
...
```json
{
  ...
  "rationale": "The source gives derivative-bound consequences of the mean value theorem. The square-root increment estimate is a locally chosen specialization using the inverse-rule derivative.",
...
```json
{
  ...
  "derivations": [
    {
      "id": "ex-mean-value-theorem-bounds-the-square-root-increment-step-1-1",
      "claim": "The interior points of $I = [1,\\infty)$ are exactly the reals $b > 1$. For $b > 1$ the neighbourhood $N_{b-1}(b)$ is contained in $(1,\\infty) \\subseteq I$, so $b$ is interior; the point $1$ is not interior, since $1 - \\varepsilon/2 \\in N_{\\varepsilon}(1)$ and $1 - \\varepsilon/2 \\notin I$ for every real $\\varepsilon > 0$; and every interior point of $I$ lies in $I$, hence is $\\ge 1$. [L6]",
      "step": "1.1",
      "inputs": [
        "L6"
      ]
    ...
    {
      "id": "ex-mean-value-theorem-bounds-the-square-root-increment-step-1-2",
      "claim": "For every real $b \\ge 1$ one has $b^{-1/2} \\le 1$. If $b = 1$ then $1^{1/2} = 1$ by [L4], so $1^{-1/2} = 1/1 = 1$. If $b > 1$ then $b^{1/2} > 1$ by [L4], and $b^{1/2} > 0$, so $b^{-1/2} = 1/b^{1/2} < 1$ by [L4] and [L5]. [L4, L5]",
      "step": "1.2",
      "inputs": [
        "L4",
        "L5"
    ...
    {
      "id": "ex-mean-value-theorem-bounds-the-square-root-increment-step-2-1",
      "claim": "By [L1] and [L2] the function $s$ is differentiable at every $b \\in I$ with $s'(b) = \\frac{1}{\\iota(2)}b^{-1/2}$, and by [L3] it is continuous at every point of $I$, hence continuous on $I$. [step 1.1, L1, L2, L3]",
      "step": "2.1",
      "inputs": [
        "L1",
        "L2",
    ...
    {
      "id": "ex-mean-value-theorem-bounds-the-square-root-increment-step-3-1",
      "claim": "At every interior point $b$ of $I$ one has $b > 1$ by step 1.1, so $b^{-1/2} > 0$ by [L4] and $b^{-1/2} \\le 1$ by step 1.2; multiplying the pair $0 \\le b^{-1/2} \\le 1$ and $0 \\le 1/\\iota(2) \\le 1/\\iota(2)$ as in [L5] gives $0 < s'(b) \\le 1/\\iota(2)$ by step 2.1, and therefore $|s'(b)| = s'(b) \\le 1/\\iota(2)$ by [L5]. [step 1.1, step 1.2, step 2.1, L4, L5]",
      "step": "3.1",
      "inputs": [
        "1.1",
        "L4",
    ...
    {
      "id": "ex-mean-value-theorem-bounds-the-square-root-increment-step-4-1",
      "claim": "Apply [L7] with $J := I$, $h := s$ and $M := 1/\\iota(2)$, a real $\\ge 0$ by [L5]: the hypotheses hold by step 2.1 for the continuity and differentiability and by step 3.1 for the bound, so $|s(x)-s(y)| \\le \\frac{1}{\\iota(2)}|x-y|$ for all $x, y \\in I$, that is $|\\sqrt{x} - \\sqrt{y}| \\le \\frac{1}{\\iota(2)}|x-y|$; $s$ is Lipschitz with constant $1/\\iota(2)$ on $I$; and $s$ is uniformly continuous on $I$. [step 2.1, step 3.1, L5, L7] ∎",
      "step": "4.1",
      "inputs": [
        "L7",
        "L5",
...
[
  {
    "source": "ex-mean-value-theorem-bounds-the-square-root-increment",
  ...
  },
  {
    ...
    "sourcePage": "the-derivative-and-mean-value-theorems-examples",
    "batch": "wave10-real-analysis",
    "target": "ex-derivative-of-the-nth-root-by-the-inverse-rule",
    "declared_target": "ex-derivative-of-the-nth-root-by-the-inverse-rule",
    "target_statement_provenance": "ai-altered",
