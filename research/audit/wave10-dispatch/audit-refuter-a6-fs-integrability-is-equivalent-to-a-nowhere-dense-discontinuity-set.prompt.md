...
## Wave 10 target — `fs-integrability-is-equivalent-to-a-nowhere-dense-discontinuity-set`

Selection: mandatory Wave 10 high/critical proof-bearing route (risk report classifies all 49 contracts critical).

Normalized current SHA-256: `9421258d6446f3b930452dd18a828c16bbb93e36b48687a36e23476da95553c2`
...
```json
{
  ...
  "rationale": "The sources combine Lebesgue’s criterion with a positive-measure nowhere-dense fat Cantor set. The item uses its indicator to refute the proposed nowhere-dense characterization.",
...
```json
{
  ...
  "derivations": [
    {
      "id": "fs-integrability-is-equivalent-to-a-nowhere-dense-discontinuity-set-step-1-1",
      "claim": "$g$ is bounded, taking only the values $0$ and $1$, so its Darboux sums and integrals are defined by [L5]. [given, L5]",
      "step": "1.1",
      "inputs": [
        "L5"
      ]
    ...
    {
      "id": "fs-integrability-is-equivalent-to-a-nowhere-dense-discontinuity-set-step-1-2",
      "claim": "**$g$ is discontinuous at every point of $S$.** Let $x \\in S$, so $g(x) = 1$, and let a real $\\rho > 0$ be given. Put $u := \\max\\{0,\\ x-\\rho\\}$ and $v := \\min\\{1,\\ x+\\rho\\}$; then $u < v$ because $0 \\le x \\le 1$ and $\\rho > 0$, and $(u,v) \\subseteq N_\\rho(x) \\cap [0,1]$ by [L8]. Since $S$ is closed and nowhere dense it contains no nonempty open set by [L1] and [L2], so there is $y \\in (u,v)$ with $y \\notin S$. Then $y \\in [0,1]$, $|y - x| < \\rho$ and $|g(x)-g(y)| = 1$, so the continuity condition fails at $x$ for $\\varepsilon := 1$. [given, L1, L2, L6, L8]",
      "step": "1.2",
      "inputs": [
        "L8",
        "L1",
    ...
    {
      "id": "fs-integrability-is-equivalent-to-a-nowhere-dense-discontinuity-set-step-1-3",
      "claim": "**$g$ is continuous at every point of $[0,1] \\setminus S$.** Let $x \\in [0,1]$ with $x \\notin S$. Since $S$ is closed, [L3] gives a real $\\rho > 0$ with $N_\\rho(x) \\cap S = \\varnothing$, so $g$ vanishes identically on $N_\\rho(x)\\cap[0,1]$ and $|g(y)-g(x)| = 0 < \\varepsilon$ there, for every $\\varepsilon > 0$. [given, L1, L3]",
      "step": "1.3",
      "inputs": [
        "L3",
        "L1"
    ...
    {
      "id": "fs-integrability-is-equivalent-to-a-nowhere-dense-discontinuity-set-step-2-1",
    ...
    {
      "id": "fs-integrability-is-equivalent-to-a-nowhere-dense-discontinuity-set-step-2-2",
      "claim": "**Every upper Darboux sum of $g$ is at least $2^{-1}$.** Let $P = (n,t)$ be a partition of $[0,1]$ and let $B := \\{\\, i < n : I_i \\cap S \\ne \\varnothing \\,\\}$. For $i \\in B$ the set $g[I_i]$ contains $1$, so $M_i = 1$ by [L6] and $g \\le 1$; for $i \\notin B$ one has $g[I_i] = \\{0\\}$ and $M_i = 0$. Hence $U(g,P) = \\sum_{i<n}M_i\\Delta_i$ is the sum of the $\\Delta_i$ with $i \\in B$, by [L5] and [L7]. [step 1.1, L5, L6, L7]",
      "step": "2.2",
      "inputs": [
        "L6",
        "L5",
    ...
    {
      "id": "fs-integrability-is-equivalent-to-a-nowhere-dense-discontinuity-set-step-2-3",
      "claim": "**Every lower Darboux sum of $g$ is $0$.** With $P$ as above and $i < n$: $t_i < t_{i+1}$ by [L4], so $(t_i,t_{i+1})$ is a nonempty open subset of $[0,1]$, and by [L1] and [L2] it is not contained in $S$; a point of it outside $S$ lies in $I_i \\cap [0,1]$ and has $g$-value $0$, so $g[I_i]$ contains $0$ and $m_i = 0$ by [L6], $g$ being nonnegative. Hence $L(g,P) = 0$ by [L5] and [L7]. [step 1.1, L1, L2, L4, L5, L6, L7]",
      "step": "2.3",
      "inputs": [
        "L4",
        "L1",
    ...
    {
      "id": "fs-integrability-is-equivalent-to-a-nowhere-dense-discontinuity-set-step-3-1",
      "claim": "The intervals $I_i$ with $i \\in B$ cover $S$: a point of $S$ lies in $[0,1] = \\bigcup_{i<n}I_i$ by [L4], hence in some $I_i$, and that $i$ is in $B$. Extending this finite list of closed intervals to a sequence by degenerate intervals $[0,0]$ ([L7]) gives a cover of $S$ all of whose partial total lengths are at most $U(g,P)$, so [L1] gives $U(g,P) \\ge 2^{-1}$. [step 2.2, L1, L4, L7]",
      "step": "3.1",
      "inputs": [
        "L4",
    ...
    {
      "id": "fs-integrability-is-equivalent-to-a-nowhere-dense-discontinuity-set-step-4-1",
      "claim": "Therefore $\\underline{\\int_0^1} g = 0$ by [L6] and step 2.3, while $\\overline{\\int_0^1} g \\ge 2^{-1}$ by step 3.1, since every upper sum is at least $2^{-1}$ and the infimum of such a set is at least $2^{-1}$. The two differ, so $g$ is not Riemann integrable by [L5]. [step 3.1, step 2.3, L5, L6]",
      "step": "4.1",
      "inputs": [
        "L6",
        "2.3",
    ...
    {
      "id": "fs-integrability-is-equivalent-to-a-nowhere-dense-discontinuity-set-step-5-1",
      "claim": "So $g$ is a bounded function on $[0,1]$ whose set of discontinuities is nowhere dense and which is not Riemann integrable; [A1] fails at $g$, and with it the claimed equivalence. [step 2.1, step 4.1, A1] ∎",
      "step": "5.1",
      "inputs": [
        "A1",
        "2.1",
  ...
  "boundaries": [
    ...
    },
    {
      "case": "iff-forward",
      "status": "checked",
      "evidence": "Statement records the integrable-but-dense-discontinuity Thomae counterexample, so the integrable-to-nowhere-dense direction is explicitly disposed of."
    },
    {
      "case": "iff-reverse",
      "status": "checked",
      "evidence": "Statement and steps 1.1-5.1 give a fat-Cantor indicator with nowhere-dense discontinuities that is not integrable."
    }
  ]
}
```
...
[
  {
    "source": "fs-integrability-is-equivalent-to-a-nowhere-dense-discontinuity-set",
