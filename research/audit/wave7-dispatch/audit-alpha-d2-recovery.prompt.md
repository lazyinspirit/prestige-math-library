...
> `research/audit/wave7-alpha.md` with your active substage, owned
...
> `wave7-alpha.md` naming files read, baseline, inherited open actions, and
...
4. **Audit every cross-batch and cross-level edge** against
   `research/audit/wave7-audit-manifest.json`: read the source item's use
...
`research/audit/wave7-rejudge-targets.json` repair list. Adjudicate only
...
per-context owner decision to `research/audit/wave7-judge-adjudications.jsonl` so
...
```
node tools/touchlog.mjs snap research/audit/wave7-touches.json "pre-a8"
node tools/step8-guard.mjs --touches research/audit/wave7-touches.json \
  --baseline "pre-a8" --adjudications research/audit/wave7-judge-adjudications.jsonl
```
...
evidence in `research/audit/wave7-alpha.md`.
