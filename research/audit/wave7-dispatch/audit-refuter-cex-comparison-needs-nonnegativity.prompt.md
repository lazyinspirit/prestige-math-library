...
- critical risk (12): 10 declared dependencies; 11 cited facts; biconditional / both-direction claim; boundary-sensitive language; analytic limiting/completeness language

## Target item — `cex-comparison-needs-nonnegativity`

Normalized current SHA-256: `c41253dd477d913f03849fbcec0bd72e4f34b1acadfd4c6efd081f9a110ffcbf`
...
```json
{
  ...
  "rationale": "The item is the standard direct-comparison theorem with its nonnegativity hypothesis removed, refuted by the negative harmonic series below the zero series. That is a recoverable alteration of the sourced theorem, not a novel mathematical claim. The explicit signed witness and verification are locally chosen.",
...
```json
{
  ...
  "derivations": [
    {
      "id": "step-1-1",
      "claim": "For every $k \\in \\mathbb{N}$, $a_k = -1/\\iota(k+1) < 0 = b_k$, so in particular $a_k \\le b_k$.",
      "step": "1.1",
      "inputs": [
        "L1",
        "given"
    ...
    {
      "id": "step-1-2",
      "claim": "The partial sums of $\\sum b_k$ are $\\sum_{j<n} 0 = 0$ for every $n$, a constant sequence, so $\\sum b_k$ converges with sum $0$.",
      "step": "1.2",
      "inputs": [
        "L2",
        "L3",
    ...
    {
      "id": "step-1-3",
      "claim": "The sequence $(a_k)$ is $(-1)$ times the sequence $j \\mapsto 1/\\iota(j+1)$, whose series is the harmonic series and diverges; since $-1 \\ne 0$, $\\sum a_k$ diverges.",
      "step": "1.3",
      "inputs": [
        "L4",
        "L5",
    ...
    {
      "id": "step-2-1",
      "claim": "So the hypotheses of the claim hold for this pair while its conclusion fails, and the claim is false.",
      "step": "2.1",
      "inputs": [
        "1.1",
        "1.2",
    ...
    {
      "id": "step-3-1",
      "claim": "The genuine comparison test is untouched: it requires $0 \\le a_k$ from some index on, and here $a_k < 0$ at every index.",
      "step": "3.1",
      "inputs": [
        "1.1",
        "L6"
  ...
  "boundaries": [
    ...
    },
    {
      "case": "zero",
      "status": "checked",
      "evidence": "steps 1.1-1.2 use b_k = 0 exactly and verify its zero partial sums converge."
    },
    {
      "case": "one",
      "status": "checked",
      "evidence": "step 1.3 uses the nonzero scalar -1 and the harmonic family shifted from index 1."
    },
    {
      "case": "degenerate",
      "status": "checked",
      "evidence": "the zero comparison series is the intended degenerate case, checked in steps 1.1-1.2."
    },
    {
      "case": "endpoints",
      "status": "checked",
      "evidence": "steps 1.1 and 3.1 show the missing lower bound 0 <= a_k fails at every index, not merely at a finite prefix."
    },
    {
...
[
  {
    "source": "cex-comparison-needs-nonnegativity",
