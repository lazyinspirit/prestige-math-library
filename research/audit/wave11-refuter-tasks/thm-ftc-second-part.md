...
- critical risk (12): 13 declared dependencies; 13 cited facts; 6 numbered proof steps; existence, choice, uniqueness, or well-definedness; boundary-sensitive language; analytic limiting/completeness language

## Target item — `thm-ftc-second-part`

Normalized current SHA-256: `b97a507ae103b3c595df871b49d55536ec6d92062a31780dea4ea9008f08e3ca`
...
```json
{
  ...
  "rationale": "The sources state the Newton-Leibniz formula for a differentiable function whose derivative is Riemann integrable, with the same conclusion. The item's proof through the first FTC and constant-difference theorem is local.",
...
```json
{
  ...
  "derivations": [
    {
      "id": "thm-ftc-second-part-step-1-1",
      "claim": "Let $P = (n,t)$ be an arbitrary partition of $[a,b]$ and let $i < n$. The restriction of $G$ to $I_i = [t_i,t_{i+1}]$ is continuous on $I_i$ and differentiable at every point of $(t_i,t_{i+1})$, with the same derivative $f$ there, by [L5] and [L1]. [given, L1, L5]",
      "step": "1.1",
      "inputs": [
        "given",
        "L1",
    ...
    {
      "id": "thm-ftc-second-part-step-2-1",
      "claim": "By [L4] applied on $I_i$ there is $\\xi_i \\in (t_i,t_{i+1})$ with $G(t_{i+1}) - G(t_i) = f(\\xi_i)\\,\\Delta_i$; since $\\xi_i \\in I_i$ and $\\Delta_i > 0$, [L2] gives $m_i\\Delta_i \\le G(t_{i+1})-G(t_i) \\le M_i\\Delta_i$. [step 1.1, L1, L2, L4, L7]",
      "step": "2.1",
      "inputs": [
        "step 1.1",
        "L1",
    ...
    {
      "id": "thm-ftc-second-part-step-3-1",
      "claim": "Step 2.1 holds for every $i < n$, so monotonicity of finite sums applies to the three families and gives $\\sum_{i<n}m_i\\Delta_i \\le \\sum_{i<n}\\bigl(G(t_{i+1})-G(t_i)\\bigr) \\le \\sum_{i<n}M_i\\Delta_i$. [step 2.1, L6]",
      "step": "3.1",
      "inputs": [
        "step 2.1",
        "L6"
    ...
    {
      "id": "thm-ftc-second-part-step-4-1",
      "claim": "The middle sum telescopes to $G(t_n) - G(t_0) = G(b) - G(a)$ by [L6] and [L1], so $L(f,P) \\le G(b)-G(a) \\le U(f,P)$ by [L2]. [step 3.1, L1, L2, L6]",
      "step": "4.1",
      "inputs": [
        "step 3.1",
        "L1",
    ...
    {
      "id": "thm-ftc-second-part-step-5-1",
      "claim": "Step 4.1 holds for **every** partition $P$, so $G(b)-G(a)$ is an upper bound of the set of lower sums and a lower bound of the set of upper sums; hence $\\underline{\\int_a^b} f \\le G(b)-G(a) \\le \\overline{\\int_a^b} f$ by [L3] and [L7]. [step 4.1, L3, L7]",
      "step": "5.1",
      "inputs": [
        "step 4.1",
        "L3",
    ...
    {
      "id": "thm-ftc-second-part-step-6-1",
      "claim": "Since $f$ is integrable the two integrals coincide with $\\int_a^b f$, so $\\int_a^b f = G(b)-G(a)$. [step 5.1, L3] ∎",
      "step": "6.1",
      "inputs": [
        "step 5.1",
        "L3"
  ...
  "boundaries": [
    {
      ...
      "reason": "thm-ftc-second-part concerns stipulated functions and points; no empty family, empty sum, or empty choice domain changes its assertion."
    },
    {
      "case": "zero",
      "status": "not_applicable",
      "reason": "thm-ftc-second-part has no counting, exponent, or value parameter whose zero case changes the proof."
    },
    {
      "case": "one",
      "status": "checked",
    ...
    },
    {
      "case": "iff-forward",
      "status": "not_applicable",
      "reason": "The Statement of thm-ftc-second-part is not a biconditional, so it has no forward iff direction."
    },
    {
      "case": "iff-reverse",
      "status": "not_applicable",
      "reason": "The Statement of thm-ftc-second-part is not a biconditional, so it has no reverse iff direction."
    }
  ],
  "finite_smoke": [],
  "risk_review": {
