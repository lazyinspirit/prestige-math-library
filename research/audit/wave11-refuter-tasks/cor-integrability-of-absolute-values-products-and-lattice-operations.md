...
- critical risk (11): 16 declared dependencies; 17 cited facts; 10 numbered proof steps; boundary-sensitive language; analytic limiting/completeness language
- top-20 manifest-edge consumer (17 outgoing relationships)

## Target item — `cor-integrability-of-absolute-values-products-and-lattice-operations`

Normalized current SHA-256: `f05393726935afc2075db8dc01c72806cddced76f29e6c40f5c0dab5447e7a45`
...
```json
{
  ...
  "rationale": "The sources give closure of Riemann-integrable functions under absolute value, products, maxima and minima. The item packages all four operations as one corollary and obtains the lattice formulas through continuous composition and algebraic identities.",
...
```json
{
  ...
  "derivations": [
    {
      "id": "cor-integrability-of-absolute-values-products-and-lattice-operations-step-1-1",
      "claim": "$f$ is bounded, so fix reals $m \\le M$ with $f[\\,[a,b]\\,] \\subseteq [m,M]$; the same for $g$, and for $f+g$ and $f-g$, which are integrable by [L2]. [given, L1, L2, choose]",
      "step": "1.1",
      "inputs": [
        "given",
        "L1",
    ...
    {
      "id": "cor-integrability-of-absolute-values-products-and-lattice-operations-step-1-2",
      "claim": "The maps $t \\mapsto |t|$ and $t \\mapsto t^{2}$ are continuous on any closed bounded interval, by [L4]. [L4]",
      "step": "1.2",
      "inputs": [
        "L4"
      ]
    ...
    {
      "id": "cor-integrability-of-absolute-values-products-and-lattice-operations-step-2-1",
      "claim": "By [L1] applied with $\\varphi(t) = |t|$ to $f$, to $g$ and to $f-g$, the functions $|f|$, $|g|$ and $|f-g|$ are integrable. [step 1.1, step 1.2, L1]",
      "step": "2.1",
      "inputs": [
        "step 1.1",
        "step 1.2",
    ...
    {
      "id": "cor-integrability-of-absolute-values-products-and-lattice-operations-step-2-2",
      "claim": "By [L1] applied with $\\varphi(t) = t^{2}$ to $f$, to $f+g$ and to $f-g$, the functions $f^{2}$, $(f+g)^{2}$ and $(f-g)^{2}$ are integrable. [step 1.1, step 1.2, L1]",
      "step": "2.2",
      "inputs": [
        "step 1.1",
        "step 1.2",
    ...
    {
      "id": "cor-integrability-of-absolute-values-products-and-lattice-operations-step-3-1",
      "claim": "By [L5], $fg = \\bigl((f+g)^{2}-(f-g)^{2}\\bigr)\\cdot 4^{-1}$ pointwise, so $fg$ is integrable by [L2]; this completes claim 1. [step 2.1, step 2.2, L2, L5]",
      "step": "3.1",
      "inputs": [
        "step 2.1",
        "step 2.2",
    ...
    {
      "id": "cor-integrability-of-absolute-values-products-and-lattice-operations-step-3-2",
      "claim": "By [L5], $\\max(f,g) = \\bigl(f + g + |f-g|\\bigr)\\cdot 2^{-1}$ and $\\min(f,g) = \\bigl(f+g-|f-g|\\bigr)\\cdot 2^{-1}$ pointwise, so both are integrable by [L2]; this is claim 2. [step 2.1, L2, L5]",
      "step": "3.2",
      "inputs": [
        "step 2.1",
        "L2",
    ...
    {
      "id": "cor-integrability-of-absolute-values-products-and-lattice-operations-step-3-3",
      "claim": "**Claim 3.** By [L6], $-|f| \\le f \\le |f|$ pointwise on $[a,b]$, and all three functions are integrable by step 2.1 and [L2]. [step 2.1, L2, L6]",
      "step": "3.3",
      "inputs": [
        "step 2.1",
        "L2",
    ...
    {
      "id": "cor-integrability-of-absolute-values-products-and-lattice-operations-step-4-1",
      "claim": "By [L3] applied twice, $-\\int_a^b|f| \\le \\int_a^b f \\le \\int_a^b |f|$, using $\\int_a^b(-|f|) = -\\int_a^b|f|$ from [L2]. [step 3.3, L2, L3]",
      "step": "4.1",
      "inputs": [
        "step 3.3",
        "L2",
    ...
    {
      "id": "cor-integrability-of-absolute-values-products-and-lattice-operations-step-5-1",
      "claim": "Hence $\\bigl|\\int_a^b f\\bigr| \\le \\int_a^b|f|$ by [L6], which is claim 3. [step 4.1, L6]",
      "step": "5.1",
      "inputs": [
        "step 4.1",
        "L6"
    ...
    {
      "id": "cor-integrability-of-absolute-values-products-and-lattice-operations-step-6-1",
      "claim": "**The oriented form.** For $u = v$ both sides are $0$ by [L7]; for $u < v$ it is claim 3 on $[u,v]$; and for $u > v$ both $\\int_u^v f$ and $\\int_u^v |f|$ are the negatives of the corresponding integrals over $[v,u]$ by [L7], so the two absolute values are unchanged and claim 3 on $[v,u]$ gives the inequality. [step 5.1, L7, L8] ∎",
      "step": "6.1",
      "inputs": [
        "step 5.1",
        "L7",
  ...
  "boundaries": [
    {
      ...
      "reason": "cor-integrability-of-absolute-values-products-and-lattice-operations concerns stipulated functions and points; no empty family, empty sum, or empty choice domain changes its assertion."
    },
    {
      "case": "zero",
      "status": "not_applicable",
      "reason": "cor-integrability-of-absolute-values-products-and-lattice-operations has no counting, exponent, or value parameter whose zero case changes the proof."
    },
    {
      "case": "one",
      "status": "not_applicable",
      "reason": "cor-integrability-of-absolute-values-products-and-lattice-operations has no counting, exponent, or unit parameter whose one case changes the proof."
    },
    {
      "case": "degenerate",
      "status": "checked",
    ...
    },
    {
      "case": "iff-forward",
      "status": "not_applicable",
      "reason": "The Statement of cor-integrability-of-absolute-values-products-and-lattice-operations is not a biconditional, so it has no forward iff direction."
    },
    {
      "case": "iff-reverse",
      "status": "not_applicable",
      "reason": "The Statement of cor-integrability-of-absolute-values-products-and-lattice-operations is not a biconditional, so it has no reverse iff direction."
    }
  ],
  "finite_smoke": [],
  "risk_review": {
...
[
  ...
  },
  {
    ...
    "sourcePage": "properties-of-the-integral-and-the-working-ftc",
    "batch": "wave11-real-analysis",
    "target": "cex-absolute-value-integrable-without-the-function",
    "declared_target": "cex-absolute-value-integrable-without-the-function",
    "target_statement_provenance": "ai-altered",
