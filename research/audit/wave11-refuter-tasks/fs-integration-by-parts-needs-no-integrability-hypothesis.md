...
- critical risk (12): 18 declared dependencies; 19 cited facts; 6 numbered proof steps; existence, choice, uniqueness, or well-definedness; boundary-sensitive language; analytic limiting/completeness language
...
## Target item — `fs-integration-by-parts-needs-no-integrability-hypothesis`

Normalized current SHA-256: `d818d46930c195bf66732ff974f306ec7b8cb8278eb5777a91b83bcb6a93d86c`
...
```json
{
  ...
  "evidence": "none",
  "urls": [],
  "rationale": "This exact undefinedness refutation is a bespoke composition of the preceding local quartic-bump function G with v(x)=x. The proof directly verifies that G is integrable while xG' remains unbounded at the displayed bump points, so one integral in the asserted equation does not denote.",
...
```json
{
  ...
  "derivations": [
    {
      "id": "fs-integration-by-parts-needs-no-integrability-hypothesis-step-1-1",
      "claim": "$u := G$ and $v$ are differentiable at every point of $[0,1]$, by [L1] and [L2]; so the hypothesis of [A1] is satisfied by this pair. [given, A1, L1, L2]",
      "step": "1.1",
      "inputs": [
        "given",
        "A1",
    ...
    {
      "id": "fs-integration-by-parts-needs-no-integrability-hypothesis-step-2-1",
      "claim": "$uv' = G \\cdot 1 = G$, which is continuous on $[0,1]$ by [L3] and therefore integrable there; so the left-hand side of [A1] exists. [step 1.1, L2, L3]",
      "step": "2.1",
      "inputs": [
        "step 1.1",
        "L2",
    ...
    {
      "id": "fs-integration-by-parts-needs-no-integrability-hypothesis-step-2-2",
      "claim": "$u'v$ is the function $x \\mapsto x\\,G'(x)$ on $[0,1]$. At the point $u_n$ its value is $u_n\\,G'(u_n) > \\alpha_n \\cdot \\tfrac{3}{16}\\iota(n+2)^{2} = \\tfrac{3}{16}\\,\\iota(n+2)$, using $\\alpha_n = 1/\\iota(n+2) > 0$ and [L1]. [step 1.1, L1, L5, L6]",
      "step": "2.2",
      "inputs": [
        "step 1.1",
        "L1",
    ...
    {
      "id": "fs-integration-by-parts-needs-no-integrability-hypothesis-step-3-1",
      "claim": "Given a real $M \\ge 0$, [L5] supplies $n$ with $\\tfrac{16}{3}M < \\iota(n+2)$, so $u_n G'(u_n) > M$ by step 2.2; hence $u'v$ is unbounded on $[0,1]$. [step 2.2, L5, L6]",
      "step": "3.1",
      "inputs": [
        "step 2.2",
        "L5",
    ...
    {
      "id": "fs-integration-by-parts-needs-no-integrability-hypothesis-step-4-1",
      "claim": "By [L4] the function $u'v$ has no Darboux sums and is not Riemann integrable on $[0,1]$, so the symbol $\\int_0^1 u'v$ appearing in [A1] does not denote a real number. [step 3.1, L4]",
      "step": "4.1",
      "inputs": [
        "step 3.1",
        "L4",
    ...
    {
      "id": "fs-integration-by-parts-needs-no-integrability-hypothesis-step-5-1",
      "claim": "So [A1] fails at this pair: its left-hand side is defined by step 2.1 and its right-hand side is not, by step 4.1, and the asserted identity is therefore not a true statement about them. [step 2.1, step 4.1, A1] ∎",
      "step": "5.1",
      "inputs": [
        "step 2.1",
        "step 4.1",
  ...
  "boundaries": [
    {
      ...
      "reason": "fs-integration-by-parts-needs-no-integrability-hypothesis concerns stipulated functions and points; no empty family, empty sum, or empty choice domain changes its assertion."
    },
    {
      "case": "zero",
      "status": "checked",
    ...
    },
    {
      ...
      "status": "checked",
      "evidence": "Step 3.1 establishes or explicitly fixes the object before the proof chooses or extremizes it."
    },
    {
      "case": "iff-forward",
      "status": "not_applicable",
      "reason": "The Statement of fs-integration-by-parts-needs-no-integrability-hypothesis is not a biconditional, so it has no forward iff direction."
    },
    {
      "case": "iff-reverse",
      "status": "not_applicable",
      "reason": "The Statement of fs-integration-by-parts-needs-no-integrability-hypothesis is not a biconditional, so it has no reverse iff direction."
    }
  ],
  "finite_smoke": [],
  "risk_review": {
...
[
  {
    "source": "fs-integration-by-parts-needs-no-integrability-hypothesis",
    "sourcePage": "properties-of-the-integral-and-the-working-ftc-examples",
    "batch": "wave11-real-analysis",
    "target": "thm-integration-by-parts",
    "declared_target": "thm-integration-by-parts",
...
[
  {
    "id": "fs-integration-by-parts-needs-no-integrability-hypothesis",
    "file": "items/fs-integration-by-parts-needs-no-integrability-hypothesis.md",
    "source": "provenance",
    "cone_size": 1,
    "logical_consumers": [],
