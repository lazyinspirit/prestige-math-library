...
- critical risk (10): 23 declared dependencies; 23 cited facts; 9 numbered proof steps; boundary-sensitive language; analytic limiting/completeness language
- top-20 manifest-edge consumer (23 outgoing relationships)

## Target item — `cex-spikes-with-integral-one-converging-pointwise-to-zero`

Normalized current SHA-256: `78947c36dad438a2b1464897a4521b653ba2818c2429de095f8d85c198ce0370`
...
```json
{
  ...
  "rationale": "The university notes give the standard moving/shrinking spike phenomenon: pointwise convergence to zero with every integral fixed at one. The item uses a specific continuous triangular tent supported on [0,1/(n+1)] and translates the construction into canonical-natural notation.",
...
```json
{
  ...
  "derivations": [
    {
      "id": "cex-spikes-with-integral-one-converging-pointwise-to-zero-step-1-1",
      "claim": "$c_n \\ge 1$ by [L1], so $0 < 1/(2c_n) < 1/c_n \\le 1$ and the three pieces of the definition subdivide $[0,1]$. [given, L1]",
      "step": "1.1",
      "inputs": [
        "given",
        "L1"
    ...
    {
      "id": "cex-spikes-with-integral-one-converging-pointwise-to-zero-step-2-1",
      "claim": "The three formulas agree at the shared endpoints: at $x = 1/(2c_n)$ both give $4c_n^{2}/(2c_n) = 2c_n$, and at $x = 1/c_n$ the second gives $0$, which is the third. So $f_n$ is a well-defined function and is continuous on $[0,1]$ by [L2], hence integrable by [L3]. [step 1.1, L2, L3]",
      "step": "2.1",
      "inputs": [
        "step 1.1",
        "L2",
    ...
    {
      "id": "cex-spikes-with-integral-one-converging-pointwise-to-zero-step-2-2",
      "claim": "On $[1/c_n, 1]$ the function $f_n$ is constantly $0$, so $\\int_{1/c_n}^{1} f_n = 0$ by [L6]; when $1/c_n = 1$ this piece is degenerate and the integral is $0$ by [L4]. [step 1.1, L4, L6]",
      "step": "2.2",
      "inputs": [
        "step 1.1",
        "L4",
    ...
    {
      "id": "cex-spikes-with-integral-one-converging-pointwise-to-zero-step-2-3",
      "claim": "**Pointwise convergence to $0$.** At $x = 0$ every $f_n(0) = 0$. For $x > 0$, [L1] gives $N$ with $1/\\iota(N+1) < x$, and for $n \\ge N$ one has $c_n = \\iota(n+1) \\ge \\iota(N+1)$, hence $1/c_n \\le 1/\\iota(N+1) < x$, so $x$ lies in the third piece and $f_n(x) = 0$. [step 1.1, given, L1, L8]",
      "step": "2.3",
      "inputs": [
        "step 1.1",
        "given",
    ...
    {
      "id": "cex-spikes-with-integral-one-converging-pointwise-to-zero-step-3-1",
      "claim": "On $[0, 1/(2c_n)]$ the function $H(x) := 2c_n^{2}x^{2}$ has $H' (x)= 4c_n^{2}x = f_n(x)$ by [L6], so $\\int_0^{1/(2c_n)} f_n = H(1/(2c_n)) - H(0) = 2c_n^{2}/(4c_n^{2}) = 1/2$ by [L5]. [step 2.1, L5, L6]",
      "step": "3.1",
      "inputs": [
        "step 2.1",
        "L5",
    ...
    {
      "id": "cex-spikes-with-integral-one-converging-pointwise-to-zero-step-3-2",
      "claim": "On $[1/(2c_n), 1/c_n]$ the function $H_2(x) := 4c_n^{2}\\bigl(x/c_n - x^{2}/2\\bigr)$ has $H_2'(x) = 4c_n^{2}(1/c_n - x) = f_n(x)$ by [L6], and $H_2(1/c_n) = 4c_n^{2}\\bigl(1/c_n^{2} - 1/(2c_n^{2})\\bigr) = 2$ while $H_2(1/(2c_n)) = 4c_n^{2}\\bigl(1/(2c_n^{2}) - 1/(8c_n^{2})\\bigr) = 3/2$; so $\\int_{1/(2c_n)}^{1/c_n} f_n = 2 - 3/2 = 1/2$ by [L5]. [step 2.1, L5, L6",
      "step": "3.2",
      "inputs": [
        "step 2.1",
        "L5",
    ...
    {
      "id": "cex-spikes-with-integral-one-converging-pointwise-to-zero-step-3-3",
      "claim": "Hence $|f_n(x) - 0| = 0 < \\varepsilon$ for all $n \\ge N$, so $f_n(x) \\to 0$ for every $x \\in [0,1]$ by [L7]. [step 2.3, L7]",
      "step": "3.3",
      "inputs": [
        "step 2.3",
        "L7"
    ...
    {
      "id": "cex-spikes-with-integral-one-converging-pointwise-to-zero-step-4-1",
      "claim": "By [L4] applied twice, $\\int_0^1 f_n = 1/2 + 1/2 + 0 = 1$ for every $n$. [step 3.1, step 3.2, step 2.2, L4]",
      "step": "4.1",
      "inputs": [
        "step 3.1",
        "step 3.2",
    ...
    {
      "id": "cex-spikes-with-integral-one-converging-pointwise-to-zero-step-5-1",
      "claim": "The pointwise limit is the zero function, which is integrable with integral $0$ by [L6], while $\\int_0^1 f_n = 1$ for every $n$ by step 4.1; so the integrals do not converge to the integral of the limit and the claim is false. [step 4.1, step 3.3, L6, L7] ∎",
      "step": "5.1",
      "inputs": [
        "step 4.1",
        "step 3.3",
  ...
  "boundaries": [
    {
      ...
      "reason": "cex-spikes-with-integral-one-converging-pointwise-to-zero concerns stipulated functions and points; no empty family, empty sum, or empty choice domain changes its assertion."
    },
    {
      "case": "zero",
      "status": "checked",
    ...
    },
    {
      "case": "iff-forward",
      "status": "not_applicable",
      "reason": "The Statement of cex-spikes-with-integral-one-converging-pointwise-to-zero is not a biconditional, so it has no forward iff direction."
    },
    {
      "case": "iff-reverse",
      "status": "not_applicable",
      "reason": "The Statement of cex-spikes-with-integral-one-converging-pointwise-to-zero is not a biconditional, so it has no reverse iff direction."
    }
  ],
  "finite_smoke": [],
  "risk_review": {
...
[
  {
    "source": "cex-spikes-with-integral-one-converging-pointwise-to-zero",
