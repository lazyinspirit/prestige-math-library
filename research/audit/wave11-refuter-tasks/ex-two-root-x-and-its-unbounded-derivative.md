...
- critical risk (15): 28 declared dependencies; 31 cited facts; 10 numbered proof steps; existence, choice, uniqueness, or well-definedness; boundary-sensitive language; quotient or equivalence-class construction; analytic limiting/completeness language
- top-20 manifest-edge consumer (28 outgoing relationships)

## Target item — `ex-two-root-x-and-its-unbounded-derivative`

Normalized current SHA-256: `93d3326a1393494a74fd798506b52487b6bf8b812c44ace92936d3190a2cf1a3`
...
```json
{
  ...
  "urls": [
    "https://clp.math.uky.edu/clp1/sec_def_deriv.html",
    "https://faculty.gvsu.edu/boelkinm/Home/ACS/sec-6-5-improper.html"
  ],
  "rationale": "The CLP university text computes (sqrt x)'=1/(2sqrt x) for x>0 and states that the derivative is undefined at zero; Active Calculus treats 1/sqrt x as an unbounded integrand at zero. The item scales by 2, proves the precise unbounded sequence, and packages those facts to explain why the proper Riemann FTC does not apply on [0,1].",
...
```json
{
  ...
  "derivations": [
    {
      "id": "ex-two-root-x-and-its-unbounded-derivative-step-1-1",
      "claim": "By [L2] the map $q(x)=x^{2}$ is continuous and injective on the order-convex $[0,\\infty)$, and its image is exactly $[0,\\infty)$: every value is $\\ge 0$, and every $a \\ge 0$ is $q(a^{1/2})$ by [L1]. [L1, L2]",
      "step": "1.1",
      "inputs": [
        "L1",
        "L2"
    ...
    {
      "id": "ex-two-root-x-and-its-unbounded-derivative-step-2-1",
      "claim": "By [L3] the inverse $g : [0,\\infty) \\to [0,\\infty)$ of $q$ is continuous, and $g(a) = a^{1/2}$ by the uniqueness in [L1]. Hence $H = 2g$ restricted to $[0,1]$ is continuous, which is claim 1. [step 1.1, L1, L3]",
      "step": "2.1",
      "inputs": [
        "step 1.1",
        "L1",
    ...
    {
      "id": "ex-two-root-x-and-its-unbounded-derivative-step-2-2",
      "claim": "Let $x \\in (0,1]$ and put $c := x^{1/2} > 0$ by [L1]. By [L5], $q$ is differentiable at $c$ with $q'(c) = \\iota(2)c = 2c \\ne 0$; so [L4] gives $g$ differentiable at $x = q(c)$ with $g'(x) = 1/(2c) = 1/(2\\sqrt{x})$. [step 1.1, L1, L4, L5, L10]",
      "step": "2.2",
      "inputs": [
        "step 1.1",
        "L1",
    ...
    {
      "id": "ex-two-root-x-and-its-unbounded-derivative-step-3-1",
      "claim": "Hence $H = 2g$ is differentiable at every $x \\in (0,1]$ with $H'(x) = 2/(2\\sqrt{x}) = 1/\\sqrt{x}$, by [L5]. [step 2.2, L5]",
      "step": "3.1",
      "inputs": [
        "step 2.2",
        "L5"
    ...
    {
      "id": "ex-two-root-x-and-its-unbounded-derivative-step-4-1",
      "claim": "**Claim 3.** For $n \\in \\mathbb{N}$ put $x_n := 1/\\iota(n+1)^{2}$, a real in $(0,1]$ by [L9]. Then $\\sqrt{x_n} = 1/\\iota(n+1)$, since that number is positive with square $x_n$ and [L1] gives uniqueness; so $H'(x_n) = \\iota(n+1)$ by step 3.1. [step 3.1, L1, L9, L10]",
      "step": "4.1",
      "inputs": [
        "step 3.1",
        "L1",
    ...
    {
      "id": "ex-two-root-x-and-its-unbounded-derivative-step-5-1",
      "claim": "$H'$ is unbounded on $(0,1]$: given a real $M \\ge 0$, [L9] supplies $n$ with $M < \\iota(n+1) = H'(x_n)$. [step 4.1, L9]",
      "step": "5.1",
      "inputs": [
        "step 4.1",
        "L9"
    ...
    {
      "id": "ex-two-root-x-and-its-unbounded-derivative-step-5-2",
      "claim": "**$H$ is not differentiable at $0$.** The difference quotient of $H$ at $0$ is $x \\mapsto 2\\sqrt{x}/x = 2/\\sqrt{x}$ for $x \\in (0,1]$, by [L1] and [L10]; at $x_n$ it takes the value $2\\iota(n+1)$, which exceeds every real by [L9]. So no real $L$ can satisfy the $\\varepsilon$-$\\delta$ condition with $\\varepsilon = 1$: any $\\delta>0$ admits some $x_n < \\delta$",
      "step": "5.2",
      "inputs": [
        "step 4.1",
        "L1",
    ...
    {
      "id": "ex-two-root-x-and-its-unbounded-derivative-step-6-1",
      "claim": "**Claim 4.** Let $u : [0,1] \\to \\mathbb{R}$ agree with $H'$ on $(0,1]$. By step 5.1, $u$ is unbounded on $[0,1]$, so it has no Darboux sums and is not Riemann integrable there, by [L8]. [step 5.1, L8]",
      "step": "6.1",
      "inputs": [
        "step 5.1",
        "L8"
    ...
    {
      "id": "ex-two-root-x-and-its-unbounded-derivative-step-7-1",
      "claim": "**The hypotheses of the second fundamental theorem both fail on $[0,1]$**, by step 5.2 and step 6.1; so [L7] gives nothing there, and $\\int_0^1 H'$ is an undefined symbol. [step 5.2, step 6.1, L7, L8]",
      "step": "7.1",
      "inputs": [
        "step 5.2",
        "step 6.1",
    ...
    {
      "id": "ex-two-root-x-and-its-unbounded-derivative-step-8-1",
      "claim": "**On $[\\eta,1]$ everything works.** There $\\sqrt{\\cdot}$ is continuous and does not vanish, so $H' = 1/\\sqrt{\\cdot}$ is continuous on $[\\eta,1]$ by [L6] and integrable there; $H$ is differentiable at every point of $[\\eta,1]$ by step 3.1; so [L7] gives $\\int_{\\eta}^{1}H' = H(1)-H(\\eta) = 2 - 2\\sqrt{\\eta}$. [step 2.1, step 3.1, L1, L6, L7] ∎",
      "step": "8.1",
      "inputs": [
        "step 2.1",
        "step 3.1",
  ...
  "boundaries": [
    {
      ...
      "reason": "ex-two-root-x-and-its-unbounded-derivative concerns stipulated functions and points; no empty family, empty sum, or empty choice domain changes its assertion."
    },
    {
      "case": "zero",
      "status": "checked",
    ...
    },
    {
      ...
      "status": "checked",
      "evidence": "Step 5.1 establishes or explicitly fixes the object before the proof chooses or extremizes it."
    },
    {
      "case": "iff-forward",
      "status": "not_applicable",
      "reason": "The Statement of ex-two-root-x-and-its-unbounded-derivative is not a biconditional, so it has no forward iff direction."
    },
    {
      "case": "iff-reverse",
      "status": "not_applicable",
      "reason": "The Statement of ex-two-root-x-and-its-unbounded-derivative is not a biconditional, so it has no reverse iff direction."
    }
  ],
  "finite_smoke": [],
  "risk_review": {
...
[
  {
    "source": "ex-two-root-x-and-its-unbounded-derivative",
