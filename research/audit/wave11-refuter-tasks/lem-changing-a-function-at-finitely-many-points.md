...
- critical risk (13): 19 declared dependencies; 20 cited facts; 16 numbered proof steps; boundary-sensitive language; induction, recursion, or minimality; analytic limiting/completeness language
- top-20 manifest-edge consumer (20 outgoing relationships)

## Target item — `lem-changing-a-function-at-finitely-many-points`

Normalized current SHA-256: `b8853fa5603ab32e813249f397bc90cd9341c6474b55cfaf3822fa293138e871`
...
```json
{
  ...
  "rationale": "The source states that changing a bounded function at finitely many points preserves Riemann integrability and the integral. The item's oscillation-budget proof is local.",
...
```json
{
  ...
  "derivations": [
    {
      "id": "lem-changing-a-function-at-finitely-many-points-step-1-1",
      "claim": "**The one-point case is proved first, for a function called $w$ so that no symbol is reused.** Let $p \\in [a,b]$ and let $w : [a,b] \\to \\mathbb{R}$ satisfy $w(x) = 0$ for every $x \\ne p$; put $K := |w(p)|$, so $|w(x)| \\le K$ for every $x$ and $w$ is bounded. [given, L1]",
      "step": "1.1",
      "inputs": [
        "given",
        "L1"
    ...
    {
      "id": "lem-changing-a-function-at-finitely-many-points-step-1-2",
      "claim": "Fix $N \\ge 1$ and write $U_N = (N,t)$ with subintervals $I_i$ and lengths $\\Delta_i = (b-a)/\\iota(N)$. Define $\\theta_i := 1$ if $p \\in I_i$ and $\\theta_i := 0$ otherwise, for $i < N$. [L3, construct]",
      "step": "1.2",
      "inputs": [
        "L3",
        "construct"
    ...
    {
      "id": "lem-changing-a-function-at-finitely-many-points-step-1-3",
      "claim": "**Setting up the induction.** Put $h := g - f$, so that $h(x) = 0$ for every $x \\notin E$, and for $j < r$ define $h_j : [a,b] \\to \\mathbb{R}$ by $h_j(x) := h(x)$ if $x = e(j)$ and $h_j(x) := 0$ otherwise. Each $h_j$ vanishes off the single point $e(j)$. [given, construct]",
      "step": "1.3",
      "inputs": [
        "given",
        "construct"
    ...
    {
      "id": "lem-changing-a-function-at-finitely-many-points-step-2-1",
      "claim": "At most two indices $i < N$ have $\\theta_i = 1$, and they are consecutive: if $p \\in I_i \\cap I_j$ with $i < j$ then $t_j \\le p \\le t_{i+1}$ and $t_{i+1} \\le t_j$, so $t_{i+1} = t_j$ and $j = i+1$ by injectivity of $t$. Also some index has $\\theta_i = 1$, since the subintervals cover $[a,b]$; let $i_0$ be the least such. [step 1.2, L3, choose]",
      "step": "2.1",
      "inputs": [
        "step 1.2",
        "L3",
    ...
    {
      "id": "lem-changing-a-function-at-finitely-many-points-step-2-2",
      "claim": "For every $x \\in [a,b]$ and every $k \\le r$, $\\sum_{j<k} h_j(x) = h(x)$ when $x = e(j_0)$ for some $j_0 < k$, and $\\sum_{j<k}h_j(x) = 0$ otherwise: in the first case all terms with $j \\ne j_0$ vanish, because $e$ is injective, and [L4] evaluates the sum; in the second every term is $0$. [step 1.3, L4]",
      "step": "2.2",
      "inputs": [
        "step 1.3",
        "L4"
    ...
    {
      "id": "lem-changing-a-function-at-finitely-many-points-step-3-1",
      "claim": "For every $i < N$: if $\\theta_i = 0$ then $w$ vanishes on $I_i$, so $m_i = M_i = 0$, where $m_i$ and $M_i$ are the extreme values of $w$ on $I_i$; and always $-K\\theta_i \\le m_i \\le M_i \\le K\\theta_i$. [step 1.2, step 2.1, L2, L8]",
      "step": "3.1",
      "inputs": [
        "step 1.2",
        "step 2.1",
    ...
    {
      "id": "lem-changing-a-function-at-finitely-many-points-step-3-2",
      "claim": "Define $y_i := \\Delta_i$ for $i = i_0$ and $y_i := 0$ otherwise, and $z_i := \\Delta_i$ for $i = i_0+1$ with $i_0 + 1 < N$, and $z_i := 0$ otherwise. Then $\\theta_i\\Delta_i \\le y_i + z_i$ for every $i < N$ by step 2.1, and $\\sum_{i<N}(y_i+z_i) \\le 2(b-a)/\\iota(N)$ by [L4] and [L3]. [step 1.2, step 2.1, L3, L4, construct]",
      "step": "3.2",
      "inputs": [
        "step 1.2",
        "step 2.1",
    ...
    {
      "id": "lem-changing-a-function-at-finitely-many-points-step-3-3",
      "claim": "Let $Q(k)$, for $k \\le r$, be the statement that the function $H_k : x \\mapsto \\sum_{j<k}h_j(x)$ is integrable on $[a,b]$ with $\\int_a^b H_k = 0$. [step 2.2, construct]",
      "step": "3.3",
      "inputs": [
        "step 2.2",
        "construct"
    ...
    {
      "id": "lem-changing-a-function-at-finitely-many-points-step-4-1",
      "claim": "By step 3.1 and monotonicity of finite sums, $U(w,U_N) - L(w,U_N) = \\sum_{i<N}(M_i-m_i)\\Delta_i \\le \\sum_{i<N}2K\\theta_i\\Delta_i \\le 4K(b-a)/\\iota(N)$, and likewise $-2K(b-a)/\\iota(N) \\le L(w,U_N)$ and $U(w,U_N) \\le 2K(b-a)/\\iota(N)$. [step 3.1, step 3.2, L2, L4, L8]",
      "step": "4.1",
      "inputs": [
        "step 3.1",
        "step 3.2",
    ...
    {
      "id": "lem-changing-a-function-at-finitely-many-points-step-4-2",
      "claim": "**Base.** $H_0$ is the constant function $0$, integrable with integral $0$ by [L5], so $Q(0)$ holds. [base, step 3.3, L5]",
      "step": "4.2",
      "inputs": [
        "base",
        "step 3.3",
    ...
    {
      "id": "lem-changing-a-function-at-finitely-many-points-step-4-3",
      "claim": "**Induction hypothesis.** Fix $k < r$ and assume $Q(k)$. [ih, step 3.3]",
      "step": "4.3",
      "inputs": [
        "ih",
        "step 3.3"
    ...
    {
      "id": "lem-changing-a-function-at-finitely-many-points-step-5-1",
      "claim": "Given a real $\\eta > 0$, [L6] supplies $N$ with $4K(b-a)/\\iota(N) < \\eta$, so $w$ satisfies Riemann's criterion and is integrable by [L1]. [step 4.1, L1, L6, L8]",
      "step": "5.1",
      "inputs": [
        "step 4.1",
        "L1",
    ...
    {
      "id": "lem-changing-a-function-at-finitely-many-points-step-5-2",
      "claim": "Moreover $|\\int_a^b w| \\le 2K(b-a)/\\iota(N)$ for every $N \\ge 1$ by step 4.1 and [L2], and the right-hand side is below every positive real by [L6]; hence $\\int_a^b w = 0$. Steps 1.1 to 5.2 therefore prove: every function on $[a,b]$ vanishing off a single point is integrable with integral $0$. [step 4.1, L2, L6, L8]",
      "step": "5.2",
      "inputs": [
        "step 4.1",
        "L2",
    ...
    {
      "id": "lem-changing-a-function-at-finitely-many-points-step-6-1",
      "claim": "$H_{k+1} = H_k + h_k$ pointwise by [L4], and $h_k$ is integrable with integral $0$ by steps 5.1 and 5.2 applied to $w := h_k$ and $p := e(k)$; so $H_{k+1}$ is integrable with $\\int_a^b H_{k+1} = 0 + 0 = 0$ by [L5], which is $Q(k+1)$. [step 5.1, step 5.2, step 1.3, step 4.3, L4, L5]",
      "step": "6.1",
      "inputs": [
        "step 5.1",
        "step 5.2",
    ...
    {
      "id": "lem-changing-a-function-at-finitely-many-points-step-7-1",
      "claim": "By [L7] with steps 4.2 and 6.1, $Q(k)$ holds for every $k \\le r$; at $k = r$, and by step 2.2, $H_r = h$, so $h$ is integrable with $\\int_a^b h = 0$. [step 2.2, step 4.2, step 4.3, step 6.1, L7]",
      "step": "7.1",
      "inputs": [
        "step 2.2",
        "step 4.2",
    ...
    {
      "id": "lem-changing-a-function-at-finitely-many-points-step-8-1",
      "claim": "Hence $g = f + h$ is integrable with $\\int_a^b g = \\int_a^b f + 0 = \\int_a^b f$ by [L5]. [step 7.1, L5, discharge-induction] ∎",
      "step": "8.1",
      "inputs": [
        "step 7.1",
        "L5",
  ...
  "boundaries": [
    {
      "case": "empty",
      "status": "checked",
      "evidence": "Statement and step 1.1 explicitly dispose of the relevant empty/nonempty alternative."
    },
    {
      "case": "zero",
      "status": "not_applicable",
      "reason": "lem-changing-a-function-at-finitely-many-points has no counting, exponent, or value parameter whose zero case changes the proof."
    },
    {
      "case": "one",
      "status": "not_applicable",
      "reason": "lem-changing-a-function-at-finitely-many-points has no counting, exponent, or unit parameter whose one case changes the proof."
    },
    {
      "case": "degenerate",
      "status": "checked",
    ...
    },
    {
      "case": "iff-forward",
      "status": "not_applicable",
      "reason": "The Statement of lem-changing-a-function-at-finitely-many-points is not a biconditional, so it has no forward iff direction."
    },
    {
      "case": "iff-reverse",
      "status": "not_applicable",
      "reason": "The Statement of lem-changing-a-function-at-finitely-many-points is not a biconditional, so it has no reverse iff direction."
    }
  ],
  "finite_smoke": [],
  "risk_review": {
