...
# One question this lane must answer explicitly, in addition to the general reading

The first Remark, "Why regularity is defined for cardinals and not for ordinals",
contains this sentence about the condition cf(alpha) = alpha:

> it fails at $\omega + 1$ and at $\omega \cdot 2$ for reasons that have
> nothing to do with size, and it holds only at $0$, at $1$, and at those
> ordinals that are already infinite cardinals.

Answer all three of the following, showing your work from the quoted dependency
texts and from standard ZF facts:

1. **State exactly the class of ordinals alpha for which cf(alpha) = alpha
   holds.** Give the characterisation, not an example.
2. **Is the quoted sentence true, false, or ambiguous between a true and a false
   reading?** Address directly the force of the word "only": does the sentence
   assert a necessary condition, an exhaustive enumeration, or both? If the two
   readings differ in truth value, say which reading a competent reader of this
   page would take and why.
3. **Does the sentence contradict anything else on this page?** Compare it with
   the item own definition of "singular" in the Definition section, with the
   second Remark, and with the `thm-regularity-of-the-alephs` Statement quoted
   above.

Do not soften your answer to agree with the item. Do not soften it to disagree
either — if the sentence is defensible as written, say so and say why.

Then give the general adversarial reading in the brief format, and finish with
the `N fatal, M nonfatal` line.
