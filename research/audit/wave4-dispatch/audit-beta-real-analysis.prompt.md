<!-- TEMPLATE. Copy into an Audit-Beta prompt and substitute 4 (wave) and
...
# Audit-Beta brief — wave 4, real-analysis batch (steps A1/A2/A4)
...
> **Model/routing rule (owner, 2026-08-02).** You are **GPT 5.6 Sol** at
> `xhigh` reasoning with a 1,000,000-token context window, dispatched through
> the Codex subscription. Do not substitute another model and do not route
> through any billed pipeline.
...
`research/audit/wave4-real-analysis.pages.json`. Nothing here was authored by
...
in `research/audit/wave4-real-analysis.provenance.jsonl`:
...
3. **Capture the full proof contract** for every proof-bearing item (owner
   decision D1) in `research/audit/wave4-real-analysis.proof-contracts.json`:
...
`research/audit/wave4-real-analysis.provenance.jsonl`, `.findings.md`,
