## Wave 10 target — `thm-finitely-many-discontinuities-integrable`

Selection: mandatory Wave 10 high/critical proof-bearing route (risk report classifies all 49 contracts critical).

Normalized current SHA-256: `4c1c749e25d1ef6f3552b2b5bdbb630d957c84d0fce7ea36dfc8887b3fd36a83`
...
```json
{
  ...
  "urls": [
    "https://www2.math.upenn.edu/~gressman/analysis/07-integrationfuncs.html"
  ],
  "rationale": "The university notes state that a bounded function with finitely many discontinuities is Riemann integrable. The item supplies a Darboux proof separating small exceptional intervals from uniformly continuous pieces.",
...
```json
{
  ...
  "derivations": [
    {
      "id": "thm-finitely-many-discontinuities-integrable-step-1-1",
      "claim": "Let a real $\\varepsilon > 0$ be given. Put $\\theta := \\varepsilon \\cdot \\bigl(4(b-a)\\bigr)^{-1}$ and $\\eta := \\varepsilon \\cdot \\bigl(16\\,M_{+}(\\iota(r)+1)\\bigr)^{-1}$, both positive reals by [L10] and [L11]. [given, L10, L11]",
      "step": "1.1",
      "inputs": [
        "L10",
        "L11"
    ...
    {
      "id": "thm-finitely-many-discontinuities-integrable-step-2-1",
      "claim": "Put $V := \\bigcup_{k<r}\\,(d_k - \\eta,\\ d_k + \\eta)$, an open set by [L5], and $K := [a,b] \\setminus V = [a,b] \\cap (\\mathbb{R}\\setminus V)$, an intersection of two closed sets, hence closed by [L5], and bounded since $K \\subseteq [a,b]$; so $K$ is compact by [L4]. [step 1.1, L4, L5, construct]",
      "step": "2.1",
      "inputs": [
        "L5",
        "L4",
    ...
    {
      "id": "thm-finitely-many-discontinuities-integrable-step-3-1",
      "claim": "$f$ is continuous at every point of $K$: a point $x \\in K$ is not any $d_k$, since $d_k \\in (d_k-\\eta, d_k+\\eta) \\subseteq V$ and $K$ misses $V$. Hence the restriction $f|_K$ is continuous on $K$ by [L7]. [step 2.1, given, L7, L11]",
      "step": "3.1",
      "inputs": [
        "L7",
        "2.1",
    ...
    {
      "id": "thm-finitely-many-discontinuities-integrable-step-4-1",
      "claim": "By [L4] applied to $f|_K$ on the compact set $K$ with the value $\\theta$, fix a real $\\delta > 0$ such that $|f(x)-f(y)| < \\theta$ for all $x,y \\in K$ with $|x-y| < \\delta$. [step 2.1, step 3.1, L4, choose]",
      "step": "4.1",
      "inputs": [
        "L4",
        "2.1",
    ...
    {
      "id": "thm-finitely-many-discontinuities-integrable-step-5-1",
      "claim": "By [L10] fix a natural $N \\ge 1$ with $1/\\iota(N) < \\delta \\cdot (b-a)^{-1}$, so that the uniform partition $U_N$ has mesh $(b-a)/\\iota(N) < \\delta$ by [L1] and [L11]. Let $P = (n,t)$ be the partition obtained from $U_N$ by inserting, one after another, those of the $2r$ points $d_k - \\eta$ and $d_k + \\eta$ with $k < r$ that lie in $[a,b]$. By [L1] each insertion leaves the mesh no larger, so $\\|P\\| < \\delta$, and $\\operatorname{pts}(P)$ contains every one of those points that lies in $[a,b]$. [step 4.1, L1, L10, L11, choose, construct]",
      "step": "5.1",
      "inputs": [
        "L10",
        "L1",
    ...
    {
      "id": "thm-finitely-many-discontinuities-integrable-step-6-1",
      "claim": "**A dichotomy for each subinterval and each $k$.** Fix $i < n$ and $k < r$, and write $c^{-} := d_k - \\eta$ and $c^{+} := d_k + \\eta$. Neither $c^{-}$ nor $c^{+}$ lies in the open interval $(t_i,t_{i+1})$: if such a point lies in $[a,b]$ it is a member of $\\operatorname{pts}(P)$ by step 5.1, hence is some $t_j$ with $j \\le n$, and no $t_j$ lies strictly between $t_i$ and $t_{i+1}$ by [L1]; and if it lies outside $[a,b]$ it is outside $[t_i,t_{i+1}]$ altogether. [step 5.1, L1, L11]",
      "step": "6.1",
      "inputs": [
        "5.1",
        "L1",
    ...
    {
      "id": "thm-finitely-many-discontinuities-integrable-step-7-1",
      "claim": "Consequently either $(t_i,t_{i+1}) \\cap (c^{-},c^{+}) = \\varnothing$, or $I_i \\subseteq [c^{-},c^{+}]$. Suppose the intersection contains a point $z$ and let $w \\in (t_i,t_{i+1})$. If $w \\le c^{-}$ then $c^{-}$ lies between $w$ and $z$, both in the order-convex set $(t_i,t_{i+1})$, so $c^{-} \\in (t_i,t_{i+1})$, contradicting step 6.1; likewise $w \\ge c^{+}$ is impossible. So $(t_i,t_{i+1}) \\subseteq (c^{-},c^{+})$. Then $c^{-} < z < t_{i+1}$ with $c^{-} \\notin (t_i,t_{i+1})$ forces $c^{-} \\le t_i$, and symmetrically $c^{+} \\ge t_{i+1}$, that is $I_i \\subseteq [c^{-},c^{+}]$. [step 6.1, L11, given]",
      "step": "7.1",
      "inputs": [
        "6.1",
        "L11"
    ...
    {
      "id": "thm-finitely-many-discontinuities-integrable-step-8-1",
      "claim": "Call $i < n$ **bad** when $I_i \\subseteq [d_k-\\eta, d_k+\\eta]$ for some $k < r$, and **good** otherwise. If $i$ is good then by step 7.1 the open interval $(t_i,t_{i+1})$ meets no $(d_k-\\eta,d_k+\\eta)$, hence $(t_i,t_{i+1}) \\subseteq K$; since $K$ is closed, [L6] gives $I_i = [t_i,t_{i+1}] \\subseteq K$. [step 2.1, step 7.1, L6]",
      "step": "8.1",
      "inputs": [
        "7.1",
        "L6",
    ...
    {
      "id": "thm-finitely-many-discontinuities-integrable-step-9-1",
      "claim": "For a good $i$: all $x,y \\in I_i$ lie in $K$ and satisfy $|x-y| \\le \\Delta_i \\le \\|P\\| < \\delta$ by [L1] and [L11], so $|f(x)-f(y)| < \\theta$ by step 4.1; therefore $\\theta$ bounds the set whose supremum is $M_i - m_i$, and $M_i - m_i \\le \\theta$ by [L2]. [step 4.1, step 5.1, step 8.1, L1, L2, L11]",
      "step": "9.1",
      "inputs": [
        "L1",
        "L11",
    ...
    {
      "id": "thm-finitely-many-discontinuities-integrable-step-9-2",
      "claim": "**Bounding the bad lengths.** For $k < r$ put $J_k := \\{\\, i < n : d_k - \\eta \\le t_i \\text{ and } t_{i+1} \\le d_k + \\eta \\,\\}$, so that $i$ is bad exactly when $i \\in J_k$ for some $k < r$, and put $h^{k}_i := \\Delta_i$ for $i \\in J_k$ and $h^{k}_i := 0$ otherwise. Each $J_k$ is a set of consecutive indices: if $i < i' < i''$ with $i, i'' \\in J_k$ then $d_k - \\eta \\le t_i \\le t_{i'}$ and $t_{i'+1} \\le t_{i''+1} \\le d_k + \\eta$, so $i' \\in J_k$. [step 8.1, L1, L11, construct]",
      "step": "9.2",
      "inputs": [
        "8.1",
        "L1",
    ...
    {
      "id": "thm-finitely-many-discontinuities-integrable-step-10-1",
      "claim": "$\\sum_{i<n} h^{k}_i \\le 2\\eta$ for each $k < r$. If $J_k = \\varnothing$ the sum is $0$. Otherwise let $p := \\min J_k$ and let $q$ be the least natural with $q > p$ and $q \\notin J_k$, which exists by [L9] because $n \\notin J_k$; by step 9.2 then $J_k = \\{\\, i : p \\le i < q \\,\\}$, since an $i \\in J_k$ with $i \\ge q$ together with $p \\in J_k$ and $p < q \\le i$ would put $q \\in J_k$. Splitting the sum at $p$ and at $q$ and discarding the vanishing outer parts ([L8]) gives $\\sum_{i<n}h^{k}_i = \\sum_{i=p}^{q-1}\\Delta_i = t_q - t_p$ by telescoping, and $p \\in J_k$, $q-1 \\in J_k$ give $t_p \\ge d_k-\\eta$ and $t_q \\le d_k+\\eta$, whence $t_q - t_p \\le 2\\eta$. [step 9.2, L8, L9, L11]",
      "step": "10.1",
      "inputs": [
        "L9",
        "9.2",
    ...
    {
      "id": "thm-finitely-many-discontinuities-integrable-step-11-1",
      "claim": "Put $\\beta_i := \\Delta_i$ for $i$ bad and $\\beta_i := 0$ for $i$ good. Then $\\beta_i \\le \\sum_{k<r}h^{k}_i$ for every $i < n$, all terms being nonnegative and a bad $i$ lying in some $J_k$; so by [L8], $\\sum_{i<n}\\beta_i \\le \\sum_{i<n}\\sum_{k<r}h^{k}_i = \\sum_{k<r}\\sum_{i<n}h^{k}_i \\le \\sum_{k<r}2\\eta = 2\\eta\\,\\iota(r)$, using step 10.1. [step 9.2, step 10.1, L8, L11]",
      "step": "11.1",
      "inputs": [
        "L8",
        "10.1",
    ...
    {
      "id": "thm-finitely-many-discontinuities-integrable-step-12-1",
      "claim": "For every $i < n$ one has $(M_i - m_i)\\Delta_i \\le \\theta\\,\\Delta_i + 2M_{+}\\,\\beta_i$: for good $i$ this is step 9.1 together with $\\beta_i \\ge 0$, and for bad $i$ it follows from $M_i - m_i \\le 2M_{+}$ in [L2] and $\\beta_i = \\Delta_i$, together with $\\theta\\Delta_i \\ge 0$. [step 9.1, step 11.1, L2, L11]",
      "step": "12.1",
      "inputs": [
        "9.1",
        "L2",
    ...
    {
      "id": "thm-finitely-many-discontinuities-integrable-step-13-1",
      "claim": "Summing step 12.1 over $i < n$ and using [L8], [L1] and step 11.1: $U(f,P) - L(f,P) \\le \\theta\\sum_{i<n}\\Delta_i + 2M_{+}\\sum_{i<n}\\beta_i \\le \\theta(b-a) + 4M_{+}\\eta\\,\\iota(r) \\le \\varepsilon\\cdot 4^{-1} + \\varepsilon\\cdot 4^{-1} < \\varepsilon$, the last estimate because $4M_{+}\\eta\\,\\iota(r) \\le 4M_{+}\\eta(\\iota(r)+1) = \\varepsilon\\cdot 4^{-1}$ by step 1.1. [step 1.1, step 11.1, step 12.1, L1, L2, L8, L10, L11]",
      "step": "13.1",
      "inputs": [
        "12.1",
        "L8",
    ...
    {
      "id": "thm-finitely-many-discontinuities-integrable-step-14-1",
      "claim": "The real $\\varepsilon > 0$ of step 1.1 was arbitrary and step 13.1 produced a partition $P$ with $U(f,P) - L(f,P) < \\varepsilon$, so [L3] applies and $f$ is Riemann integrable on $[a,b]$. [step 1.1, step 13.1, L3] ∎",
      "step": "14.1",
      "inputs": [
        "1.1",
        "13.1",
...
## Complete current text of every declared or cited item (34)
