## Wave 10 target — `cor-differentiable-implies-continuous`

Selection: mandatory Wave 10 high/critical proof-bearing route (risk report classifies all 49 contracts critical).

Normalized current SHA-256: `dcbefbfd2e99f1d371197f43423e3a7302886a51cbbbd16a0ee14d4fa6082688`
...
```json
{
  ...
  "rationale": "Both sources state that differentiability implies continuity. The proof is a local Carathéodory-factor argument rather than a verbatim source proof.",
...
```json
{
  ...
  "derivations": [
    {
      "id": "cor-differentiable-implies-continuous-step-1-1",
      "claim": "Fix a function $\\varphi : A \\to \\mathbb{R}$, continuous at $c$, with $f(x) - f(c) = \\varphi(x)(x - c)$ for every $x \\in A$. [L1, choose]",
      "step": "1.1",
      "inputs": [
        "L1"
      ]
    ...
    {
      "id": "cor-differentiable-implies-continuous-step-1-2",
      "claim": "The identity $x \\mapsto x$ on $A$ and every constant function on $A$ are continuous at $c$; hence so is $x \\mapsto x - c$, which is the sum of the identity and the constant function with value $-c$. [L2]",
      "step": "1.2",
      "inputs": [
        "L2"
      ]
    ...
    {
      "id": "cor-differentiable-implies-continuous-step-2-1",
      "claim": "The pointwise product $x \\mapsto \\varphi(x)(x - c)$ is continuous at $c$, being the product of two functions on $A$ continuous at $c$. [step 1.1, step 1.2, L2]",
      "step": "2.1",
      "inputs": [
        "1.1",
        "1.2",
    ...
    {
      "id": "cor-differentiable-implies-continuous-step-3-1",
      "claim": "For every $x \\in A$ one has $f(x) = f(c) + \\varphi(x)(x - c)$, so $f$ is the sum of the constant function with value $f(c)$ and the product of step 2.1. [step 1.1, L1]",
      "step": "3.1",
      "inputs": [
        "2.1",
        "1.1",
    ...
    {
      "id": "cor-differentiable-implies-continuous-step-4-1",
      "claim": "A sum of two functions continuous at $c$ is continuous at $c$, so $f$ is continuous at $c$. [step 2.1, step 3.1, L2, L3]",
      "step": "4.1",
      "inputs": [
        "2.1",
        "3.1",
    ...
    {
      "id": "cor-differentiable-implies-continuous-step-5-1",
      "claim": "The point $c$ was an arbitrary point of $A$, a limit point of $A$, at which $f$ is differentiable; applying step 4.1 at every point of a set $S \\subseteq A$ on which $f$ is differentiable gives continuity of $f$ at every point of $S$. [step 3.1, L3] ∎",
      "step": "5.1",
      "inputs": [
        "4.1",
        "3.1",
...
[
  ...
  },
  {
    ...
    "sourcePage": "the-derivative-and-mean-value-theorems",
    "batch": "wave10-real-analysis",
    "target": "cex-absolute-value-is-not-differentiable-at-zero",
    "declared_target": "cex-absolute-value-is-not-differentiable-at-zero",
