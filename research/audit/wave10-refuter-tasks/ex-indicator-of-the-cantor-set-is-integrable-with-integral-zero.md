## Wave 10 target — `ex-indicator-of-the-cantor-set-is-integrable-with-integral-zero`

Selection: mandatory Wave 10 high/critical proof-bearing route (risk report classifies all 49 contracts critical).

Normalized current SHA-256: `f218a0cce1ac59acd6e42c84791d8c9c79989123f92469fd653997be004ef9a8`
...
```json
{
  ...
  "rationale": "The lecture notes state that the Cantor-set indicator is Riemann integrable with integral zero. The item verifies its discontinuity set and applies Lebesgue’s criterion.",
...
```json
{
  ...
  "derivations": [
    {
      "id": "ex-indicator-of-the-cantor-set-is-integrable-with-integral-zero-step-1-1",
      "claim": "$\\mathbf{1}_C$ takes only the values $0$ and $1$, so it is bounded on $[0,1]$ and its Darboux sums and integrals are defined by [L5]. [given, L5]",
      "step": "1.1",
      "inputs": [
        "L5"
      ]
    ...
    {
      "id": "ex-indicator-of-the-cantor-set-is-integrable-with-integral-zero-step-1-2",
      "claim": "**Discontinuity on $C$.** Let $x \\in C$, so $\\mathbf{1}_C(x) = 1$, and let a real $\\rho > 0$ be given. By [L8] the set $(u,v) \\subseteq N_\\rho(x)\\cap[0,1]$ is a nonempty open interval, so by [L1] it contains a point $y \\notin C$; then $y \\in [0,1]$, $|y-x| < \\rho$ and $|\\mathbf{1}_C(x) - \\mathbf{1}_C(y)| = 1$. So the continuity condition fails at $x$ for $\\varepsilon := 1$. [given, L1, L8]",
      "step": "1.2",
      "inputs": [
        "L8",
        "L1"
    ...
    {
      "id": "ex-indicator-of-the-cantor-set-is-integrable-with-integral-zero-step-1-3",
      "claim": "**Continuity off $C$.** Let $x \\in [0,1]$ with $x \\notin C$. Since $C$ is closed, [L2] gives a real $\\rho > 0$ with $N_\\rho(x) \\cap C = \\varnothing$, so $\\mathbf{1}_C$ vanishes on $N_\\rho(x)\\cap[0,1]$ and $|\\mathbf{1}_C(y) - \\mathbf{1}_C(x)| = 0 < \\varepsilon$ there for every $\\varepsilon > 0$. [given, L1, L2]",
      "step": "1.3",
      "inputs": [
        "L2",
        "L1"
    ...
    {
      "id": "ex-indicator-of-the-cantor-set-is-integrable-with-integral-zero-step-2-1",
      "claim": "So the set of discontinuities of $\\mathbf{1}_C$ in $[0,1]$ is exactly $C$, which has measure zero by [L1]; by [L3] and $0 < 1$, $\\mathbf{1}_C$ is Riemann integrable on $[0,1]$. [step 1.1, step 1.2, step 1.3, L1, L3]",
      "step": "2.1",
      "inputs": [
        "L1",
        "L3",
    ...
    {
      "id": "ex-indicator-of-the-cantor-set-is-integrable-with-integral-zero-step-2-2",
    ...
    {
      "id": "ex-indicator-of-the-cantor-set-is-integrable-with-integral-zero-step-3-1",
      "claim": "The set of lower sums is $\\{0\\}$, so $\\underline{\\int_0^1}\\mathbf{1}_C = 0$ by [L6]; and $\\mathbf{1}_C$ is integrable by step 2.1, so $\\int_0^1 \\mathbf{1}_C = 0$ by [L5]. [step 2.1, step 2.2, L5, L6] ∎",
      "step": "3.1",
      "inputs": [
        "L6",
        "2.1",
...
[
  {
    "source": "ex-indicator-of-the-cantor-set-is-integrable-with-integral-zero",
