## Wave 10 target — `ex-derivatives-of-polynomials-computed`

Selection: mandatory Wave 10 high/critical proof-bearing route (risk report classifies all 49 contracts critical).

Normalized current SHA-256: `87cd734ca238a277541d547a9153b66167be4a519963262ca73d2fa713b87ef6`
...
```json
{
  ...
  "rationale": "The source supplies the power and algebra rules underlying polynomial differentiation. The particular displayed polynomial computations and their stepwise derivation are worked specializations.",
...
```json
{
  ...
  "derivations": [
    {
      "id": "ex-derivatives-of-polynomials-computed-step-1-1",
      "claim": "Let $c \\in \\mathbb{R}$, a limit point of $\\mathbb{R}$ by [L5]. By [L1] the functions $x \\mapsto x^{4}$, $x \\mapsto x^{1}$ and $x \\mapsto x^{0}$ are differentiable at $c$ with derivatives $\\iota(4)c^{3}$, $\\iota(1)c^{0} = 1$ and $0$ respectively, using [L4]. [L1, L4, L5]",
      "step": "1.1",
      "inputs": [
        "L5",
        "L1",
    ...
    {
      "id": "ex-derivatives-of-polynomials-computed-step-1-2",
      "claim": "Put $u(x) := x^{2} + 1$ and $v(x) := x - 1$, both functions on $\\mathbb{R}$, and let $c \\in \\mathbb{R}$ with $c \\ne 1$. [construct]",
      "step": "1.2",
      "inputs": [
        "given"
      ]
    ...
    {
      "id": "ex-derivatives-of-polynomials-computed-step-2-1",
      "claim": "**Claim 1.** The function $p$ is the sum of the scalar multiples $\\iota(3)\\cdot(x \\mapsto x^{4})$, $(-\\iota(5))\\cdot(x \\mapsto x^{1})$ and $\\iota(2)\\cdot(x \\mapsto x^{0})$, so by the sum and scalar-multiple rules of [L2] it is differentiable at $c$ with $p'(c) = \\iota(3)\\iota(4)c^{3} + (-\\iota(5)) \\cdot 1 + \\iota(2) \\cdot 0 = \\iota(12)c^{3} - \\iota(5)$, the last step by [L3]. [step 1.1, L2, L3]",
      "step": "2.1",
      "inputs": [
        "L2",
        "L3",
    ...
    {
      "id": "ex-derivatives-of-polynomials-computed-step-2-2",
      "claim": "The functions $u$ and $v$ are differentiable at every real $c$ with $u'(c) = \\iota(2)c$ and $v'(c) = 1$: $u$ is the sum of $x \\mapsto x^{2}$ and the constant $1$, whose derivatives at $c$ are $\\iota(2)c^{1} = \\iota(2)c$ and $0$ by [L1] and [L4]; and $v$ is the sum of $x \\mapsto x^{1}$ and the constant $-1$. [step 1.1, L1, L2, L4]",
      "step": "2.2",
      "inputs": [
        "L1",
        "L4",
    ...
    {
      "id": "ex-derivatives-of-polynomials-computed-step-3-1",
      "claim": "**Claim 2.** By step 1.2 one has $v(c) = c - 1 \\ne 0$, and $\\{x \\in \\mathbb{R} : v(x) \\ne 0\\}$ is exactly $A_0$. So the quotient rule of [L2] applies: $c \\in A_0$, the point $c$ is a limit point of $A_0$, and $r = (u/v)|_{A_0}$ is differentiable at $c$ with $r'(c) = \\bigl(u'(c)v(c) - u(c)v'(c)\\bigr)/v(c)^{2} = \\bigl(\\iota(2)c\\,(c-1) - (c^{2}+1)\\bigr)/(c-1)^{2}$. [step 1.2, step 2.2, L2]",
      "step": "3.1",
      "inputs": [
        "1.2",
        "L2",
    ...
    {
      "id": "ex-derivatives-of-polynomials-computed-step-4-1",
      "claim": "Expanding the numerator: $\\iota(2)c(c-1) - (c^{2}+1) = \\iota(2)c^{2} - \\iota(2)c - c^{2} - 1 = \\bigl(\\iota(2)-1\\bigr)c^{2} - \\iota(2)c - 1 = c^{2} - \\iota(2)c - 1$, the last equality because $\\iota(2) - 1 = 1$ by [L3]. So $r'(c) = \\bigl(c^{2} - \\iota(2)c - 1\\bigr)/(c-1)^{2}$. [step 3.1, L3, L4]",
      "step": "4.1",
      "inputs": [
        "L3",
        "3.1",
    ...
    {
      "id": "ex-derivatives-of-polynomials-computed-step-5-1",
      "claim": "Both claims are verified: claim 1 by step 2.1 and claim 2 by steps 3.1 and 4.1. [step 2.1, step 4.1] ∎",
      "step": "5.1",
      "inputs": [
        "2.1",
        "3.1",
...
[
  {
    "source": "ex-derivatives-of-polynomials-computed",
