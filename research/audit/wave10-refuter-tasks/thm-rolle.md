## Wave 10 target — `thm-rolle`

Selection: mandatory Wave 10 high/critical proof-bearing route (risk report classifies all 49 contracts critical).

Normalized current SHA-256: `345de4d47602f9da96c0ce98169d851cb2e190efaf5e12ddd91f374787b253f3`
...
```json
{
  ...
  "rationale": "The sources state Rolle’s theorem with continuity on the closed interval and differentiability on its interior. The proof explicitly treats the constant and interior-extremum cases.",
...
```json
{
  ...
  "derivations": [
    {
      "id": "thm-rolle-step-1-1",
      "claim": "The set $[a,b]$ is nonempty and compact, and $f$ is continuous on it. [L1]",
      "step": "1.1",
      "inputs": [
        "L1"
      ]
    ...
    {
      "id": "thm-rolle-step-1-2",
      "claim": "Since $a < b$, the open interval $(a,b)$ is nonempty; fix $m := (a+b)/2 \\in (a,b)$. [L6, choose]",
      "step": "1.2",
      "inputs": [
        "L6"
      ]
    ...
    {
      "id": "thm-rolle-step-2-1",
      "claim": "By [L2], applied with $A := K := [a,b]$, fix $p, q \\in [a,b]$ with $f(q) \\le f(x) \\le f(p)$ for every $x \\in [a,b]$. [step 1.1, L2, choose]",
      "step": "2.1",
      "inputs": [
        "L2",
        "1.1"
    ...
    {
      "id": "thm-rolle-step-3-1",
      "claim": "**Case A: at least one of $p, q$ lies in $(a,b)$.** Fix such a point and call it $c$. By [L3] the point $c$ is interior to $[a,b]$, and $f$ is differentiable at $c$ because $c \\in (a,b)$. By step 2.1 and [L4], $f$ has a local maximum at $c$ if $c$ is the point $p$, and a local minimum at $c$ if it is the point $q$; either way a local extremum. So [L5] gives $f'(c) = 0$, and $c \\in (a,b)$. [step 2.1, L3, L4, L5]",
      "step": "3.1",
      "inputs": [
        "L3",
        "2.1",
    ...
    {
      "id": "thm-rolle-step-3-2",
      "claim": "**Case B: neither $p$ nor $q$ lies in $(a,b)$.** A point of $[a,b]$ outside $(a,b)$ satisfies $a \\le x \\le b$ and not $a < x < b$, hence equals $a$ or $b$; so $p, q \\in \\{a, b\\}$ and, since $f(a) = f(b)$, both $f(p)$ and $f(q)$ equal $f(a)$. By step 2.1, every $x \\in [a,b]$ satisfies $f(a) = f(q) \\le f(x) \\le f(p) = f(a)$, so $f(x) = f(a)$. Thus $f$ is the constant function with value $f(a)$ on $[a,b]$. [step 2.1]",
      "step": "3.2",
      "inputs": [
        "2.1"
      ]
    ...
    {
      "id": "thm-rolle-step-4-1",
      "claim": "In case B, [L7] gives that $f$ is differentiable at every point of $[a,b]$ with derivative $0$; in particular $f'(m) = 0$, and $m \\in (a,b)$ by step 1.2. [step 1.2, step 3.2, L7]",
      "step": "4.1",
      "inputs": [
        "L7",
        "1.2",
    ...
    {
      "id": "thm-rolle-step-5-1",
      "claim": "The two cases are exhaustive, since either at least one of $p, q$ lies in $(a,b)$ or neither does. Case A supplies a point $c \\in (a,b)$ with $f'(c) = 0$ by step 3.1, and case B supplies the point $m$ by step 4.1. [step 3.1, step 4.1] ∎",
      "step": "5.1",
      "inputs": [
        "3.1",
        "4.1"
