...
- critical risk (10): 6 declared dependencies; 9 cited facts; boundary-sensitive language; induction, recursion, or minimality; analytic limiting/completeness language

## Target item — `lem-nth-term-test`

Normalized current SHA-256: `7ebe5086f92a63b4597406477c5ab20bf89c2b48fce81db086a57d8615ffe85a`
...
```json
{
  ...
  "rationale": "Hunter Theorem 4.9 states exactly that convergence of a real series forces its terms to tend to zero; its contrapositive is logically identical. The item's arbitrary-starting-index paragraph is only the library's shift convention. The proof follows Hunter's Cauchy-tail argument with local indexing.",
...
```json
{
  ...
  "derivations": [
    {
      "id": "step-1-1",
      "claim": "The sequence $(s_{n+1})_{n \\in \\mathbb{N}}$ is the $1$-st tail of $(s_n)$, and it converges to $S$.",
      "step": "1.1",
      "inputs": [
        "A1",
        "L2"
    ...
    {
      "id": "step-1-2",
      "claim": "From the recursion clause, $a_n = s_{n+1} - s_n$ for every $n \\in \\mathbb{N}$.",
      "step": "1.2",
      "inputs": [
        "L1",
        "algebra"
    ...
    {
      "id": "step-2-1",
      "claim": "Hence $(a_n)$ is the difference of two sequences converging to $S$, so it converges to $S - S = 0$.",
      "step": "2.1",
      "inputs": [
        "1.1",
        "1.2",
    ...
    {
      "id": "step-3-1",
      "claim": "That is the claim, and its contrapositive is the assertion that a series whose terms do not tend to $0$ diverges.",
      "step": "3.1",
      "inputs": [
        "2.1"
      ]
  ...
  "boundaries": [
    ...
    },
    {
      "case": "zero",
      "status": "checked",
      "evidence": "step 2.1 computes the term limit as S - S = 0, including the identically zero series."
    },
    {
      "case": "one",
      "status": "checked",
      "evidence": "step 1.1 uses the first tail (s_{n+1}) and step 1.2 identifies the corresponding first difference a_n."
    },
    {
      "case": "degenerate",
      "status": "checked",
      "evidence": "steps 1.1-2.1 remain valid when every partial sum equals the same constant, in particular for the zero series."
    },
    {
      "case": "endpoints",
      "status": "checked",
      "evidence": "Statement and steps 1.1-2.1 cover n = 0 and the general starting-index form by the stated shift j -> j+m."
    },
    {
