...
# Adversarial proof reading — `thm-exponential-correspondence-is-continuous`

## The item under review, in full

`items/thm-exponential-correspondence-is-continuous.md`
...
### `lem-tube-lemma-for-a-compact-metric-factor` — lemma — Tube lemma: if $K$ is a compact subset of a metric space $X$, $Z$ is a topological space and $N$ is open in $X \\times Z$ with $K \\times \\{z_0\\} \\subseteq N$, then $K \\times W \\subseteq N$ for some open $W \\ni z_0$
...
Read `thm-exponential-correspondence-is-continuous` adversarially against the dependency text above and return
