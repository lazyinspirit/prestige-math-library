...
# Certify the wave-5 repair of `fs-consecutive-differences-null-implies-cauchy`

**Item file:** `items/fs-consecutive-differences-null-implies-cauchy.md` — read it in full from disk, and read every
...
## The repair, as a diff from `7e4f64e` to `HEAD`
...
```diff
(no diff in this range — certify the current text as it stands)
```
