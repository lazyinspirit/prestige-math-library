...
# Adversarial proof reading — `thm-compact-hausdorff-tychonoff-from-the-ultrafilter-lemma`

## The item under review, in full

`items/thm-compact-hausdorff-tychonoff-from-the-ultrafilter-lemma.md`
...
### `lem-continuous-images-of-universal-nets-are-universal` — lemma — The image of a universal net under any map is universal, and a continuous map preserves its limits
...
### `cor-compactness-via-universal-nets` — corollary — Assuming the ultrafilter lemma, a space is compact if and only if every universal net converges
...
### `thm-hausdorff-iff-net-limits-are-unique` — theorem — A topological space is Hausdorff if and only if every net has at most one limit
...
Read `thm-compact-hausdorff-tychonoff-from-the-ultrafilter-lemma` adversarially against the dependency text above and return
