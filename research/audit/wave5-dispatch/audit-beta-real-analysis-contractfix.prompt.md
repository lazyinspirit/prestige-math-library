<!-- TEMPLATE. Copy into an Audit-Beta prompt and substitute 5 (wave) and
     real-analysis. The published-page audit workflow is AUDIT-WORKFLOW.md; this
     brief is its Beta-side half for steps A1, A2, and A4. -->

# Audit-Beta brief — wave 5, real-analysis batch (steps A1/A2/A4)
...
> **Model/routing rule (owner, 2026-08-02; rerouted 2026-08-05).** You are
> **`claude-opus-5`** at `xhigh` effort. Do not substitute another model and do
> not route through any billed pipeline.
>
...
`research/audit/wave5-real-analysis.pages.json`. Nothing here was authored by
...
in `research/audit/wave5-real-analysis.provenance.jsonl`:
...
3. **Capture the full proof contract** for every proof-bearing item (owner
   decision D1) in `research/audit/wave5-real-analysis.proof-contracts.json`:
...
`research/audit/wave5-real-analysis.provenance.jsonl`, `.findings.md`,
...
# Wave 5, A4 gate repair — your batch's proof contract

You are the `wave5-real-analysis` Audit-Beta. A4's gates halted the wave on THREE
`citation-uses` errors in the contract you wrote. Nothing mathematical is alleged
and no item text is in question — this is your contract file's bookkeeping.

`proof-contract.mjs` requires every entry in `citations[]` to carry a non-empty
`uses` array naming every proof step that cites that fact. Three of yours are
empty:
...
Edit **`research/audit/wave5-real-analysis.proof-contracts.json`** only.

For each, open the item in `items/`, read its Proof, and list the exact step ids
(`1.1`, `2.3`, …) whose written justification cites that fact. The checker
compares your list against the steps whose text actually contains the fact token,
so a guess fails and an over-broad list fails too.

**If a fact turns out to be cited by NO step, that is a finding, not a formatting
problem.** Do not invent a `uses` entry to satisfy the gate: an unused `[L#]`
means either the fact is dead and should be removed from the item, or a step
relies on it without saying so. Report which, and do not paper over it.

Do not touch any other batch's contract, any item's mathematics, or the
provenance ledgers. When done, the orchestrator re-merges and re-runs A4's gates.
