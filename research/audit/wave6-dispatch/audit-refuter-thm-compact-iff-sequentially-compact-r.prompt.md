...
## Target item — `thm-compact-iff-sequentially-compact-r`

Normalized current SHA-256: `7de1e8885bee680a6afccb6c42b3a8fbd89dca2a0992fdfbccddaf333847540f`
...
```json
{
  ...
  "derivations": [
    {
      "id": "step-1-1",
      "claim": "For the forward implication assume $K$ is compact; then $K$ is closed and bounded by [L2], so [L5] supplies $\\ell, u$ with $\\ell \\le y \\le u$ for every $y \\in K$. Let $(x_k)$ be any sequence with $x_k \\in K$ for every $k \\in \\mathbb{N}$.",
      "step": "1.1",
      "inputs": [
        "assume-hyp",
        "L2",
    ...
    {
      "id": "step-1-2",
      "claim": "For the backward implication assume $K$ is sequentially compact.",
      "step": "1.2",
      "inputs": [
        "assume-hyp",
        "L1"
    ...
    {
      "id": "step-2-1",
      "claim": "The sequence of step 1.1 is bounded: put $M := \\max\\{|\\ell|, |u|\\}$ by [L10]; for each $k$, from $\\ell \\le x_k \\le u$ we get $x_k \\le u \\le |u| \\le M$ and $-x_k \\le -\\ell \\le |\\ell| \\le M$, so $|x_k| \\le M$ by [L9]. By [L3] there are a strictly increasing $n$ and a real $L$ with $x_{n_j} \\to L$; every term $x_{n_j}$ lies in $K$ and $K$ is closed, so $L \\in K$ by [L4]. Hence every sequence in $K$ has a subsequence converging in $K$, that is, $K$ is sequentially compact.",
      "step": "2.1",
      "inputs": [
        "step 1.1",
        "L1",
    ...
    {
      "id": "step-2-2",
      "claim": "A sequentially compact $K$ is closed: let $y \\in \\overline{K}$; by [L4] there is a sequence $(a_k)$ with $a_k \\in K$ for all $k$ and $a_k \\to y$; by sequential compactness some subsequence $(a_{n_j})$ converges to a point $z \\in K$; but that subsequence also converges to $y$ by [L7], and limits are unique by [L7], so $z = y$ and $y \\in K$. Hence $\\overline{K} \\subseteq K$, so $\\overline{K} = K$ and $K$ is closed by [L4].",
      "step": "2.2",
      "inputs": [
        "step 1.2",
        "L1",
    ...
    {
      "id": "step-2-3",
      "claim": "A sequentially compact $K$ is bounded: suppose it is not. Then for every $k \\in \\mathbb{N}$ the set $Y_k := \\{\\, y \\in K : y > k \\text{ or } y < -k \\,\\}$ is nonempty, since $Y_k = \\varnothing$ would mean $-k \\le y \\le k$ for every $y \\in K$ and make $K$ bounded by [L5]. Use [L6] to fix $f$ with $f(k) \\in Y_k$ and put $x_k := f(k)$; then $x_k \\in K$, and $|x_k| > k$ for every $k$, because $x_k > k \\ge 0$ gives $|x_k| = x_k > k$ while $x_k < -k \\le 0$ gives $|x_k| = -x_k > k$ by [L9] and [L8]. By sequential compactness some subsequence $(x_{n_j})$ converges, hence is bounded by some real $M$ with $|x_{n_j}| \\le M$ for all $j$ by [L7]; by [L8] fix a natural $j \\ge 1$ with $M < j$, and then $|x_{n_j}| > n_j \\ge j > M$ by [L7] and [L8], which contradicts $|x_{n_j}| \\le M$. So $K$ is bounded.",
      "step": "2.3",
      "inputs": [
        "step 1.2",
        "L1",
    ...
    {
      "id": "step-3-1",
      "claim": "A sequentially compact $K$ is therefore closed by step 2.2 and bounded by step 2.3, hence compact by [L2].",
      "step": "3.1",
      "inputs": [
        "step 2.2",
        "step 2.3",
    ...
    {
      "id": "step-4-1",
      "claim": "Step 2.1 is the forward implication and step 3.1 the backward one, so for subsets of $\\mathbb{R}$ compactness and sequential compactness coincide.",
      "step": "4.1",
      "inputs": [
        "step 2.1",
        "step 3.1"
  ...
  "boundaries": [
    {
      ...
      "reason": "The thm-compact-iff-sequentially-compact-r statement has no empty-set or empty-family boundary."
    },
    {
      "case": "zero",
      "status": "not_applicable",
      "reason": "The thm-compact-iff-sequentially-compact-r statement has no zero-valued boundary."
    },
    {
      "case": "one",
      "status": "checked",
    ...
    },
    {
      "case": "degenerate",
      "status": "checked",
      "evidence": "The statement and step 2.3 explicitly treat the equality, singleton, constant, or nonempty degeneracy."
    },
    {
      "case": "endpoints",
      "status": "not_applicable",
      "reason": "The thm-compact-iff-sequentially-compact-r statement has no interval or extended-real endpoint convention."
    },
    {
    ...
    },
    {
      "case": "iff-forward",
      "status": "checked",
      "evidence": "The forward direction is discharged in step 1.1 (with the final synthesis in step 4.1)."
    },
    {
      "case": "iff-reverse",
      "status": "checked",
      "evidence": "The reverse direction is discharged in step 1.2 (with the final synthesis in step 4.1)."
    }
  ],
  "finite_smoke": [],
  "risk_review": {
