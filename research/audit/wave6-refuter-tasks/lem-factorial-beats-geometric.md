## Target item — `lem-factorial-beats-geometric`

Normalized current SHA-256: `3807d889d69b762a1a3e624fb6237a6ff8c207e1e73d209b7ba1050f193ad769`
...
```json
{
  ...
  "derivations": [
    {
      "id": "step-1-1",
      "claim": "Each $k!$ is a product of the positive reals $\\iota(j+1)$, $j < k$, hence positive, and $(k+1)! = k! \\cdot \\iota(k+1)$; also $M = |x| \\ge 0$.",
      "step": "1.1",
      "inputs": [
        "given",
        "L1",
    ...
    {
      "id": "step-1-2",
      "claim": "For every $k \\in \\mathbb{N}$ one has $|x^{k}| = M^{k}$: at $k = 0$ both sides are $|1| = 1 = M^{0}$, and if $|x^{k}| = M^{k}$ then $|x^{k+1}| = |x^{k} x| = |x^{k}||x| = M^{k} M = M^{k+1}$, so this follows by induction on $k$.",
      "step": "1.2",
      "inputs": [
        "given",
        "L2",
    ...
    {
      "id": "step-1-3",
      "claim": "Take a natural $N \\ge 1$ with $M < \\iota(N)$ and put $\\lambda := M/\\iota(N)$ and $A := M^{N}/N!$. Then $0 \\le \\lambda < 1$, since $0 \\le M < \\iota(N)$ and $\\iota(N) > 0$, and $A \\ge 0$.",
      "step": "1.3",
      "inputs": [
        "given",
        "L1",
    ...
    {
      "id": "step-1-4",
      "claim": "The statement $P(0)$ holds, with equality: $M^{N+0}/(N+0)! = M^{N}/N! = A = A \\cdot 1 = A\\lambda^{0}$.",
      "step": "1.4",
      "inputs": [
        "given",
        "A1",
    ...
    {
      "id": "step-1-5",
      "claim": "Fix $j \\in \\mathbb{N}$ and assume $P(j)$, that is $M^{N+j}/(N+j)! \\le A\\lambda^{j}$.",
      "step": "1.5",
      "inputs": [
        "A1",
        "ih"
    ...
    {
      "id": "step-2-1",
      "claim": "Then $P(j+1)$ holds. Indeed $M^{N+j+1}/(N+j+1)! = \\big(M^{N+j}/(N+j)!\\big)\\big(M/\\iota(N+j+1)\\big)$, and $N + j + 1 > N$ gives $\\iota(N+j+1) > \\iota(N) > 0$, hence $0 \\le M/\\iota(N+j+1) \\le M/\\iota(N) = \\lambda$; since also $0 \\le M^{N+j}/(N+j)! \\le A\\lambda^{j}$ by step 1.5 and $A\\lambda^{j} \\ge 0$, multiplying the two nonnegative inequalities gives $M^{N+j+1}/(N+j+1)! \\le A\\lambda^{j}\\lambda = A\\lambda^{j+1}$.",
      "step": "2.1",
      "inputs": [
        "step 1.5",
        "A1",
    ...
    {
      "id": "step-3-1",
      "claim": "By the induction principle $P(j)$ holds for every $j \\in \\mathbb{N}$, and $M^{N+j}/(N+j)! \\ge 0$ always, so $0 \\le M^{N+j}/(N+j)! \\le A\\lambda^{j}$ for every $j$.",
      "step": "3.1",
      "inputs": [
        "step 1.4",
        "step 2.1",
    ...
    {
      "id": "step-4-1",
      "claim": "Since $|\\lambda| = \\lambda < 1$, the sequence $(\\lambda^{j})_j$ converges to $0$, hence so does $(A\\lambda^{j})_j$; the constant sequence $0$ also converges to $0$, so the squeeze theorem applied to step 3.1 shows that the $N$-th tail $j \\mapsto M^{N+j}/(N+j)!$ converges to $0$, and therefore $(M^{k}/k!)_k$ converges to $0$. Finally $|x^{k}/k! - 0| = |x^{k}|/k! = M^{k}/k!$ by steps 1.1 and 1.2, so $x^{k}/k! \\to 0$.",
      "step": "4.1",
      "inputs": [
        "step 3.1",
        "step 1.1",
  ...
  "boundaries": [
    ...
    },
    {
      "case": "degenerate",
      "status": "checked",
      "evidence": "The statement and step 1.4 explicitly treat the equality, singleton, constant, or nonempty degeneracy."
    },
    {
      "case": "endpoints",
      "status": "not_applicable",
      "reason": "The lem-factorial-beats-geometric statement has no interval or extended-real endpoint convention."
    },
    {
    ...
    },
    {
      "case": "iff-forward",
      "status": "not_applicable",
      "reason": "The lem-factorial-beats-geometric statement is not a biconditional requiring a forward implication."
    },
    {
      "case": "iff-reverse",
      "status": "not_applicable",
      "reason": "The lem-factorial-beats-geometric statement is not a biconditional requiring a reverse implication."
    }
  ],
  "finite_smoke": [],
  "risk_review": {
