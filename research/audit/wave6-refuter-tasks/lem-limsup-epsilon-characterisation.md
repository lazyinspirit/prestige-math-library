## Target item — `lem-limsup-epsilon-characterisation`

Normalized current SHA-256: `4405d72125007608290066405169420ccfc0022974cf9e3bf4a7980e44c8e1d2`
...
```json
{
  ...
  "derivations": [
    {
      "id": "step-1-1",
      "claim": "For the forward implication of claim 1, assume $L = \\Lambda$ and let $\\varepsilon > 0$ be an arbitrary real.",
      "step": "1.1",
      "inputs": [
        "assume-hyp",
        "L1"
    ...
    {
      "id": "step-1-2",
      "claim": "For the converse implication of claim 1, assume that for every real $\\varepsilon > 0$ the sequence satisfies $x_k < L + \\varepsilon$ eventually and $x_k > L - \\varepsilon$ frequently.",
      "step": "1.2",
      "inputs": [
        "assume-hyp",
        "L3"
    ...
    {
      "id": "step-2-1",
      "claim": "Under the assumption of step 1.1, $L + \\varepsilon > L = \\Lambda$, so $L + \\varepsilon$ is not a lower bound of $\\{s_n\\}$, since $\\Lambda$ is the greatest lower bound; by totality there is $n$ with $s_n < L + \\varepsilon$. For every $k \\ge n$ we have $x_k \\le s_n$, hence $x_k < L + \\varepsilon$; so $x_k < L + \\varepsilon$ eventually.",
      "step": "2.1",
      "inputs": [
        "step 1.1",
        "L1",
    ...
    {
      "id": "step-2-2",
      "claim": "Under the assumption of step 1.1, fix $n \\in \\mathbb{N}$. Then $\\Lambda \\le s_n$ because $\\Lambda$ is a lower bound of $\\{s_n\\}$, and $L - \\varepsilon < L = \\Lambda$, so $L - \\varepsilon < s_n$. Hence $L - \\varepsilon$ is not an upper bound of $T_n$, for an upper bound $u$ of $T_n$ satisfies $s_n \\le u$; by totality of the order on $\\mathbb{R}$ there is therefore $k \\ge n$ with $x_k > L - \\varepsilon$. As $n$ was arbitrary, $x_k > L - \\varepsilon$ frequently.",
      "step": "2.2",
      "inputs": [
        "step 1.1",
        "L1",
    ...
    {
      "id": "step-2-3",
      "claim": "Under the assumption of step 1.2, let $\\varepsilon > 0$ be a real and take $N$ with $x_k < L + \\varepsilon$ for all $k \\ge N$. Then $L + \\varepsilon$ is an upper bound of $T_N$, so $s_N \\le L + \\varepsilon$ by leastness, and $\\Lambda \\le s_N$ because $\\Lambda$ is a lower bound of $\\{s_n\\}$; hence $\\Lambda \\le L + \\varepsilon$.",
      "step": "2.3",
      "inputs": [
        "step 1.2",
        "L1",
    ...
    {
      "id": "step-2-4",
      "claim": "Under the assumption of step 1.2, let $\\varepsilon > 0$ be a real and fix $n$. There is $k \\ge n$ with $x_k > L - \\varepsilon$, and $x_k \\le s_n$, so $L - \\varepsilon < s_n$ and in particular $L - \\varepsilon \\le s_n$. As $n$ was arbitrary, $L - \\varepsilon$ is a lower bound of $\\{s_n\\}$, so $L - \\varepsilon \\le \\Lambda$ by greatest-lower-boundedness.",
      "step": "2.4",
      "inputs": [
        "step 1.2",
        "L1",
    ...
    {
      "id": "step-3-1",
      "claim": "Taking $\\varepsilon = 1$ in steps 2.3 and 2.4 gives $L - 1 \\le \\Lambda \\le L + 1$ with $L \\pm 1$ real, so $\\Lambda$ is neither $+\\infty$ nor $-\\infty$ and is therefore a real number. Suppose $\\Lambda > L$ and put $\\delta := \\Lambda - L > 0$; choosing a natural $m \\ge 1$ with $1/m < \\delta$ and applying step 2.3 with $\\varepsilon = 1/m$ gives $\\Lambda \\le L + 1/m < L + \\delta = \\Lambda$, which is impossible. Suppose instead $\\Lambda < L$ and put $\\delta := L - \\Lambda > 0$; choosing $m \\ge 1$ with $1/m < \\delta$ and applying step 2.4 with $\\varepsilon = 1/m$ gives $L - 1/m \\le \\Lambda$, that is $\\delta = L - \\Lambda \\le 1/m < \\delta$, again impossible. By trichotomy $\\Lambda = L$.",
      "step": "3.1",
      "inputs": [
        "step 2.3",
        "step 2.4",
    ...
    {
      "id": "step-4-1",
      "claim": "Steps 2.1 and 2.2 prove the forward implication of claim 1 and step 3.1 proves its converse, so claim 1 holds.",
      "step": "4.1",
      "inputs": [
        "step 2.1",
        "step 2.2",
    ...
    {
      "id": "step-5-1",
      "claim": "For claim 2, note that $L = \\liminf_k x_k$ holds exactly when $-L = -\\liminf_k x_k = \\limsup_k(-x_k)$, since negation is injective on $\\overline{\\mathbb{R}}$. Applying claim 1 to the sequence $(-x_k)$ and the real number $-L$, that holds exactly when for every real $\\varepsilon > 0$ one has $-x_k < -L + \\varepsilon$ eventually and $-x_k > -L - \\varepsilon$ frequently. Negating each of the two inequalities reverses it, turning them into $x_k > L - \\varepsilon$ eventually and $x_k < L + \\varepsilon$ frequently, which is claim 2.",
      "step": "5.1",
      "inputs": [
        "step 4.1",
        "L5",
  ...
  "boundaries": [
    {
      ...
      "reason": "The lem-limsup-epsilon-characterisation statement has no empty-set or empty-family boundary."
    },
    {
      "case": "zero",
      "status": "not_applicable",
      "reason": "The lem-limsup-epsilon-characterisation statement has no zero-valued boundary."
    },
    {
      "case": "one",
      "status": "checked",
    ...
    },
    {
      "case": "degenerate",
      "status": "not_applicable",
      "reason": "The lem-limsup-epsilon-characterisation statement has no parameter degeneracy that changes the claim."
    },
    {
      "case": "endpoints",
      "status": "checked",
    ...
    },
    {
      "case": "iff-forward",
      "status": "checked",
      "evidence": "The forward direction is discharged in step 1.1 (with the final synthesis in step 5.1)."
    },
    {
      "case": "iff-reverse",
      "status": "checked",
      "evidence": "The reverse direction is discharged in step 1.1 (with the final synthesis in step 5.1)."
    }
  ],
  "finite_smoke": [],
  "risk_review": {
