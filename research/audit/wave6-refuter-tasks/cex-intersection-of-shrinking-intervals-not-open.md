## Target item — `cex-intersection-of-shrinking-intervals-not-open`

Normalized current SHA-256: `5a1d5a23f13258f0e3f5125ed6ffff86a8f55218c73725f4b32978aa1c9a3300`
...
```json
{
  ...
  "derivations": [
    {
      "id": "step-1-1",
      "claim": "Each $U_k$ is a nonempty open subset of $\\mathbb{R}$, being the interval $(-1/k, 1/k)$ with $1/k > 0$, so the family $\\{\\, U_k : k \\ge 1 \\,\\}$ is an instance of the claim [A1], which asserts that its intersection is open.",
      "step": "1.1",
      "inputs": [
        "A1",
        "L1",
    ...
    {
      "id": "step-1-2",
      "claim": "$0 \\in U_k$ for every $k \\ge 1$, since $|0| = 0 < 1/k$ by [L4].",
      "step": "1.2",
      "inputs": [
        "L1",
        "L4"
    ...
    {
      "id": "step-2-1",
      "claim": "If $x \\ne 0$ then $|x| > 0$ by [L4], so [L3] supplies a natural $n \\ge 1$ with $1/n < |x|$, and $x \\in U_n$ would give $|x| < 1/n$, which trichotomy forbids; hence $x \\notin U_n$. With step 1.2 this gives $\\bigcap_{k \\ge 1} U_k = \\{0\\}$.",
      "step": "2.1",
      "inputs": [
        "step 1.2",
        "L3",
    ...
    {
      "id": "step-3-1",
      "claim": "The singleton $\\{0\\}$ is not open, since for every real $\\varepsilon > 0$ the point $\\varepsilon \\cdot 2^{-1}$ lies in $N_\\varepsilon(0)$ and differs from $0$ by [L4]. So the family of step 1.1 consists of open sets and its intersection, computed in step 2.1, is not open: the claim [A1] is refuted.",
      "step": "3.1",
      "inputs": [
        "step 1.1",
        "step 2.1",
  ...
  "boundaries": [
    {
      ...
      "reason": "The cex-intersection-of-shrinking-intervals-not-open statement has no empty-set or empty-family boundary."
    },
    {
      "case": "zero",
      "status": "checked",
    ...
    },
    {
      "case": "one",
      "status": "not_applicable",
      "reason": "The cex-intersection-of-shrinking-intervals-not-open statement has no unit or first-index boundary."
    },
    {
      "case": "degenerate",
      "status": "checked",
    ...
    },
    {
      ...
      "status": "not_applicable",
      "reason": "The cex-intersection-of-shrinking-intervals-not-open proof makes no selection from a set whose nonemptiness needs a separate discharge."
    },
    {
      "case": "iff-forward",
      "status": "not_applicable",
      "reason": "The cex-intersection-of-shrinking-intervals-not-open statement is not a biconditional requiring a forward implication."
    },
    {
      "case": "iff-reverse",
      "status": "not_applicable",
      "reason": "The cex-intersection-of-shrinking-intervals-not-open statement is not a biconditional requiring a reverse implication."
    }
  ],
  "finite_smoke": [],
  "risk_review": {
...
[
  {
    "source": "cex-intersection-of-shrinking-intervals-not-open",
    "sourcePage": "topology-of-r-examples",
    "batch": "wave6-real-analysis-limits",
    "target": "fs-arbitrary-intersection-of-open-is-open",
    "declared_target": "fs-arbitrary-intersection-of-open-is-open",
