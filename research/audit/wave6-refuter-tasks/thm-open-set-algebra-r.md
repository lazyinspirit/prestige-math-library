## Target item — `thm-open-set-algebra-r`

Normalized current SHA-256: `6c804b78cf2d16da1159ab7f0df19e63cc319021b93b66f10cda90afb68967cd`
...
```json
{
  ...
  "derivations": [
    {
      "id": "step-1-1",
      "claim": "Let $x \\in \\bigcup \\mathcal{U}$. Then $x \\in U$ for some $U \\in \\mathcal{U}$, and $U$ is open, so there is $\\varepsilon > 0$ with $N_\\varepsilon(x) \\subseteq U \\subseteq \\bigcup \\mathcal{U}$; as $x$ was arbitrary, $\\bigcup \\mathcal{U}$ is open, which is claim 1.",
      "step": "1.1",
      "inputs": [
        "given",
        "L1",
    ...
    {
      "id": "step-1-2",
      "claim": "Now let $U$ and $V$ be open and let $x \\in U \\cap V$; fix $\\varepsilon > 0$ with $N_\\varepsilon(x) \\subseteq U$ and $\\eta > 0$ with $N_\\eta(x) \\subseteq V$.",
      "step": "1.2",
      "inputs": [
        "given",
        "L1",
    ...
    {
      "id": "step-2-1",
      "claim": "Put $\\delta := \\min\\{\\varepsilon, \\eta\\}$, which is one of $\\varepsilon, \\eta$ and hence $> 0$, and satisfies $\\delta \\le \\varepsilon$ and $\\delta \\le \\eta$; then $N_\\delta(x) \\subseteq N_\\varepsilon(x) \\subseteq U$ and $N_\\delta(x) \\subseteq N_\\eta(x) \\subseteq V$, so $N_\\delta(x) \\subseteq U \\cap V$, and as $x$ was arbitrary $U \\cap V$ is open.",
      "step": "2.1",
      "inputs": [
        "step 1.2",
        "L2",
    ...
    {
      "id": "step-2-2",
      "claim": "The family $\\{\\, \\mathbb{R} \\setminus F : F \\in \\mathcal{F} \\,\\}$ consists of open sets by [L1], so its union is open by step 1.1; that union is $\\mathbb{R} \\setminus \\bigcap \\mathcal{F}$ by [A1], so $\\bigcap \\mathcal{F}$ is closed, which is claim 3.",
      "step": "2.2",
      "inputs": [
        "step 1.1",
        "A1",
    ...
    {
      "id": "step-3-1",
      "claim": "Claim 2 now follows by induction on $n$: for $n = 0$ the intersection is $U_0$, which is open by hypothesis; and if $W := U_0 \\cap \\dots \\cap U_n$ is open then $U_0 \\cap \\dots \\cap U_{n+1} = W \\cap U_{n+1}$ is an intersection of two open sets, hence open by step 2.1.",
      "step": "3.1",
      "inputs": [
        "step 2.1",
        "given"
    ...
    {
      "id": "step-4-1",
      "claim": "Each $\\mathbb{R} \\setminus F_i$ is open by [L1], so $(\\mathbb{R} \\setminus F_0) \\cap \\dots \\cap (\\mathbb{R} \\setminus F_n)$ is open by step 3.1; that set is $\\mathbb{R} \\setminus (F_0 \\cup \\dots \\cup F_n)$ by [A1], so $F_0 \\cup \\dots \\cup F_n$ is closed, which is claim 4.",
      "step": "4.1",
      "inputs": [
        "step 3.1",
        "A1",
    ...
    {
      "id": "step-5-1",
      "claim": "Claims 1, 2, 3 and 4 are steps 1.1, 3.1, 2.2 and 4.1 respectively, so arbitrary unions and finite intersections of open sets are open, and arbitrary intersections and finite unions of closed sets are closed.",
      "step": "5.1",
      "inputs": [
        "step 1.1",
        "step 2.2",
  ...
  "boundaries": [
    ...
    },
    {
      ...
      "reason": "The thm-open-set-algebra-r statement has no interval or extended-real endpoint convention."
    },
    {
    ...
    },
    {
      "case": "iff-forward",
      "status": "not_applicable",
      "reason": "The thm-open-set-algebra-r statement is not a biconditional requiring a forward implication."
    },
    {
      "case": "iff-reverse",
      "status": "not_applicable",
      "reason": "The thm-open-set-algebra-r statement is not a biconditional requiring a reverse implication."
    }
  ],
  "finite_smoke": [],
  "risk_review": {
...
[
  ...
  },
  {
    ...
    "sourcePage": "topology-of-r",
    "batch": "wave6-real-analysis-limits",
    "target": "cex-intersection-of-shrinking-intervals-not-open",
    "declared_target": "cex-intersection-of-shrinking-intervals-not-open",
    "target_statement_provenance": null,
...
### `fs-arbitrary-intersection-of-open-is-open`
