...
- high risk (7): 10 declared dependencies; 10 cited facts; induction, recursion, or minimality

## Target item — `ex-geometric-function-taylor-remainder`

Normalized current SHA-256: `2291a8ea600b4a897d24d6b3020f510cd24f22d5b99a00a6bfd5027278099933`
...
```json
{
  ...
  "rationale": "The source develops the Taylor polynomials for the geometric rational function and the geometric-series coefficients. The item records the finite exact algebraic remainder x^{n+1}/(1-x), valid for every x not equal to 1, rather than using a convergence claim.",
...
```json
{
  "citations": [
    ...
    },
    {
      ...
      "source_section": "Statement",
      "quote": "Let $n\\in\\mathbb N$, and suppose $f$ has derivatives through order $n+1$ on the closed interval between $a$ and $x$, with the usual endpoint continuity. If $|f^{(n+1)}(t)|\\le M$ throughout that interval, then\n$$|R_{n,a}f(x)|\\le \\frac{M}{\\iota((n+1)!)}|x-a|^{n+1}.$$",
      "uses": [
        "3.1"
      ]
    }
  ...
  "derivations": [
    {
      "id": "ex-geometric-function-taylor-remainder-step-1-1",
      "claim": "Induction gives $f^{(j)}(x)=\\iota(j!)(1-x)^{-j-1}$, hence $f^{(j)}(0)/\\iota(j!)=1$. [L1]",
      "step": "1.1",
      "inputs": [
        "L1"
      ]
    ...
    {
      "id": "ex-geometric-function-taylor-remainder-step-2-1",
      "claim": "Multiplying $\\sum_{j=0}^{n}x^j$ by $1-x$ telescopes to $1-x^{n+1}$. Subtracting from $1/(1-x)$ gives the stated remainder. [step 1.1, L1, algebra]",
      "step": "2.1",
      "inputs": [
        "step 1.1",
        "L1",
    ...
    {
      "id": "ex-geometric-function-taylor-remainder-step-3-1",
      "claim": "This exact expression agrees with the qualitative estimate supplied by [L2] on every closed interval avoiding $1$. [step 2.1, L2] ∎",
      "step": "3.1",
      "inputs": [
        "step 2.1",
        "L2"
  ...
  "boundaries": [
    {
      ...
      "reason": "ex-geometric-function-taylor-remainder concerns stipulated functions and points; no empty family, empty sum, or empty choice domain changes its assertion."
    },
    {
      "case": "zero",
      "status": "not_applicable",
      "reason": "ex-geometric-function-taylor-remainder has no counting, exponent, or value parameter whose zero case changes the proof."
    },
    {
      "case": "one",
      "status": "not_applicable",
      "reason": "ex-geometric-function-taylor-remainder has no counting, exponent, or unit parameter whose one case changes the proof."
    },
    {
      "case": "degenerate",
      "status": "not_applicable",
      "reason": "ex-geometric-function-taylor-remainder has no interval-length or denominator parameter that may collapse degenerately."
    },
    {
      "case": "endpoints",
      "status": "not_applicable",
      "reason": "ex-geometric-function-taylor-remainder contains no interval-endpoint assertion requiring a separate endpoint case."
    },
    {
    ...
    },
    {
      "case": "iff-forward",
      "status": "not_applicable",
      "reason": "The Statement of ex-geometric-function-taylor-remainder is not a biconditional, so it has no forward iff direction."
    },
    {
      "case": "iff-reverse",
      "status": "not_applicable",
      "reason": "The Statement of ex-geometric-function-taylor-remainder is not a biconditional, so it has no reverse iff direction."
    }
  ],
  "finite_smoke": [],
  "risk_review": {
...
[
  {
    "source": "ex-geometric-function-taylor-remainder",
  ...
  },
  {
    ...
    "sourcePage": "darboux-lhopital-and-taylor-examples",
    "batch": "wave11-real-analysis",
    "target": "cor-taylor-remainder-bound",
    "declared_target": "cor-taylor-remainder-bound",
