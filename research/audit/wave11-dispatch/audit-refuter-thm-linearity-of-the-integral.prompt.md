...
- critical risk (12): 15 declared dependencies; 18 cited facts; 18 numbered proof steps; biconditional / both-direction claim; analytic limiting/completeness language

## Target item — `thm-linearity-of-the-integral`

Normalized current SHA-256: `5ddedcbca79ba5bf988a1a3b132e584d4a611897d1903879f7faed8341c96809`
...
```json
{
  ...
  "derivations": [
    {
      "id": "thm-linearity-of-the-integral-step-1-1",
      "claim": "$f$, $g$, $f+g$ and $\\lambda f$ are bounded on $[a,b]$, so all their Darboux sums and integrals are defined. [given, L3]",
      "step": "1.1",
      "inputs": [
        "given",
        "L3"
    ...
    {
      "id": "thm-linearity-of-the-integral-step-1-2",
      "claim": "For every partition $P = (n,t)$ and every $i < n$: $f(x) + g(x) \\le M_i(f) + M_i(g)$ for $x \\in I_i$, so $M_i(f) + M_i(g)$ is an upper bound of $(f+g)[I_i]$ and $M_i(f+g) \\le M_i(f) + M_i(g)$ by [L6]; dually $m_i(f+g) \\ge m_i(f) + m_i(g)$. [L3, L6]",
      "step": "1.2",
      "inputs": [
        "L3",
        "L6"
    ...
    {
      "id": "thm-linearity-of-the-integral-step-1-3",
      "claim": "Fix partitions $P_1$ and $P_2$ with $U(f,P_1) - L(f,P_1) < \\varepsilon \\cdot 2^{-1}$ and $U(g,P_2) - L(g,P_2) < \\varepsilon \\cdot 2^{-1}$, and put $P := P_1 \\vee P_2$. [given, L1, L4, choose]",
      "step": "1.3",
      "inputs": [
        "given",
        "L1",
    ...
    {
      "id": "thm-linearity-of-the-integral-step-1-4",
      "claim": "**Claim 2, the case $\\lambda = 0$.** Then $\\lambda f$ is the constant function $0$, integrable with integral $0 = 0 \\cdot \\int_a^b f$. [L8]",
      "step": "1.4",
      "inputs": [
        "L8"
      ]
    ...
    {
      "id": "thm-linearity-of-the-integral-step-1-5",
      "claim": "**Claim 2, the case $\\lambda > 0$.** For every partition $P$ and every $i < n$, $\\lambda M_i(f)$ is an upper bound of $(\\lambda f)[I_i]$, and any upper bound $w$ of $(\\lambda f)[I_i]$ gives the upper bound $w/\\lambda$ of $f[I_i]$, whence $M_i(f) \\le w/\\lambda$ and $\\lambda M_i(f) \\le w$; so $M_i(\\lambda f) = \\lambda M_i(f)$ by [L6], and dually $m_i(\\lambda f) = \\lambda m_i(f)$. [L3, L6, L9]",
      "step": "1.5",
      "inputs": [
        "L3",
        "L6",
    ...
    {
      "id": "thm-linearity-of-the-integral-step-1-6",
      "claim": "**Claim 2, the case $\\lambda = -1$.** For every $P$ and $i < n$, $(-f)[I_i] = -\\bigl(f[I_i]\\bigr)$, so $M_i(-f) = -m_i(f)$ and $m_i(-f) = -M_i(f)$ by [L7]. [L3, L7]",
      "step": "1.6",
      "inputs": [
        "L3",
        "L7"
    ...
    {
      "id": "thm-linearity-of-the-integral-step-2-1",
      "claim": "By [L4], $U(f,P) - L(f,P) < \\varepsilon\\cdot 2^{-1}$ and $U(g,P) - L(g,P) < \\varepsilon \\cdot 2^{-1}$. [step 1.3, L4]",
      "step": "2.1",
      "inputs": [
        "step 1.3",
        "L4"
    ...
    {
      "id": "thm-linearity-of-the-integral-step-2-2",
      "claim": "Summing the inequalities of step 1.2 over $i < n$ against the positive weights $\\Delta_i$ and using [L5] gives $L(f,P) + L(g,P) \\le L(f+g,P) \\le U(f+g,P) \\le U(f,P) + U(g,P)$. [step 1.2, L3, L5, L9]",
      "step": "2.2",
      "inputs": [
        "step 1.2",
        "L3",
    ...
    {
      "id": "thm-linearity-of-the-integral-step-2-3",
      "claim": "With step 1.5 and [L5], $U(\\lambda f,P) = \\lambda U(f,P)$ and $L(\\lambda f, P) = \\lambda L(f,P)$ for $\\lambda > 0$; hence $U(\\lambda f,P)-L(\\lambda f,P) = \\lambda\\bigl(U(f,P)-L(f,P)\\bigr)$, which [L1] makes smaller than any prescribed positive number by choosing $P$ suitably, so $\\lambda f$ is integrable. [step 1.5, L1, L3, L5, L9]",
      "step": "2.3",
      "inputs": [
        "step 1.5",
        "L1",
    ...
    {
      "id": "thm-linearity-of-the-integral-step-2-4",
      "claim": "With step 1.6 and [L5], $U(-f,P) = -L(f,P)$ and $L(-f,P) = -U(f,P)$, so $U(-f,P)-L(-f,P) = U(f,P)-L(f,P)$ and $-f$ is integrable by [L1]; and by [L7] applied to the sets of Darboux sums, $\\overline{\\int_a^b}(-f) = -\\underline{\\int_a^b} f$ and $\\underline{\\int_a^b}(-f) = -\\overline{\\int_a^b} f$, so $\\int_a^b(-f) = -\\int_a^b f$. [step 1.6, L1, L2, L5, L7]",
      "step": "2.4",
      "inputs": [
        "step 1.6",
        "L1",
    ...
    {
      "id": "thm-linearity-of-the-integral-step-3-1",
      "claim": "Hence $U(f+g,P) - L(f+g,P) \\le \\bigl(U(f,P)-L(f,P)\\bigr) + \\bigl(U(g,P)-L(g,P)\\bigr) < \\varepsilon$, so $f+g$ is integrable by [L1], $\\varepsilon > 0$ having been arbitrary. [step 1.1, step 2.1, step 2.2, L1, L9]",
      "step": "3.1",
      "inputs": [
        "step 1.1",
        "step 2.1",
    ...
    {
      "id": "thm-linearity-of-the-integral-step-3-2",
      "claim": "Moreover the set of lower sums of $\\lambda f$ is $\\lambda$ times the set of lower sums of $f$, and a supremum scales by a positive factor, by the argument of step 1.5 applied to that set; so $\\underline{\\int_a^b}\\lambda f = \\lambda \\underline{\\int_a^b} f$, and likewise for the upper integrals, giving $\\int_a^b \\lambda f = \\lambda\\int_a^b f$. [step 1.5, step 2.3, L2, L6, L9]",
      "step": "3.2",
      "inputs": [
        "step 1.5",
        "step 2.3",
    ...
    {
      "id": "thm-linearity-of-the-integral-step-4-1",
      "claim": "Both $\\int_a^b(f+g)$ and $\\int_a^b f + \\int_a^b g$ lie in the interval from $L(f,P)+L(g,P)$ to $U(f,P)+U(g,P)$: the first by [L2] and step 2.2, the second by [L2] applied to $f$ and to $g$ separately. [step 2.1, step 2.2, step 3.1, L2]",
      "step": "4.1",
      "inputs": [
        "step 2.1",
        "step 2.2",
    ...
    {
      "id": "thm-linearity-of-the-integral-step-4-2",
      "claim": "**Claim 2 for $\\lambda < 0$.** Then $-\\lambda > 0$ and $\\lambda f = (-\\lambda)(-f)$, so steps 2.3, 2.4 and 3.2 give integrability and the required identities and $\\int_a^b \\lambda f = (-\\lambda)\\int_a^b(-f) = (-\\lambda)\\bigl(-\\int_a^b f\\bigr) = \\lambda\\int_a^b f$. [step 2.3, step 3.2, step 2.4, L9]",
      "step": "4.2",
      "inputs": [
        "step 2.3",
        "step 3.2",
    ...
    {
      "id": "thm-linearity-of-the-integral-step-5-1",
      "claim": "That interval has length less than $\\varepsilon$ by step 2.1, so $\\bigl|\\int_a^b(f+g) - \\int_a^b f - \\int_a^b g\\bigr| < \\varepsilon$; as $\\varepsilon > 0$ was arbitrary the difference is $0$, which is claim 1. [step 2.1, step 4.1, L9]",
      "step": "5.1",
      "inputs": [
        "step 2.1",
        "step 4.1",
    ...
    {
      "id": "thm-linearity-of-the-integral-step-5-2",
      "claim": "Claim 2 is now proved in all three cases $\\lambda = 0$, $\\lambda > 0$ and $\\lambda < 0$, which are exhaustive by trichotomy. [step 1.4, step 2.3, step 3.2, step 4.2, L9]",
      "step": "5.2",
      "inputs": [
        "step 1.4",
        "step 2.3",
    ...
    {
      "id": "thm-linearity-of-the-integral-step-6-1",
      "claim": "**Claim 3.** By claim 2 the functions $\\lambda f$ and $\\mu g$ are integrable with integrals $\\lambda\\int_a^b f$ and $\\mu\\int_a^b g$, and by claim 1 their sum is integrable with the sum of those integrals. [step 5.1, step 5.2]",
      "step": "6.1",
      "inputs": [
        "step 5.1",
        "step 5.2"
    ...
    {
      "id": "thm-linearity-of-the-integral-step-7-1",
      "claim": "**Claim 4.** If $u < v$ then $u, v \\in [a,b]$ and claim 3 applies verbatim on $[u,v]$; if $u = v$ both sides are $0$ by [L10]; and if $u > v$ then applying the case $u < v$ to the pair $(v,u)$ and multiplying by $-1$ gives the identity, by [L10]. [step 6.1, L10] ∎",
      "step": "7.1",
      "inputs": [
        "step 6.1",
        "L10"
  ...
  "boundaries": [
    {
      ...
      "reason": "thm-linearity-of-the-integral concerns stipulated functions and points; no empty family, empty sum, or empty choice domain changes its assertion."
    },
    {
      "case": "zero",
      "status": "checked",
      "evidence": "Statement includes the zero boundary, and step 1.4 treats or preserves that case."
    },
    {
      "case": "one",
      "status": "checked",
      "evidence": "Statement includes the unit/first-index boundary, and step 1.3 treats or preserves that case."
    },
    {
      "case": "degenerate",
      "status": "checked",
    ...
    },
    {
      "case": "iff-forward",
      "status": "not_applicable",
      "reason": "The Statement of thm-linearity-of-the-integral is not a biconditional, so it has no forward iff direction."
    },
    {
      "case": "iff-reverse",
      "status": "not_applicable",
      "reason": "The Statement of thm-linearity-of-the-integral is not a biconditional, so it has no reverse iff direction."
    }
  ],
  "finite_smoke": [],
  "risk_review": {
