...
## Target item — `ex-polynomial-integrals-by-the-ftc`

Normalized current SHA-256: `134682706ea58a91339b026f2bbeead6926f18848478200825ce122ac647c68a`
...
```json
{
  ...
  "urls": [
    "https://people.eecs.berkeley.edu/~demmel/ma128a_Spr02/LectureNotes/Integration.html"
  ],
  "rationale": "The university notes state exactly integral_0^1 x^m dx=1/(m+1) for nonnegative integer m. The item translates the integer into the library's canonical real embedding and supplies local FTC and consistency checks.",
...
```json
{
  ...
  "derivations": [
    {
      "id": "ex-polynomial-integrals-by-the-ftc-step-1-1",
      "claim": "$p_m$ is continuous on $[0,1]$, hence integrable there, by [L3]. [L3]",
      "step": "1.1",
      "inputs": [
        "L3"
      ]
    ...
    {
      "id": "ex-polynomial-integrals-by-the-ftc-step-1-2",
      "claim": "Define $H : [0,1] \\to \\mathbb{R}$ by $H(x) := x^{m+1}/\\iota(m+1)$; this is legitimate because $\\iota(m+1) > 0$ by [L6]. [L6, L8, construct]",
      "step": "1.2",
      "inputs": [
        "L6",
        "L8",
    ...
    {
      "id": "ex-polynomial-integrals-by-the-ftc-step-2-1",
      "claim": "By [L1] with $n := m+1 \\ge 1$ and [L2], $H$ is differentiable at every point of $[0,1]$ with $H'(x) = \\iota(m+1)\\,x^{m}/\\iota(m+1) = x^{m} = p_m(x)$. [step 1.2, L1, L2, L6]",
      "step": "2.1",
      "inputs": [
        "step 1.2",
        "L1",
    ...
    {
      "id": "ex-polynomial-integrals-by-the-ftc-step-3-1",
      "claim": "By [L4] applied to $H$ on $[0,1]$, whose derivative $p_m$ is integrable by step 1.1, $\\int_0^1 x^{m} = H(1)-H(0)$. [step 1.1, step 2.1, L4]",
      "step": "3.1",
      "inputs": [
        "step 1.1",
        "step 2.1",
    ...
    {
      "id": "ex-polynomial-integrals-by-the-ftc-step-4-1",
      "claim": "By [L5], $H(1) = 1^{m+1}/\\iota(m+1) = 1/\\iota(m+1)$ and $H(0) = 0^{m+1}/\\iota(m+1) = 0$, since $m+1 \\ge 1$; so $\\int_0^1 x^{m} = 1/\\iota(m+1)$. [step 1.2, step 3.1, L5, L6]",
      "step": "4.1",
      "inputs": [
        "step 1.2",
        "step 3.1",
    ...
    {
      "id": "ex-polynomial-integrals-by-the-ftc-step-5-1",
      "claim": "**First check, at $m = 0$.** There $p_0$ is the constant function $1$ by [L5], so $\\int_0^1 p_0 = 1\\cdot(1-0) = 1$ by [L7], while the formula gives $1/\\iota(1) = 1/1 = 1$. The two agree. [step 4.1, L5, L6, L7]",
      "step": "5.1",
      "inputs": [
        "step 4.1",
        "L5",
    ...
    {
      "id": "ex-polynomial-integrals-by-the-ftc-step-6-1",
      "claim": "**Second check, monotonicity in $m$.** By [L5], $x^{m+1} \\le x^{m}$ for every $x \\in [0,1]$, so $\\int_0^1 x^{m+1} \\le \\int_0^1 x^{m}$ by [L7]; the formula gives $1/\\iota(m+2) \\le 1/\\iota(m+1)$, which holds by [L6] and [L8]. The two agree. [step 4.1, L5, L6, L7, L8] ∎",
      "step": "6.1",
      "inputs": [
        "step 4.1",
        "L5",
  ...
  "boundaries": [
    {
      ...
      "reason": "ex-polynomial-integrals-by-the-ftc concerns stipulated functions and points; no empty family, empty sum, or empty choice domain changes its assertion."
    },
    {
      "case": "zero",
      "status": "checked",
    ...
    },
    {
      "case": "iff-forward",
      "status": "not_applicable",
      "reason": "The Statement of ex-polynomial-integrals-by-the-ftc is not a biconditional, so it has no forward iff direction."
    },
    {
      "case": "iff-reverse",
      "status": "not_applicable",
      "reason": "The Statement of ex-polynomial-integrals-by-the-ftc is not a biconditional, so it has no reverse iff direction."
    }
  ],
  "finite_smoke": [],
  "risk_review": {
...
[
  {
    "source": "ex-polynomial-integrals-by-the-ftc",
