...
- critical risk (16): 10 declared dependencies; 11 cited facts; 10 numbered proof steps; biconditional / both-direction claim; existence, choice, uniqueness, or well-definedness; boundary-sensitive language; analytic limiting/completeness language

## Target item — `lem-integrability-on-a-subinterval`

Normalized current SHA-256: `34e40205ce10cbd7d285acdbb5ded9bc6e6b7fc741129ace4b07734a67ac1430`
...
```json
{
  ...
  "rationale": "The source states that Riemann integrability on [a,b] implies integrability on every closed subinterval, with the same hypotheses and direction. The Darboux-criterion proof in the item is local.",
...
```json
{
  ...
  "derivations": [
    {
      "id": "lem-integrability-on-a-subinterval-step-1-1",
      "claim": "$g$ is bounded on $[c,d]$, since $[c,d] \\subseteq [a,b]$ and $f$ is bounded on $[a,b]$, integrability presupposing boundedness. [given, L7]",
      "step": "1.1",
      "inputs": [
        "given",
        "L7"
    ...
    {
      "id": "lem-integrability-on-a-subinterval-step-1-2",
      "claim": "Let a real $\\varepsilon > 0$ be given, and fix a partition $P$ of $[a,b]$ with $U(f,P) - L(f,P) < \\varepsilon$. [given, L1, choose]",
      "step": "1.2",
      "inputs": [
        "given",
        "L1",
    ...
    {
      "id": "lem-integrability-on-a-subinterval-step-2-1",
      "claim": "Put $P' := (P + c) + d$, a partition of $[a,b]$ refining $P$ whose point set contains $c$ and $d$. [step 1.2, L2, construct]",
      "step": "2.1",
      "inputs": [
        "step 1.2",
        "L2",
    ...
    {
      "id": "lem-integrability-on-a-subinterval-step-3-1",
      "claim": "By [L3] applied to the pair $(P, P')$, $U(f,P') - L(f,P') \\le U(f,P) - L(f,P) < \\varepsilon$. [step 1.2, step 2.1, L3]",
      "step": "3.1",
      "inputs": [
        "step 1.2",
        "step 2.1",
    ...
    {
      "id": "lem-integrability-on-a-subinterval-step-3-2",
      "claim": "Write $P' = (n',t')$ and fix the unique indices $p, q \\le n'$ with $t'_p = c$ and $t'_q = d$; then $p < q$, because $c < d$ and $t'$ is increasing on those indices. [step 2.1, L6, choose]",
      "step": "3.2",
      "inputs": [
        "step 2.1",
        "L6",
    ...
    {
      "id": "lem-integrability-on-a-subinterval-step-4-1",
      "claim": "Define $Q := (q-p,\\ s)$ by $s_i := t'_{p+i}$ for $i \\le q-p$ and $s_k := d$ for $k \\ge q-p$. Then $s_0 = t'_p = c$, $s_{q-p} = t'_q = d$, and $s_i < s_{i+1}$ for $i < q-p$ by [L6], with $q - p \\ge 1$; so $Q$ is a partition of $[c,d]$, its $i$-th subinterval is $[t'_{p+i}, t'_{p+i+1}] = I'_{p+i}$ and its $i$-th length is $\\Delta'_{p+i}$. [step 3.2, L6, constr",
      "step": "4.1",
      "inputs": [
        "step 3.2",
        "L6",
    ...
    {
      "id": "lem-integrability-on-a-subinterval-step-5-1",
      "claim": "For $i < q-p$ the $i$-th subinterval of $Q$ is $I'_{p+i} \\subseteq [c,d]$, and $g$ agrees with $f$ there, so the extreme values of $g$ on it are $M'_{p+i}$ and $m'_{p+i}$; hence $U(g,Q) - L(g,Q) = \\sum_{i<q-p}\\bigl(M'_{p+i} - m'_{p+i}\\bigr)\\Delta'_{p+i}$ by [L4] and [L5]. [step 4.1, L4, L5]",
      "step": "5.1",
      "inputs": [
        "step 4.1",
        "L4",
    ...
    {
      "id": "lem-integrability-on-a-subinterval-step-6-1",
      "claim": "Every term $\\bigl(M'_j - m'_j\\bigr)\\Delta'_j$ is nonnegative by [L4], and splitting $\\sum_{j<n'}$ first at $p$ and then at $q$ exhibits $\\sum_{i<q-p}\\bigl(M'_{p+i}-m'_{p+i}\\bigr)\\Delta'_{p+i}$ as one of the three pieces of $\\sum_{j<n'}\\bigl(M'_j-m'_j\\bigr)\\Delta'_j$, the other two being nonnegative; so the displayed sum is at most $U(f,P') - L(f,P')$. [step ",
      "step": "6.1",
      "inputs": [
        "step 5.1",
        "L4",
    ...
    {
      "id": "lem-integrability-on-a-subinterval-step-7-1",
      "claim": "Combining, $U(g,Q) - L(g,Q) \\le U(f,P') - L(f,P') < \\varepsilon$. [step 3.1, step 5.1, step 6.1]",
      "step": "7.1",
      "inputs": [
        "step 3.1",
        "step 5.1",
    ...
    {
      "id": "lem-integrability-on-a-subinterval-step-8-1",
      "claim": "Since $\\varepsilon > 0$ was arbitrary and $g$ is bounded, [L1] applies on $[c,d]$ and $g$ is integrable there. [step 1.1, step 1.2, step 7.1, L1] ∎",
      "step": "8.1",
      "inputs": [
        "step 1.1",
        "step 1.2",
  ...
  "boundaries": [
    {
      ...
      "reason": "lem-integrability-on-a-subinterval concerns stipulated functions and points; no empty family, empty sum, or empty choice domain changes its assertion."
    },
    {
      "case": "zero",
      "status": "not_applicable",
      "reason": "lem-integrability-on-a-subinterval has no counting, exponent, or value parameter whose zero case changes the proof."
    },
    {
      "case": "one",
      "status": "not_applicable",
      "reason": "lem-integrability-on-a-subinterval has no counting, exponent, or unit parameter whose one case changes the proof."
    },
    {
      "case": "degenerate",
      "status": "checked",
    ...
    },
    {
      "case": "iff-forward",
      "status": "not_applicable",
      "reason": "The Statement of lem-integrability-on-a-subinterval is not a biconditional, so it has no forward iff direction."
    },
    {
      "case": "iff-reverse",
      "status": "not_applicable",
      "reason": "The Statement of lem-integrability-on-a-subinterval is not a biconditional, so it has no reverse iff direction."
    }
  ],
  "finite_smoke": [],
  "risk_review": {
