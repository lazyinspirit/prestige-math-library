...
- high risk (7): 8 declared dependencies; 6 cited facts; induction, recursion, or minimality

## Target item — `lem-taylor-polynomial-derivatives`

Normalized current SHA-256: `bdbe1786d0ba97b69a990cc6a4e7924b9415d535424b118b9785ffc0643a318e`
...
```json
{
  ...
  "rationale": "The source states that the Taylor polynomial matches every derivative of f through order n at the centre. This is the same claim and range of orders; the termwise finite-sum calculation is local.",
...
```json
{
  ...
  "derivations": [
    {
      "id": "lem-taylor-polynomial-derivatives-step-1-1",
      "claim": "At $r=0$ the formula is the definition. [base, given]",
      "step": "1.1",
      "inputs": [
        "base",
        "given"
    ...
    {
      "id": "lem-taylor-polynomial-derivatives-step-1-2",
      "claim": "Assuming the formula at $r<n$, differentiate termwise. The term indexed $j$ acquires $\\iota(j-r)$, which cancels $\\iota((j-r)!)$ to $\\iota((j-r-1)!)$; the $j=r$ constant term disappears. This is the formula at $r+1$. [ih, L1, L2, algebra]",
      "step": "1.2",
      "inputs": [
        "ih",
        "L1",
    ...
    {
      "id": "lem-taylor-polynomial-derivatives-step-2-1",
      "claim": "At $x=a$, only the $j=r$ term survives and equals $f^{(r)}(a)$. Subtraction from $f^{(r)}(a)$ gives the remainder assertion. [step 1.2, L1, algebra]",
      "step": "2.1",
      "inputs": [
        "step 1.2",
        "L1",
    ...
    {
      "id": "lem-taylor-polynomial-derivatives-step-3-1",
      "claim": "The formula and both consequences hold through order $n$. [step 1.1, step 2.1, discharge-induction] ∎",
      "step": "3.1",
      "inputs": [
        "step 1.1",
        "step 2.1",
  ...
  "boundaries": [
    {
      ...
      "reason": "lem-taylor-polynomial-derivatives concerns stipulated functions and points; no empty family, empty sum, or empty choice domain changes its assertion."
    },
    {
      "case": "zero",
      "status": "not_applicable",
      "reason": "lem-taylor-polynomial-derivatives has no counting, exponent, or value parameter whose zero case changes the proof."
    },
    {
      "case": "one",
      "status": "not_applicable",
      "reason": "lem-taylor-polynomial-derivatives has no counting, exponent, or unit parameter whose one case changes the proof."
    },
    {
      "case": "degenerate",
      "status": "not_applicable",
      "reason": "lem-taylor-polynomial-derivatives has no interval-length or denominator parameter that may collapse degenerately."
    },
    {
      "case": "endpoints",
      "status": "not_applicable",
      "reason": "lem-taylor-polynomial-derivatives contains no interval-endpoint assertion requiring a separate endpoint case."
    },
    {
    ...
    },
    {
      "case": "iff-forward",
      "status": "not_applicable",
      "reason": "The Statement of lem-taylor-polynomial-derivatives is not a biconditional, so it has no forward iff direction."
    },
    {
      "case": "iff-reverse",
      "status": "not_applicable",
      "reason": "The Statement of lem-taylor-polynomial-derivatives is not a biconditional, so it has no reverse iff direction."
    }
  ],
  "finite_smoke": [],
  "risk_review": {
