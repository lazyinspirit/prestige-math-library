...
## Target item — `cor-taylor-remainder-bound`

Normalized current SHA-256: `cf990a4a35d7d1fca0b4fc477c0eae1403a3c9d31bb040f99010c24f5245ef7a`
...
```json
{
  ...
  "rationale": "Grinshpan's university Real Analysis note states the Lagrange remainder formula under the corresponding derivative hypotheses; taking the displayed derivative value at the intermediate point under the uniform bound gives exactly M|x-a|^{n+1}/(n+1)!. The repository's proof is a direct local application of its preceding remainder theorem.",
...
```json
{
  "citations": [
    {
      "fact": "L1",
      "source": "cor-taylor-lagrange-and-cauchy-remainders",
      "source_section": "Statement",
      "quote": "Under the hypotheses of [[thm-taylor-schlomilch-roche-remainder]], there are points $\\xi_L,\\xi_C$ between $a$ and $x$ such that\n$$R_{n,a}f(x)=\\frac{f^{(n+1)}(\\xi_L)}{\\iota((n+1)!)}(x-a)^{n+1}$$\nand\n$$R_{n,a}f(x)=\\frac{f^{(n+1)}(\\xi_C)}{\\iota(n!)}(x-\\xi_C)^n(x-a).$$",
      "uses": [
        "1.1"
      ]
    },
  ...
  "derivations": [
    {
      "id": "cor-taylor-remainder-bound-step-1-1",
      "claim": "If $x=a$, then $R_{n,a}f(a)=0$, so the estimate is immediate. If $x\\ne a$, [L1] gives $$R_{n,a}f(x)=\\frac{f^{(n+1)}(\\xi)}{\\iota((n+1)!)}(x-a)^{n+1}$$ for some point $\\xi$ strictly between $a$ and $x$. [L1]",
      "step": "1.1",
      "inputs": [
        "L1"
      ]
    ...
    {
      "id": "cor-taylor-remainder-bound-step-2-1",
      "claim": "In the case $x\\ne a$, take absolute values in step 1.1, use $|f^{(n+1)}(\\xi)|\\le M$, and divide by the positive factorial. Together with the case $x=a$, this proves the estimate. [step 1.1, L2, algebra] ∎",
      "step": "2.1",
      "inputs": [
        "step 1.1",
  ...
  "boundaries": [
    {
      ...
      "reason": "cor-taylor-remainder-bound concerns stipulated functions and points; no empty family, empty sum, or empty choice domain changes its assertion."
    },
    {
      "case": "zero",
      "status": "not_applicable",
      "reason": "cor-taylor-remainder-bound has no counting, exponent, or value parameter whose zero case changes the proof."
    },
    {
      "case": "one",
      "status": "not_applicable",
      "reason": "cor-taylor-remainder-bound has no counting, exponent, or unit parameter whose one case changes the proof."
    },
    {
      "case": "degenerate",
      "status": "not_applicable",
      "reason": "cor-taylor-remainder-bound has no interval-length or denominator parameter that may collapse degenerately."
    },
    {
      "case": "endpoints",
      "status": "not_applicable",
      "reason": "cor-taylor-remainder-bound contains no interval-endpoint assertion requiring a separate endpoint case."
    },
    {
    ...
    },
    {
      "case": "iff-forward",
      "status": "not_applicable",
      "reason": "The Statement of cor-taylor-remainder-bound is not a biconditional, so it has no forward iff direction."
    },
    {
      "case": "iff-reverse",
      "status": "not_applicable",
      "reason": "The Statement of cor-taylor-remainder-bound is not a biconditional, so it has no reverse iff direction."
    }
  ],
  "finite_smoke": [],
  "risk_review": {
