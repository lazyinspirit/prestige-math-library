...
- critical risk (9): 10 declared dependencies; 8 cited facts; boundary-sensitive language; quotient or equivalence-class construction

## Target item — `thm-taylor-schlomilch-roche-remainder`

Normalized current SHA-256: `0e54c0599dd9670fc5fb62faed038ce45c9b28f712fe9a53e4c2b332d9c108b8`
...
```json
{
  ...
  "rationale": "The source gives the Schlomilch/Roche remainder formula with the same parameter and interpolating point. The item restricts the parameter to an integer between 1 and n+1, states relative-interval regularity explicitly, and uses the library's factorial and integer-power conventions.",
...
```json
{
  ...
  "derivations": [
    {
      "id": "thm-taylor-schlomilch-roche-remainder-step-1-1",
      "claim": "Define $\\Phi(t):=f(x)-\\sum_{j=0}^{n}f^{(j)}(t)(x-t)^j/\\iota(j!)$ and $\\Psi(t):=(x-t)^p$. Telescoping after differentiating the sum gives $\\Phi'(t)=-f^{(n+1)}(t)(x-t)^n/\\iota(n!)$, while $\\Psi'(t)=-\\iota(p)(x-t)^{p-1}$. [L1, L3, algebra]",
      "step": "1.1",
      "inputs": [
        "L1",
        "L3",
    ...
    {
      "id": "thm-taylor-schlomilch-roche-remainder-step-1-2",
      "claim": "We have $\\Phi(a)=R_{n,a}f(x)$, $\\Phi(x)=0$, $\\Psi(a)=(x-a)^p$, and $\\Psi(x)=0$. Also $\\Psi'\\ne0$ on $(a,x)$, because $p\\ge1$, $\\iota(p)>0$, and $x-t>0$. [given, L1, L4, algebra]",
      "step": "1.2",
      "inputs": [
        "given",
        "L1",
    ...
    {
      "id": "thm-taylor-schlomilch-roche-remainder-step-2-1",
      "claim": "Apply [L2] to $\\Phi,\\Psi$. For some $\\xi\\in(a,x)$, $\\Phi(a)/\\Psi(a)=\\Phi'(\\xi)/\\Psi'(\\xi)=f^{(n+1)}(\\xi)(x-\\xi)^{n+1-p}/(\\iota(p)\\iota(n!))$. [step 1.1, step 1.2, L2]",
      "step": "2.1",
      "inputs": [
        "step 1.1",
        "step 1.2",
    ...
    {
      "id": "thm-taylor-schlomilch-roche-remainder-step-3-1",
      "claim": "Multiply by $(x-a)^p$. If $x<a$, interchange the interval endpoints; the same algebraic identity results. [step 2.1, algebra] ∎",
      "step": "3.1",
      "inputs": [
        "step 2.1",
        "algebra"
  ...
  "boundaries": [
    {
      ...
      "reason": "thm-taylor-schlomilch-roche-remainder concerns stipulated functions and points; no empty family, empty sum, or empty choice domain changes its assertion."
    },
    {
      "case": "zero",
      "status": "not_applicable",
      "reason": "thm-taylor-schlomilch-roche-remainder has no counting, exponent, or value parameter whose zero case changes the proof."
    },
    {
      "case": "one",
      "status": "not_applicable",
      "reason": "thm-taylor-schlomilch-roche-remainder has no counting, exponent, or unit parameter whose one case changes the proof."
    },
    {
      "case": "degenerate",
      "status": "checked",
    ...
    },
    {
      "case": "endpoints",
      "status": "checked",
      "evidence": "Statement specifies the closed or oriented interval convention; step 3.1 handles the endpoint data used by the argument."
    },
    {
    ...
    },
    {
      "case": "iff-forward",
      "status": "not_applicable",
      "reason": "The Statement of thm-taylor-schlomilch-roche-remainder is not a biconditional, so it has no forward iff direction."
    },
    {
      "case": "iff-reverse",
      "status": "not_applicable",
      "reason": "The Statement of thm-taylor-schlomilch-roche-remainder is not a biconditional, so it has no reverse iff direction."
    }
  ],
  "finite_smoke": [],
  "risk_review": {
...
[
  {
    "source": "thm-taylor-schlomilch-roche-remainder",
