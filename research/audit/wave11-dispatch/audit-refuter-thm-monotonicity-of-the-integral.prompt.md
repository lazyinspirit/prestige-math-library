...
- critical risk (9): 10 declared dependencies; 9 cited facts; boundary-sensitive language; analytic limiting/completeness language

## Target item — `thm-monotonicity-of-the-integral`

Normalized current SHA-256: `c09760939f726571eff0f6ae229150f2f03a61b6ef16e4138c6b372640bba237`
...
```json
{
  ...
  "derivations": [
    {
      "id": "thm-monotonicity-of-the-integral-step-1-1",
      "claim": "**Claim 1.** Under [A1] the constant $0$ is a lower bound of $f$ on $[a,b]$, so [L1] applies with $m' := 0$ and gives $\\underline{\\int_a^b} f \\ge 0$. [A1, L1]",
      "step": "1.1",
      "inputs": [
        "A1",
        "L1"
    ...
    {
      "id": "thm-monotonicity-of-the-integral-step-1-2",
      "claim": "**Claim 2.** Under [A2] the function $h := g - f$ satisfies $h(x) \\ge 0$ for every $x \\in [a,b]$, and $h$ is integrable with $\\int_a^b h = \\int_a^b g - \\int_a^b f$ by [L3]. [A2, L3, L4]",
      "step": "1.2",
      "inputs": [
        "A2",
        "L3",
    ...
    {
      "id": "thm-monotonicity-of-the-integral-step-2-1",
      "claim": "Since $f$ is integrable, $\\int_a^b f = \\underline{\\int_a^b} f \\ge 0$ by [L2]. [step 1.1, L2]",
      "step": "2.1",
      "inputs": [
        "step 1.1",
        "L2"
    ...
    {
      "id": "thm-monotonicity-of-the-integral-step-3-1",
      "claim": "By claim 1 applied to $h$, $\\int_a^b g - \\int_a^b f \\ge 0$, that is $\\int_a^b f \\le \\int_a^b g$. [step 2.1, step 1.2, L4]",
      "step": "3.1",
      "inputs": [
        "step 2.1",
        "step 1.2",
    ...
    {
      "id": "thm-monotonicity-of-the-integral-step-4-1",
      "claim": "**Claim 3.** Under [A3], [L1] applied to $f$ with $m' := m$ and $M' := M$ gives $m(b-a) \\le \\underline{\\int_a^b} f$ and $\\overline{\\int_a^b} f \\le M(b-a)$, and both integrals equal $\\int_a^b f$ by [L2]. [A3, L1, L2] ∎",
      "step": "4.1",
      "inputs": [
        "A3",
        "L1",
  ...
  "boundaries": [
    {
      ...
      "reason": "thm-monotonicity-of-the-integral concerns stipulated functions and points; no empty family, empty sum, or empty choice domain changes its assertion."
    },
    {
      "case": "zero",
      "status": "checked",
    ...
    },
    {
      "case": "one",
      "status": "not_applicable",
      "reason": "thm-monotonicity-of-the-integral has no counting, exponent, or unit parameter whose one case changes the proof."
    },
    {
      "case": "degenerate",
      "status": "checked",
    ...
    },
    {
      "case": "iff-forward",
      "status": "not_applicable",
      "reason": "The Statement of thm-monotonicity-of-the-integral is not a biconditional, so it has no forward iff direction."
    },
    {
      "case": "iff-reverse",
      "status": "not_applicable",
      "reason": "The Statement of thm-monotonicity-of-the-integral is not a biconditional, so it has no reverse iff direction."
    }
  ],
  "finite_smoke": [],
  "risk_review": {
