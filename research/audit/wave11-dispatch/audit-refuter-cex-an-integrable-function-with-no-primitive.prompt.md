...
- critical risk (16): 23 declared dependencies; 22 cited facts; 11 numbered proof steps; biconditional / both-direction claim; boundary-sensitive language; quotient or equivalence-class construction; analytic limiting/completeness language
- top-20 manifest-edge consumer (23 outgoing relationships)

## Target item — `cex-an-integrable-function-with-no-primitive`

Normalized current SHA-256: `1e2c0cc10985ccca6539657cf8f0b916612a7e8f05d8d841c94998749afcd12e`
...
```json
{
  ...
  "rationale": "The sources give the sign function's integral function as an absolute-value function and Darboux's theorem that derivatives cannot jump. The item packages these standard facts as an explicit refutation of the claim that every Riemann-integrable function has a primitive.",
...
```json
{
  ...
  "derivations": [
    {
      "id": "cex-an-integrable-function-with-no-primitive-step-1-1",
      "claim": "$s$ is bounded, with $|s| \\le 1$, and continuous at every point of $[-1,1]$ other than $0$: near a point $x \\ne 0$ it is locally constant. So $s$ is integrable on $[-1,1]$, and likewise on $[-1,0]$ and on $[0,1]$, by [L1]. [given, L1]",
      "step": "1.1",
      "inputs": [
        "given",
        "L1"
    ...
    {
      "id": "cex-an-integrable-function-with-no-primitive-step-1-2",
      "claim": "**Assume [A1].** Then $G' = s$ everywhere on $[-1,1]$. [assume-contra, A1]",
      "step": "1.2",
      "inputs": [
        "assume-contra",
        "A1"
    ...
    {
      "id": "cex-an-integrable-function-with-no-primitive-step-2-1",
      "claim": "On $[-1,0]$ the function $s$ agrees with the constant $-1$ except at the single point $0$, so $\\int_{-1}^{0} s = -1\\cdot(0-(-1)) = -1$ by [L2]; on $[0,1]$ it agrees with the constant $1$ except at $0$, so $\\int_0^1 s = 1$. [step 1.1, L2]",
      "step": "2.1",
      "inputs": [
        "step 1.1",
        "L2"
    ...
    {
      "id": "cex-an-integrable-function-with-no-primitive-step-2-2",
      "claim": "On $I_{+} := [0,1]$ the function $u_{+}(x) := G(x) - x$ is continuous, by [L5], and differentiable at every interior point $x \\in (0,1)$ with $u_{+}'(x) = s(x) - 1 = 0$; so $u_{+}$ is constant on $[0,1]$ by [L4], say $G(x) = x + c_1$ there. [step 1.2, L4, L5, choose]",
      "step": "2.2",
      "inputs": [
        "step 1.2",
        "L4",
    ...
    {
      "id": "cex-an-integrable-function-with-no-primitive-step-2-3",
      "claim": "On $I_{-} := [-1,0]$ the function $u_{-}(x) := G(x) + x$ is continuous and differentiable at every $x \\in (-1,0)$ with $u_{-}'(x) = s(x)+1 = 0$; so $G(x) = -x + c_2$ on $[-1,0]$. [step 1.2, L4, L5, choose]",
      "step": "2.3",
      "inputs": [
        "step 1.2",
        "L4",
    ...
    {
      "id": "cex-an-integrable-function-with-no-primitive-step-3-1",
      "claim": "By [L3], $\\int_{-1}^{1} s = \\int_{-1}^{0} s + \\int_0^1 s = -1 + 1 = 0$. [step 2.1, L3]",
      "step": "3.1",
      "inputs": [
        "step 2.1",
        "L3"
    ...
    {
      "id": "cex-an-integrable-function-with-no-primitive-step-3-2",
      "claim": "By [L3] again, $F(x) = \\int_{-1}^{x} s$ equals $-1 + x$ for $x \\in [0,1]$ and equals $-(x-(-1)) = -x-1$ for $x \\in [-1,0]$, by [L2] applied on the relevant piece; in both cases $F(x) = |x| - 1$ by [L7]. [step 2.1, L2, L3, L7]",
      "step": "3.2",
      "inputs": [
        "step 2.1",
        "L2",
    ...
    {
      "id": "cex-an-integrable-function-with-no-primitive-step-3-3",
      "claim": "Evaluating both formulas at $x = 0$ gives $c_1 = G(0) = c_2$; write $c$ for the common value, so $G(x) = |x| + c$ for every $x \\in [-1,1]$ by [L7]. [step 2.2, step 2.3, L7]",
      "step": "3.3",
      "inputs": [
        "step 2.2",
        "step 2.3",
    ...
    {
      "id": "cex-an-integrable-function-with-no-primitive-step-4-1",
      "claim": "The difference quotient of $G$ at $0$ is $x \\mapsto (G(x)-G(0))/x = |x|/x$, which equals $1$ for $x > 0$ and $-1$ for $x < 0$ by [L7]; so its right-hand limit at $0$ is $1$ and its left-hand limit is $-1$. [step 3.3, L7]",
      "step": "4.1",
      "inputs": [
        "step 3.3",
        "L7"
    ...
    {
      "id": "cex-an-integrable-function-with-no-primitive-step-5-1",
      "claim": "By [L6] the two-sided limit of that quotient at $0$ does not exist, so $G$ is not differentiable at $0$; this contradicts [A1]. [step 4.1, A1, L6]",
      "step": "5.1",
      "inputs": [
        "step 4.1",
        "A1",
    ...
    {
      "id": "cex-an-integrable-function-with-no-primitive-step-6-1",
      "claim": "Hence no such $G$ exists: $s$ is integrable on $[-1,1]$ by step 1.1 and has no primitive there, so the claim is false. [step 1.1, step 3.1, step 5.1, discharge-contradiction] ∎",
      "step": "6.1",
      "inputs": [
        "step 1.1",
        "step 3.1",
  ...
  "boundaries": [
    {
      ...
      "reason": "cex-an-integrable-function-with-no-primitive concerns stipulated functions and points; no empty family, empty sum, or empty choice domain changes its assertion."
    },
    {
      "case": "zero",
      "status": "not_applicable",
      "reason": "cex-an-integrable-function-with-no-primitive has no counting, exponent, or value parameter whose zero case changes the proof."
    },
    {
      "case": "one",
      "status": "checked",
    ...
    },
    {
      ...
      "status": "checked",
      "evidence": "Step 2.2 establishes or explicitly fixes the object before the proof chooses or extremizes it."
    },
    {
      "case": "iff-forward",
      "status": "not_applicable",
      "reason": "The Statement of cex-an-integrable-function-with-no-primitive is not a biconditional, so it has no forward iff direction."
    },
    {
      "case": "iff-reverse",
      "status": "not_applicable",
      "reason": "The Statement of cex-an-integrable-function-with-no-primitive is not a biconditional, so it has no reverse iff direction."
    }
  ],
  "finite_smoke": [],
  "risk_review": {
