...
- critical risk (11): 7 declared dependencies; 6 cited facts; boundary-sensitive language; quotient or equivalence-class construction; analytic limiting/completeness language

## Target item — `thm-lhopital-infinity-over-infinity`

Normalized current SHA-256: `ee5bee65be38af173c7df7274440362626b4ce40da54a98b6dc08f040d89cb24`
...
```json
{
  ...
  "rationale": "The sources give the infinity-over-infinity rule for finite and infinite endpoints. The item consolidates endpoint modes, extended-real derivative limits, absolute divergence, and eventual fixed signs in the library's conventions. Its Cauchy-mean-value proof is local.",
...
```json
{
  ...
  "derivations": [
    {
      "id": "thm-lhopital-infinity-over-infinity-step-1-1",
      "claim": "Fix a base point $a$ inside the domain. For variable $x$ farther toward the limiting end, [L1] gives $\\frac{f(x)-f(a)}{g(x)-g(a)}=\\frac{f'(\\xi_x)}{g'(\\xi_x)}$, where $\\xi_x$ lies between $a$ and $x$. [given, L1]",
      "step": "1.1",
      "inputs": [
        "given",
        "L1"
    ...
    {
      "id": "thm-lhopital-infinity-over-infinity-step-2-1",
      "claim": "First choose $a$ sufficiently far toward the end that the derivative quotient is as close to $L$ as required throughout the remaining tail. Then the quotient of increments has the same bound for every later $x$. [step 1.1, L2, choose]",
      "step": "2.1",
      "inputs": [
        "step 1.1",
        "L2",
    ...
    {
      "id": "thm-lhopital-infinity-over-infinity-step-3-1",
      "claim": "Since $|g(x)|\\to\\infty$, $g(a)/g(x)\\to0$; since the increment quotient is bounded in the finite-$L$ case, the identity $\\frac{f(x)}{g(x)}=\\frac{f(x)-f(a)}{g(x)-g(a)}\\left(1-\\frac{g(a)}{g(x)}\\right)+\\frac{f(a)}{g(x)}$ gives the finite conclusion. For $L=\\pm\\infty$, choose the derivative-quotient lower or upper bound first and then make the two fixed-base term",
      "step": "3.1",
      "inputs": [
        "step 2.1",
        "L2",
    ...
    {
      "id": "thm-lhopital-infinity-over-infinity-step-4-1",
      "claim": "Thus the quotient has limit $L$ in every stated mode. [step 3.1] ∎",
      "step": "4.1",
      "inputs": [
        "step 3.1"
      ]
  ...
  "boundaries": [
    {
      ...
      "reason": "thm-lhopital-infinity-over-infinity concerns stipulated functions and points; no empty family, empty sum, or empty choice domain changes its assertion."
    },
    {
      "case": "zero",
      "status": "not_applicable",
      "reason": "thm-lhopital-infinity-over-infinity has no counting, exponent, or value parameter whose zero case changes the proof."
    },
    {
      "case": "one",
      "status": "checked",
    ...
    },
    {
      "case": "degenerate",
      "status": "not_applicable",
      "reason": "thm-lhopital-infinity-over-infinity has no interval-length or denominator parameter that may collapse degenerately."
    },
    {
      "case": "endpoints",
      "status": "not_applicable",
      "reason": "thm-lhopital-infinity-over-infinity contains no interval-endpoint assertion requiring a separate endpoint case."
    },
    {
    ...
    },
    {
      "case": "iff-forward",
      "status": "not_applicable",
      "reason": "The Statement of thm-lhopital-infinity-over-infinity is not a biconditional, so it has no forward iff direction."
    },
    {
      "case": "iff-reverse",
      "status": "not_applicable",
      "reason": "The Statement of thm-lhopital-infinity-over-infinity is not a biconditional, so it has no reverse iff direction."
    }
  ],
  "finite_smoke": [],
  "risk_review": {
...
[
  {
    "source": "thm-lhopital-infinity-over-infinity",
