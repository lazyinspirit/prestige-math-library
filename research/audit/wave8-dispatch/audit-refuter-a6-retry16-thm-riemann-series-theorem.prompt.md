...
Your entire response must be at most 450 words.  Reason internally; do not
reproduce, summarize, or quote the supplied item or dependencies except for the
single exact claim needed in a finding.  A clean result should use only the
four required lines below.  This length bound changes no reading obligation:
you must still inspect the complete target and every supplied dependency.
