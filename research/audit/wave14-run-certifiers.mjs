...
const index = JSON.parse(readFileSync(join(audit, "wave14-certifier-index.json"), "utf8"));
const dispatchDir = join(audit, "wave14-dispatch");
const brief = "research/audit/wave14-certifier-brief.md";
const concurrency = 6;
const tasks = index.targets.map((entry) => ({ label: `a6r2-${entry.id}`, path: entry.task }));

const passed = ({ label }) => {
  const path = join(dispatchDir, `certifier-${label}.result.json`);
  if (!existsSync(path)) return false;
  try { const row = JSON.parse(readFileSync(path, "utf8")); return row.ok === true && row.model === "gpt-5.6-terra" && /VERDICT:\s*CERTIFIED/.test(row.tail ?? ""); }
  catch { return false; }
};
const pending = tasks.filter((task) => !passed(task));
console.log(`wave14 r2 certifiers: ${tasks.length} prepared; ${tasks.length - pending.length} certified; ${pending.length} pending; concurrency ${concurrency}`);
let cursor = 0;
const failures = [];
const runOne = (task) => new Promise((resolve) => {
  const started = Date.now();
  const child = spawn(process.execPath, ["tools/dispatch.mjs", "--role", "certifier", "--brief", brief, "--label", task.label, "--run", "wave14", "--task", task.path, "--timeout", "7200", "--json"], { cwd: root, stdio: ["ignore", "pipe", "pipe"] });
  let stdout = "", stderr = "";
...
console.log(`wave14 r2 certifiers: ${tasks.length - failures.length}/${tasks.length} dispatches passed; ${failures.length} failed`);
