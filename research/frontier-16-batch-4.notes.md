...
| Hu–Li–Li–Yan, *Calculus: Rigor, Concision, Clarity* — `https://www.math.hkust.edu.hk/~majhu/Math024/Notes.pdf` | §7.1.1, Example 7.1.7, printed pp. 189–190 | sine-period arc length and the ellipse/elliptic-integral reduction |
| Coffman–Pan–Zhang, “Continuous solutions of nonlinear Cauchy–Riemann equations and pseudoholomorphic curves in normal coordinates” — `https://arxiv.org/pdf/1410.1582` | Remark 3.6, printed p. 12 | $x^2\sin(1/x^2)$ differentiable with unbounded derivative |
| Terence Tao, *An Epsilon of Room: Pages from Year Three of a Mathematical Blog*, preliminary version — `https://terrytao.files.wordpress.com/2010/02/epsilon.pdf` | Example 1.6.28, printed p. 88 | tangent homeomorphism; boundedness and completeness not topological |
...
| Thomas Lam, *21-236 Recitation Notes* — `https://cims.nyu.edu/~tjl8195/21_236_Recitation_Notes_Final.pdf` | §4.3, Definition 4.2 and the following example paragraph, printed p. 25 | rectifiability as finite length; continuous compact-interval curves of infinite length; the graph of $x\sin(1/x)$ as an example |

### Source-scouting disposition (2026-08-20)

- **Same-source recovery — Coffman–Pan–Zhang.** The author-hosted URL did not yield a full body through the scouting channel. The arXiv PDF is the same paper and carries Remark 3.6, with the $x^2\sin(1/x^2)$ statement, at printed p. 12. Coverage now uses `https://arxiv.org/pdf/1410.1582` and preserves the author-hosted URL as `original_url`; the harvested row and locator remain faithful.
- **Re-harvest — Tao.** The CiteSeerX endpoint did not yield a full body. Tao's own `https://terrytao.files.wordpress.com/2010/02/epsilon.pdf` is the complete preliminary *An Epsilon of Room: Pages from Year Three of a Mathematical Blog*. Because its pagination differs from the discarded citation, I treated it as a re-harvest: Example 1.6.28 at printed p. 88 contains both recorded claims about the tangent homeomorphism and the failure of boundedness and completeness to be topological properties. Coverage preserves the CiteSeerX endpoint as `original_url`.
- **Re-harvest — Lebl II.** The run's liveness gate records HTTP 404 and no archive recovery for the obsolete draft-v2.0 URL. The current author-hosted version 6.3 is complete. I read §11.8.4 at printed pp. 200–201 and rewrote the harvested heading to the current edition's symmetric finite exponential sum and Dirichlet-kernel formula. Coverage preserves the dead draft URL as `original_url`.
- **Re-harvest — HKUST calculus text.** The run's liveness gate records HTTP 403 and no archive recovery for `Notes_2007.pdf`. The authors' complete current `Math024/Notes.pdf` carries the same development in §7.1.1, Example 7.1.7, at printed pp. 189–190. I reread that example and rewrote its `contents` rows to its ellipse-perimeter, second-kind elliptic-integral, rescaled-sine, and non-elementarity claims. Coverage preserves the blocked 2007 URL as `original_url`.
- **Re-harvest — rectifiable graph.** Kazdan's complete Problem Set 10 falls below `source-fetch-check.mjs`'s minimum PDF length even though Problem 2 contains the intended claim. The replacement is Thomas Lam's complete `21-236 Recitation Notes`, whose browser parse is well above that floor. I read the narrow §4.3 range at printed p. 25: Definition 4.2 defines rectifiability by finite length, and the immediately following paragraph states that a continuous compact-interval curve may have infinite length, naming the graph of $x\sin(1/x)$ as an example. Coverage re-harvests that range and preserves Kazdan's URL as `original_url`.
- **Removed redundant short source — harmonic sine series.** Coskunuzer's complete Problem Set 8 also falls below the verifier's minimum PDF length. Its Problem 7 supplied only local uniform convergence away from the endpoints; the retained full-length NCSU course text already carries the stronger scaffolded result in Example 5.18(b), namely pointwise but nonuniform convergence of $\sum\sin(nx)/n$. The UTD row was removed instead of attributing its heading to a different document.
- **Mechanical dead end.** The required stamping command was rerun after these changes. It returned `EAI_AGAIN` for all 12 retained source hostnames, including the browser-verified arXiv, author WordPress, current Lebl, current HKUST, UC Davis, Minnesota, ANU, Auburn, NCSU, and NYU PDFs. Non-escalated `curl --head` checks independently returned `curl: (6) Could not resolve host`, confirming that shell DNS is unavailable across hosts rather than that the replacement documents returned bad bodies. Consequently `source-fetch-check.mjs` cannot create or validate any `fetch_verified` stamp in this runtime. No escalation-free shell-network path is available; the exact remaining blocker is a rerun of the same `--stamp` command in a runtime whose Node process has DNS access. The final command result in this scouting round was `0/12 source(s) fetch-verified, 12 FAILED`.
...
| `ex-sine-period-arc-length-as-a-complete-elliptic-integral` | literature-derived | ai-altered | HKUST Example 7.1.7 and Hall Example 3.1.2; local proof derives the exact special value. |
...
| `ex-sine-harmonic-series-pointwise-not-uniform` | literature-derived | ai-altered | NCSU Example 5.18; proof uses Dirichlet cancellation and an explicit uniform-Cauchy obstruction. |
| `fs-a-continuous-function-on-a-compact-interval-has-a-rectifiable-graph` | ai-altered | ai-altered | Universal false claim distilled from Lam §4.3 and the published classical witness; proof is rebuilt locally to respect the B-leaf rule. |
...
- `coverage-checklist` exits successfully with zero errors and zero warnings after the source re-harvest.
