...
- Work only on dispatched `covers` batches, one item at a time. Find their owning group in `research/phase-2-catchup-24-alpha-groups.json`; the dispatch label may include batch numbers. Other batches in that group may still be authoring. Read each assigned manifest, `research/phase-2-catchup-24-step6-scope-16.json`, current authored items and A/B pages, contracts, and dependencies needed to verify the arguments. Do not edit unfinished or unassigned batches. Record dependencies on changing frontier suppliers for the final cross-batch audit; never assume a missing proof.
