...
Reconcile the dependency records under `briefs/tasks/frontier-dependency-ledger.md`
against every repair before returning current verdicts.
...
If `research/phase-2-wave-1-owner-published-repairs.json` exists, read it and its
receipt. Only the designated review group closes those named, owner-authorized
early repairs. Independently audit the items and affected interfaces; after
acceptance record delegated verification and ledger dispositions as specified
in the receipt. A judge pass alone is not audit closure. This grants no edits
to other published content and waives no scaffold requirement.

Apply the source-resolution standard in `briefs/tasks/alpha-group.md`. Judge
alternate proofs on soundness and adequate dependencies, not availability of
the original. Respect documented drops; retain unresolved owner escalations
and identify exact mathematical gaps rather than demanding the original again.

Refresh and check `research/phase-2-wave-1-alpha-<your-group>-scope-decisions.json`
with `tools/scope-decisions.mjs`. Resolve every new pending row from current
evidence before the check.

Write `research/phase-2-wave-1-alpha-<your-group>-recheck.md` with one disposition per
