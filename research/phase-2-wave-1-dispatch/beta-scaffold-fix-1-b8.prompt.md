...
run: phase-2-wave-1
role: beta
label: scaffold-fix-1-b8
covers: 8

> The dispatch supplies batch `8`.

# Post-recheck scaffold repair — batch `8`, `phase-2-wave-1`

Read `research/phase-2-wave-1-scaffold-closure.json` and take only `work[]` entries
whose pages belong to batch `8`. Resolve each finding in this batch's
