...
`research/phase-2-wave-1-step8-owner-prerequisite-repairs.jsonl` with
...
# Final Adjudicator queue — phase-2-wave-1, group g, round 1

This is the exact queue frozen in `research/phase-2-wave-1-step8-fa-g-round-1.json`. It contains 2 item(s).
Work in the numbered order below. Do not substantively review the next item until the recorder accepts the current one.

## 1. `def-qid-restricted-blockade-with-empty-blocks` (run)

1. Read `items/def-qid-restricted-blockade-with-empty-blocks.md`, its cited dependencies, pair/page context, proof contract, judge and Alpha evidence, and this group's conventions.
...
3. Write concrete evidence to `research/phase-2-wave-1-step8-fa-g-1-def-qid-restricted-blockade-with-empty-blocks.md`, including exact source URLs and what they support, or explain why the mathematics was familiar.
...
```bash
node tools/step8-terminal-resolution.mjs record --run phase-2-wave-1 --id def-qid-restricted-blockade-with-empty-blocks --resolved-by final-adjudicator --group g --queue research/phase-2-wave-1-step8-fa-g-round-1.json --state-dir /home/lazyinspirit/Projects/prestige-math-library/.autopilot/phase-2-wave-1 --disposition accepted-after-review --source-status verified --basis-file research/phase-2-wave-1-step8-fa-g-1-def-qid-restricted-blockade-with-empty-blocks.md
node tools/step8-terminal-resolution.mjs record --run phase-2-wave-1 --id def-qid-restricted-blockade-with-empty-blocks --resolved-by final-adjudicator --group g --queue research/phase-2-wave-1-step8-fa-g-round-1.json --state-dir /home/lazyinspirit/Projects/prestige-math-library/.autopilot/phase-2-wave-1 --disposition repaired --source-status verified --basis-file research/phase-2-wave-1-step8-fa-g-1-def-qid-restricted-blockade-with-empty-blocks.md
```
...
## 2. `lem-qid-logarithmic-and-constant-divisibility` (run)

1. Read `items/lem-qid-logarithmic-and-constant-divisibility.md`, its cited dependencies, pair/page context, proof contract, judge and Alpha evidence, and this group's conventions.
2. Independently adjudicate the Terra rejudge rejection and decide whether the current Sol repair is correct. If unfamiliar or uncertain, use web search and verify against authoritative sources.
3. Write concrete evidence to `research/phase-2-wave-1-step8-fa-g-2-lem-qid-logarithmic-and-constant-divisibility.md`, including exact source URLs and what they support, or explain why the mathematics was familiar.
4. Either accept the current Sol repair or independently repair it and its directly required local metadata/contracts. You may author new missing-dependency lemmas and register them as specified in the FA brief. If the repair changes an existing run-local direct dependency, record the exact final-adjudicator prerequisite-repair licence. Run focused checks. Do not append a Sol adjudication or request another judge call for the already rejudged consumer; new lemmas still require their first engine-managed judgment.
5. Record the exact final bytes with exactly one of these commands:

```bash
node tools/step8-terminal-resolution.mjs record --run phase-2-wave-1 --id lem-qid-logarithmic-and-constant-divisibility --resolved-by final-adjudicator --group g --queue research/phase-2-wave-1-step8-fa-g-round-1.json --state-dir /home/lazyinspirit/Projects/prestige-math-library/.autopilot/phase-2-wave-1 --disposition accepted-after-review --source-status verified --basis-file research/phase-2-wave-1-step8-fa-g-2-lem-qid-logarithmic-and-constant-divisibility.md
node tools/step8-terminal-resolution.mjs record --run phase-2-wave-1 --id lem-qid-logarithmic-and-constant-divisibility --resolved-by final-adjudicator --group g --queue research/phase-2-wave-1-step8-fa-g-round-1.json --state-dir /home/lazyinspirit/Projects/prestige-math-library/.autopilot/phase-2-wave-1 --disposition repaired --source-status verified --basis-file research/phase-2-wave-1-step8-fa-g-2-lem-qid-logarithmic-and-constant-divisibility.md
```
