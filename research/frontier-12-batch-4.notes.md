...
The scaffold contains four pages and 61 items: `pi-the-equivalent-characterizations` has 14 A-page items, its companion has 7; `line-integrals-and-the-gradient-theorem` has 31 A-page items, its companion has 9. Neither A page approaches the active 60-item ceiling. **No split is proposed.** The exact cut is therefore not applicable, and no result was trimmed to avoid a split.
...
The library defines pi as twice the least positive zero of cosine. This page proves that the same number is the least positive zero of sine, half the least common period of sine and cosine, the length of a unit semicircle, every circle's circumference-to-diameter ratio, half the common limit approached from below and above by regular inscribed and circumscribed polygon perimeters, and the Riemann area of the unit disc. “Once around” and the local graph-area convention are explicit, preventing traversal and measure-theory ambiguities.

Three analytic representations are proved without assuming a geometric formula: the Gregory–Leibniz series with a finite integral remainder, Wallis's product from its integral recurrence and squeeze, and Viète's product from the finite double-angle identity and the published small-angle limit. Wallis also yields de Moivre's central-binomial asymptotic by direct algebra. A final ledger records every characterization and the precise theorem establishing it.
...
- The Archimedean theorem treats the regular inscribed and circumscribed perimeters as `2n sin(pi/n)` and `2n tan(pi/n)`, proves their opposite monotonicities, and identifies their common limit with the independently established unit-circle circumference `2 pi`.
...
- The de Moivre corollary must rewrite the central binomial quotient as the reciprocal double-factorial ratio in the finite Wallis product, prove `sqrt(pi n) binom(2n,n)/4^n -> 1`, and define the displayed asymptotic notation by that ratio limit.
...
- The ledger is a theorem, not a remark: it states one constant and names the exact zero, period, length, polygonal-limit, area, ratio, series, and product equalities already proved.
...
`frontier-12-batch-4.coverage.json` records 84 source-anchored headings/results: 56 `included`, 14 `inline`, 5 `already-published`, and 9 `out-of-scope`. Thus 70 source entries feed new scaffolded items, 5 were confirmed on disk, and 9 were declined. The canonical checklist adds 18 `included` entries and one `deferred` entry, the Basel problem, homed at `the-riemann-zeta-function` (order 345).
...
2. Rutgers Mathematics 373, *Workshop 9 Solutions: Pi and the AGM*, Problem 1 opening and parts (a)–(b), PDF pp. 1–2: <https://sites.math.rutgers.edu/courses/373/373-f03/w9sol.pdf>. This supplies the formulas and two-sided monotone limit under repeated polygon doubling; the scaffold proves the all-`n` monotonicity from the published derivative criterion.
3. Imperial College London, *History of Mathematics, Problems VI solutions*, PDF pp. 1–2, Questions 1–3: <https://www.ma.imperial.ac.uk/~bin06/HoM/HoM17/m3hsoln6.pdf>. This supplies the named Viète and Wallis developments and the de Moivre central-binomial consequence; Girard remains out of scope.
4. David Galvin, *Primitives and techniques of integration*, §13.2, PDF pp. 9–11 (printed pp. 321–323): <https://www3.nd.edu/~dgalvin1/10860/10860_S20/book/Sec13.pdf>. This is the second independent treatment of the Wallis recurrence, squeeze, product, and central-binomial asymptotic.
5. H. Jerome Keisler, *Elementary Calculus: An Infinitesimal Approach*, Chapter 4A §4.4, PDF pp. 213–214: <https://people.math.wisc.edu/~hkeisler/chapter_4a.pdf>. Keisler takes unit-disc area as the definition of pi, making it a useful independent convention check against this library's zero-based definition.

### Line-integral sources actually read

1. Jiří Lebl, *Basic Analysis II*, compound locator: §9.2 Definition 9.2.5 through Example 9.2.18 and the following arc-length paragraph at <https://www.jirka.org/ra/html/sec_pathintegral.html>; §9.3 opening through Definition 9.3.8 at <https://www.jirka.org/ra/html/sec_pathind.html>; and §10.6 Definition 10.6.1 through Example 10.6.5 plus Exercises 10.6.4–10.6.5 at <https://www.jirka.org/ra/html/sec_mvgreenstheorem.html>. The §10.6 clauses supply the load-bearing source support for positive orientation, graph regions, both half-proofs, the Type III theorem, shared-arc cancellation, and area formulas.
2. Jean-Baptiste Campesato, *Poincaré Lemma* (University of Toronto MAT237 notes), §§1–2, Definition 1 through Proposition 12, PDF pp. 1–7: <https://www.math.toronto.edu/campesat/ens/1920/poincare.pdf>. This is the independent full-note treatment of exactness, closedness, star-shaped domains, coordinate and radial potential constructions, and the punctured-plane counterexample.
3. Nathaniel Donaldson, *Math 2E*, §16.4, the Definition and Green theorem on PDF p. 1, the Calculating Areas corollary on p. 3, and **A Sketch Proof of Green's Theorem** on p. 5: <https://www.math.uci.edu/~ndonalds/math2e/16-4-green.pdf>. This remains corroboration only. No authored `[F#]` fact may use Donaldson as its cited clause; the corresponding exact Lebl §10.6 clause must be the load-bearing citation.

The three Lebl URLs are recorded as one source entry with a compound locator because they are one work. Campesato is independent for the Poincaré material; Donaldson corroborates the Green-theorem outline but is not a load-bearing proof source.
...
Lebl's book carries its Creative Commons notice on the book site; OpenStax is openly licensed. No explicit redistribution licence was found in the Rutgers, Imperial, Galvin, Keisler-chapter, Toronto, or Donaldson PDFs at the passages read. Those sources are therefore used only for factual research and locator-backed paraphrase. No proof text, figures, exercises, or source-specific prose is copied into the scaffold. Item titles name classical results; final statements and proofs will be newly worded and will retain source-reference URLs.
...
- The de Moivre central-binomial asymptotic is no longer declined: it is scaffolded as `cor-central-binomial-coefficient-asymptotic-from-wallis` and follows algebraically from the finite Wallis products.
- Lebl Proposition 11.4.2(9) states `sin(x) <= x` for `x >= 0`. That exact one-sided statement is not a pi characterization and is not imported; no scaffold item depended on the previously misquoted stronger absolute-value form.
- The Basel problem is deferred to `the-riemann-zeta-function` (order 345), where the zeta-function or Fourier-series machinery needed for its proof belongs.
...
`cor-archimedean-reciprocal`, `cor-differentiable-implies-continuous`, `cor-pi-is-the-first-positive-sine-zero`, `cor-sin-x-over-x-limit`, `cor-trigonometric-parity-and-pythagorean-identity`, `def-binomial-coefficient`, `def-canonical-natural`, `def-darboux-integral`, `def-derivative`, `def-factorial-and-falling-factorial`, `def-p-norms-on-rn`, `def-path-polygonal-length-and-rectifiability-in-rn`, `def-pi-via-first-positive-cosine-zero`, `def-sequence`, `def-series`, `def-tangent-cotangent-secant-cosecant`, `lem-integral-elementary-bounds`, `lem-polygonal-length-is-monotone-under-refinement`, `thm-algebra-of-derivatives`, `thm-algebra-of-limits`, `thm-arc-length-is-invariant-under-monotone-reparametrization`, `thm-binomial-closed-formula`, `thm-c1-paths-have-length-equal-to-the-integral-of-speed`, `thm-continuous-implies-integrable`, `thm-integration-by-parts`, `thm-monotonicity-from-the-derivative`, `thm-of-square-roots`, `thm-quarter-turn-values-and-shift-formulas`, `thm-sine-and-cosine-addition-formulas`, `thm-sine-and-cosine-derivatives`, `thm-sine-cosine-signs-monotonicity-and-ranges`, `thm-sine-cosine-zero-sets-and-fundamental-period`, `thm-squeeze`, `thm-substitution`, and `thm-tangent-cotangent-secant-cosecant-derivatives-and-periods`.
...
| `thm-archimedean-polygon-perimeter-characterization-of-pi` | `literature-derived` | `ai-altered` | Rutgers Workshop 9 Problem 1 supplies the two regular-polygon formulas and the monotone common limit under doubling. The final proof strengthens this to all `n` by rebuilding the formulas, monotonicities, and limit from the published arc-length, derivative, and trigonometric dependencies. |
...
| `lem-wallis-integrals-recurrence-and-squeeze` | `literature-derived` | `ai-altered` | Imperial Problems VI Question 3 and Galvin §13.2 independently supply the Wallis recurrence and squeeze, checked against the published integration-by-parts machinery. The final proof is newly organized rather than copied. |
| `thm-wallis-product-for-pi` | `literature-derived` | `ai-altered` | Imperial Problems VI Question 3 and Galvin §13.2 independently supply the product. The final proof is newly organized around the published recurrence, squeeze, and limit machinery. |
| `cor-central-binomial-coefficient-asymptotic-from-wallis` | `literature-derived` | `ai-altered` | Imperial Question 3's de Moivre note and Galvin §13.2 give the asymptotic; the scaffold derives its exact ratio limit from the local finite Wallis product and published binomial formula. |
...
| `thm-pi-equivalent-characterizations-ledger` | `ai-altered` | `ai-altered` | a checkable synthesis of the separately sourced A-page characterizations, including the Rutgers-backed Archimedean polygon limit. The final proof will be newly organized around the listed on-disk dependencies, so it is altered rather than copied. |
...
| `def-type-i-type-ii-and-elementary-green-regions` | `literature-derived` | `not-applicable` | Lebl §10.6 is the exact load-bearing source for the graph-region types and their conjunction in the elementary proof; Donaldson §16.4 is corroborative only. The finite-union wording is normalized to the dispatch's explicit gluing hypothesis. |
| `def-positive-orientation-for-elementary-region-boundaries` | `literature-derived` | `not-applicable` | Lebl Definition 10.6.2 is the exact load-bearing source for the region-on-the-left convention; Donaldson's opening definition only corroborates it. The graph-arc traversal makes it usable without the Jordan curve theorem. |
| `lem-green-type-i-boundary-identity` | `literature-derived` | `ai-altered` | Lebl's Type I half-proof is the load-bearing source; Donaldson's sketch stage 1 is corroborative only. The final proof is reorganized around the library's graph-region Fubini theorem and explicit piecewise-C1 boundary convention. |
| `lem-green-type-ii-boundary-identity` | `literature-derived` | `ai-altered` | Lebl's Type II half-proof is the load-bearing source; Donaldson's sketch stage 2 is corroborative only. The final proof is reorganized around the published symmetric Jordan-Fubini assertion. |
| `lem-green-boundary-cancellation-under-finite-gluing` | `literature-derived` | `ai-altered` | Lebl Example 10.6.5 is the load-bearing source for oppositely traversed internal arcs; Donaldson's sketch stage 3 corroborates it. The scaffold also states the separate zero-extension proof of region-integral additivity. |
| `thm-greens-theorem-for-finite-unions-of-elementary-regions` | `literature-derived` | `ai-altered` | Lebl Theorem 10.6.4's proved Type III case is the load-bearing source for the exact narrowed theorem; Donaldson's sketch is corroborative only. Arbitrary Jordan domains are excluded. |
| `cor-area-as-a-line-integral-for-elementary-regions` | `literature-derived` | `ai-altered` | Lebl Exercise 10.6.4 is the load-bearing source for the three coordinate boundary formulas; Donaldson's Calculating Areas corollary corroborates them. The final derivation uses the local Green theorem and Jordan content. |
