...
### measures-and-their-basic-properties — 49 items

1. `def-nonnegative-extended-series` — definition — Series in the nonnegative extended real line
2. `thm-tonelli-for-nonnegative-double-series` — theorem — Tonelli's theorem for double series of nonnegative extended real numbers
3. `def-measure` — definition — Measures on sigma-algebras
4. `def-measure-space` — definition — Measure spaces
5. `def-finitely-additive-set-function` — definition — Finitely additive nonnegative set functions
6. `def-finite-sigma-finite-and-semifinite-measures` — definition — Finite, sigma-finite, and semifinite measures
7. `def-probability-measure` — definition — Probability measures and probability spaces
8. `def-measure-null-set-and-almost-everywhere` — definition — Measure-null sets and almost-everywhere statements relative to a measure
9. `def-complete-measure-space` — definition — Complete measure spaces
10. `fs-subsets-of-null-sets-are-always-measurable` — false-statement — FALSE: every subset of a measure-null set is measurable
11. `prop-measure-monotonicity` — proposition — Measures are monotone
12. `prop-measure-of-a-set-difference` — proposition — Measure of a set difference when the smaller set has finite measure
13. `thm-finite-and-countable-subadditivity-of-measures` — theorem — Finite and countable subadditivity of measures
14. `thm-continuity-from-below-for-measures` — theorem — Continuity from below for measures
15. `thm-continuity-from-above-for-measures` — theorem — Continuity from above when one set has finite measure
16. `def-counting-measure` — definition — Counting measure on an arbitrary set
17. `prop-counting-measure-is-a-measure` — proposition — Counting measure is a measure
18. `fs-continuity-from-above-without-finiteness` — false-statement — FALSE: continuity from above needs no finiteness hypothesis
19. `prop-two-set-measure-identity` — proposition — The two-set measure identity $\mu(A\cup B)+\mu(A\cap B)=\mu(A)+\mu(B)$
20. `thm-finite-inclusion-exclusion-for-measures` — theorem — Inclusion-exclusion for a nonempty finite family of finite-measure sets
21. `prop-null-symmetric-difference-preserves-measure` — proposition — Sets whose symmetric difference is null have the same measure
22. `thm-countable-additivity-and-set-function-continuity` — theorem — Countable additivity and continuity of finitely additive set functions
23. `thm-first-borel-cantelli` — theorem — The first Borel-Cantelli lemma for measures
24. `thm-measure-of-set-liminf` — theorem — The measure of a set liminf is at most the liminf of the measures
25. `thm-measure-of-set-limsup` — theorem — The limsup of the measures is at most the measure of the set limsup under a finite-union bound
26. `cor-dominated-convergence-for-sets` — corollary — Measures converge for a convergent sequence of sets contained in one finite-measure set
27. `prop-null-sets-form-a-sigma-ideal-in-a-complete-space` — proposition — Null sets are closed under countable unions and, in a complete space, under arbitrary subsets
28. `def-completion-of-a-measure-space` — definition — The completion sigma-algebra and completed set function of a measure space
29. `lem-completion-domain-is-a-sigma-algebra` — lemma — The completion domain is a sigma-algebra
30. `lem-completed-measure-is-well-defined` — lemma — The completed measure is independent of the representing measurable set
31. `thm-completion-of-a-measure-space` — theorem — Every measure space has a unique complete extension to its completion
32. `def-restriction-of-a-measure` — definition — Restriction of a measure to a measurable set
33. `prop-restriction-is-a-measure` — proposition — The restriction of a measure to a measurable set is a measure
34. `def-dirac-measure` — definition — The Dirac set function at a point
35. `prop-dirac-measure-is-a-probability-measure` — proposition — A Dirac set function is a probability measure
36. `def-nonnegative-weighted-sum-of-measures` — definition — Nonnegative scalar multiples and countable weighted sums of measures
37. `thm-nonnegative-weighted-sums-of-measures` — theorem — Nonnegative scalar multiples and countable weighted sums of measures are measures
38. `thm-measures-on-finite-sigma-algebras-are-atomic` — theorem — A measure on a finite sigma-algebra is a finite weighted sum over its atoms
39. `thm-measures-on-countable-discrete-spaces-are-weighted-dirac-sums` — theorem — Every measure on a countable discrete space is its weighted sum of Dirac measures
40. `lem-semifinite-measures-have-arbitrarily-large-finite-subsets` — lemma — Assuming countable choice, an infinite-measure set in a semifinite measure space has arbitrarily large finite-measure subsets
41. `def-semifinite-part-of-a-measure` — definition — The semifinite part of a measure
42. `thm-semifinite-part-is-a-measure` — theorem — Assuming countable choice, the semifinite part is a semifinite measure and equals the original measure exactly when it is semifinite
43. `thm-semifinite-and-zero-infinity-decomposition` — theorem — Assuming countable choice, every measure is the sum of its semifinite part and a zero-infinity-valued measure
44. `lem-finite-measure-uniqueness-on-a-pi-system` — lemma — Finite measures agreeing on a generating pi-system and on the whole space are equal
45. `thm-measure-uniqueness-on-a-sigma-finite-pi-system` — theorem — Measures agreeing on a generating pi-system are equal under an increasing finite-measure exhaustion from that pi-system
46. `fs-generating-families-determine-measures` — false-statement — FALSE: agreement on an arbitrary generating family determines a measure
47. `fs-every-finitely-additive-set-function-is-a-measure` — false-statement — FALSE: every finitely additive nonnegative set function on a sigma-algebra is a measure
48. `fs-countable-additivity-without-disjointness` — false-statement — FALSE: measures are additive on arbitrary countable unions
49. `fs-zero-on-singletons-implies-zero-measure` — false-statement — FALSE: a measure on an infinite set that vanishes on every singleton is the zero measure
...
- The main A page has 49 items. It is below the hard 60-item ceiling, so no split is proposed. No result was pruned to stay below the ceiling.
...
4. Set limits: `thm-measure-of-set-liminf` uses increasing tail intersections; `thm-measure-of-set-limsup` uses decreasing tail unions and the exact finite-union bound. The corollary proves both numerical squeeze directions.
...
All 58 final ids were checked against both `items/` and `research/plan-spec.json` with:

```sh
for id in <the 58 manifest ids>; do rg -q "(^id: $id$|\"id\": \"$id\")" items research/plan-spec.json; done
```
