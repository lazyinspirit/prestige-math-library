...
- Batch-scoped precheck: exit 0, with all 49 proof-bearing items clean and no auto-repair output.
- Batch-scoped rendercheck: exit 0 across all 58 items and both pages; frontmatter, delimiters, wikilinks, KaTeX, and display blocks are clean.
- `node tools/content-policy.mjs research/frontier-10-batch-3.pages.json`: exit 0, with 58 scoped items and no errors or warnings.
...
- `node tools/proof-contract.mjs research/frontier-10-batch-3.proof-contracts.json --strict`: exit 0, with 49 of 49 proof-bearing items checked and no errors or warnings.
- `node tools/prosecheck.mjs`: exit 0 across the shared tree, with no errors; its 580 heuristic warnings are permitted positional/count warnings.
- **Shared-tree blocker:** the required bare precheck, depcheck, and rendercheck invocations cannot be left green while other frontier-10 batches are actively authoring in the same workspace. At final rerun, bare precheck reported 51 failures among 2,456 proof-bearing files, bare depcheck reported 48 errors, and bare rendercheck reported 19 errors across 3,402 files. Every reported failure belongs to out-of-scope Riemann–Stieltjes, improper-integral, change-of-variables, or determinant-dependent files. No batch-3 item or page appears in those failure lists. I did not edit another batch's owned artifacts.

## Confidence and unverified matters

Confidence is high in the final item inventory, mathematical dependency closure, proof formatting, boundary handling, and harvest fidelity for the named ranges. I checked all 118 coverage dispositions against the displayed source and canonical headings, re-opened all published dependency interfaces, re-opened Donaldson's Corollary 23.15, and used the authored batch-2 interfaces exactly as stated. I did not verify licensing beyond what the inspected source pages/PDFs exposed, so four sources remain deliberately marked citation-only. I did not edit the later published real/complex polynomial items, prose scaffolds, `plan-spec.json`, or another batch's owned artifacts.
