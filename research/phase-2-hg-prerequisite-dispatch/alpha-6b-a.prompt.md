...
label: 6b-a
covers: 1
output: research/phase-2-hg-prerequisite-alpha-a-6b.md

# Step 6b — routed group adjudication

Work only on the dispatched group. For every batch, read its
`research/phase-2-hg-prerequisite-step6-scope-1.json`, reader report and findings JSON, refuter
...
Compare each post-reader hash snapshot with `research/phase-2-hg-prerequisite-step6-hash-1-pre-6b.json`.
...
For each other changed carrier, write `post-reader:1:<id>` with route `touched`
...
Write `research/phase-2-hg-prerequisite-alpha-<g>-6b.md` and
`research/phase-2-hg-prerequisite-alpha-<g>-6b-decisions.json` with `{version:1,run,group,decisions}`.
