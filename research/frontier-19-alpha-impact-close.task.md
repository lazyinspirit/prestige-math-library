# Impact-receipt closure, run `frontier-19`

The `impact-receipt` gate is red. The receipt at `research/frontier-19-impact.json`
...
Read the final entry of `research/frontier-19-touches.json`; call its exact `label`
`<end>`. Reproduce the gate from the repo root against that endpoint:

```
node tools/impact-audit.mjs --touches research/frontier-19-touches.json \
  --from pre-author --to <end> --receipt research/frontier-19-impact.json
```
...
anything** (`research/frontier-19-dispatch/`). **No permission prompts of any
