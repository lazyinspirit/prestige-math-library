...
label: 6c-lead
covers: all
output: research/frontier-23-alpha-6c.md

# Step 6c — cross-batch audit and closure

Read `research/frontier-23-cross-group-edges.json`, the post-6b carriers, and every
...
`research/frontier-23-6c-verdicts.jsonl`; use the exact kind and verdict vocabulary
accepted by `tools/cross-group-edges.mjs`. Obtain a current carrier hash with
`node tools/cross-group-edges.mjs carrier --run frontier-23 --id <id>` after edits.
...
Write `research/frontier-23-alpha-6c.md` with the evidence, disposition, edits, and
