...
coverage harvest currently uses Emily Riehl, *Category Theory in Context*,
Chapter 1 §§1.1–1.5 and §1.7, and Steven Awodey–Andrej Bauer,
*Introduction to Categorical Logic*, Chapter 1 §§1.1–1.4.3.  The current design
keeps the A page at (not over) the 60-item ceiling, so no split is presently
indicated; this remains deliberately visible because any Step-3 addition would
force a split.
...
- B page `categories-functors-and-natural-transformations-examples` (order
  360): **23 proof-bearing leaves**.  No B item depends on another B item.
- Pair total: **83 items**; proof-contract scope: **58 items**.
...
`frontier-10-batch-7.coverage.json` records **154 source headings**:

| disposition | headings |
|---|---:|
| included as a dedicated scaffold item | 110 |
| absorbed inline into a named item | 20 |
| deferred to an identified later page | 15 |
| out of this pair's subject scope | 9 |

Thus the source-heading yield is **130 accepted/scaffolded or inline versus 24
declined**.  Counts are heading dispositions, not unique item counts: independent
sources deliberately overlap, and the manifest contains 83 unique items.

The declines most likely to be challenged are these:

1. **Determinant naturality.**  The prose plan explicitly requested it, but
   `the-determinant-of-a-linear-operator` has no published items.  Defining
   determinants, general linear groups and the unit-group target here would
   import an entire unbuilt linear-algebra block.  It remains deferred, not
   silently dropped.
2. **Dual, double-dual and non-naturality examples.**  The dual-space functor,
...
3. **Initial/terminal uniqueness.**  It is elementary and could be proved here,
   ...
   here.
4. **Representables, the hom bifunctor, presheaves and Yoneda.**  All are
...
correspondence, torsion-decomposition non-naturality, Riesz representation, and
the matrix/finite-dimensional-vector-space equivalence.  Each requires a named
subject block beyond this pair.  By contrast, Riehl's singleton natural
transformation, opposite-group natural isomorphism, and natural set-arithmetic
isomorphisms were added to B rather than declined after the detailed harvest.
...
|---|---|---|
| Emily Riehl, *Category Theory in Context*, Chapter 1 §§1.1–1.3, selected §1.4 (Definition 1.4.1 and Examples/Proposition 1.4.4–1.4.11), §1.5, selected §1.6 (Definitions 1.6.4, 1.6.14 and Lemmas 1.6.5, 1.6.16), §1.7; <https://emilyriehl.github.io/files/context.pdf> | Primary textbook spine: categories, duality, mono/epi, functors, naturality, equivalences, skeletons, diagrams and 2-categorical composition. | Author-hosted under the author's stated special arrangement with Dover.  Cite and paraphrase only. |
...
Every one of the following **50 direct external item dependencies** was then
...
`def-cartesian-product`, `def-continuous-map-top`,
...
All load-bearing dependencies are either earlier within A or published; there
are no same-run/cross-batch edges and no forward reference kept as a dependency.
No opened published dependency contained an unambiguous falsehood.
...
2. **Equivalence and Choice.**  The primary equivalence criterion is “fully
   ...
   choice is smuggled into a large-category claim.
...
The following groups exhaust all 83 items.  Each source-backed item will carry
...
### B-page examples, counterexamples and false statements (23)

- These 16 use `statement: literature-derived`, `proof: ai-altered`:
...
and one entry for every one of the 58 proof-bearing items.  It records exact
on-disk clauses for published dependencies, planned clauses for same-pair
dependencies, three numbered-step obligations per item, and all eight boundary
dispositions.  At Step 5 the author must synchronize every “Planned Statement”
...
2. Replace the existing eight-entry `requires` paragraph with the 13 page ids in
...
4. Replace the determinant/double-dual sentences in the B paragraph with:
   `The determinant and dual-space examples remain recorded deferrals because their planned prerequisite pages are unbuilt.  B instead includes the published-prerequisite examples in the batch manifest: functor categories and quivers, the fundamental and action groupoids, pointed sets versus partial maps, free and forgetful functors, products, inverse-image/open/closed functors, singleton and complement naturality, opposite groups, natural set arithmetic, group actions, and change of basepoint.`
...
The manifest's 83 ids were checked with
`fs.existsSync('items/' + id + '.md')`; **0 existed**.  A corroborating
`rg -n 'def-category$|thm-small-categories-form-a-strict-two-category|ex-action-groupoid-orbits-and-stabilizers' items research/plan-spec.json research/plan-combinatorics-and-categories.md`
...
was edited.  There is no cross-batch dependency, external fallback, or
