...
- [Lebl, *Basic Analysis I*, §6.3](https://jirilebl.github.io/ra/realanal.pdf): independent scalar Picard proof, exponential iterates, quadratic blowup, nonuniqueness for the square-root field, the Dirichlet right-hand side with no solution, and the integrating-factor exercise.
...
It reports three mechanically verified sources: Fonda, Bruckner–Bruckner–Thomson, and Lebl. The remaining canonical Teschl URL returns `EAI_AGAIN` in this shell sandbox, so no `fetch_verified` object was added for it. This is an environment blocker, not a claim that the URL is dead: the complete PDF was fetched through web research, but a network-enabled engine process must produce its byte count and hash.
...
|---|---|---|---|
| `def-first-order-ode-initial-value-problem-and-solution` | literature-derived | not-applicable | [Teschl Ch. 2](https://www.mat.univie.ac.at/~gerald/ftp/book-ode/ode.pdf) and [Lebl §6.3](https://jirilebl.github.io/ra/realanal.pdf) supplies the convention; the library statement fixes notation and boundary clauses without a proof component. |
...
| `prop-first-order-ivp-is-equivalent-to-a-volterra-integral-equation` | literature-derived | ai-altered | [Teschl Ch. 2](https://www.mat.univie.ac.at/~gerald/ftp/book-ode/ode.pdf) and [Lebl §6.3](https://jirilebl.github.io/ra/realanal.pdf) supplies the statement or exact witness. The Step-5 derivation is locally reorganized around published dependencies and the proof contract. |
...
| `def-maximal-ode-solution` | literature-derived | not-applicable | [Teschl Ch. 2](https://www.mat.univie.ac.at/~gerald/ftp/book-ode/ode.pdf) and [Lebl §6.3](https://jirilebl.github.io/ra/realanal.pdf) supplies the convention; the library statement fixes notation and boundary clauses without a proof component. |
...
## Source-scouting round 1 — 2026-08-20

The unstamped sources were both on `picard-lindelof-and-first-order-odes`. The gauge-integral pair was already mechanically stamped and was not changed.

- Teschl replacement URL at the end of round 1: `https://radon.mat.univie.ac.at/~gerald/ftp/book-ode/ode.pdf`. This was selected as a University of Vienna host variant of the author-authorised preliminary AMS text. The fetched web body was identified as the complete PDF dated April 2012; Chapter 2 begins at printed p. 33, ends at p. 58, and has the same §§2.1–2.7 headings used by the existing harvest. The prior `https://www.mat.univie.ac.at/~gerald/ftp/book-ode/ode.pdf` was retained as `original_url`. This was a same-source host substitution, so no `contents` row changed.
- Lebl replacement URL: `https://jirilebl.github.io/ra/realanal.pdf`. Lebl's author page names this GitHub Pages URL as the official mirror of the current Volume I PDF, version 6.3 dated 2026-05-15. The source retains §6.3 and its numbering; the prior `https://www.jirka.org/ra/html/sec_picard.html` remains as `original_url`. This is the same current textbook edition in PDF form, so no `contents` row changed.
- Rejected alternate for Teschl: the LSU-hosted `https://www.math.lsu.edu/~wolenski/Teschl/Teschl.pdf` is a July 2009 version with a shorter and differently numbered Chapter 2. It is not the harvested edition and was not used. The NZDR copy is a March 2011 draft and was rejected for the same reason.
- Mechanical result after replacement: `node tools/source-fetch-check.mjs --coverage research/frontier-16-batch-3.coverage.json --stamp` still cannot reach either replacement host from this shell sandbox and reports `EAI_AGAIN`; the unchanged stamped Fonda and BBT records remain valid. A network-enabled engine process must compute the two missing stamps. No digest or byte count was inferred from web-search metadata.

## Source-scouting round 2 — 2026-08-20

The unstamped check now isolates only Teschl at the `radon` host; the engine has mechanically stamped Lebl's replacement PDF. I restored `https://www.mat.univie.ac.at/~gerald/ftp/book-ode/ode.pdf`, the author's canonical download linked from his official book page, and retained the failed `radon` variant as `original_url`.

- The canonical author URL was fetched as the complete PDF. Its preliminary-version notice names the AMS, its colophon is dated April 2012, and its table of contents puts Chapter 2 at printed pp. 33–58 with exactly §§2.1–2.7 harvested in this batch. This is the same body and edition, so the locator and all `contents` rows remain unchanged.
