...
## Your cluster — `w2-group-actions`

This is an **enrichment of an already published pair** — the six published items stay untouched and every one of your assigned items is `status: draft`. It exists because the published page shipped without the orbit-stabiliser theorem, so the whole point is that the new machinery is correct.

The chain to attack: `thm-orbit-stabilizer` -> `thm-class-equation` -> `thm-p-group-fixed-point-congruence` -> `thm-nontrivial-center-of-a-finite-p-group` -> `thm-cauchy-theorem-for-finite-groups`, plus the Burnside/Cauchy-Frobenius count and `thm-jordans-derangement-theorem`.

Specific traps. Orbit-stabiliser is a **bijection between the orbit and the coset space**, not a numerical identity, and the numerical form needs finiteness — check which is stated and which is used. The class equation needs the centre split off and the sum taken over **distinct** non-central classes, one representative each; a double count there is silent. Cauchy-Frobenius counts orbits as `(1/|G|) * sum |Fix(g)|` and needs `|G|` finite and nonzero. The p-group fixed-point congruence needs the action set finite. Check every left-versus-right action convention: this page has coset actions, conjugation actions and translation actions, and a reversed convention makes `gHg^{-1}` versus `g^{-1}Hg` statements subtly wrong.

## Your assigned items

Every one of these is `high` or `critical` on `risk-report.mjs`. Read each in full, from disk, with every dependency it cites.

**group-actions-and-cayleys-theorem** (20)
...
**group-actions-and-cayleys-theorem-examples** (10)
