...
## Your cluster — `w2-det-cat`

Two unrelated pages sharing one refuter; treat them separately.

**Determinants.** The whole page works over a **commutative ring**, and that is the discipline to check. The Leibniz definition and multilinearity are ring-level. Invertibility is not: over a ring, \(A\) invertible implies \(\det A\) a **unit**, and the converse needs the adjugate, which this page deliberately does not build. `cor-invertible-matrix-has-unit-determinant` and `thm-real-square-matrix-invertible-iff-determinant-nonzero` are the two sides — check neither is stated more strongly than it is proved, and check nothing on the page silently uses "nonzero determinant implies invertible" over a ring. `lem-rigidity-of-alternating-multilinear-matrix-functions` and `thm-determinant-is-the-unique-normalized-alternating-multilinear-function` are the uniqueness spine; check the alternating-versus-antisymmetric distinction is respected (they differ in characteristic 2, and `cex-antisymmetric-need-not-be-alternating-in-characteristic-two` is the witness).

**Categories.** These are the items not already assigned to the two category refuters. Hold every one to the page's size conventions in `rem-category-theory-class-and-size-conventions`: a construction that needs all objects of a large category as a set is fatal. Check the duality principle, the mono/epi calculus, the initial/terminal/zero-object items, and `thm-eckmann-hilton-argument` for whether the interchange law is applied with the right units.

## Your assigned items

Every one of these is `high` or `critical` on `risk-report.mjs`. Read each in full, from disk, with every dependency it cites.

**categories-functors-and-natural-transformations** (12)
...
**categories-functors-and-natural-transformations-examples** (4)
...
**determinants-of-matrices-over-a-commutative-ring** (5)
...
**determinants-of-matrices-over-a-commutative-ring-examples** (3)

- `cex-antisymmetric-need-not-be-alternating-in-characteristic-two`
- `ex-row-operations-on-a-singular-matrix-over-z-mod-six`
- `fs-determinant-is-additive-on-matrices`
