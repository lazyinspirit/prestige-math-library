...
`research/frontier-10-batch-3.coverage.json`. Per A page: at least two
...
```
node tools/coverage-checklist.mjs research/frontier-10-batch-3.coverage.json
```
...
- `research/frontier-10-batch-3.pages.json` — the machine scaffold
- `research/frontier-10-batch-3.notes.md` — source ledger, provenance
  rationales, convention disagreements, split proposals, licensing notes
- `research/frontier-10-batch-3.coverage.json` — the harvest
- `research/frontier-10-batch-3.proof-contracts.json` — begun at scaffold time
...
# frontier-10, batch 3 — step-3 repair round

You are **Beta-frontier-10-3**, returning to repair your own scaffold after
Alpha's step-3 breadth-and-depth review.

## Read this first

`research/frontier-10-alpha-step3-scaffold-review.md` — **your sections are
§5 `polynomial-rings-and-roots` (B4)**. Read the whole file's opening and its F1 section too; read your own
sections in full and treat every "add" as required, not advisory.

Your pair is `insufficient` for B4. **F1 needs no action from you**: the owner approved re-homing `def-finite-sum-in-a-commutative-monoid` and `lem-finite-sum-reindexing-and-fubini` to `rings-subrings-and-integral-domains` (order 46), which is below your page at order 52, so your four citations are now legal backward edges. Keep citing the same two ids. Do correct your notes: §"Forward references, cross-batch dependencies and blockers" says "Forward references kept: none", which was false against disk when written.

## What you may change

Only your own four artifacts:

- `research/frontier-10-batch-3.pages.json`
- `research/frontier-10-batch-3.notes.md`
- `research/frontier-10-batch-3.coverage.json`
- `research/frontier-10-batch-3.proof-contracts.json`
...
node tools/coverage-checklist.mjs research/frontier-10-batch-3.coverage.json
node tools/content-policy.mjs research/frontier-10-batch-3.pages.json --manifest-only --rehomed research/frontier-10-rehomed.json
```
