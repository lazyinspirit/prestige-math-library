<!-- Dispatched as-is. `frontier-17` and `9` are substituted by tools/dispatch.mjs
...
# Scaffolding brief — run `frontier-17`, batch `9` (steps 1 and 2)
...
You are **Beta-9**, running **GPT 5.6 Sol via the Codex subscription plan
...
`research/frontier-17-batch-9.notes.md`: current substage, owned artifact paths,
...
```
node tools/source-fetch-check.mjs --coverage research/frontier-17-batch-9.coverage.json --stamp
```
...
```
node tools/coverage-checklist.mjs research/frontier-17-batch-9.coverage.json
```
...
- `research/frontier-17-batch-9.pages.json` — your batch's page objects in final
...
- `research/frontier-17-batch-9.notes.md` — prose-scaffold amendments as precise
  ...
  route, and necessity.
- `research/frontier-17-batch-9.proof-contracts.json` — the version-1
...
run: frontier-17
role: beta
label: source-scout-1-b9
covers: 9

> **Source-scouting round.** Your batch number is in the "This dispatch"
> section appended below; substitute it wherever `9` appears.

# Batch `9` of run `frontier-17` — scout a live full-text URL
...
```
node tools/source-fetch-check.mjs --coverage research/frontier-17-batch-9.coverage.json
```
...
`research/frontier-17-batch-9.coverage.json` and `.notes.md` only. Keep
