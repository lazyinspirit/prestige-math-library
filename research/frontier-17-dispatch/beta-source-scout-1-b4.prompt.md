...
run: frontier-17
role: beta
label: source-scout-1-b4
covers: 4

> **Source-scouting round.** Your batch number is in the "This dispatch"
> section appended below; substitute it wherever `4` appears.

# Batch `4` of run `frontier-17` — scout a live full-text URL
...
```
node tools/source-fetch-check.mjs --coverage research/frontier-17-batch-4.coverage.json
```
...
`research/frontier-17-batch-4.coverage.json` and `.notes.md` only. Keep
