...
> section appended below; substitute it wherever `1` appears.

# Batch `1` of run `frontier-17` — scout a live full-text URL
...
```
node tools/source-fetch-check.mjs --coverage research/frontier-17-batch-1.coverage.json
```
...
`research/frontier-17-batch-1.coverage.json` and `.notes.md` only. Keep
