...
The area A page has 16 items and its companion has 6. The improper-integral A page has 14 items and its companion has 4. Neither A page approaches the 60-item ceiling, so no split is proposed.
...
Recommendation: approve. The source and this page use “simple polygon” for the compact filled region. The published later item `def-polygonal-arc-and-polygon` uses “polygon” for the boundary curve.
...
```text
**DEFS.** Translation of a set; the parallelogram $P(p;v,w)$ and the triangle $T(A,B,C)$ in $\mathbb{R}^2$; base and height of a chosen nonzero side; a simple polygonal region as a compact planar region bounded by an irredundant simple closed finite polygonal chain, and a triangulation of that region. Record, as non-load-bearing forward orientation, that `def-polygonal-arc-and-polygon` later calls the boundary chain a polygon and `thm-polygonal-jordan-curve` supplies the corresponding complementary-region theorem.
```
...
The coverage artifact contains 75 harvested headings: 26 `included`, 9 `inline`, 20 `already-published`, and 20 `out-of-scope`. There are no `deferred` rows. The area warning is explained by the mature prerequisites: the source ranges independently re-prove boundary criteria, graph-nullity, C1 preservation of null sets, linear content scaling, and matrix factorization that are already published. None of those results was dropped for want of a lemma.

The source harvest added the interior-diagonal lemma, triangulation theorem, triangulation face count, shoelace formula, uniform-tail lemma, comparison theorem, and the explicit parameter counterexample. No page needs a split after those additions.

## Published dependencies opened and closure

Every proposed external dependency was opened from `items/` and its actual primary section read. All 61 are `status: published`; 23 have `provenance.statement: literature-derived` and 38 have `provenance.statement: ai-altered`. None is AI-generated and none is legacy-unclassified. Every home is strictly earlier than order 288.00007; the standard square-root items are homed on `foundations-of-the-real-numbers` and `roots-and-rational-powers`, and the standard right-triangle example is not a dependency.
...
The contracts contain 36 citations to published clauses, all verified as exact substrings of the cited `Statement` or `Definition`. No published load-bearing claim was found false, so no published-dependency repair is proposed.
...
| `def-simple-polygonal-region-and-triangulation` | literature-derived | not-applicable | ETH Definitions 4.1 and 4.7, reconciled with Erickson's region/boundary discussion. Cite ETH and Erickson. |
...
The highest-risk boundaries are zero radius; zero chosen base vectors; repeated and collinear triangle vertices; irredundant polygon vertices; the $n=3$ induction base; internal edges counted twice; cyclic final-to-first shoelace indexing; and zero-content overlaps along full closed triangle edges.
...
- The Gaussian proof is decomposed into finiteness/positivity, the product-square identity, the polar plane evaluation, and the final positive-square-root theorem.
...
The highest-risk boundaries are the empty open set; $j=0$ versus $j+1$ in exhaustion cores; the empty complement of $\mathbb R^n$; avoiding Countable Choice in the recursive neighborhoods; finite versus infinite nonnegative integral values; excluding infinity-minus-infinity; relative parameter endpoints; increment zero in difference quotients; separate improper tails; the polar origin and full-period seam; and the two ratios of lower rectangle endpoints in the unequal-exhaustion example.
...
- `def-simple-polygonal-region-and-triangulation`: both `A simple polygonal region is a compact planar region whose boundary is an irredundant simple closed finite polygonal chain.` and `A triangulation is a finite family of closed triangles with union the polygon and pairwise intersections empty or a common face.`
...
Translation invariance first identifies graph area with Jordan content and gives the disc content $\pi r^2$. Orthogonal projection turns a two-dimensional determinant into base times perpendicular height, yielding determinant and base--height formulas for parallelograms and triangles, including their degenerate cases. Interior diagonals then triangulate simple polygonal regions; finite additivity sums the triangle contents, and cancellation of internal edges gives the shoelace formula.
...
Compact Jordan exhaustions define nonnegative improper multiple integrals and show that signed exhaustion-independent convergence is absolute. An integrable dominator gives uniform tail control, from which continuity and differentiation under a parameter-dependent integral follow. For the Gaussian, comparison proves one-dimensional convergence, product Fubini identifies its square with a plane integral, and polar change of variables evaluates that plane integral as $\pi$, giving the positive value $\sqrt\pi$.

Both B pages must have frontmatter only and no authored summary body.

## Per-page item lists

### `areas-of-elementary-plane-figures` — 16 items
...
5. `def-parallelogram-and-triangle-in-r2` — definition — Parallelograms and triangles in R²
6. `def-base-and-height-for-plane-figures` — definition — Base and perpendicular height for a chosen side of a plane figure
7. `lem-determinant-base-height-identity-in-r2` — lemma — Determinant equals base times distance to the spanning line in R²
8. `thm-parallelogram-content-and-base-height-formula` — theorem — Parallelogram content by determinant and base times height
9. `thm-triangle-content-and-base-height-formula` — theorem — Triangle content by determinant and half base times height
10. `cor-triangle-content-zero-iff-collinear` — corollary — A triangle has zero content iff its vertices are collinear
11. `def-simple-polygonal-region-and-triangulation` — definition — Simple polygonal regions and their triangulations
12. `lem-simple-polygon-has-an-interior-diagonal` — lemma — Every nontriangular simple polygon has an interior diagonal
13. `thm-simple-polygon-admits-a-triangulation` — theorem — Every simple polygon admits a triangulation
14. `cor-simple-polygon-triangulation-has-n-minus-two-triangles` — corollary — Every triangulation of a simple n-gon has n−2 triangles
15. `thm-simple-polygon-content-is-the-sum-of-triangle-contents` — theorem — Polygon content is the sum of triangle contents
16. `thm-shoelace-formula-for-simple-polygons` — theorem — Shoelace formula for a counterclockwise simple polygon
...
### `improper-and-parameter-dependent-multiple-integrals` — 14 items
...
11. `lem-gaussian-integral-is-finite-and-positive` — lemma — Gaussian integral is finite and positive
12. `lem-gaussian-square-is-the-plane-gaussian-integral` — lemma — Gaussian square equals the plane Gaussian integral
13. `lem-plane-gaussian-integral-in-polar-coordinates` — lemma — Plane Gaussian integral equals π
14. `thm-gaussian-integral` — theorem — Gaussian integral equals √π
