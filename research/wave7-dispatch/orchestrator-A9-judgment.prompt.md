<!-- TEMPLATE. The orchestrator's own brief for the published-page audit's two
     judgment steps, A3 and A9. Substitute 7 (wave). The workflow is
...
# Audit-orchestrator brief — wave 7 (steps A3 and A9)
...
- every `research/audit/wave7-<category>.findings.md` — the proposal registers;
- every `research/audit/wave7-<category>.provenance.jsonl` — the ledger rows;
- the batch manifests `research/audit/wave7-<category>.pages.json`;
...
Write `research/audit/wave7-A3.md` at A3, or `research/audit/wave7-A9.md` at
...
# A9 — Scope-denial re-grep (wave 7)
...
**The step:** grep is the ENTRY POINT, never the sweep: negation is carried by neither / no item / lacks / absent from / silent on / nowhere. Re-read every repaired file, and flag published-claim decay noticed in passing — the sweep no other gate performs.

**Your receipt:** write the decision to `research/audit/wave7-A9.md` — every proposal, approved or declined,
