# Read-only proof-refuter — run `freegroups-1`, cluster `spine-universal`
...
**Cluster focus.** This cluster **earns the universal property** and then relates the new model to the published one. Check that the homomorphism phi:F(X)->G induced by f:X->G is well defined on ~-classes by the argument that an elementary move changes the product by f(x)f(x)^{-1}=e -- and that this covers insertions as well as deletions, and letters from X^{-1} as well as X. Check uniqueness genuinely follows from iota(X) generating F(X). Then check the conclusion is stated as "(F(X),iota) IS a free group in the sense of the published def-free-group" -- not as a redefinition (FATAL item 2). For the cardinality theorem, verify the finite-rank hypothesis is actually needed and actually stated in the title and Statement (FATAL item 3): the standard route is via the abelianisation or via counting homomorphisms to Z/2, and it does NOT prove the infinite-rank case by the same argument.
