# Read-only proof-refuter — run `freegroups-1`, cluster `cyclic-torsion-conjugacy`
...
**Cluster focus.** Cyclic reduction and its two consequences. Check lem-cyclic-reduction-normal-form handles the case where the cyclic reduction is empty and the case of a word that is already cyclically reduced, and that "conjugate to a cyclically reduced word" is proved with the conjugator exhibited. thm-free-groups-are-torsion-free must handle the identity element correctly -- the statement is that every NON-IDENTITY element has infinite order -- and its proof usually needs that a nonempty cyclically reduced word has powers whose reduced length grows, which requires care exactly when the first and last letters are inverse to each other (that case is excluded by cyclic reducedness -- check the proof says so). thm-conjugacy-of-cyclically-reduced-words is a biconditional about cyclic permutation: check both directions and check whether it claims uniqueness it did not prove.
