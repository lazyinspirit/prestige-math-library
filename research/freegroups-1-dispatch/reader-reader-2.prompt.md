...
<!-- TEMPLATE. Copy into a Beta-n-i audit prompt and substitute freegroups-1 and 2. -->
...
Update your batch's `research/levelfreegroups-1-batch-2.proof-contracts.json` whenever
...
## YOUR ASSIGNMENT — reader 2 of 2

Audit these 19 items in full. Reader 1 covers the rest; do not duplicate them.
...
**Your highest-priority target is `def-tietze-transformations` and
`thm-tietze-transformations-connect-finite-presentations`.** The equivalence of
the three-move package with the classical four moves is load-bearing: if it
fails, completeness is FALSE. Check the renaming simulation move by move against
the stated legality conditions. Also verify each of the three `cex-` witnesses by
direct computation — do not accept the stated arithmetic.
