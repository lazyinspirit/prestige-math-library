## Step 6a — independent read of batch `<i>`
...
`research/frontier-14-batch-<i>.pages.json`.

**Write your report to:** `<output>`

Both `<i>` and `<output>` are substituted by the dispatcher from values the
engine computed for this unit. They are never copied from another batch's file —
that is how a reader once wrote its report over another reader's, destroying
eleven fatal findings.
...
- an `[F#]`/`[L#]` restatement wider than the item it cites. **Open the cited
  item on disk.** Ten of twenty-five step-8 rows on the previous run were this.
