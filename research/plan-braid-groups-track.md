...
Status: research-backed prose scaffold reconciled against actual suppliers,
2026-09-08. This document
...
The planned but unbuilt topology supplier
`fibrations-fiber-bundles-and-homotopy-exact-sequences` supplies the definition
of a locally trivial bundle, the numerable-bundle-to-Hurewicz-fibration
interface, and the long exact homotopy sequence. The track does not silently
treat its empty inventory as published. The former dependency on the much
broader future differential-topology page
`isotopy-extension-and-embedding-theory-beyond-whitney` has been removed:
finite point motions are constructed locally in BG-4, every topological disk
mapping class is represented by the endpoint of an explicit smooth point
motion, and only the smooth relative arc-extension case is then proved from
the published vector-field/flow interfaces. Thus no topological taming or
Schoenflies assertion is hidden in the replacement. Published `the-fundamental-group`,
`covering-spaces-and-lifting`, `classification-of-covering-spaces`,
`partitions-of-unity-and-paracompactness`, and the algebraic group/module
foundations are reused at item level.
...
|---|---|---|---|
| `def-geometric-braid-with-setwise-endpoints` (definition) | `[def-product-topology]` | Define a braid as $n$ disjoint paths $b_j:I\to D^2\times I$ for which height projection restricts to the identity, so every strand is the graph of a continuous point motion; the bottom and top endpoint sets are both $Q_n$, and their matching records the endpoint permutation. This parametrized level-preserving definition, rather than an unqualified tame-link embedding, is what makes slicing continuous. | GM §1.2--§1.3, printed pp. 4--5; Farb--Margalit §9.1.1, printed pp. 251--253; BB §1.2, printed pp. 4--6. |
| `def-braid-isotopy-relative-top-and-bottom` (definition) | `[def-geometric-braid-with-setwise-endpoints, def-homotopy-relative-and-path-homotopy]` | An isotopy is a continuous two-parameter family through level-preserving geometric braids, fixes the bottom and top endpoint sets throughout, and never permits a collision. Individual strand labels are transported continuously but need not return to the same top endpoint; pure does not mean the same thing as isotopy relative each strand. | GM §1.2--§1.3, pp. 4--5; Farb--Margalit §9.1.1, pp. 251--253. |
...
| `thm-fadell-neuwirth-forgetful-fibration` (theorem) | `[lem-forgetting-configuration-points-is-locally-trivial, def-locally-trivial-fiber-bundle, cor-metric-spaces-admit-subordinate-partitions-of-unity, thm-numerable-fiber-bundles-are-hurewicz-fibrations]` | For a connected manifold $M$ without boundary of dimension at least two and $m,n\ge1$, forgetting the last $n$ points gives a locally trivial bundle $F_{m+n}(M)\to F_m(M)$ with fiber $F_n(M\setminus\{m\text{ retained points}\})$. For the load-bearing disk case, $F_m(\operatorname{int}D^2)$ is an open subspace of Euclidean space and hence metric; refine the displayed trivializing cover by a subordinate partition of unity, so the bundle is numerable and therefore a Hurewicz fibration. | FN Theorems 1--2, pp. 112--114; GM Theorem 2.1 for the planar one-point case; Hatcher §4.2 Proposition 4.48 and the paracompact-base strengthening, printed pp. 379--380. |
...
| `lem-boundary-fixed-disk-evaluation-has-continuous-local-point-motion-sections` (lemma) | `[def-unordered-configuration-space]` | Choose pairwise disjoint coordinate disks about the points of $Q_n$ and the corresponding labelled neighborhood of $[Q_n]$. In each disk choose a Lipschitz cutoff $\chi_i$ and, after shrinking the target disk so $\lVert y_i-q_i\rVert\operatorname{Lip}(\chi_i)<1$, use $x\mapsto x+\chi_i(x)(y_i-q_i)$. This is a boundary-fixed bi-Lipschitz homeomorphism, depends continuously on $y_i$ in the compact-open topology, and the disjointly supported product sends $Q_n$ to $\{y_i\}$. Translate the construction by an arbitrary homeomorphism to obtain local sections over every configuration. | BB §1.3, proof of Theorem 1; Farb--Margalit §9.1.4 pp. 256--257; direct supported-point-motion construction. |
| `lem-a-smooth-finite-disk-arc-system-isotopy-extends-relative-boundary-and-marked-points` (lemma) | `[def-time-dependent-vector-field-and-evolution-operator, lem-a-vector-field-along-an-embedded-submanifold-extends-to-a-neighbourhood-and-globally-when-closed, thm-compactly-supported-time-dependent-vector-fields-have-global-evolution-on-a-compact-time-interval]` | Let $F_t$ be a smooth isotopy of one compact smooth proper arc in a finite smooth arc system, constant on collars of its fixed endpoints, with its interior disjoint from the marked points and arcs already fixed. Differentiate $F_t$, extend its velocity smoothly from the compact isotopy track in $[0,1]\times D^2$ by slice charts and a partition of unity, and multiply by a cutoff that vanishes on the endpoint set, the other marked points, the fixed arcs, and $\partial D^2$ away from the allowed endpoint collar. The published global-evolution theorem integrates this field; its evolution carries $F_0$ to $F_t$ and fixes every prescribed set. Compose the finitely many one-arc extensions. This is exactly the smooth relative arc variant of Proposition 1.11; it neither asserts a topological-arc chart nor invokes Schoenflies. | Farb--Margalit §§1.2.5--1.2.7, Proposition 1.11 and the explicit statement that the arc variants hold, printed pp. 35--38; library vector-field extension and time-dependent-flow interfaces. |
| `lem-evaluation-on-an-unordered-marked-set-is-a-numerable-bundle-and-fibration` (lemma) | `[def-unordered-configuration-space, lem-boundary-fixed-disk-evaluation-has-continuous-local-point-motion-sections, def-locally-trivial-fiber-bundle, cor-metric-spaces-admit-subordinate-partitions-of-unity, thm-numerable-fiber-bundles-are-hurewicz-fibrations]` | Evaluation $\operatorname{Homeo}^+(D^2,\partial D^2)\to C_n(\operatorname{int}D^2)$, $h\mapsto h(Q_n)$, is a locally trivial bundle whose fiber is the subgroup preserving $Q_n$ setwise: a local section $s$ gives $h\mapsto(h(Q_n),s(h(Q_n))^{-1}h)$. The finite-group quotient of the Euclidean configuration space is metrizable, so subordinate partitions make this bundle numerable; the inherited theorem then makes it a Hurewicz fibration. | BB §1.3, proof of Theorem 1; Farb--Margalit §9.1.4 pp. 256--257; Hatcher §4.2, printed pp. 379--380. |
| `def-boundary-map-from-point-motions-to-punctured-disk-mapping-classes` (definition) | `[lem-evaluation-on-an-unordered-marked-set-is-a-numerable-bundle-and-fibration, def-boundary-fixed-mapping-class-group-of-a-punctured-disk]` | Lift a based configuration loop starting at the identity homeomorphism and take the component of its endpoint in the fiber. | BB §1.3; Farb--Margalit Theorem 9.1 proof. |
...
| `thm-the-evaluation-bundle-boundary-map-is-an-isomorphism-for-the-disk` (theorem) | `[thm-alexander-contractibility-of-the-boundary-fixed-disk-homeomorphism-group, lem-evaluation-on-an-unordered-marked-set-is-a-numerable-bundle-and-fibration, lem-the-point-motion-boundary-map-is-a-well-defined-homomorphism, thm-long-exact-sequence-of-homotopy-groups-of-a-fibration]` | The low-degree tail of the fibration long exact sequence, together with $\pi_0=\pi_1=0$ for the total homeomorphism group, makes $\pi_1(C_n(D^2))\to\pi_0(\operatorname{Homeo}^+(D^2,\partial D^2;Q_n))$ bijective. | BB §1.3, Theorem 1; Farb--Margalit Theorem 9.1. |
| `thm-braid-group-is-the-boundary-fixed-mapping-class-group-of-the-punctured-disk` (theorem) | `[thm-geometric-braids-are-the-fundamental-group-of-unordered-configurations, thm-the-evaluation-bundle-boundary-map-is-an-isomorphism-for-the-disk, def-time-dependent-vector-field-and-evolution-operator, thm-compactly-supported-time-dependent-vector-fields-have-global-evolution-on-a-compact-time-interval]` | Compose the configuration/strand equivalence with the evaluation boundary isomorphism; match each geometric half twist to the supported half-twist mapping class. Also record the load-bearing smooth-representative refinement: lift a configuration loop to an ordered point path, use compact separation to approximate it rel endpoints by a smooth collision-free path constant near the endpoints, and homotope by straight interpolation inside the same separation bound. Disjoint smooth bump fields following the point velocities combine to a compactly supported time-dependent field; its evolution gives a boundary-fixed diffeomorphism whose endpoint represents the same mapping class. Thus every class in this topological mapping-class group has a smooth representative, without assuming that the image of an arc under an arbitrary homeomorphism is smooth. | GM §1.4; BB §1.3, Theorem 1; Farb--Margalit §1.2.6 and §9.1.3--§9.1.4, especially Proposition 1.11. |
...
**Deferred proof joint.** The point-pushing map is defined here, but its
injectivity is not. In the fibration exact sequence its kernel is the image of
$\pi_2(F_{n-1}(D^2))$; BG-5 first proves that group vanishes and only then
states the short exact point-pushing theorem. This prevents the Birman exact
sequence from bootstrapping the asphericity argument that proves its left-hand
injectivity.
...
| proposed item (kind) | deps | exact content and proof route | source locator |
|---|---|---|---|
| `lem-a-finitely-punctured-disk-retracts-to-a-wedge-of-circles` (lemma) | `[def-homotopy-equivalence, thm-fundamental-group-of-finite-wedge-of-circles, thm-covering-space-lifting-criterion, thm-homotopy-lifting-for-covering-maps, thm-higher-dimensional-spheres-are-simply-connected]` | Construct disjoint arcs from puncture circles to the boundary and collapse the complement to a finite wedge. Construct the wedge's universal cover as the tree of reduced words and contract that tree. For $k\ge2$, lift based maps $S^k\to\bigvee S^1$ and their nullhomotopies through the cover to prove vanishing higher homotopy; the published wedge theorem supplies the stated free fundamental-group basis. | FN §II and §IV; GM discussion after Theorem 2.1; Hatcher §1.3, pp. 58--72. |
| `lem-planar-configuration-spaces-have-vanishing-pi-two-by-simultaneous-induction` (lemma) | `[thm-fadell-neuwirth-forgetful-fibration, lem-a-finitely-punctured-disk-retracts-to-a-wedge-of-circles, thm-long-exact-sequence-of-homotopy-groups-of-a-fibration]` | Starting at $F_1(\mathbb C)=\mathbb C$, the LES segment $\pi_2(\text{fiber})\to\pi_2(F_n)\to\pi_2(F_{n-1})$ and the wedge-of-circles fiber force $\pi_2(F_n)=0$ for every $n$. | FN Theorem 2 and Corollary 2.2, pp. 113--114; GM §2.1. |
...
| `thm-point-pushing-is-the-kernel-of-forgetting-a-puncture` (theorem) | `[def-point-pushing-homomorphism-for-a-puncture, thm-pure-braid-forgetting-a-strand-short-exact-sequence, cor-pure-braids-are-pure-punctured-disk-mapping-classes, thm-long-exact-sequence-of-homotopy-groups-of-a-fibration]` | Identify the fiber-inclusion homomorphism in the now-short-exact Fadell--Neuwirth sequence with the point-pushing map and transport it through the pure braid/mapping-class isomorphisms. Thus point pushing is injective and its image is exactly the kernel of forgetting the last puncture. | FN Theorems 2--3 and §IV, pp. 113--117; Farb--Margalit §4.2 and §9.1.4, pp. 101--105 and 256--257. |
...
| `thm-ordered-planar-configuration-spaces-are-aspherical` (theorem) | `[thm-fadell-neuwirth-forgetful-fibration, lem-a-finitely-punctured-disk-retracts-to-a-wedge-of-circles, lem-planar-configuration-spaces-have-vanishing-pi-two-by-simultaneous-induction, thm-long-exact-sequence-of-homotopy-groups-of-a-fibration]` | For every $k\ge3$, the LES has zero adjacent fiber groups and reduces $\pi_k(F_n)$ to $\pi_k(F_{n-1})$; combine with the separately closed $k=2$ induction and contractible base case. | FN Corollaries 2.1--2.3, pp. 114--115; GM §2. |
| `cor-unordered-planar-configuration-spaces-are-aspherical` (corollary) | `[thm-ordered-planar-configuration-spaces-are-aspherical, thm-ordered-configurations-cover-unordered-configurations-regularly, thm-covering-space-lifting-criterion, thm-homotopy-lifting-for-covering-maps, thm-higher-dimensional-spheres-are-simply-connected]` | For $k\ge2$, lift every based $S^k$ map through the ordered cover and lift nullhomotopies uniquely; this proves that a covering induces an isomorphism on $\pi_k$, so the unordered configuration space is aspherical. | FN §II; GM §2; Hatcher §1.3, pp. 60--62. |
| `def-standard-pure-braid-generators` (definition) | `[def-elementary-geometric-half-twist, prop-the-artin-presentation-surjects-onto-geometric-braids]` | Define $A_{ij}$ as the geometric image of $\sigma_{j-1}\cdots\sigma_{i+1}\sigma_i^2\sigma_{i+1}^{-1}\cdots\sigma_{j-1}^{-1}$ under the already constructed Artin-to-geometric surjection and verify its endpoint permutation is trivial. No injectivity of the Artin presentation is used here. | GM §2, formulas for Artin pure generators. |
...
| `lem-trivial-action-on-standard-meridians-fixes-the-stem-arc-system-up-to-isotopy` (lemma) | `[def-standard-meridians-of-a-punctured-disk, thm-the-punctured-disk-fundamental-group-is-free-on-standard-meridians, thm-braid-group-is-the-boundary-fixed-mapping-class-group-of-the-punctured-disk]` | Choose the smooth representative supplied by BG-4. Equality of $h(s_i)m_{h(i)}h(s_i)^{-1}$ with $s_im_is_i^{-1}$ first forces $h(i)=i$ by free reduction/peripheral conjugacy, then lifts the two smooth proper arcs to the universal cover of the punctured disk. Their equal endpoint lifts cobound bigons; the smooth relative-boundary arc bigon criterion removes finitely many bigons and gives a smooth isotopy rel endpoint collars. | Farb--Margalit §§1.2.4--1.2.7, printed pp. 31--38; GM §1.6. |
| `lem-a-standard-stem-arc-system-can-be-straightened-by-boundary-and-puncture-fixed-ambient-isotopy` (lemma) | `[lem-trivial-action-on-standard-meridians-fixes-the-stem-arc-system-up-to-isotopy, lem-a-smooth-finite-disk-arc-system-isotopy-extends-relative-boundary-and-marked-points]` | Apply the smooth disk-arc extension lemma to the preceding relative arc isotopy, cut along $s_1$, and repeat. At stage $i$ the support avoids the previously fixed arcs, so earlier arcs, every puncture, and the outer boundary remain fixed. | Farb--Margalit Proposition 1.11 and §1.2.7, pp. 36--38; local BG-4 smooth extension lemma. |
...
| `lem-an-lkb-kernel-braid-fixes-every-standard-adjacent-edge-up-to-isotopy` (lemma) | `[lem-the-fork-noodle-pairing-detects-essential-intersections, def-lawrence-krammer-bigelow-representation, thm-braid-group-is-the-boundary-fixed-mapping-class-group-of-the-punctured-disk]` | Choose the smooth representative supplied by BG-4. For each standard adjacent edge $E_i$, compare all fork/noodle pairings before and after that representative. Apply the Key Lemma to eliminate intersections with the complementary standard noodles; the smooth relative arc criterion gives an isotopy rel endpoint collars, chosen successively away from edges already fixed. | Bigelow 2001 §3.3 and Lemma 3.6, pp. 9--10; Farb--Margalit §§1.2.4--1.2.7. |
| `lem-a-punctured-disk-mapping-class-fixing-all-standard-adjacent-edges-is-a-boundary-twist-power` (lemma) | `[lem-an-lkb-kernel-braid-fixes-every-standard-adjacent-edge-up-to-isotopy, lem-a-smooth-finite-disk-arc-system-isotopy-extends-relative-boundary-and-marked-points]` | Apply the smooth disk-arc extension lemma to ambient-isotope the mapping class to fix $E_1\cup\cdots\cup E_{n-1}$ pointwise. A regular neighborhood is a once-bordered disk containing all punctures; Alexander-contract the restriction there. The complement is an annulus, whose boundary-fixed mapping class group is generated by its core twist, identified with the full boundary twist $\Delta^2$. | Bigelow 2001 end of §3.3, pp. 9--10; Farb--Margalit Proposition 1.11, Alexander lemma and annulus computation, §§1.2.6, 2.2.1--2.2.2. |
...
| `def-homflypt-polynomial-from-the-hecke-markov-trace` (definition) | `[thm-the-ocneanu-markov-trace-exists-and-is-unique, def-closure-of-a-geometric-braid]` | Since $T_n^{-1}=q^{-1}T_n+(q^{-1}-1)$, put $z_-=q^{-1}(z+1-q)$. First localize the trace ring so that $z$ and $z_-$ are units, then adjoin a unit $u$ with $u^2=z_-/z$; set $v=(uz)^{-1}$ and define $P(\widehat\beta)=u^{e(\beta)}v^{n-1}\operatorname{tr}_n(\pi(\beta))$, followed by the source's explicit variable change to the standard HOMFLYPT normalization. The localization is part of the definition, not an implicit division in $\mathbb Z[q^{\pm1},z]$. | BB §4.3, equations after Theorem 12. |
...
| proposed item (kind) | deps | exact content and proof route | source locator |
|---|---|---|---|
| `def-bigraded-matrix-factorization-with-potential` (definition) | `[def-graded-ring-module-bimodule-and-internal-shift]` | Over $S=\mathbb Q[a,x_1,\ldots,x_r]$ with $\deg a=(2,0)$ and $\deg x_i=(0,2)$, a factorization of $w$ is a free bigraded $S$-module $M^0\oplus M^1$ with odd $d$ of bidegree $(1,1)$ and $d^2=w$; define bidegree-zero morphisms and homotopies of degree $(-1,-1)$. This is not declared to be an ordinary chain complex: unless $w=0$, its differential does not square to zero. | KR II, opening definitions, printed pp. 1--3. |
