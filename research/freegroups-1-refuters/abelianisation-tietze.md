# Read-only proof-refuter — run `freegroups-1`, cluster `abelianisation-tietze`
...
**Cluster focus.** Two independent developments. For the abelianisation: check G/[G,G] is proved abelian and that its universal property is stated with the right direction; check the free-abelian conclusion for F(X) really establishes a free BASIS in the abelian category and does not just assert Z^n. For Tietze: reader 2 repaired an overlapping-alphabet omission in def-tietze-transformations and judged it nonfatal under the 30-second rule -- **re-examine that judgement independently from disk**. Then attack thm-tietze-transformations-connect-finite-presentations, which reader 2 named the mandatory high-risk read: its reverse direction builds a common presentation from two presentations of the same group, and that construction is where a plausible proof most often fails. Check it produces a genuinely FINITE sequence of transformations and that the intermediate presentations are finite.
