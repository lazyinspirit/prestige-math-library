# Read-only proof-refuter — run `freegroups-1`, cluster `examples-counterexamples`
...
**Cluster focus.** All the concrete content. **The presentation examples are where FATAL item 3 bites hardest**: each claims a specific presented group IS isomorphic to a named group. A complete argument needs a surjection from the presented group onto the target (usually by von Dyck, checking the relators die) AND an upper bound on the presented group's order matching the target's -- typically by a normal-form / rewriting argument showing every element has one of at most n canonical forms. **Verify the upper bound is actually proved and not skipped.** Check each: finite cyclic, dihedral (order 2n, and check the dihedral convention used matches the library's published one), Klein four, Z^2 (infinite -- the counting argument does NOT apply, so check what replaces it), and S_3. For the free-group examples, check the empty-set case really yields the trivial group and the one-generator case really yields Z, both proved via the universal property rather than asserted. For the three cex-delete-only-relator items, recompute the arithmetic yourself from scratch -- do not accept the stated word manipulations -- and confirm each states a claim about ONE named syntactic procedure, never about decidability (FATAL item 1).
