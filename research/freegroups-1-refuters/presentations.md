# Read-only proof-refuter — run `freegroups-1`, cluster `presentations`
...
**Cluster focus.** The presentation machinery. prop-normal-closure-is-products-of-conjugates must prove BOTH inclusions -- that the set of finite products of conjugates is a normal subgroup containing the set, and that it sits inside every normal subgroup containing the set -- and must handle the empty product / trivial case. thm-von-dyck is the run's largest single proof and is a biconditional: check each direction separately and check the induced map is well defined on the quotient by the normal closure exactly. prop-equality-of-words-in-a-presentation is also a biconditional. cor-every-finite-group-is-finitely-presented needs a genuinely FINITE relator set -- check the argument does not quietly present an infinite one, and check it does not overstate to "every group".
