...
2.1 Suppose $n\ge2$ and fix the transposition $\tau=(0\,1)$. Left multiplication $\sigma\mapsto\tau\sigma$ is an involutive bijection from the even fibre of sign to the odd fibre, because $\operatorname{sgn}(\tau)=-1$. [step 1.1, L1]

3.1 The even and odd fibres are disjoint and have union $S_n$, and step 2.1 gives them equal finite cardinality. The sum rule and [L3] therefore give $n!=|S_n|=|A_n|+|A_n|=2|A_n|$. [step 2.1, L1, L3]

4.1 If $n=0$ or $n=1$, sign has image $\{1\}$, so its kernel is all of $S_n$ and $A_n=S_n$. [step 3.1, L1] ∎
