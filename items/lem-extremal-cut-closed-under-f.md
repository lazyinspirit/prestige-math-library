...
status: draft
origin: session
deps: [def-bw-extremal, lem-admissible-set-exists, def-chain-complete-poset]
justified_by: []
aliases: []
landmark: false
...
1.2 Membership of $M_x$ gives $y \le x$ or $f(x) \le y$, and the relation $y \le x$ holds exactly when $y < x$ or $y = x$. [A2]
...
2.2 In the case $y = x$, we get $f(y) = f(x)$, and $f(x) \le f(x)$ by reflexivity, so $f(y) \in M$ satisfies the second alternative, hence $f(y) \in M_x$. [step 1.4, step 1.1, A2]

2.3 In the case $f(x) \le y$, progressivity gives $y \le f(y)$, so $f(x) \le f(y)$ by transitivity, hence $f(y) \in M_x$. [step 1.5, L2, step 1.1, A2]
