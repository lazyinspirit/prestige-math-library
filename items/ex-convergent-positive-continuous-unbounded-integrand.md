...
2.1 By [L2], additivity, and [L3], the total integral of all spikes is $\sum_{k=1}^\infty2^{-k-2}<\infty$. Given $\varepsilon>0$, choose $K$ so that both the geometric spike tail from $K$ and the baseline tail $1/(1+K)$ are below $\varepsilon/2$. For $K\le u<v$, nonnegativity bounds the integral over every partial spike by the full spike area, so $\int_u^v f<\varepsilon$. The Cauchy criterion therefore gives convergence of $\int_0^\infty f$. [step 1.1, L2, L3]

3.1 At every positive integer $k$, $f(k)\ge s_k(k)=k$. Integers occur arbitrarily far out, so $f$ is unbounded on every tail. [given] ∎
