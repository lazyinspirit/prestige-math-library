...
[F2] Let $(X,d)$ be a metric space (def-metric-space). $(X,d)$ is **complete** if every Cauchy sequence in $(X,d)$ converges to a point of $X$; a subset $A\subseteq X$ is called complete when the metric subspace $(A,d_A)$ is complete. ([[def-complete-metric-space]]).

[F3] Throughout, $\mathbb{R}$ is the complete ordered field (def-real-numbers) and a **sequence of reals** is a function $a : \mathbb{N} \to \mathbb{R}$ (def-sequence), written $(a_k)$; recall that $\mathbb{N}$ contains $0$. The **sequence of partial sums** of a sequence $(a_k)$ of reals is $s_n:=\sum_{k<n}a_k$, so that $s_0=0$ and $s_{n+1}=s_n+a_n$; the **series** $\sum a_k$ **converges** when $(s_n)$ converges, and its **sum** is then that limit. ([[def-series]]).
