...
provenance:
  ...
  proof: ai-altered
deps: [def-cech-complete-space, def-dependent-choice, thm-compact-iff-fip, thm-the-separation-implication-chain, def-baire-space, thm-a-compact-hausdorff-space-is-regular-and-normal]
justified_by: []
aliases: []
landmark: true
...
2.1 Starting in a prescribed nonempty relatively open set, use regularity and density to choose nested nonempty ambient open sets with compact closures, each lying in the next $G_\delta$ layer and the next dense open set. The ambient space of step 1.1 is compact Hausdorff, so [F6] is what supplies its regularity; the separation chain [F4] does not state that implication. [step 1.1, F3, F4, F5, F6]
