...
[L1] The standard unit vectors satisfy $z=\sum_j z_je_j$, $\langle z,e_j\rangle=z_j$, and $\lVert e_j\rVert_2=1$; the Euclidean norm satisfies Cauchy--Schwarz, homogeneity, and the triangle inequality ([[def-euclidean-inner-product]], [[lem-standard-basis-of-f-n]], [[thm-cauchy-schwarz-and-the-euclidean-norm]]).
...
1.2 Conversely, $z=\sum_jz_je_j$ and the norm axioms in [L1] give $\lVert z\rVert_2\le\sum_j|z_j|$. Applying this to every chord gives $\ell_P(\gamma)\le\sum_{j=1}^nV(\gamma_j,P)\le\sum_{j=1}^n\operatorname{Var}(\gamma_j)$ for every partition. [given, L1, L2, L3]
