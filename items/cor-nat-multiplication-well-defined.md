...
status: published
origin: session
deps: [thm-recursion, def-natural-numbers, def-nat-addition]
aliases: []
landmark: false
...
2.1 Apply the recursion theorem with $A = \mathbb{N}$, $a = 0$, $f = \mathrm{add}_m$: there is a unique $p_m : \mathbb{N} \to \mathbb{N}$ with $p_m(0) = 0$ and $p_m(\sigma(n)) = \mathrm{add}_m(p_m(n)) = p_m(n) + m$. [L1, step 1.1]
