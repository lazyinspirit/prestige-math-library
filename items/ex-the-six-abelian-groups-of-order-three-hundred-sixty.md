...
[L3] Let $n\ge1$, and write its canonical prime factorisation as $n=\prod_{i<r}p_i^{a_i}$, with the $p_i$ distinct and $a_i>0$. Then the number of isomorphism classes of abelian groups of order $n$ is $$\prod_{i<r}P(a_i),$$ where $P(a)$ is the number of partitions of $a$. For $n=1$ one has $r=0$, so the empty product is $1$. ([[cor-number-of-abelian-groups-of-a-given-finite-order]]).
