...
- **Why the mesh of the coarse partition is the right quantity.** Step 4.2 is the only place where the mesh hypothesis is spent, and it is spent through the quantitative clause of [[lem-refinement-inequalities]]: adding the at most $n_0 - 1$ interior points of $P_0$ to $P$ can change each Darboux sum by at most $2M_{+}$ times the total length of the affected subintervals, and each of those has length below $\delta$. The number $n_0$ is fixed **before** $\delta$ is chosen, which is why the argument is not circular.
