...
**Given:** The four-point set $X=\{NW,NE,SW,SE\}$ and the family $\mathcal G$ of the north and south rows and the west and east columns.
...
1.2 Intersecting rows with columns produces every singleton, so $\sigma_X(\mathcal G)=\mathcal P(X)$; a singleton intersection is absent from $\mathcal G$, so $\mathcal G$ is not a pi-system. [given, L2, L3]

2.1 Every row and column contains exactly one $\mu$-atom and one $\nu$-atom of mass $1/2$, so the measures agree on $\mathcal G$. [given, step 1.1, algebra]

3.1 The measures are unequal because $\mu(\{NW\})=1/2$ and $\nu(\{NW\})=0$, despite steps 2.1 and 1.2. Thus the example violates exactly the pi-system hypothesis of [L3]. [step 1.1, step 2.1, step 1.2, L3] ∎
