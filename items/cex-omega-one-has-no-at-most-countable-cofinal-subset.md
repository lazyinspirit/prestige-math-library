...
**What is deliberately not said.** In the usual vocabulary this item says $\operatorname{cf}(\omega_1) > \omega$, or "$\omega_1$ is regular". Neither the cofinality function nor the word regular is defined in this library, so the statement is made in terms of subsets only ([[def-cofinal-subset-of-an-ordinal]]). Nothing is lost: the applications, such as the non-normality of the deleted Tychonoff plank, use exactly the subset form.
