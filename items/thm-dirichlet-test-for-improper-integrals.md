...
1.1 Choose $M$ with $|F|\le M$. In clause 1, apply [L1] on $[u,v]$. Every partial integral $\int_u^xf=F(x)-F(u)$ has absolute value at most $2M$, so
...
2.1 Reversing orientation proves the $-\infty$ case. At a finite endpoint, use a primitive based at a fixed nonsingular point and take the corresponding one-sided limits; the same estimates are unchanged. ∎
