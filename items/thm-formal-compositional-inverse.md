...
**Given:** The composition laws of [[thm-formal-composition-laws]], the unit criterion of [[thm-formal-power-series-unit-criterion]], and coefficient extensionality from [[prop-coefficient-extraction-linearity-and-extensionality]].
...
2.1 Apply the construction to $g$: its linear coefficient $a_1^{-1}$ is a unit, so there is $h$ with $g\circ h=x$. Associativity gives $f=f\circ x=f\circ(g\circ h)=(f\circ g)\circ h=x\circ h=h$. Thus $g\circ f=x$ as well, and any two-sided inverse is the already unique solution of $f\circ g=x$. [step 1.2, given]
