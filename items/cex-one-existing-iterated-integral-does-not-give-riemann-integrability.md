...
[L1] Rational and irrational numbers are dense, and a nonzero scalar multiple of the Dirichlet function is nonintegrable ([[def-dirichlet-and-thomae-functions]]).
...
1.2 For fixed $x\ne0$, the $y$-section is $x$ times the Dirichlet function and is nonintegrable by [L1]. Its lower and upper integrals are respectively $\min(x,0)$ and $\max(x,0)$; integrating these in $x$ gives $-1/2$ and $1/2$. [L1, given]
