...
$$f(x)=\begin{cases}1,&x=0\text{ or }x=2^{-n}\text{ for some integer }n\ge1,\\0,&\text{otherwise.}\end{cases}$$

Then $f$ is Riemann integrable with integral zero on every subinterval. Its integral function $F(x)=\int_0^x f$ is therefore identically zero, so $F'(0)=0$, although $f$ is discontinuous at $0$ and $f(0)=1$.
...
1.2 Given $\varepsilon>0$, choose $N$ with $2^{-N}<\varepsilon/2$ by [L1]. Put the finitely many spikes $2^{-1},\ldots,2^{-(N-1)}$ in disjoint partition intervals of total length below $\varepsilon/2$, and put all remaining spikes in $[0,2^{-N}]$. The resulting upper sum is below $\varepsilon$. [given, L1, construct]
...
4.1 Along the spike sequence $2^{-n}\to0$, the values are $1$, while arbitrarily close nonspike points have value $0$; thus $f$ is discontinuous at $0$ and $F'(0)=0\ne f(0)=1$. [given, step 3.1, L1] ∎
