...
Let $(F(X),i)$ be a free group on $X$, let
$q:F(X)\to F(X)^{\mathrm{ab}}$ be the abelianisation map, and put
$i_{\mathrm{ab}}=q\circ i$. Then
$(F(X)^{\mathrm{ab}},i_{\mathrm{ab}})$ is a free abelian group on $X$.

## Facts & Assumptions

**Given:** A free group $(F(X),i)$, its quotient $F(X)^{\mathrm{ab}}=F(X)/[F(X),F(X)]$, its canonical quotient map $q$, and $i_{\mathrm{ab}}=q\circ i$.
...
2.1 By [L2], construct a homomorphism $\overline f:F(X)^{\mathrm{ab}}\to A$ with $f=\overline f\circ q$; then $\overline f\circ i_{\mathrm{ab}}=\overline f\circ q\circ i=f\circ i=u$. [L2, step 1.2, construct]
