...
[L12] Finite sums: splitting, additivity, scaling, monotonicity in the terms, telescoping, and the interchange $\sum_{i<n}\sum_{j<m}c_{i,j} = \sum_{j<m}\sum_{i<n}c_{i,j}$, which follows from additivity by induction ([[def-finite-sum]], [[lem-finite-sum-laws]], [[thm-induction-principle]]).
...
- **The exhaustion $D = \bigcup_k E_{1/\iota(k+1)}$ is proved here and not quoted.** [[thm-discontinuity-set-is-f-sigma]] proves the closely related statement that $D$ is the trace on the domain of an $F_\sigma$ subset of $\mathbb{R}$, which is not the pointwise identity step 2.2 needs; the identity is short and is derived directly from [[thm-continuity-iff-oscillation-zero]] and the Archimedean property.
