...
verification:
  ...
  verified:
    ...
    verdict: pass
    date: 2026-09-09
    scope: "Local author repair of contraction notation and review of the proof and used published supplier interfaces; no independent judge or whole-closure certification."
    delegated_by: "Owner-authorized U-C19 repair audit"
sources:
  scraped: []
  references:
...
$$ Q_i=\{m\in M:m/1\in N_{\mathfrak p_i}\}. $$
This is the inverse image under the canonical map $M\to M_{\mathfrak p_i}$;
that map need not be injective. In particular, each isolated primary component is
...
2.1 Fact [L3] recovers $Q_i$ by contracting $(Q_i)_{\mathfrak p_i}$ back to $M$. Since step 1.1 identifies $(Q_i)_{\mathfrak p_i}$ with $N_{\mathfrak p_i}$, this gives $$ Q_i=\{m\in M:m/1\in N_{\mathfrak p_i}\}. $$ By [L5], the set of component primes depends only on $M/N$, so $\mathfrak p_i$ occurs and is isolated in every minimal primary decomposition. Applying the same localization-and-contraction argument to any such decomposition recovers its $\mathfrak p_i$-component from the same right-hand side. The assumed Axiom of Choice supplies the choice hypotheses of the primary-characterization and associated-prime results used through [L2], [L3], and [L5]; the finitely many selections in step 1.1 require only finite choice. Hence the isolated component is unique. [L3, L5, step 1.1]
