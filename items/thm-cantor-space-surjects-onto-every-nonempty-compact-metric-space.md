...
kind: theorem
title: "Under Dependent Choice, every nonempty compact metric space is a continuous image of Cantor space"
status: published
origin: session
provenance:
...
Assume Dependent Choice. Every nonempty compact metric space is the image of a continuous surjection from Cantor space $\{0,1\}^{\mathbb N}$.

## Facts & Assumptions

**Given:** The objects, hypotheses, and choice principles stated above.

[F1] If $K$ is a nonempty compact metric space, there is a rooted levelled tree $T=\bigcup_{n\in\mathbb N}T_n$, finitely branching with every level $T_n$ finite and nonempty — so $T$ itself is infinite — and nonempty compact sets $(K_s)_{s\in T}$ such that $T_0$ has one root $r$ with $K_r=K$, every node has a finite nonempty set of children whose sets cover its set, every child set is contained in its parent set, and $\operatorname{diam}(K_s)\le2^{-n}$ for $s\in T_n$ after a harmless rescaling of the metric. ([[lem-finite-refining-small-diameter-covers-of-compact-metric-spaces]]).
