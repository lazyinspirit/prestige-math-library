...
kind: theorem
title: "For every positive-sized square matrix over a commutative ring, A adj(A)=adj(A) A=det(A) I"
status: draft
origin: session
provenance:
...
1.1 The $(i,j)$ entry of $A\operatorname{adj}(A)$ is $\sum_k a_{ik}C_{jk}(A)$. If $i=j$, [L1] makes this $\det(A)$. [L1, L4]
...
1.3 The $(i,j)$ entry of $\operatorname{adj}(A)A$ is $\sum_k C_{ki}(A)a_{kj}$. It equals $\det(A)$ when $i=j$ by column expansion. When $i\neq j$, it is the column-$i$ expansion of the matrix obtained by replacing column $i$ by column $j$, whose determinant is $0$ because it has two equal columns. [L1, L3, L4]
