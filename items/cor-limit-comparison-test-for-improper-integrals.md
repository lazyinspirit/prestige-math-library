...
1.1 With tolerance $L/2$, the limit gives $L/2<f/g<3L/2$ sufficiently near the singular end. Thus $(L/2)g\le f\le(3L/2)g$. Applying [L2] in both directions proves the equivalence.

2.1 If $L=0$, eventually $f/g\le1$, so $f\le g$. If $f/g\to+\infty$, eventually $g\le f$. The one-way conclusions again follow from [L2]. ∎
