...
1.2 Conversely, insert $c$ into an arbitrary partition $P$ of $[a,b]$. The refined sum splits into a sum on $[a,c]$ and one on $[c,b]$, while inserting a point does not decrease the sum, because the corresponding increment is replaced by two increments whose absolute values dominate it by the triangle inequality. Hence $V(f,P)\le\operatorname{Var}_{[a,c]}(f)+\operatorname{Var}_{[c,b]}(f)$. [L1, L2, L3]
