...
2.1 Reversing the recorded effects in [L3] expresses $\det(A)$ in terms of $\det(U)$, with one factor $-1$ per swap, one nonzero factor $c$ per row scaling by $c$, and no factor for a row addition. [step 1.2, L3]
