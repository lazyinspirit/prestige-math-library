...
3.1 Claim (a): applying [L1] to the families $(\kappa_\xi)_{\xi \in \beta}$ and the constant family $\lambda_\xi = \kappa$, which satisfy $\kappa_\xi < \kappa$ by step 1.1, gives $\sum_{\xi \in \beta}\kappa_\xi < \prod_{\xi \in \beta}\kappa = \kappa^{\lvert \beta\rvert} = \kappa^{\beta}$ by [L2] and [L8], since $\beta$ is a cardinal; with step 2.1 this is $\kappa < \kappa^{\operatorname{cf}(\kappa)}$. [step 1.1, step 2.1, L1, L2, L8]
