...
status: draft
origin: session
deps: [def-ultrafilter, def-filter, def-maximal-element]
justified_by: []
aliases: []
landmark: true
...
- The filter $\mathcal{G}$ built at step 1.3 is the filter generated by
  $\mathcal{U} \cup \{A\}$: its members are the supersets of the sets
  $C \cap A$, which are exactly the intersections of finite lists in
  $\mathcal{U} \cup \{A\}$ once several $C$'s are absorbed into one
  ([[lem-fip-generates-filter]]). Step 2.1 is the check that this family has the
  finite intersection property, and that check is exactly where
