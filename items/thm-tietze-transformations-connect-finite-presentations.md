...
provenance:
  ...
  proof: ai-altered
deps: [def-tietze-transformations, prop-tietze-transformations-preserve-presented-groups, prop-equality-of-words-in-a-presentation, thm-von-dyck, def-relators-relations-and-finite-presentations, prop-canonical-quotient-map, thm-induction-principle]
justified_by: []
...
1.2 Conversely, fix an isomorphism $\phi:G_{\mathcal P}\to G_{\mathcal Q}$ and first rename generators if necessary so that $X\cap Y=\varnothing$. By surjectivity in [L3], for each $x\in X$ choose a word $v_x(Y)$ representing $\phi([x])$, and for each $y\in Y$ choose a word $w_y(X)$ representing $\phi^{-1}([y])$; only the finitely many choices indexed by $X\cup Y$ are made, successively by [L4]. [L3, L4, given, choose]
