...
- **Why the witnesses are all in one space.** The cofinite topology on an infinite set makes every finite set closed and every infinite set dense, so it separates the four combinations of $\operatorname{int}$, $\overline{\ \cdot\ }$, $\cup$ and $\cap$ with a single pair of sets and a single family. The same four failures occur in $\mathbb{R}$ with its usual topology, and the sharpest form of the first is on the companion page: the interiors of $\mathbb{Q}$ and of its complement are both empty while the interior of their union is everything ([[cex-interior-of-a-union-is-larger]]).
