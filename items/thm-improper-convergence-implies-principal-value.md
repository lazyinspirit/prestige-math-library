...
1.1 At an interior point $c$, the two truncated terms in [L3] tend separately to the two finite one-sided values. Their sum therefore tends to the sum in [L1].

1.2 On the real line, split at zero. As $R\to\infty$, $\int_{-R}^0f$ and $\int_0^Rf$ tend separately to their two tail values, so their sum tends to the mixed value. Split-point invariance [L2] removes any dependence on zero.

2.1 These arguments prove only that independent limits imply the coupled limit; the coupled limit can exist by cancellation even when neither independent limit is finite. ∎
