...
deps: [def-a-monotone-sublist-of-a-finite-list-of-reals, def-canonical-natural,
       ...
       lem-nat-trichotomy, thm-nat-linear-order, lem-nat-discrete, def-ordered-field, def-injection-surjection-bijection,
...
1.1 If $m=0$ or $n=0$, then $mn=0$ and the empty list has no sublist of the positive lengths $m+1$ and $n+1$, proving the assertion in these boundary cases. Hence for the construction below assume $m,n\ge1$. [given, L1, L2]

1.2 The index bijection. Define $\beta : m \times n \to \mathbb{N}$ by $\beta(b,j) := b\,n + j$. Its values lie in $mn$: from $j < n$ and $b < m$ we get $j + 1 \le n$ and $b + 1 \le m$, so $\beta(b,j) + 1 = bn + (j+1) \le bn + n = \sigma(b)\cdot n \le mn$ by [L1], whence $\beta(b,j) < mn$. [L1, construct]

1.3 $\beta$ is injective. Suppose $\beta(b,j) = \beta(b',j')$ with $(b,j), (b',j') \in m \times n$. If $b < b'$ then $\sigma(b) \le b'$, so $\beta(b,j) < bn + n = \sigma(b)\,n \le b'n \le \beta(b',j')$ by [L1], a contradiction; symmetrically $b' < b$ is impossible, so $b = b'$ by [L1], and then $j = j'$ by cancellation. [L1, construct]

2.1 The list. By [L2] and [L3], the injection $\beta$ of step 1.3 is a bijection of $m \times n$ onto $mn$, so every index $i < mn$ is $\beta(b,j)$ for exactly one pair, and $$a_{\beta(b,j)} \;:=\; \iota\big(b\,n + n - j\big) \qquad (b<m,\ j<n)$$ defines a list $a : (mn) \to \mathbb{R}$. Write $\mathrm{blk}(\beta(b,j)) := b$ for the **block** of an index. [step 1.2, step 1.3, L2, L3, construct]
...
3.2 Across blocks the values increase, and the block is monotone in the index. Let $b < b' < m$ and $j, j' < n$. Then $bn + n - j \le bn + n = \sigma(b)\,n \le b'n < b'n + (n - j')$, the last step because $n - j' \ge 1$; so $a_{\beta(b,j)} < a_{\beta(b',j')}$ by [L5]. Moreover $\beta(b,j) < \beta(b',j')$, by the computation of step 1.3; equivalently, $\mathrm{blk}$ is nondecreasing along the index order. [step 1.3, step 2.1, L1, L5]
...
4.2 No strictly increasing sublist of length $m+1$. Let $s : L \to mn$ be a strictly increasing sublist. The map $\mathrm{blk}\circ s : L \to m$ is injective: if $p < q < L$ had $\mathrm{blk}(s(p)) = \mathrm{blk}(s(q))$, then $s(p) < s(q)$ lie in one block, so $a_{s(p)} > a_{s(q)}$ by step 3.1, contradicting that the sublist increases. Hence $L \le m$ by [L4] and natural-order trichotomy, so $L \ne m+1$. [step 2.1, step 3.1, L1, L4, L6]

4.3 No strictly decreasing sublist of length $n+1$. Let $s : L \to mn$ be a strictly decreasing sublist and let $p < q < L$. Then $s(p) < s(q)$, so $\mathrm{blk}(s(p)) \le \mathrm{blk}(s(q))$ by step 3.2; and $\mathrm{blk}(s(p)) < \mathrm{blk}(s(q))$ would give $a_{s(p)} < a_{s(q)}$ by step 3.2, contradicting that the sublist decreases. Thus, if $L>0$, all the $s(p)$ lie in the block of $s(0)$, say $s(p)=\beta(b,j_p)$, and $p\mapsto j_p$ is injective into $n$ because $s$ is injective and $\beta$ is a bijection. If $L=0$, the empty map is already an injection $L\to n$. In either case [L4] and natural-order trichotomy give $L\le n$, so $L\ne n+1$. [step 1.1, step 2.1, step 3.2, L1, L4, L6]

5.1 Together with the boundary cases in step 1.1, the list of step 2.1 is therefore a pairwise distinct list of $mn$ reals with no strictly increasing sublist of length $m+1$ and no strictly decreasing sublist of length $n+1$, which is the assertion. [step 1.1, step 4.1, step 4.2, step 4.3, discharge-construct] ∎
...
- **The degenerate cases are discharged first.** If $m = 0$ or $n = 0$ then $m \times n$ and $mn$ are both empty, and step 1.1 proves directly that the empty list has neither required positive-length sublist.
