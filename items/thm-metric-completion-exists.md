...
- **The quadrilateral estimate does all the work.** Step 1.1 is used three times: to show $\big(d(x_n,y_n)\big)_n$ is Cauchy, to show $\rho$ is independent of representatives, and implicitly in the triangle inequality for $\widehat{d}$. It is the reverse triangle inequality ([[lem-metric-reverse-triangle]]) applied in each argument separately and added.
