...
provenance:
  ...
  proof: ai-altered
deps: [def-c-one-map-and-local-inverse, def-invertible-euclidean-linear-map, thm-euclidean-inverse-function-theorem, thm-algebra-of-total-derivatives, thm-chain-rule-for-total-derivatives, lem-euclidean-linear-maps-have-matrices-and-are-bounded]
justified_by: []
...
[L1] A $C^1$ map with invertible derivative has a local $C^1$ inverse ([[thm-euclidean-inverse-function-theorem]]).

[L2] Total derivatives obey linear algebra and chain rules; direct expansion of
the defining formula gives the displayed block derivative of $H$
...
1.1 Define $H:U\to\mathbb R^{m+n}$ by $H(x,y):=(x,F(x,y))$.  It is $C^1$, and [L2] gives $DH(a,b)(u,v)=(u,D_xF(a,b)u+D_yF(a,b)v)$. If $B:=D_yF(a,b)^{-1}$, the displayed derivative has the two-sided inverse $(r,s)\longmapsto (r,B(s-D_xF(a,b)r))$. Thus $DH(a,b)$ is invertible.  [L2, L3, given, algebra]
