...
[L2] If $n,k\in\mathbb N$ and $k\le n$, then $\binom{n}{k}\cdot k!\cdot (n-k)! = n!$, equivalently $\binom nk\,k!=n^{\underline k}$ ([[thm-binomial-closed-formula]]).
...
3.1 If $N<k$, then $\binom Nk=0$ by the definition of the binomial coefficient. If $k\le N$, [L2] gives $\binom Nk\,k!=N^{\underline k}\le N^k$, so again $\binom Nk\le N^k/k!$. Since $N\le2^{k/2}$, the left side in step 2.1 is therefore at most $2^{1+k/2}/k!$ in either case. At $k=3$ this is $2^{5/2}/6<1$; thereafter the ratio of the bound for $k+1$ to that for $k$ is $\sqrt2/(k+1)<1$. Hence the strict inequality holds for every $k\ge3$. [step 2.1, L2, algebra]
