...
kind: lemma
title: "Exactness at the homology of the right complex"
status: draft
origin: pipeline
provenance:
...
one has
$$\operatorname{im}(H_n(B)\to H_n(C))=\ker(\partial_n:H_n(C)\to H_{n-1}(A)).$$
...
2.1 Exactness of that categorical segment gives $$\operatorname{im}(H_n(B)\to H_n(C))=\ker(\partial_n),$$ as required. [L1, step 1.1] ∎
