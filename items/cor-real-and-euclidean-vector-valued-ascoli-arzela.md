...
1.3 For the Euclidean common bound, fix unit variation. Equicontinuity gives, at every $x\in X$, a neighbourhood $U_x$ on which $\lVert f(y)-f(x)\rVert_2<1$ for all $f\in\mathcal F$. Compactness gives a finite subcover $U_{x_1},\ldots,U_{x_m}$. [given]
...
3.1 Pointwise boundedness gives numbers $M_i$ with $\lVert f(x_i)\rVert_2\leq M_i$ for every $f$. For $y\in X$, choose $i$ with $y\in U_{x_i}$; then $\lVert f(y)\rVert_2<1+M_i\leq1+\max_iM_i$. Thus one Euclidean bound serves all $f$ and $y$, and the converse from a common bound to pointwise boundedness is immediate. [step 1.3] ∎
