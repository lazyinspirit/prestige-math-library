...
[L2] Finite sums and products, defined ONLY for sequences $\mathbb{N} \to \mathbb{R}$ ([[def-finite-sum]], [[lem-finite-sum-laws]]): the recursion clauses $\sum_{k<0} a_k = 0$ and $\sum_{k<j+1} a_k = \sum_{k<j} a_k + a_j$; splitting of sums and products at any index $m \le m'$, $\sum_{k<m'} a_k = \sum_{k<m} a_k + \sum_{k=m}^{m'-1} a_k$, where the tail $\sum_{k=m}^{m'-1} a_k$ is by definition the shifted sum $\sum_{t<m'-m} a_{m+t}$ ([[def-finite-sum]]), and likewise for products; scaling $\sum_{k<m} \lambda a_k = \lambda \sum_{k<m} a_k$; and the constant sum $\sum_{i<m}\lambda = \iota(m)\lambda$. Every $\sum$ and $\prod$ written below is therefore a sum or product in $\mathbb{R}$.
...
[L5] Rational arithmetic, the order on $\mathbb{Q}$ and the embedding ([[lem-rat-positive-denominator]], [[def-rationals]], [[def-rat-operations]], [[def-rat-order]], [[lem-of-q-embeds]], [[lem-of-naturals-positive]]): on a representative with positive denominator, $m/N \ge 0$ holds exactly when $m \ge 0$ in $\mathbb{Z}$ ([[def-rat-order]], with the order on $\mathbb{Z}$ as in [[def-int-order]]); a nonnegative integer is the image of a unique natural and a positive integer the image of a unique natural $\ge 1$ ([[lem-nat-embeds-int]]), which is the step that licenses reading such an $m$, and such a denominator, as a natural number; every single rational has a representative with positive denominator ([[lem-rat-positive-denominator]] states exactly this, for one rational; the passage to a common denominator for finitely many is NOT quoted from it and is carried out by the induction inside step 1.1); $\iota : \mathbb{Q} \to \mathbb{R}$ is an injective order-preserving field homomorphism, so $\iota(0) = 0$, $\iota(u + v) = \iota(u) + \iota(v)$ and $\iota(uv) = \iota(u)\iota(v)$; on an integer $\iota$ is $m \mapsto m \cdot 1_\mathbb{R}$, with $\iota(m) > 0$ for a natural $m \ge 1$ and $m \mapsto \iota(m)$ injective on $\{1, 2, \dots\}$, hence injective on all of $\mathbb{N}$ since $\iota(0) = 0 < \iota(m)$ there.
...
1.1 Common denominator, by an induction written out rather than asserted. The claim at $j$ is: any rationals $v_0, \dots, v_{j-1}$ admit a natural $N \ge 1$ and integers $p_0, \dots, p_{j-1}$ with $v_k = p_k/N$ for every $k < j$. At $j = 0$ take $N = 1$, there being no $p_k$ to produce. Assume the claim at $j$ and let $v_0, \dots, v_j$ be given: applying it to $v_0, \dots, v_{j-1}$ yields a natural $N' \ge 1$ and integers $p'_k$ with $v_k = p'_k/N'$, and $v_j$ has a representative $v_j = a/b$ with $b > 0$, hence with $b$ a natural $\ge 1$; then $N := N'b$ is a natural $\ge 1$, and the integers $p_k := p'_k b$ for $k < j$ and $p_j := a N'$ satisfy $p_k/N = (p'_k b)/(N' b) = p'_k/N' = v_k$ and $p_j/N = (aN')/(bN') = a/b = v_j$, since $(x, y) \sim (xc, yc)$ for $c \ne 0$. The claim at $j = n$, applied to $w_0, \dots, w_{n-1}$, fixes a natural $N \ge 1$ and integers $m_0, \dots, m_{n-1}$ with $w_k = m_k/N$. Finally each $m_k$ is a natural: $w_k \ge 0$ read on the positive-denominator representative $m_k/N$ gives $m_k \ge 0$ in $\mathbb{Z}$, and a nonnegative integer is the image of a unique natural. [L5, L6]
...
5.1 Its sum and product, by a second induction written out rather than asserted. The claim at $j \le n$ is $\sum_{i<M_j} c_i = \sum_{k<j} \iota(m_k) x_k$ and $\prod_{i<M_j} c_i = \prod_{k<j} x_k^{\,m_k}$. At $j = 0$ we have $M_0 = 0$ and all four expressions are the empty sum $0$ or the empty product $1$. Assume the claim at $j < n$. Since $M_j \le M_{j+1}$, splitting gives $\sum_{i<M_{j+1}} c_i = \sum_{i<M_j} c_i + \sum_{i=M_j}^{M_{j+1}-1} c_i$, and the tail is by definition $\sum_{t<m_j} c_{M_j+t}$, since $M_{j+1} - M_j = m_j$; every one of its terms equals $x_j$, because $M_j \le M_j + t < M_{j+1}$ for $t < m_j$, so the constant sum evaluates it as $\iota(m_j)x_j$, and therefore $\sum_{i<M_{j+1}} c_i = \sum_{k<j} \iota(m_k)x_k + \iota(m_j)x_j = \sum_{k<j+1} \iota(m_k)x_k$ by the recursion clause for finite sums. The same computation with products in place of sums, the constant product in place of the constant sum, gives $\prod_{i<M_{j+1}} c_i = \big(\prod_{k<j} x_k^{\,m_k}\big) x_j^{\,m_j} = \prod_{k<j+1} x_k^{\,m_k}$. At $j = n$, where $M_n = N$ by step 3.1, this reads $\sum_{i<N} c_i = \sum_{j<n} \iota(m_j) x_j$ and $\prod_{i<N} c_i = \prod_{j<n} x_j^{\,m_j}$. [step 3.1, step 4.1, L2, L3, L6]
