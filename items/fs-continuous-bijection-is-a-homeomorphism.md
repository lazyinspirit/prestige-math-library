...
- **What rescues the claim.** A continuous bijection from a compact space to a Hausdorff space is a homeomorphism, because it is then a closed map. Neither compactness nor the Hausdorff property is defined in this library, so that repair is recorded here as orientation only and is used nowhere.
