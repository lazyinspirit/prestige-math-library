...
- **The exponent is rational, and that is not a restriction here.** At this page's position in the reading order, [[def-rational-power]] is the available exponent construction. Both $1/2$ and every exponent used above are rational, so claim 4 is intentionally local to rational $\alpha$; real powers are introduced later in [[def-real-power]].
