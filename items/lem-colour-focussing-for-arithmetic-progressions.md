...
1.1 For $r=1$, apply $V(m-1,k)$ inside the first half of an interval twice as long. Its monochromatic $(m-1)$-term progression has a positive difference and its next term still lies in the full interval, giving one focused progression. [base]

2.1 Assume $r>1$ and let $n=F(m,k,r-1)$. Partition a sufficiently long interval into consecutive blocks of length $2n$. By [L1] there are $k^{2n}$ possible block colour vectors. Use $V(m-1,k^{2n})$ on the sequence of block vectors to obtain identically coloured blocks whose indices are $b,b+t,\ldots,b+(m-2)t$. [ih, L1]

2.2 In the first selected block, the induction hypothesis gives either a monochromatic $m$-term progression, which finishes, or $r-1$ colour-focused progressions $A_i=\{a_i+j d_i:0\le j<m-1\}$ focused at $f$. For the second alternative define $A_i'=\{a_i+j(d_i+2nt):0\le j<m-1\}$. Its $j$th term occupies the same relative position in block $b+jt$ as the $j$th term of $A_i$ in block $b$, so identical block vectors preserve its colour. [step 2.1, ih]

3.1 The progressions $A_i'$ are focused at $f+(m-1)2nt$. The points $f,f+2nt,\ldots,f+(m-2)2nt$ form another monochromatic $(m-1)$-term progression focused there. Its colour differs from every colour of the $A_i$, since equality with one would make $A_i\cup\{f\}$ a monochromatic $m$-term progression. Thus the second alternative contains $r$ focused progressions of distinct colours. [step 2.2]
