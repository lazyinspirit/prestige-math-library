...
1.3 If $c\neq0$, put $b=E(1/c)$. Then $b>0$, $b\neq1$, and $\log b=1/c$, so [F2] gives $\log_bx=c\log x$. Thus the nonzero members are exactly the constant-multiple forms underlying logarithms to bases, while the zero member requires no division. [F1, L4, F2, algebra]
