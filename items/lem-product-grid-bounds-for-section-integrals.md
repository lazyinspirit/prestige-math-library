...
1.1 For a cell $I$ of $P$ and a cell $J$ of $R$, put $m_{IJ}:=\inf_{I\times J}f$ and $M_{IJ}:=\sup_{I\times J}f$. Regrouping the finite sums over the product grid gives $L(f,P\times R)=\sum_I\operatorname{vol}(I)\sum_Jm_{IJ}\operatorname{vol}(J)$ and the analogous formula with $M_{IJ}$ for the upper sum. [L1, given]

2.1 If $x\in I$, then $\inf_J f_x\ge m_{IJ}$ and $\sup_Jf_x\le M_{IJ}$ for every $J$. Hence $\ell_B(x)\ge\sum_Jm_{IJ}\operatorname{vol}(J)$ and $u_B(x)\le\sum_JM_{IJ}\operatorname{vol}(J)$. Taking the infimum or supremum over $x\in I$ preserves these inequalities. [L1, step 1.1, algebra]

3.1 Multiply the cellwise inequalities by $\operatorname{vol}(I)$ and sum over $I$. Together with $\ell_B\le u_B$, this gives the displayed chain. Exchanging the coordinate blocks gives the symmetric chain. [step 2.1, algebra] ∎
