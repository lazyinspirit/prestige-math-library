...
provenance:
  ...
  proof: ai-altered
deps: [def-the-solution-set-condition, thm-the-free-group-functor-is-left-adjoint-to-the-underlying-set-functor, thm-quotient-group-universal-property, def-normal-subgroup]
aliases: []
landmark: false
...
[L3] Kernels are normal subgroups, and normal subgroups are subsets of their ambient group ([[def-normal-subgroup]]).
...
2.1 Given $f:S\to U(G)$, extend it by [L1] to $\widehat f:F(S)\to G$ and put $N=\ker\widehat f$. By [L3], $N$ is normal, and [L2] gives a unique $\bar f:F(S)/N\to G$ with $\widehat f=\bar f q_N$. Therefore $f=U(\bar f)\eta_N$. [step 1.1, L1, L2, L3]
