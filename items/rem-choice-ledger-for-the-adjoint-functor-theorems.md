...
The objectwise theorems [[thm-general-adjoint-functor-theorem-objectwise-form]] and [[thm-special-adjoint-functor-theorem-objectwise-form]] produce an initial comma object for one fixed ambient object. Their functor forms require a supplied family of those comma objects so that no proper-class selection is hidden in assembling the adjoint. The chosen-well-powered SAFT branch additionally supplies representative sets. The direct SAFT branch instead assumes the relevant class intersections and their preservation explicitly; it never treats a proper class as a small diagram.
