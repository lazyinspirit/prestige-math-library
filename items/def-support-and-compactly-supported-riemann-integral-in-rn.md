...
A compactly supported $f$ is **compactly supported Riemann integrable** if there is a closed rectangle $Q$ with $\operatorname{supp}f\subseteq\operatorname{int}Q$ such that $f|_Q$ is Riemann integrable. Its integral over Euclidean space is defined by
