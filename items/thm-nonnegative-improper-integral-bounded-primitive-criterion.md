...
1.1 At $+\infty$, $F(R)=\int_a^R f$ is nondecreasing by [L1]. If $F(R)$ converges, its range is bounded. Conversely, if its range is bounded above, the integer sequence $F(n)$ is bounded and nondecreasing, so [L2] gives $F(n)\to S=\sup_nF(n)$.
...
2.1 Reciprocal truncations and the same squeeze prove the finite-endpoint forms; reversing orientation proves the $-\infty$ form. Moving the initial finite endpoint is harmless by [L3]. ∎
