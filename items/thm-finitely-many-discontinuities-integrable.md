...
[L8] Finite sums: splitting, additivity, scaling, monotonicity in the terms, telescoping, and the interchange $\sum_{i<n}\sum_{k<r}c_{i,k} = \sum_{k<r}\sum_{i<n}c_{i,k}$, which follows from additivity by induction on $r$ ([[def-finite-sum]], [[lem-finite-sum-laws]], [[thm-induction-principle]]).
