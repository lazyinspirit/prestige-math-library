...
- **`authorship`** (every future mathematical-content item, including
  definitions, propositions, theorems, lemmas, corollaries, examples,
  counterexamples, false statements, and mathematical remarks):
  `ai-generated`, `ai-altered`, or `literature-derived`. This is a reader-facing
  disclosure of where the mathematical text came from. It is neither a claim of
  novelty nor a substitute for a citation. `literature-derived` means faithful
  source text; a material AI reformulation, extension, repair, or generated
  proof is `ai-altered`; content not both well-established and documented in
  reliable literature is `ai-generated`. The latter receives a heightened truth
  audit, including a counterexample search whenever its truthfulness is in
  doubt. Whether a local proof is included is independently shown by
  `proved_here` and, when absent, the fuchsia ‡ marker.
