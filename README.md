...
- **`authorship`** (every future theorem, lemma, and corollary):
  ...
  proof is `ai-altered`; a result formulated without faithfully deriving it from
  one identified source is `ai-generated`. Whether a local proof is included is
  independently shown by `proved_here` and, when absent, the fuchsia ‡ marker.
