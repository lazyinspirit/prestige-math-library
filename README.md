...
- **Verification** (accumulative): mechanical precheck (always, both origins);
  ...
  **GLM 5.2** (`z-ai/glm-5.2`) primary since 2026-07-25, with GPT-5.4 then Gemini
  as fallback — never a Claude model. (`deepseek/deepseek-v4-flash` was trialled
  and reverted the same day: it passed an injected, blatantly false claim.) GLM and
  DeepSeek are both in the production *generator* lineup, so they are used as judge
  for `session` items ONLY; judging a `pipeline` item with either
