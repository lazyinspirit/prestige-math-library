# Wave 4 audit — A8 rejection lane `a8-14-ex-one-point-compactifications-of-the-li`
...
`items/ex-one-point-compactifications-of-the-line-and-of-the-naturals.md` — read the whole file from disk, including frontmatter
...
> Step 4.1 sets C={s in R : |s|<=M} and calls it a closed bounded interval, then uses L1 to make W=R*\C a set of the second kind. L1/def-one-point-compactification require C closed in R and compact, but L9 (via thm-heine-borel-r) only establishes compactness; no cited fact shows C is topologically closed, and def-interval's naming of closed intervals is explicitly just a name, not a proof of closedness (parallel to def-metric-ball's disclaimer).
