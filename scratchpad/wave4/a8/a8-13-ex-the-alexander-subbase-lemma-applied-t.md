# Wave 4 audit — A8 rejection lane `a8-13-ex-the-alexander-subbase-lemma-applied-t`
...
`items/ex-the-alexander-subbase-lemma-applied-to-a-closed-interval.md` — read the whole file from disk, including frontmatter
...
> L4 asserts the order of R is total but cites only def-real-order and def-interval; def-real-order's own remarks defer trichotomy/totality to thm-reals-ordered-field, which L4 omits. This total-order fact is what licenses step 4.1's dichotomy u<b or u>=b>a giving L=L_{<b} cup L_{>a}. Sibling batch items (e.g. the Sorgenfrey example) correctly cite thm-reals-ordered-field for this exact claim, showing the omission here is a real citation gap, not house style.
