# Wave 4 audit — A8 rejection lane `a8-06-ex-an-aleph-fixed-point`
...
`claude-opus-5` and the rejecting judge here is `deepseek-v4-pro`. You are
...
`items/ex-an-aleph-fixed-point.md` — read the whole file from disk, including frontmatter
...
> Step 1.1 attempts to define T by recursion using the rule h ↦ ℵ_{⋃ran(h)}. This rule is not a total class function as required by [L2] because the aleph operation is only defined for ordinal arguments; ran(h) may contain non-ordinals, making ⋃ran(h) not an ordinal and the rule undefined. The proof does not address this, so the recursion is not justified by the cited fact.
