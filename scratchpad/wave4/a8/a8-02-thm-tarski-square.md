# Wave 4 audit — A8 rejection lane `a8-02-thm-tarski-square`
...
`items/thm-tarski-square.md` — read the whole file from disk, including frontmatter
...
> Step 4.1 first case wrongly forbids an injection kappa to A: L2 says Hartogs number admits no injection into A, but that case only shows fixed a in A injects kappa into A via f, which contradicts L2 only if A itself is the target, which is exactly what happens here so this is fine; however step 3.1 fails to verify f is a bijection onto B not just B, and step 5.1 cites L7 for finite well-orderability but L7 in this item's Facts differs from lem-pigeonhole cited (claim 1 is about no injection sigma(n) to n, not finiteness), a minor mismatch but not fatal within 30s.
