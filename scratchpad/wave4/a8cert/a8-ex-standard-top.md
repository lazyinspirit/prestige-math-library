...
**What was repaired here, and by whom:** The first Remark ended "Every theorem on the companion page that concludes something interesting from compactness carries a Hausdorff hypothesis for exactly this reason". Claude Sonnet 5 rejected the item because that is false: on library/topology/compactness.md, thm-compactness-under-continuous-maps claims 1 and 2, thm-compact-iff-fip, thm-closed-subspace-of-a-compact-space-is-compact, thm-finite-products-of-compact-spaces, the tube lemma and thm-tychonoff all conclude a great deal from compactness with no Hausdorff hypothesis at all. The repair narrows the claim to SEPARATION conclusions and states the covering conclusions that carry no separation hypothesis. Check the narrowed claim against the actual page. No new wikilink was added, so deps are unchanged. Stale judge block and obsolete verified stamp removed.
...
Read the CURRENT text of `items/ex-compactness-in-the-standard-topologies.md` from disk and decide independently.
