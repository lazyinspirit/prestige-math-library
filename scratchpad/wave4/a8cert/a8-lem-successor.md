...
**What was repaired here, and by whom:** The last Remark ended "the cardinal successor is always written $\aleph(\kappa)$ on this page". Claude Sonnet 5 rejected the item because that is false about its own page: the sibling def-aleph-and-beth-hierarchies, on the same A page, defines kappa-plus := aleph(kappa) and declares a notation rule in force on the page reserving the superscript on a CARDINAL letter for the successor cardinal. The repair replaces the final clause with the true statement, deliberately WITHOUT a wikilink, because def-aleph-and-beth-hierarchies comes LATER in the page reading order and a link would be an undeclared forward reference. Stale judge block and obsolete verified stamp removed.
...
Read the CURRENT text of `items/lem-successor-cardinal-exists.md` from disk and decide independently.
