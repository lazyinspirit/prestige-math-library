# Contract-rework certification, run `{{run}}`
...
newest `beta-contract-rework-*.result.json` in `research/{{run}}-dispatch/` —
...
4. **On a pass: close the ledger row IN PLACE** — one defect, one row: set its
   ...
   obligation row with `node tools/obligations.mjs close --run {{run}} --id
