...
# Batch `<i>` of run `{{run}}` — post-recheck scaffold repair
...
**`research/{{run}}-scaffold-closure.json`** — read `work[]`. Each entry names
...
map in `research/{{run}}-scope-ledger.json`, and touch nothing outside your
batch.

## What to do

1. For each of your findings: apply the remedy to your batch's scaffold files
   (`research/{{run}}-batch-<i>.pages.json`, `.coverage.json`, `.notes.md`),
