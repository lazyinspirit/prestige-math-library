...
- `{{run}}-batch-<i>.pages.json`, `{{run}}-batch-<i>.notes.md`, and
  `{{run}}-batch-<i>.coverage.json`;
- `{{run}}-beta-<i>.task.md` and its assigned design sections;
- `{{run}}-alpha-groups.json`: find your group by `covers`, then read
  `{{run}}-alpha-GROUP-step3-verdicts.json` and
  `{{run}}-alpha-GROUP-step3-scaffold-review.md` using that group's label.

Report missing inputs; do not substitute another run's artifacts. Preserve and
complete `research/{{run}}-batch-<i>.proof-contracts.json`, creating it if absent.
