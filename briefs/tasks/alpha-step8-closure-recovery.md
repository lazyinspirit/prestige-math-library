# Step 8 — exact closure recovery, run `{{run}}`
...
standalone, open `research/{{run}}-step8-scope.json` and find the group whose
batches you were dispatched against before anything else — every scope rule below
is relative to it.

## Exact scope

Read `research/{{run}}-judge-closure.json`. Its `unadjudicated_rows` array names
each `(id, model, context_sha256)` rejection with no exact Alpha outcome.
Recompute that join against `research/{{run}}-judge.jsonl` and
`research/{{run}}-judge-adjudications.jsonl`; exclude null, capacity, transport
...
`research/{{run}}-step8-scope.json`. A row belonging to another group is that
...
2. **Append** exactly one row to
   `research/{{run}}-judge-adjudications.jsonl` with
...
`research/{{run}}-alpha-step8-closure-recovery-<label>.md`, where `<label>` is
