...
- Work on your dispatched group, one item at a time. Read each batch manifest, `research/<run>-step6-scope-<i>.json`, current authored items and A/B pages, contracts, and dependencies needed to verify the arguments.
...
- Write `research/<run>-alpha-<g>-6b.md` with edits, source evidence, local suppliers, required shared-plan amendments, published findings, checks, and blockers.
- Write `research/<run>-alpha-<g>-6b-decisions.json` as `{version:1,run,group,decisions}`. Preserve supplemental gate decisions. The engine stamps current content, manifest, and contract hashes and rejects missing or stale decisions.
