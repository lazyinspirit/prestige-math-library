...
if (!touchesArg || !baselineLabel || !manifestsArg || !outArg || !scopeOutArg) {
  console.error('usage: node tools/step8-changes.mjs --touches <ledger.json> --baseline <label> --manifests <batch.pages.json,...> --out <receipt.json> --scope-out <changes.pages.json> [--check] [--root <repo>]');
  process.exit(2);
}
...
console.log(`step8-changes: ${created.length} created, ${modified.length} modified, ${deleted.length} deleted since ${JSON.stringify(baselineLabel)}`);
