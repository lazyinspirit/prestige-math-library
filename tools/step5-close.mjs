#!/usr/bin/env node
// Freeze Step 5 after exact routing, currency, plan, and ledger checks pass.
// Later stages may rewrite items, but cannot rewrite the Step-5 evidence.
...
const fail = (message) => { console.error(`step5-close: ${message}`); process.exit(1); };
if (!run || !['close', 'verify'].includes(mode)) {
  console.error('usage: node tools/step5-close.mjs close|verify --run <run> [--root <repo>]');
  process.exit(2);
}

const receiptPath = R('research', `${run}-step5-closure.json`);
const researchNames = () => readdirSync(R('research'));
const artifactNames = () => researchNames().filter((name) =>
  name.startsWith(`${run}-step5-hash-`)
  || name.startsWith(`${run}-step5-scope-`)
...
if (mode === 'verify') {
  ...
  if (JSON.stringify(Object.keys(currentArtifacts)) !== JSON.stringify(Object.keys(receipt.artifacts ?? {}).sort())) {
    errors.push('the exact Step-5 artifact set changed');
  }
  ...
  if (sha256(JSON.stringify(ledgerRows)) !== receipt.step5_ledger_rows_sha256
    ...
    errors.push('Step-5 defect-ledger rows changed');
  }
  if (errors.length) fail(errors.join('; '));
  console.log(`step5-close: verified ${Object.keys(receipt.artifacts ?? {}).length} frozen artifact(s) for ${run}`);
  process.exit(0);
}
...
console.log(`step5-close: closed ${run} over ${Object.keys(artifacts).length} frozen artifact(s)`);
