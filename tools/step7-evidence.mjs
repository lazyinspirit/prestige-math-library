// Shared, strict evidence primitives for Step 7.
//
// Step 7 used to parse the same append-only ledgers three different ways. Two
...
export function loadStep7JudgeEvidence(judgePath, adjudicationsPath, { allowMissingAdjudications = true } = {}) {
  ...
  for (const entry of adjudications.rows) {
    ...
    if (typeof row?.id !== 'string' || !row.id
      ...
      errors.push(`${entry.path}:${entry.line}: malformed Step-7 adjudication row`);
      continue;
    }
    const key = rejectionKey(row);
...
export function step7AlertId(alert) {
  const source = alert.source === 'step7-rejection' ? 'step7-rejection' : 'step6-read';
  const sourceRejection = source === 'step7-rejection'
    ? {
