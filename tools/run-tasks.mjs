...
/** Batch numbers, from planning's own output. Never configured in a second place. */
...
function stageSources() {
  ...
  while (pending.length) {
    ...
    sources.push(source);
    for (const match of source.matchAll(/(?:from\s+|new URL\()(?:'(\.\/[^']+)'|"(\.\/[^"]+)")/g)) {
...
function stageTaskGroups() {
  ...
  const groups = [];
  const importPath = R('research', `${run}-checkpoint-import.json`);
  const continuation = existsSync(importPath) && readJson(importPath).run === run
    && readJson(importPath).authorized_entry === 'post-review';
  for (const m of src.matchAll(/task:\s*(\[[^\]]*\]|`[^`]*`|'[^']*'|"[^"]*")/g)) {
    ...
    if (continuation && cands.every(c => c.endsWith('-alpha-step1-drift.task.md'))) continue;
...
for (const b of batches) {
  ...
  for (const p of pages) {
    ...
    lines.push('');
    if (design.length > 1) {
      lines.push('Read every listed design location. Record which one controls and why.');
      lines.push('');
    }
    lines.push('Read the complete design section and preserve its scope, conventions, warnings, and proof route. This generated file contains no mathematical review.');
    lines.push('');
    lines.push('Compare the design with `research/plan-spec.json`. The current plan controls this run; record every conflict in the batch notes.');
    lines.push('');
  }
...
{
  const dest = R('research', `${run}-planning-notes.md`);
  if (existsSync(dest) && !force) {
    skipped.push(`research/${run}-planning-notes.md (exists)`);
  } else {
    const rows = [];
    for (const b of batches) {
    ...
    writeFileSync(dest, body);
    written.push(`research/${run}-planning-notes.md`);
  }
}
