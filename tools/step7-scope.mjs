#!/usr/bin/env node
// step7-scope — partition step-7 adjudication across the group Alphas, and give
// each one the full content of its own group before it starts.
//
// WHY THIS EXISTS. Step 7 was a single lead Alpha with `units: ['all']`,
...
//
//   2. NO CONTENT CONTEXT. The lead's task file carried the step-7 RULES and
...
//
// Usage:
//   node tools/step7-scope.mjs render --run <run>
//   node tools/step7-scope.mjs check  --run <run>
//
// `render` writes:
//   research/<run>-step7-scope.json          the partition, machine-readable
//   research/<run>-alpha-<label>-step7.task.md   one per group, self-contained
//
...
if (!run || !['render', 'check', 'digests', 'published'].includes(mode)) {
  console.error('usage: node tools/step7-scope.mjs <render|check|digests|published> --run <run> [--out <receipt.json>] [--allow-pending-alerts]');
  process.exit(2);
}
...
const scopePath = R('research', `${run}-step7-scope.json`);
...
const terminalPath = R('research', `${run}-step7-terminal-resolutions.jsonl`);
const closurePath = R('research', `${run}-judge-closure.json`);
const crossPath = R('research', `${run}-step7-cross-group.jsonl`);
const alertsPath = R('research', `${run}-step7-alerts.json`);
...
/** Every step-6 digest on disk. Missing or malformed ones are skipped here; the
...
function readGroups() {
  if (!existsSync(groupsPath)) {
    fail(`no ${rel(groupsPath)} — \`2-assign\` has not produced the group assignment yet. `
      + 'Step 7 cannot be partitioned before the partition exists.');
  }
...
function openRejections() {
  // `6-scope` deliberately renders this partition before the judge sweep so
  // the group readers can start alongside it. At that point no judge ledger
  // exists yet, and the honest rejection set is empty. Once `6-judge` has
  // written the ledger, the later `7-scope` render takes the strict path below
  // and partitions every recorded rejection.
  if (!existsSync(judgePath)) return [];
  const evidence = loadStep7JudgeEvidence(judgePath, adjPath);
...
function collectAlerts(groups, index, { includeConcerns = true } = {}) {
  ...
  for (const digest of readAllDigests()) {
    ...
    for (const raw of includeConcerns ? (digest.concerns ?? []) : []) {
      ...
      alert.alert_id = step7AlertId(alert);
  ...
  for (const raw of readJsonl(crossPath)) {
    ...
    if (actualOwner !== alert.owning_group || actualOwner === alert.from_group)
      problems.push(`${alert.alert_id}: ownership does not match the Step-7 scope`);
    if (!alert.finding.trim()) problems.push(`${alert.alert_id}: finding is empty`);
    ...
    if (!source || typeof source.id !== 'string' || typeof source.model !== 'string'
      ...
      problems.push(`${alert.alert_id}: Step-7 alert requires an exact source_rejection tuple`);
    alerts.push(alert);
  }
  const ids = new Set();
...
/** The derived context every per-group step-7 file opens with.
 *
...
function groupHeader(g, index, seam, rejections, alerts, phase = 'step7') {
  const reading = phase === 'step6';
  ...
  if (!reading) {
    ...
    L.push('## What you recorded at step 6');
    L.push('');
    if (existsSync(d)) {
      L.push(`\`${rel(d)}\` is what a group Alpha for this group wrote during step 6,`);
      ...
      L.push('**No step-6 digest exists for this group.** The reading half did not run or did');
  ...
  if (reading) {
    ...
    L.push(`in \`research/${run}-step7-cross-group.jsonl\` as`);
  ...
  if (!reading) {
    ...
    L.push('## Step-6 reader warnings');
    L.push('');
    if (!incoming.length) {
      L.push('None. No Step-6 reader warning targets an item you own.');
    } else {
      L.push(`${incoming.length} warning(s) a Step-6 reader recorded in items you own.`);
      ...
      L.push('');
      L.push(`Append one owning-group disposition per warning to \`research/${run}-step7-alert-decisions.jsonl\`.`);
      L.push('A Step-6 reader warning may be adjudicated `confirmed_fatal` and repaired with exact');
      L.push('pre/post guard hashes. A later Step-7 cross-group alert still requires a real targeted');
...
if (mode === 'render') {
  ...
  const scope = {
    ...
    groups: groups.map((g) => {
      ...
      return {
        label: g.label,
        batches: g.covers,
        task: `research/${run}-alpha-${g.label}-step7.task.md`,
  ...
  console.log(`step7-scope: ${groups.length} group(s), ${index.pageOwner.size} page(s), `
  ...
  if (orphans.length) {
    console.error(`\nstep7-scope: ${orphans.length} rejection(s) belong to no batch manifest:`);
...
// The gate for the step-6 reading half. A read-only Alpha cannot write a file, so
...
if (mode === 'digests') {
  ...
  if (bad.length) {
    console.error(`step7-scope --digests: ${bad.length} problem(s):`);
    for (const b of bad) console.error(`  ${b}`);
    process.exit(1);
  }
  console.log(`step7-scope --digests: ${groups.length} group(s) read their pairs, ${read} item(s) opened across the level`);
  process.exit(0);
}
...
const reportProblems = () => {
  ...
  console.error(`step7-scope --check: ${problems.length} problem(s):`);
  for (const p of problems) console.error(`  ${p}`);
  process.exit(1);
};

if (!existsSync(scopePath)) {
  fail(`no ${rel(scopePath)} — stage \`7-scope\` has not rendered the partition. `
...
// 4. Every Step-6 reader warning and later cross-group alert has a stable
...
// The same checker closes both renders. During `6-scope` the ledger does not
// exist yet, so there cannot be a judge-backed cross-group alert to validate.
// `7-scope` runs after the sweep and takes the strict evidence path.
const evidence = existsSync(judgePath)
  ? loadStep7JudgeEvidence(judgePath, adjPath)
  : { errors: [], answers: new Map(), rejections: new Map() };
problems.push(...evidence.errors);
for (const alert of alertReceipt.alerts.filter((entry) => entry.source === 'step7-rejection')) {
...
for (const alert of alertReceipt.alerts) {
  ...
  if (decision.outcome === 'confirmed_fatal') {
    ...
    } else if (alert.source !== 'step6-read') {
      problems.push(`${alert.alert_id}: only a Step-6 reader warning may directly license a fatal repair`);
...
reportProblems();
console.log(`step7-scope --check: ${scope.groups.length} group(s) scoped, `
