...
export function finalAdjudicatorQueueProblems(queue, { run, id, group } = {}) {
  const errors = [];
  if (queue?.version !== 1 || queue?.stage !== '7-rejudge')
    errors.push('queue must be version 1 for stage 7-rejudge');
...
export function terminalResolutionPath(run) {
  return `research/${run}-step7-terminal-resolutions.jsonl`;
}
...
function freezeClosureEvidence(root, run, text) {
  ...
  const dir = join(root, 'research', `${run}-step7-terminal-evidence`);
...
export function terminalEvidence(root, run, id, stateDir = '.autopilot') {
  ...
  const cyclesPath = join(root, 'research', `${run}-step7-rejudge-cycles.json`);
...
function main() {
  ...
  console.log(`step7-terminal-resolution: recorded ${id} at ${now.context_sha256}`);
}
