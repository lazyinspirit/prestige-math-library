...
/** Screenshot paths are generated by the immediately preceding capture stage.
 * Reading the manifest keeps Sigma's image set identical to the gate's page set. */
const renderImages = (ctx: any, phase: 'initial' | 'final'): string[] => {
  const file = R(ctx, 'research', `${ctx.run}-render-${phase}.json`);
  if (!existsSync(file)) return [];
  try { return JSON.parse(readFileSync(file, 'utf8')).captures.map((r: any) => String(r.image)); }
  catch { return []; }
};
...
export const stages = [
  ...
  {
    id: '10-render-initial-v2', label: 'initial real-page render capture', units: () => ['all'],
    artifacts: (ctx) => `research/${ctx.run}-render-initial.json`,
    pattern: resultPattern('tool', 'render-initial-v2'), concurrency: 1,
    plan: (ctx) => [{ role: 'tool', label: 'render-initial-v2', job: 'bookkeeping-mechanical',
      covers: ['all'], timeout: 1800, argv: ['node', 'tools/render-capture.mjs', '--run', ctx.run, '--phase', 'initial'] }],
    gates: (ctx) => [gate('rendercheck', ['node', 'tools/rendercheck.mjs']),
      gate('render-manifest-initial', ['node', 'tools/render-manifest.mjs', '--run', ctx.run, '--phase', 'initial']), closureGate(ctx)],
  },
  {
    id: '10-sigma-initial-v2', label: 'Sigma initial visual adjudication', units: () => ['all'],
    artifacts: (ctx) => `research/${ctx.run}-sigma-initial.json`,
    pattern: resultPattern('sigma', 'initial-v2'), concurrency: 1,
    plan: (ctx) => [{ role: 'sigma', label: 'initial-v2', job: 'adjudication', covers: ['all'],
      brief: 'briefs/sigma.md',
      task: [`research/${ctx.run}-sigma-initial.task.md`, 'briefs/tasks/sigma-initial.md'], timeout: 14400,
      images: renderImages(ctx, 'initial'), outputSchema: 'briefs/schemas/sigma.json',
      resultArtifact: `research/${ctx.run}-sigma-initial.json` }],
    gates: (ctx) => [gate('sigma-initial', ['node', 'tools/visual-adjudication.mjs', '--run', ctx.run, '--phase', 'initial']), closureGate(ctx)],
  },
  {
    id: '10-tau-baseline-v2', label: 'pre-Tau whole-tree baseline', units: () => ['all'],
    pattern: resultPattern('tool', 'tau-baseline-v2'), concurrency: 1,
    plan: (ctx) => [{ role: 'tool', label: 'tau-baseline-v2', job: 'bookkeeping-mechanical', covers: ['all'],
      argv: ['node', 'tools/visual-repair.mjs', 'baseline', '--run', ctx.run] }],
    gatesWaived: 'The baseline is checked byte-for-byte by the Tau guard after repair.',
  },
  {
    id: '10-tau-repair-v2', label: 'Tau Sigma-scoped repair', units: () => ['all'],
    artifacts: (ctx) => `research/${ctx.run}-tau.json`,
    pattern: resultPattern('tau', 'repair-v2'), concurrency: 1,
    plan: (ctx) => [{ role: 'tau', label: 'repair-v2', job: 'authoring', covers: ['all'],
      brief: 'briefs/tau.md', timeout: 14400 }],
    gatesWaived: 'Tau cannot self-certify; the next mechanical stage compares its entire diff with Sigma and the baseline.',
  },
  {
    id: '10-tau-guard-v2', label: 'Tau scope and no-content-loss guard', units: () => ['all'],
    artifacts: (ctx) => `research/${ctx.run}-tau-rejudge-targets.json`,
    pattern: resultPattern('tool', 'tau-guard-v2'), concurrency: 1,
    plan: (ctx) => [{ role: 'tool', label: 'tau-guard-v2', job: 'bookkeeping-mechanical', covers: ['all'],
      argv: ['node', 'tools/visual-repair.mjs', 'check', '--run', ctx.run] }],
    gates: (ctx) => [...repoWide(ctx), ...contractGates(ctx, { reviewed: true }),
      closureGate(ctx, { pendingRejudge: true }), ledgerGate(ctx)],
  },
  {
    id: '10-post-tau-v2', label: 'post-Tau repair snapshot', units: () => ['all'],
    pattern: resultPattern('tool', 'snap-post-tau-v2'), concurrency: 1,
    plan: (ctx) => [{ role: 'tool', label: 'snap-post-tau-v2', job: 'bookkeeping-mechanical', covers: ['all'],
      argv: ['node', 'tools/touchlog.mjs', 'snap', touchesPath(ctx), 'post-tau-v2'] }],
    gatesWaived: 'This snapshot is the fatal-only baseline for any rejudge convergence repair.',
  },
  {
    id: '10-tau-rejudge-v2', label: 'paired rejudge after Tau', units: () => ['all'],
    pattern: resultPattern('tool', 'tau-rejudge-v2'), concurrency: 1,
    plan: (ctx) => [{ role: 'tool', label: 'tau-rejudge-v2', job: 'judgement', covers: ['all'], timeout: 43200,
      argv: ['node', 'tools/tau-rejudge.mjs', '--run', ctx.run] }],
    gates: (ctx) => [
      gate('step8-guard', ['node', 'tools/step8-guard.mjs', '--touches', touchesPath(ctx),
        '--baseline', 'post-tau-v2', '--adjudications', `research/${ctx.run}-judge-adjudications.jsonl`]),
      ...repoWide(ctx), ...contractGates(ctx, { reviewed: true }), closureGate(ctx), ledgerGate(ctx),
    ],
    maxFixRounds: 3,
    onGateFailure: async ({ ctx, executor, stage, round, prevRoundAt = null }: any) => {
      const closure = readClosure(ctx);
      if (!closure || closure.closed) return;
      if (closure.needs_rejudge?.length) {
        const reason = prevRoundAt ? judgeOutageSince(ctx, prevRoundAt) : null;
        if (reason) return { outage: { reason } };
        executor.start(stage, { role: 'tool', label: `tau-rejudge-round-${round}`, job: 'judgement', covers: [],
          timeout: 43200, argv: ['node', 'tools/judge-sweep.mjs', '--ledger', `research/${ctx.run}-judge.jsonl`,
            '--cost', `research/${ctx.run}-judge-cost.jsonl`, '--items', closure.needs_rejudge.join(',')] });
        return;
      }
      executor.start(stage, { role: 'alpha', label: `tau-adjudicate-round-${round}`, job: 'adjudication', covers: ['all'],
        brief: 'briefs/alpha.md', task: [`research/${ctx.run}-alpha-rejudge.task.md`, `research/${ctx.run}-alpha-step8.task.md`],
        timeout: 21600 });
    },
  },
  {
  ...
  {
    id: '10-render-final-v2', label: 'final real-page render capture', units: () => ['all'],
    artifacts: (ctx) => `research/${ctx.run}-render-final.json`,
    pattern: resultPattern('tool', 'render-final-v2'), concurrency: 1,
    plan: (ctx) => [{ role: 'tool', label: 'render-final-v2', job: 'bookkeeping-mechanical',
      covers: ['all'], timeout: 1800, argv: ['node', 'tools/render-capture.mjs', '--run', ctx.run, '--phase', 'final'] }],
    gates: (ctx) => [gate('rendercheck', ['node', 'tools/rendercheck.mjs']),
      gate('render-manifest-final', ['node', 'tools/render-manifest.mjs', '--run', ctx.run, '--phase', 'final']), closureGate(ctx)],
  },
  {
    id: '10-sigma-final-v2', label: 'Sigma final visual adjudication', units: () => ['all'],
    artifacts: (ctx) => `research/${ctx.run}-sigma-final.json`,
    pattern: resultPattern('sigma', 'final-v2'), concurrency: 1,
    plan: (ctx) => [{ role: 'sigma', label: 'final-v2', job: 'adjudication', covers: ['all'],
      brief: 'briefs/sigma.md',
      task: [`research/${ctx.run}-sigma-final.task.md`, 'briefs/tasks/sigma-final.md'], timeout: 14400,
      images: renderImages(ctx, 'final'), outputSchema: 'briefs/schemas/sigma.json',
      resultArtifact: `research/${ctx.run}-sigma-final.json` }],
    // Deliberately no repair loop. A final reject preserves the corpus exactly
    // as-is and blocks; it never authorises content removal or endless polishing.
    gates: (ctx) => [gate('sigma-final', ['node', 'tools/visual-adjudication.mjs', '--run', ctx.run,
      '--phase', 'final', '--require-approved']), closureGate(ctx)],
  },
  {
