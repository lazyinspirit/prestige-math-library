...
test('the actual Step-8 guard admits a new fatal-repair lemma and rejects an unrelated one', () => {
  const root = mkdtempSync(join(tmpdir(), 'step8-new-lemma-'));
  ...
  try {
    ...
    for (const file of readdirSync(sourceTools).filter((name) => name.endsWith('.mjs'))) {
      if (file === 'step8-guard.mjs') copyFileSync(join(sourceTools, file), join(root, 'tools', file));
    ...
    writeFileSync(join(root, 'research/touches.json'), JSON.stringify({ snapshots: [
      { label: 'pre-step8', hashes: { 'thm-consumer': shortHash(itemHashGuard(before)) } },
    ] }));
    ...
    const run = () => spawnSync(process.execPath, ['tools/step8-guard.mjs',
      '--touches', 'research/touches.json', '--baseline', 'pre-step8',
    ...
    assert.ok(JSON.parse(result.stdout).errors.some((error: any) =>
      error.code === 'step8-creation' && error.id === 'lem-unrelated'));
  } finally {
