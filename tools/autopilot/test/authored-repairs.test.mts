...
import { repairGateBatch } from '../stages/authored-repairs.mts';
...
test('an open defect in the failing detector holds instead of redispatching content repair', async () => {
  ...
  try {
    ...
    const result = await repairGateBatch({ ctx: { repo, run: 'r' }, round: 2,
      stage: { id: '5a-import-join', gates: () => [{ id: 'import-author-boundary-audit', argv: ['node', 'tools/boundary-audit.mjs'] }] },
...
test('a repair wave gives one group every overlapping gate and disjoint groups separate writers', async () => {
  ...
  try {
    ...
    await repairGateBatch({ ctx: { repo, run: 'r' }, stage: { id: '5a-adjudicate' }, round: 1,
    ...
    const evidence = JSON.parse(readFileSync(join(repo, 'research/r-5a-adjudicate-gate-batch-1.json'), 'utf8'));
    ...
    plans.length = 0;
    await repairGateBatch({ ctx:{repo,run:'r'},stage:{id:'5a-adjudicate'},round:2,
