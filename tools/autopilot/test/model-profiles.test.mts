...
test('steps 5, 6, and 7 select the requested stage-specific profiles', () => {
  ...
  assert.equal(selected(authorStage, author), MODEL_PROFILE_NAMES.deepseekXhigh1m);
  ...
  assert.equal(selected(readStage, readStage.plan(ctx, ['1'])[0]), MODEL_PROFILE_NAMES.deepseekXhigh1m);
  ...
  for (const plan of plans.filter((candidate: any) => candidate.role === 'alpha-group-read')) {
    assert.equal(selected(judgeStage, plan), MODEL_PROFILE_NAMES.gpt54Xhigh1m);
  }
...
test('every model-backed Step 9 and Step 10 dispatch inherits DeepSeek, including repairs', () => {
  for (const s of stages.filter((candidate: any) => /^(?:9|10)-/.test(candidate.id))) {
    for (const role of ['alpha', 'alpha-high', 'alpha-report', 'beta']) {
      assert.equal(selected(s, { role, job: 'audit' }), MODEL_PROFILE_NAMES.deepseekXhigh1m,
        `${s.id}/${role}`);
    }
