#!/usr/bin/env node
// Explicit migration for a run that completed 7-rejudge before the rebuilt
...
const receiptPath = resolve(root, value('--out', `research/${run}-step7-cutover.json`));
...
if (!run || !['prepare', 'check'].includes(command)) {
  console.error('usage: node tools/step7-cutover.mjs <prepare|check> --run <run> [--dispatch-dir <dir>] [--out <receipt>] [--root <repo>]');
  process.exit(2);
}
...
function completedRejudge() {
  ...
  const stage = state.stages?.['7-rejudge'];
...
function validationProblems(receipt, completed) {
  ...
  for (const id of new Set([...Object.keys(receipt.item_hashes ?? {}), ...Object.keys(now)])) {
    if (receipt.item_hashes?.[id] !== now[id]) errors.push(`${id}: item changed after frozen Step-7 cutover`);
  }
  return errors;
}

if (command === 'prepare') {
  const render = spawnSync(process.execPath, ['tools/step7-scope.mjs', 'render', '--run', run],
  ...
  if (!completed) {
    console.log('step7-cutover: ordinary pre-rejudge run; no frozen migration needed');
    process.exit(0);
  }
  ...
  if (existsSync(receiptPath)) {
    ...
    console.log(`step7-cutover: ${errors.length ? 'FAIL' : 'OK'} — existing frozen migration retained`);
  ...
  console.log(`step7-cutover: froze ${Object.keys(receipt.item_hashes).length} item(s) after completed rejudge`);
  process.exit(0);
}
...
const completed = completedRejudge();
if (!completed) errors.push('the successful, gate-complete historical 7-rejudge evidence is missing');
errors.push(...validationProblems(loaded.receipt, completed));
console.log(`step7-cutover: ${errors.length ? 'FAIL' : 'OK'} — frozen post-rejudge migration`);
for (const error of errors) console.error(`ERROR ${error}`);
process.exit(errors.length ? 1 : 0);
