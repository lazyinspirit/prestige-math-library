...
Finite cardinality supplies graph order, size, degree counts and the enumeration of two-element subsets, while [[thm-double-counting]] supplies the incidence identity behind the handshake lemma. The general definition [[def-equivalence-relation]], from the required Binary Operations, Monoids, Groups and Subgroups development, is applied to reachability. The counts [[thm-two-element-subsets-count]] and [[cor-cardinality-of-the-power-set]] license the complete-graph and labelled-graph enumerations.
