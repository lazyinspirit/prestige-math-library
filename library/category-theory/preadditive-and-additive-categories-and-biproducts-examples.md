---
page: preadditive-and-additive-categories-and-biproducts-examples
title: "Preadditive and Additive Categories and Biproducts — Examples"
status: draft
items: []
examples: [ex-the-biproduct-of-two-abelian-groups-computed-as-a-matrix-calculus,
           ex-abelian-groups-modules-and-vector-spaces-are-additive,
           ex-a-ring-viewed-as-a-one-object-preadditive-category-with-its-matrices,
           ex-the-idempotent-completion-of-a-ring-is-the-finitely-generated-projective-modules,
           cex-pointed-sets-are-not-additive,
           cex-commutative-monoids-are-semiadditive-and-not-additive,
           ex-additive-functors-out-of-the-matrix-category,
           ex-a-preadditive-category-with-two-objects-and-a-nonzero-hom-group]
---

These examples show the page’s abstract claims in the familiar settings that
motivate them: abelian groups and modules, matrix categories, pointed sets,
commutative monoids, and a minimal two-object preadditive category.
