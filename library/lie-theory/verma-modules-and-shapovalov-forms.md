...
This page constructs $M(\lambda)$ from a fixed triangular decomposition, then
uses its normalized bilinear contravariant form to detect its unique maximal
submodule.  The determinant is a block invariant, so its equality is always
understood up to a nonzero basis scalar.
