...
The two-step $\mathbb Z/4$ filtration then displays every term and map of an initial exact couple and its first derivation. Its nonzero $(1,-1)$ spectral term is retained: finite filtration does not mean first-quadrant support. A separate cohomological example has zero $d_0,d_1$ and a nonzero $d_2$, and computes each arrow of the five-term exact sequence.
