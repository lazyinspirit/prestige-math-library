---
page: dedekind-domains-and-ideal-classes-examples
title: "Dedekind Domains and Ideal Classes Examples"
status: published
items: []
examples: [ex-pid-as-dedekind-domain, ex-semilocal-dedekind-domain-is-a-pid, ex-fractional-ideal-in-the-integers, ex-ideal-inverse-computation, ex-steinitz-class-rank-two-module, ex-dedekind-localisation-is-dvr, ex-two-generators-for-a-dedekind-ideal, ex-divisor-of-a-fractional-ideal, ex-nondedekind-ideal-not-invertible]
---

These examples work through the standard concrete consequences of the Dedekind package: PIDs and semilocal Dedekind domains, explicit fractional-ideal arithmetic in $\mathbb Z$, localizations at a prime, the two-generator construction, and the divisor/class translation. The final example also records a singular one-dimensional domain where local principality fails, so invertibility fails with it.
